...
- **§0.132 on dev — Jeff's checks (Admin, accelerep.netlify.app, hard-refresh to `index-DL6xNwD_.js`):** Settings → Integrations → Automations → + New automation → the Tasks group offers **Task overdue (past its due date)** with the note on how it fires; pick it → Conditions offers Days overdue, Due date, Priority; Actions → Create task with "Chase: {{title}} ({{days_overdue}}d late)" → Create. Settings → Sales process → Lead scoring → Predictive scoring → **Train now** → on dev the answer is "Needs at least 20 decided (Converted / Dead) leads to train — this workspace has N." (the dev org had 1 decided lead of 25 at §0.123; nothing is stored). A Post to Slack rule firing on dev (Slack connected there?) shows the message as a section with "Automation: *rule* · trigger · record" under it and an Open Accelerep button — else the run history still reads "send_slack: skipped — Slack is not connected".
