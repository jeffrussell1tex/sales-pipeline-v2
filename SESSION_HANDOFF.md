...
**Nineteenth session (16 September 2026, Jeff: "Claude, do the ritual please and we will continue working" — then "Claude recomended this enhancement and I agree. I have setup a developer account with Intuit · Quote → job → invoice → QuickBooks. The quote-to-job link and the invoice record can be built now. The QuickBooks export waits on your Intuit developer account, an app with redirect URIs for both sites, and a sandbox company." — then, on the first cut: "On quote line leave it definable by admin. They can create the types of products and services to be associated with line items in the price book" — then, mid-build: "you said you need the secret ID but last time I uploaded one to you holy hell was raised that I needed to change it and it is a security risk" — then "logged in") — CLOSE.** The ritual passed (tree `deno.lock` only, the CLI's two lines; `dev` == `origin/dev` == `9d49c00`; `origin/master` `a38e0c6` an ancestor, behind by ONLY the ship-record docs commit — exactly the §5 expectation; every fingerprint present; copies identical; dev serving `index-M4Pa72MW.js`, prod `index-BkLKFS3b.js`, both as recorded). **Three findings at the open, docs only:** the state header's Verified line still read "§0.143, NOT shipped … prod serves `index-DfRbwLhy.js`" while the Updated line above it recorded the twenty-eighth ship (it lagged five batches — refreshed this session); this file's fast staleness check stopped at §0.145 while §0.146–§0.148 stood in the state doc (fixed below); §4's heading still describes the twenty-fifth ship (left — the sessions above it are the record). **Read against the tree before building:** `dispatch_jobs` carried `invoice_amount` / `invoice_status` / `invoice_paid_at` that no UI wrote and the dispatcher PUT accepted from any client; a job could link a deal but not a quote; NOTHING in the app set a quote to Accepted (the server handled the word; no control sent it); no invoice record anywhere; and TWO price books — Settings → Quoting → Price book edits a demo-seeded `priceBookProducts` list that nothing reads, while quotes draw from the `products` table under Quotes → Catalog (a finding, state §9). → **§0.149 `4fe1efc`** (functions AND bundle AND a schema change — additive, nullable, both databases first through `db/apply-invoices.mjs`, 30 columns and the indexes read back on each): `dispatch_jobs.quote_id`; the `invoices` table with four `quickbooks_*` columns reserved; `src/utils/invoices.js` (pure — the transitions, line cleaning, totals to the cent with tax on taxable lines only, terms → due date, the quote-line mapping, the customer rule, and the Admin's product & service types: three built-ins always, custom ones added, `lineKindFor` the one rule); `quote-to-job.mjs` (an Accepted quote → ONE unscheduled job with the quote's lines at net price, the customer resolved from the deal or created, the Dispatch switch checked BEFORE the quote is read, the quote read by org, 409 with the job on a repeat); `invoices.mjs` (a draft from the job's lines, `INV-YYYY-NNNN`, terms from the quote else Net 30, totals never from the body, one live invoice per job, draft → issued → paid or void, the job's three invoice columns RE-MIRRORED from its live invoice on every write and returned on the same response; Technicians 403; Admin-only draft-only delete); `dispatch-jobs.mjs` drops the three mirror columns from its PUT allowlist and its upsert's `set` (**guide §18b44, new: a mirror column has one writer**) and exports `normaliseJob` / `nextJobNumber`; `settings.mjs` carries `productTypes` in both halves; QuotesTab: "Customer accepted this quote →" (editors, on Approved / Sent / Negotiating) and the Dispatch job card (off-switch note; Create; then the job and its invoice), the Catalog's Type select the org's list; DispatchTab: the Invoice block on a job (create, edit as a draft, issue, mark paid, void — every write adopts the server's invoice AND job row) and the **Invoices** sub-tab; Settings → Quoting → **Product & service types** (new card and panel); the audit log files `invoice` under Data. +20 unit (`tests/invoices.test.mjs`), +9 integration (`tests/integration/invoices.itest.mjs` — three orgs, the custom type's line proven material), 12 mutants, the schema guard and audit-coverage extended. **Six gates (173 files; 99 function files), 883/884 unit (the April-fixture skip), 179/179 integration, 571/571 mutations after a printed green baseline (568/568 on the first cut; no restore residue either time), build guard OK 2,577 kB `index-DlCzV-dZ.js`, `dist/` cleared.** One earlier unit red was mine: `tests/date-local.test.mjs` caught a `toISOString().slice(0, 10)` in the new module — replaced with a UTC-anchored day read part by part. **OBSERVED in the pane (localhost:8888, Jeff — Admin, the Accelerep Test org `org_3BDQ…`, three memberships):** the accept (PUT 200) → "Create dispatch job →" → POST 201 `JOB-2026-0008` (`quoteId`, the deal's account and opportunity, "From quote Q-2026-010 v2", a dispatch customer created for the deal's account); Dispatch → Jobs → the job → Create invoice → POST 201 `INV-2026-0001` (two `fee` lines, $24,200, "Net 30 · Annual", due 2026-10-16, the job mirrored) → Issue → PUT 200; the Invoices sub-tab ("$24,200.00 outstanding", the row); Settings → Product & service types → "Equipment" + Material → Save → PUT 200; Quotes → Catalog → + Add product → the Type select lists Recurring, One-time, Service, Equipment. No console errors. **Pane lessons (memory-worthy):** the pane opened on Clerk's sign-in wall this time (a fresh browser context — Jeff signed in); the mutation harness must NOT run while `netlify dev` is up (its rewrites broke Vite's module graph — every module path came back as HTML) and the gate chain's build re-armed the stale `dist/` — stop the server, `rm -rf dist node_modules/.vite`, restart; the Task Overdue modal took the first click again. **NOT observed:** Mark paid and Void (the invoice is left ISSUED on the test org for Jeff to see); the draft editor's line edits; the rep path as Karen. **NOT shipped; NOT pushed at the time of writing** — `dev` ahead of `origin/master` by the ship-record docs commit `9d49c00`, **`4fe1efc` (CODE — §0.149: functions, bundle, schema)** and this handoff; a ship is `git push origin dev:master`, his call — **the schema is already in both databases (guide §18c), so the ship is safe in that respect.** **On the Intuit secret — Jeff's worry, answered in chat:** the QuickBooks half will read `INTUIT_CLIENT_ID` / `INTUIT_CLIENT_SECRET` (names to be agreed) from Netlify's environment on both sites, set by Jeff's hand in the Netlify UI like the calendar providers' secrets; nothing is ever pasted into chat, the repo or a file Claude reads — all Claude needs is the variable NAMES. Docs: state §0.149, the header's Updated / Verified / Batch lines and §9, guide §18b44 and its header, in the code commit; this handoff last. **Then Jeff, on the two-price-books finding: "well the catalog (change the name to Price Book) that feed the quotes should stay" → `ffb3de1`** (bundle only): Quotes → Catalog is **Price Book** (the sub-tab, the builder rail, the copy); the Settings → Quoting → Price book card OPENS Quotes → Price Book (behind the panels' leave guard); `PriceBookDetail.jsx` DELETED with its three pins; the `priceBookProducts` key left as dead data (state §9). Six gates, 883/884 unit (+1 scan, −1 with the file), 179/179 integration, 571/571 mutations after a printed green baseline, build guard OK `index-BycitA9a.js`, `dist/` cleared. **OBSERVED in the pane (Jeff — Admin, Accelerep Test; the server restarted after the harness with `dist/` and the Vite cache cleared):** Settings → the Price book card → the Quotes tab on its Price Book sub-tab, "+ Add product" present, no "Catalog" anywhere, no console errors. **NOT shipped. PUSHED to dev (Jeff: "push dev" — he means to keep working from another computer while travelling; the options given: a clone with a hand-made `.env` from his password manager, Claude Code on the web off the GitHub repo, or the desktop app's Remote Control with this PC left on; then "I will just wait and continue my work here later"):** `9d49c00..27bf80c dev -> dev` at 02:15:16 UTC 17 Sep; **LANDED** — accelerep.netlify.app served `index-BycitA9a.js` from 02:16:07 UTC (the hash poll, 51 s after the push; before it `index-M4Pa72MW.js`), and `invoices` and `quote-to-job` answer 401 unauthenticated on the dev site (deployed, refusing without a token; every function rebuilt — `db/schema.ts` changed). CI for `27bf80c`: queued at 02:15:19 UTC; the verdict not yet read at the time of writing (a poll on the public Actions API was running). `dev` == `origin/dev`; `dev` ahead of `origin/master` by `9d49c00`, `4fe1efc`, `6559166`, `ffb3de1`, `27bf80c` and this landing line. **NOT yet observed by Jeff on dev.**

**PROD SHIPPED — the TWENTY-EIGHTH ship (Jeff: "the org fix works … Ship", 16 Sep).** Pre-flight (gated — the push ran only because every check held): tree `deno.lock` only; `dev` == `origin/dev` == `a38e0c6`; `origin/master` `8e2d137` an ancestor; `git diff origin/master..dev -- db/schema.ts` EMPTY; seven code files (`App.jsx`, `AdminView.jsx`, `SettingsTab.jsx`, `PriceBookDetail.jsx`, two tests, the harness). SHIPPED — the TWENTY-EIGHTH ship, 19:39:50 UTC 16 Sep: `git push origin dev:master`, `master` `8e2d137` → `a38e0c6` (three commits, ONE of code — `334a079`, §0.148; bundle only, no function, no schema change); salespipelinetracker.com served `index-BkLKFS3b.js` from 19:40:22 UTC (the hash poll, 32 s after the push; before it `index-D8uZRWUK.js`), 2,612,582 B, `pk_live_` ×2 and `pk_test_` ×1 (the SDK’s constants); Netlify’s record (deploy `6aaaf0887daf930008d20e3b`, branch master, context production, commit `a38e0c6`, published 19:40:18 UTC, deploy time 25 s): 86 functions, NONE rebuilt (every build stamp unchanged from 13:32 — the bundle-only proof), `function_schedules` unchanged, secret scan clean over 467 files, "2 new files uploaded"; prod’s heartbeats after the deploy: task-reminders ok at 19:41:02 UTC (report-deliveries ok at 19:40:13, the run just before publish); the `Tests` workflow on the master push GREEN (unit, gates, integration).** **OBSERVED on dev by Jeff before the ship (his screenshots, `index-M4Pa72MW.js`): the Recently changed row "Report delivery job report delivery sent"; the Price book with list price, cost, bundle prices in the app’s face and SKUs typewriter; and, in his words, "the org fix works" — the feed and health tile change with the cards on an org switch. Also: "claude browser shoeing the app" — the desktop app’s Browser pane opens accelerep.netlify.app for him now (it had not, earlier in the session; from this side it always reached the sign-in page).** `dev` then ahead of `master` only by this handoff. **The harness’s file locks, for the record:** two consecutive runs of this batch died restoring an UNRELATED mutant ("UNKNOWN: open" — `activityView.js`, then `report-deliveries.mjs`); each was restored from HEAD after the diff showed only the mutant, and the third run reported 559/559. No dev server was running; VS Code (ten processes) and Defender’s real-time scan are the two candidates; Jeff was given the Defender folder-exclusion steps (his call — a security setting) and the cheaper experiment (close VS Code during a run). Open, small (state §9): the landing page’s cards show zeros for the second before `settings` arrives after a hard refresh.

**Eighteenth session, after the TWENTY-SEVENTH ship (16 September 2026, Jeff: "change it in price book"; "which jobs tile are you referring to"; "please do fix the report report isse"; and four prod screenshots) — CLOSE.** **OBSERVED on prod by Jeff (his screenshots):** the Accelerep Test org at 3 of 7 with "Scheduled jobs" ABSENT from the not-passing list (the check green; dev’s same line reads "not enabled on this site (JOBS_ENABLED)", correct — jobs run on prod alone); his other org (`org_3DwnytSa…`, one member) at 4 of 7, "All checks passing ✓" (MFA 1 of 1). Read-only, the audit log: that org’s daily 12:00 Chicago report "Delas stuck more than 14 days" — the one a Send now had blocked yesterday under the old dedup rule — `report_delivery.sent` at **17:00:46 UTC = 12:00:46 Chicago**, its first scheduled slot after the twenty-sixth ship; the Test org’s "Test Report #1" at 15:35:09 UTC (his inbox). The "Jobs tile" of earlier handoffs is the landing page’s health line "Scheduled jobs running" (`jobsCheck`); closed. **Two findings from the same screenshots, both fixed in §0.148, `334a079`** (bundle only): the Recently changed card read "Report report delivery sent" (the actor’s first word); and one screenshot showed the OTHER org’s feed and health above the Test org’s cards — `AdminView`’s two mount-time loads were `[]`-keyed (§0.125’s rule had not reached this view); now `activeOrgId` is in the app context, the tab hands it down, both effects key on it and clear the last org’s state first. Plus the Price book’s seven money sites onto `T.sans`. +3 tests (861), 3 mutants. Six gates (171 files; 96 function files), **861/861 unit, 170/170 integration, 559/559 mutations after a printed green baseline** (the first run for this batch died restoring an unrelated mutant on a Windows file lock — restored from HEAD, rerun), build guard OK 2,551 kB `index-M4Pa72MW.js`, `dist/` cleared. **NOT shipped; pushed to dev 19:05:12 UTC 16 Sep; LANDED — accelerep.netlify.app served `index-M4Pa72MW.js` from 19:05:43 UTC 16 Sep (the hash poll). NOT yet observed by Jeff.** Docs: state §0.148, the header, §9 (the cards’ loading state) in the code commit; this handoff last.

**PROD SHIPPED — the TWENTY-SEVENTH ship (Jeff: "ship prod", 16 Sep).** Pre-flight: tree `deno.lock` only; `dev` == `origin/dev` == `8e2d137`; `origin/master` `459c321` an ancestor; `git diff origin/master..dev -- db/schema.ts` EMPTY; three code files (`QuotesTab.jsx`, its test, the harness). SHIPPED — the TWENTY-SEVENTH ship, 16:39:20 UTC 16 Sep: `git push origin dev:master`, `master` `459c321` → `8e2d137` (four commits, ONE of code — `0bccf4b`, §0.147; bundle only, no function, no schema change); salespipelinetracker.com served `index-D8uZRWUK.js` from 16:39:52 UTC (the hash poll, 32 s after the push; before it `index-BqHYVvqM.js`), 2,612,669 B, `pk_live_` ×2 and `pk_test_` ×1 (the SDK’s constants); Netlify’s record (deploy `6aaac63a061575000806b069`, branch master, context production, commit `8e2d137`, published 16:39:49 UTC, deploy time 22 s): 86 functions, NONE rebuilt (every function’s build stamp unchanged from 13:32 — the bundle-only proof), `function_schedules` unchanged, secret scan clean over 467 files, "2 new files uploaded"; prod’s heartbeats after the deploy: report-deliveries and task-reminders both ok at 16:40:14 UTC; the `Tests` workflow on the master push: GREEN (gates, unit, integration)** `dev` then ahead of `master` only by this handoff. **What prod now has:** the builder’s notes on the customer preview and the print fallback, Edit in builder as a sand bar, the compare cards’ totals in the app’s face (§0.147) — observed by Jeff on dev before the ship ("Test out perfectly"), not yet on prod.

**Eighteenth session, after the TWENTY-SIXTH ship (16 September 2026, Jeff, with four prod screenshots: "Logging looks like it caught everything … the PDF preview of the quote did not add the notes section … highlight the edit in builder bar with a more noticeable color … once a quote is saved the dollar number font is still wrong on the list total, discount %, and Net … Report delivered as scheduled") — CLOSE.** **OBSERVED on prod by Jeff (his screenshots):** the Audit log with the job’s own row (`report_delivery.sent`, actor "Report delivery job") beside his `saved_report.scheduled`, `quote.created` and four `quote.updated`; "Test Report #1 — Opportunities · All time" in his inbox at 10:35 AM — the scheduled delivery, delivered. **§0.147, `0bccf4b`** (bundle only): (1) the builder’s notes were saved and the PDF FUNCTION rendered them, but the CUSTOMER PREVIEW never printed them and the print FALLBACK (when the function does not answer) carried neither notes nor payment terms — both now do, the fallback escaped through `esc`; which surface Jeff saw is unknown from here (a downloaded .pdf is the function’s, a print dialog the fallback’s). (2) Edit in builder is the sand bar (`T.gold` / `T.goldInk`). (3) The saved card’s totals (`{r.value}` — a seventh money site the §0.146 regex did not know) onto `T.sans`. +1 test (858), 2 mutants; a JSX comment inside a parenthesised expression broke the parse — every gate and the build said so, moved. Six gates (171 files; 96 function files), **858/858 unit, 170/170 integration, 556/556 mutations after a printed green baseline**, build guard OK 2,551 kB `index-BZQXq9Av.js`, `dist/` cleared. **NOT shipped; pushed to dev 15:52:18 UTC 16 Sep; LANDED — accelerep.netlify.app served `index-BZQXq9Av.js` from 15:52:49 UTC 16 Sep (the hash poll). OBSERVED by Jeff on dev — "Test out perfectly" (the compare cards’ totals, the sand bar, the NOTES block on the preview).** Docs: state §0.147 and the header in the code commit; this handoff last.

**PROD SHIPPED — the TWENTY-SIXTH ship (Jeff: "ship prod", 16 Sep).** Pre-flight: tree `deno.lock` only; `dev` == `origin/dev` == `459c321`; `origin/master` `930e276` an ancestor; `git diff origin/master..dev -- db/schema.ts` EMPTY; 51 files, 32 under `netlify/functions/`. SHIPPED — the TWENTY-SIXTH ship, 13:31:49 UTC 16 Sep: `git push origin dev:master`, `master` `930e276` → `459c321` (twelve commits, six of code — §0.142 + the Send-now correction, the range label, §0.143, §0.144, §0.145, §0.146; 32 function files AND the bundle; no schema change, `netlify.toml` and `package.json` untouched); salespipelinetracker.com served `index-BqHYVvqM.js` from 13:32:44 UTC (the hash poll, 55 s after the push; before it `index-DfRbwLhy.js`), 2,611,945 B, `pk_live_` ×2 and `pk_test_` ×1 (the SDK’s constants); Netlify’s record (deploy `6aaa9a4a7ee201000863bcde`, branch master, context production, commit `459c321`, published 13:32:37 UTC, deploy time 40 s): 86 functions, the 32 changed ones rebuilt at 13:32:30–36 UTC, `function_schedules` unchanged (`report-deliveries` still `*/5 * * * *`), 3 redirect rules, 1 header rule, secret scan clean over 467 files, "2 new files uploaded"; the `Tests` workflow on the master push GREEN (unit, gates, integration); prod’s heartbeats after the deploy: task-reminders ok at 13:33:03 UTC, report-deliveries ok at 13:35:09 UTC (0 due, 0 sent, 0 failed) — the first run of the shipped job** `dev` then ahead of `master` only by this handoff. **What prod now has that it did not:** every write path audits with the caller named (§0.143); the quote picker’s honest placeholder and the Send-now dedup fix (§0.142 / §0.140’s correction — the other org’s 12:00 report is safe to Send now); a delivered custom-range report names its range; money in the app’s face on Quotes (§0.146). Nothing observed on prod by Jeff yet — his §5 list.

**Eighteenth session, the last change (15 September 2026, Jeff, the §0.138 look PASSED — "All looks good - 1 change" — with the Catalog’s list-price column circled: "Can we make the dollar amounts be the same font as the text in the rest of the app?") — CLOSE.** **§0.146, `fffbfb8`** (bundle only): the six money sites in `QuotesTab.jsx` (the Catalog’s list price; a line item’s net price and total, and the totals rows, in the preview; the compare view’s line totals; the configurator’s totals) onto `fontFamily: T.sans`; the seven identifier sites (quote numbers, SKUs) keep monospace. +1 test (857), 1 mutant. Six gates (171 files; 96 function files), **857/857 unit, 170/170 integration, 554/554 mutations after a printed green baseline**, build guard OK 2,551 kB `index-leQ6sUwD.js`, `dist/` cleared. **NOT shipped; pushed to dev 23:34:19 UTC 15 Sep; LANDED — accelerep.netlify.app served `index-leQ6sUwD.js` from 23:34:50 UTC 15 Sep (the hash poll). NOT yet observed by Jeff.** Open to him: Settings → Price book sets list price, cost and bundle price in monospace as well — left alone; his call. **The §0.138 dev look — DONE (his word, with the screenshot). Prod’s Jobs tile — still his.** Docs: state §0.146 and the header in the code commit; this handoff last.

**Eighteenth session, the CI read — the other half (15 September 2026, Jeff, with the integration job’s log open: the one `not ok`) — CLOSE.** `not ok 10 - tests/integration/job-heartbeat.itest.mjs` … `[@netlify/neon] Failed to instantiate Neon client: connection string is not provided … (NETLIFY_DATABASE_URL)` at `db/index.ts:8` from the suite’s line 28. The heartbeat suite was the ONE of nineteen without the two opening statements (refuse without `DATABASE_URL_TEST`; redirect `NETLIFY_DATABASE_URL` to it) before its imports. On the runner, no `.env`, the client threw: red since 13 Sep. **On this machine the same suite ran against the APPLICATION database** — `.env`’s `NETLIFY_DATABASE_URL` is the shared `main` branch (`ep-soft-morning…`; the test database is `ep-jolly-darkness…`) — its `before`/`after` deletes of `itest_hb_*` and six tests’ writes on `itest-hb-a/b.example.com`, on every local `test:int`, reported green. Read-only, both databases, before and after the fix: **no residue** (the app database holds the nine real site rows only; the test database’s `site_job_heartbeats` was EMPTY — the suite had never once run there). **§0.145, `b809237`** (one suite, one new scan test, the harness; no function, no bundle): the two statements at the top of the suite; `tests/itest-targets-test-db.test.mjs` pins them in every `*.itest.mjs` before its first import and checks `test:int` names every suite; guide §18b43 (hard rule). Six gates (171 files; 96 function files), **856/856 unit** (855 + the fixture skip only in a shallow clone), **170/170 integration — the heartbeat suite on the test database for the first time (6/6 alone, then the chain)**, **553/553 mutations after a printed green baseline**. Pushed 23:10:18 UTC 15 Sep. **The run for `b809237`, read from the public jobs API: unit SUCCESS, gates SUCCESS, integration SUCCESS — the first fully green run since 13 Sep.** Docs: state §0.145, §9 (both CI items struck), the header; guide §18b43 — in the code commit; this handoff last.

**Eighteenth session, the CI read (15 September 2026, Jeff: "tell me how to get you the not ok info you need" — then, signed in to GitHub with the unit job’s log open, the first `not ok` line) — CLOSE, half of it.** The `unit` job’s ONLY failure, red on every run since 13 Sep: `not ok 130 - the April commit itself is flagged in both files it touched …` → `Command failed: git show bf4a3c5:netlify/functions/pipeline-alerts.mjs` / `fatal: invalid object name 'bf4a3c5'`. `tests/check-fnscope.test.mjs` reads the April commit out of git as its regression fixture; `actions/checkout@v4` clones ONE commit, so the object never existed on the runner — and every machine with the history passed, which is why nothing reproduced it (Node 22, UTC, bare `node --test` all green here). The other two `not ok` hits are a passing test whose NAME contains "NOT ok". **§0.144, `5df4736`** (the workflow and one test file; no function, no bundle): the `unit` job checks out with `fetch-depth: 0` so the fixture proof RUNS in CI; the test `git cat-file -e`s the commit first and `t.skip`s in words without it (proved in a `--depth 1` clone: 5 pass, 1 skipped, 0 fail). Pushed 22:55:32 UTC; **the run for `5df4736` read from the public jobs API: `unit` SUCCESS, `gates` SUCCESS, `integration` FAILURE at "Org-isolation integration tests (real Neon test branch)"** — the first green `unit` job since 13 Sep. **The `integration` job’s cause is NOT read** — the suites do not touch git, they set their own `SETTINGS_ENCRYPTION_KEY` default, `.env` has none either (so local == CI there), and `test:int` passes here 170/170 against `DATABASE_URL_TEST`; whether the CI secret points at the SAME database as this machine’s `.env` is unknown from here. Jeff was asked for that job’s first `not ok` line (§5). Docs: state §0.144, §9, the header — in the code commit; this handoff last.

**Eighteenth session, continued once more (15 September 2026, Jeff, with the Audit log’s last 24 hours on screen: "this is last 24 hours" — then "change audit so that it shows all the things that you think it should") — CLOSE.** Read against the tree first: THIRTEEN endpoint files wrote audit rows (`accounts`, `activities`, `contacts`, `leads`, `opportunities`, `tasks` — their `*.cleared` mass-deletes and the bulk stage move; `users`, `user-role`, `settings`, `audit-stream`, `coaching-notes`, `integration-requests`, `lead-requests`; `audit-log` and `users-sync` write their own rows) — seventeen action words in all — and THIRTY-ONE functions that insert, update, delete, send or hand data to a model wrote NONE: saved reports and their deliveries, Claude’s readings and the deal scorer, quotes and quote emails, products, automations, webhooks (a secret rotation), API keys, documents (seven paths), the dashboard, DSRs, export runs and schedules, SPIFF claims, calendar disconnects, dismissed recommendations, backups (a RESTORE), merges and their undo, invites (the UI’s warn-list named `user.invited`; nothing wrote it), web-form leads, Train now, and all of dispatch. **§0.143, `d48b3f6`** (functions AND bundle; no schema change): `_lib.auditAs(orgId, clerkUserId, fields)` resolves the caller’s name (never throwing into the write) and every one of the thirty-one calls it after its write, with a word for what happened (the per-file vocabulary is in state §0.143 — `quote.submitted`, `webhook.secret_rotated`, `backup.restored`, `account.merged`, `dispatch_equipment.checked_out`, `lead.received` as `'Web form'`, the job’s `report_delivery.sent` for SCHEDULED sends only), a detail that never carries a secret (a webhook’s HOST, a key’s PREFIX, a merge’s undo id, a status as `from → to`), a delete’s name read before the delete; `AuditDetail.jsx`’s `mapEntityTypeToCat` knows every new type. **The guard: `tests/audit-coverage.test.mjs`** — every `netlify/functions/*.mjs` that calls `.insert(` / `.update(` / `.delete(` must call `auditAs(` or `writeAudit(`, or sit in `EXEMPT` with a reason (the log itself, the three heartbeat jobs, Resend’s inbound, the rules engine, `auth`, `crypto`, `public-api`’s usage stamp, `calendar-oauth-start`, `users-sync`); four that write no table must audit anyway. **`check:fnscope` caught the batch’s own bug** — `report-prompt.mjs`’s restructure declared `body` twice; the response is `reply`. Guide §18b42 (hard rule). +6 tests (854), 6 mutants; `tests/report-prompt.test.mjs` repointed. Six gates (171 files; 96 function files), **854/854 unit, 170/170 integration, 552/552 mutations after a printed green baseline**, build guard OK 2,551 kB `index-D5O2AGMO.js`, `dist/` cleared. **Not observed in a browser this batch** (a row writes on the next action of each kind; §5 says what to look for). **NOT shipped; pushed to dev 22:42:35 UTC 15 Sep (`ddac313..d48b3f6 dev -> dev`); LANDED — accelerep.netlify.app served `index-D5O2AGMO.js` from 22:43:23 UTC 15 Sep (the hash poll). OBSERVED by Jeff on dev (his screenshot of the Audit log: saved report created and scheduled, the Claude reading under security, quote created and updated, two dispatch_job.updated beside the client’s dispatch.schedule.override — one PUT per builder step; every Actor "Jeff Russell"). One finding from the same screenshot: the client’s `addAudit` still posts the bare words `update` / `create` / `delete` for contacts, deals and tasks (state §9, added).** The three earlier flags stand (§5): the CI `not ok` line, the site key, the held §0.138 / prod-Jobs-tile checks. Docs: state §0.143, the header, §9; guide §18b42 — in the code commit; this handoff last.

**Eighteenth session, the ship and after (15 September 2026, Jeff: "ship dev" — then "I have scheduled reports to be emailed and they do not seem to be being sent" — then, with a screenshot of the Start-quote picker: "It looks to me like all the templates are fake place holders … remove the fake ones and just add a place holder … and have that go away if actual templates are available") — CLOSE.** **PROD SHIPPED — the TWENTY-FIFTH ship.** Pre-flight: tree `deno.lock` only; `dev` == `origin/dev` == `930e276`; `origin/master` `a4f4589` an ancestor; `git diff origin/master..dev -- db/schema.ts` EMPTY; 13 commits, SIX code (`e955801` §0.138, `e302346` §0.139, `a83be59` §0.140, `5ea58b8`, `ac22b82` §0.141, `1482fc5`), 43 files — `netlify.toml`, seven under `netlify/functions/` (`_aiKey`, `_reportPromptShape`, `report-prompt` new; `ai-score`, `report-deliveries`, `settings`), src, tests, scripts, docs. `git push origin dev:master` at 21:10:08 UTC: `master` `a4f4589` → `930e276`. salespipelinetracker.com served `index-DfRbwLhy.js` from 21:10:47 UTC (the hash poll, 39 s after the push; before it `index-zv28zhMt.js`), 2,610,617 B, `pk_live_` ×2 and `pk_test_` ×1 (the SDK constant), the batch's literals present ("Claude reads report prompts", "ASKING CLAUDE", "Built-in interpreter", "Custom range"); GET-less POST to `/.netlify/functions/report-prompt` answers 401 (deployed, behind Clerk). Netlify's record (deploy `6aa9b434ce5f0200086b1ffa`, branch master, context production, commit `930e276`, published 21:10:43 UTC, deploy time 29 s): **86 functions** (83 before — `_aiKey`, `_reportPromptShape`, `report-prompt` new), `function_schedules` with **`report-deliveries` at `*/5 * * * *`**, 3 redirect rules, 1 header rule, secret scan clean over 463 files, "2 new files uploaded". Prod's heartbeat read after: the job ran on the new cron at 21:15, 21:20, 21:25 UTC (ok, 9 reports read, 0 due) — every five minutes, as designed. **NOT yet observed on prod by Jeff** — his §5 list. **Then Jeff: the scheduled reports "do not seem to be being sent."** Read from the database, read-only, every `saved_reports.config.delivery`: FOUR schedules. Jeff's "Calls & Meetings Per Rep This Month" (Accelerep Test, daily 15:50 Chicago) was SAVED at 15:48 Chicago; Karen's "Deals stuck for 14 or more days" (daily 13:50) at 13:46 — each minutes before its own time, under the HOURLY job then on prod (fires on the hour, `clock.hour === d.hour`): the next run (16:00, 14:00) was not their hour, and the every-five-minutes cron landed at 16:10 Chicago, after both windows. Nothing was wrong; they fire tomorrow within five minutes of their time. The third, in Jeff's other org (`org_3Dwnyt…`, daily 12:00), carried `lastDeliveredAt` 16:29 UTC = 11:29 Chicago — a **Send now** — and the 12:00 run 31 minutes later said "already delivered this window" (the 20-hour daily gap) and skipped it: a design flaw. The fourth is off. → **§0.140's correction, in `4d6b0f5`** (functions AND bundle): two stamps — `lastSentAt` (any send; the dialog's "Last sent") and `lastDeliveredAt` (a SCHEDULED send only; the dedup key) — `cleanDelivery` allowlists both, the endpoint carries both across a client save, the integration test asserts a Send now leaves `lastDeliveredAt` null, one mutant. **And §0.142, the same commit** (bundle): Jeff was right twice — `TemplatePickerModal` drew four hard-coded cards ("SMB Starter" … with INVENTED win rates and products sliced from any catalog) and the parent ignored the pick (every card made the same blank quote); the org's template library (Settings → Quoting → Quote templates) stores a name and a description and NO line items, and its own fallback is four more invented entries an Admin's Save persists (state §9). Now `src/utils/quoteTemplates.js` (pure): `usableQuoteTemplates()` offers a saved template only when its product ids resolve in the catalog (none can today), `templateLineItems()` seeds the configurator's own shape, and the modal shows ONE placeholder — "No quote templates yet. When an Admin builds templates with line items under Settings → Quoting → Quote templates, they appear here and a pick starts the quote with them. For now, start from scratch below." — plus Start from scratch; the pick is honoured (`handleNewQuoteForOpp(oppId, template)`). +1 test file (3), the delivery tests widened, 4 mutants. Six gates (171 files; 96 function files), **848/848 unit, 170/170 integration, 546/546 mutations after a printed green baseline**, build guard OK 2,550 kB `index-uCWPULHc.js`, `dist/` cleared. **OBSERVED in the pane (localhost:8888, Jeff — Admin):** Quotes → Start quote on Heartland Freight LLC → the modal: "Pick a template to begin", the one placeholder card in the words above, "+ Start from scratch", Cancel / Create quote — none of "SMB Starter", "Growth Package", "Trial → Paid", "avg win rate" in the DOM (a script checked). The Send-now stamp was not driven in the pane (the integration test is the proof). **NOT shipped; pushed to dev 21:37:51 UTC 15 Sep (`930e276..4d6b0f5 dev -> dev`); LANDED — accelerep.netlify.app served `index-uCWPULHc.js` from 21:38:31 UTC (the hash poll, 40 s after the push). NOT yet observed by Jeff on dev.** Docs: state §0.142, the §0.140 correction, the header and §9, in the code commit; the state doc's ship record in the same commit; this handoff last.

**Eighteenth session, continued further (15 September 2026, Jeff: "would you spin up a local host instance again" — then "I added the claude key and turned on BYOK") — CLOSE.** The local instance (`netlify dev`, caches cleared first) was up within a minute; Jeff installed a BYOK key on the Accelerep Test org through Settings → Features & AI. **OBSERVED in the pane — Claude Opus 5 READING PROMPTS, three times, 10–20 s each, `usingOrgKey: true`:** (1) "Who is closing the most this quarter" (a paraphrase the built-in reader cannot do) → READ BY · Claude · Opportunities · Closed won · Owner · # of deals + Revenue · Bar · Q4, drawn: Jeff Russell 2, Karen Russell 2, no notes. (2) "What's slipping? Deals in Negotiation/Review nobody has touched in 3 weeks, and our win rate" → the note "Win rate cannot be expressed with the available metrics, so it was left out" and the title "Stalled Negotiation/Review deals (no activity 21+ days)" — but NO filters, 20 of 20 deals. (3) "Deals in Negotiation/Review nobody has touched in 3 weeks" → Days since last activity ≥ 21 · Open deals, 7 of 20 — and "Not a filter on Opportunities, left out: stage". **Two findings, read from a new function-log line (the model's tool input BEFORE the allowlists), both fixed → `1482fc5`** (functions AND bundle): (a) the required-but-empty `from` / `to` strings came back filled with stray parameter text, and in (2) the whole `where` list had spilled into `to` — the schema now asks for the word `none` when there is no date, the system prompt says so and adds "a condition that is only in the name is lost", `validateReading` reads anything but a date as no bound; (b) **the engine's text filters were built from kind-`text` dimensions only and Stage is kind `stage` — so the `stage` filter the built-in interpreter emits for "in Proposal" had been DROPPED by `resolveWhere` since §0.139** (its test checked the definition, not the run; state §0.139 carries the correction) — `textFilters` includes kind `stage`, a test runs the interpreter's own stage filter through the engine, one mutant. Re-read after the fixes: prompt (2) again → Stage is Negotiation/Review · Days since last activity ≥ 21 · Open deals · # of deals · Revenue · KPI card · All time, the win-rate note, drawn 2 of 20 / $142K; the logged model input `from: "none", to: "none"` with the three filters. Six gates (170 files; 96 function files), **845/845 unit, 170/170 integration, 542/542 mutations after a printed green baseline**, build guard OK 2,549 kB `index-Be4l2aWO.js`, `dist/` cleared. Pane side-note: the rail's text box does not take ctrl+a from the pane, so a re-typed prompt lands mid-sentence (Claude read the doubled sentence correctly anyway); the picker's box is the clean way in. **NOT shipped; pushed to dev 21:02:14 UTC 15 Sep (`84e711f..1482fc5 dev -> dev`); LANDED — accelerep.netlify.app served `index-Be4l2aWO.js` from 21:02:54 UTC (the hash poll, 40 s after the push). Claude's reading is observed on LOCAL with Jeff's key; the dev SITE still carries the rejected site key unless the BYOK key was installed on the dev site's org too (it was installed through localhost — the same database, the same org row, so it IS on dev).** Docs: state §0.141 (the observations, both findings) and the §0.139 correction, in the code commit; this handoff last. The session's earlier entries follow.

**Eighteenth session, continued (15 September 2026, Jeff: "can we make it where a user chooses claude or deterministic? Or more specifically if Claude is enabled by the admin can we have it use Claude as a default and if Claude isnt avaialable use deterministic (and have a note that AI assistance is avaialble if admin turns Claude on)" — and mid-work: "you used the old time picker I hate in the scheduled delivery of reports. PLease update this") — CLOSE.** **Read against the code before building:** the AI settings surface — `aiScoringEnabled` gates deal scoring but NO panel sets it (nothing in the app can turn scoring on; §9); `ai-score.mjs` carried its own copy of the key decrypt; `settings.mjs` imports `encrypt`/`decrypt` from `crypto.mjs`; the house time control is `src/components/ui/TimeDropdown.jsx` (a portaled listbox, type-to-search, `stepMinutes`) whose header says it replaces the native input everywhere — §0.140's first cut had used `<input type="time">`. → **§0.141 `ac22b82`** (functions AND bundle; no schema change): the switch `settings.extra.aiReportPromptsEnabled` (both halves of `settings.mjs`, a client default, an Admin's control "Claude reads report prompts" in Settings → Features → AI · Model & access, off by default, its caption saying what leaves the app and with which key); `_aiKey.mjs` (the org's BYOK key else the site's — `ai-score.mjs` on it too); `report-prompt.mjs` with the pure `_reportPromptShape.mjs` (auth → the switch, checked BEFORE any key is read → a key; the org's settings row and roster only, no CRM table; Claude Opus 5 at low effort, raw fetch like `ai-score.mjs`, one strict tool `set_report` whose schema is the builder's vocabulary, the org's stage and roster names and the interpreter's own reading rules in the system prompt; the answer through `resolveDefinition` / `resolveWhere`, every dropped id named; a refusal, a non-tool answer or an API error each an `unavailable` reason); `reportPrompt.js` gains `understoodFor(definition)` (the chips describe the definition whichever reader made it — the built-in reader uses it too) and `promptVocabulary()`; `applyPrompt` async — Claude first when the switch is on ("ASKING CLAUDE…", one at a time), the built-in reader for every other answer with a note in words (off → "AI assistance is available: an Admin can turn "Claude reads report prompts" on under Settings → Features → AI"; no key; a rejected key; unreachable), the banner's first chip READ BY · Claude / Built-in interpreter; the delivery dialog's At on `TimeDropdown` at five-minute steps (the job's cadence), the native input gone from ReportsTab. +6 tests (18), 7 mutants; the report-delivery and history-feed pins repointed. Six gates (170 files; 96 function files), **845/845 unit, 170/170 integration, 541/541 mutations after a printed green baseline**, build guard OK 2,549 kB `index-DBD7v9I0.js`, `dist/` cleared. **OBSERVED in the pane (localhost:8888, Jeff — Admin, the Accelerep Test org):** Settings → Features & AI → the new switch under "AI · Model & access" → ON → its caption "On · the report builder's "Ask AI" sends the prompt sentence (never your records) to Claude with the site key (no BYOK key installed); the built-in reader takes over if Claude cannot answer." → Save changes (PUT 200); Reports → Saved reports → + Create report → the starter "How many leads by source this quarter" → GENERATE → **the POST to `/report-prompt` went out (the switch persisted) and answered `{ unavailable: true, reason: 'error', status: 401 }` — `netlify dev` injects the DEV SITE's `ANTHROPIC_API_KEY` and Anthropic answered `authentication_error: invalid x-api-key`: the dev site's site key is not a working key.** The fallback did its job: READ BY · Built-in interpreter · Leads · Lead source · # of leads · Bar · Q4, seven groups drawn, and a note — which, after the follow-up in the same commit, reads "Claude rejected the Anthropic key this workspace uses (invalid or expired) — an Admin can replace it under Settings → Features → AI (BYOK)". **Claude's own reading was therefore NOT observed — no valid key was available to the pane; the function's contract is pinned by the tests and its answer is validated, but the first real reading is Jeff's, with a key.** Then Deliver on his "Calls & Meetings Per Rep This Month" (a report HE had already built from a prompt and saved on dev — Activity · Rep · # of activities · Type is one of Call, Meeting · a custom range — the prompt builder in his own hands) → the dialog's At is the house dropdown ("8:00 AM", no `<input type="time">` in the DOM) → opened: 288 options at five-minute steps → 9:15 AM picked → the trigger reads "9:15 AM" → Delivery on UNTICKED (a new schedule defaults to on; prod's job runs against this database) → Save schedule → the PUT 200 row `hour: 9, minute: 15, enabled: false`. Also seen: Karen's shared report already carries "⏱ Every day · 13:50 America/Chicago · email to 1" — a minute-level schedule Jeff set on dev today, live on prod from the ship. **NOT shipped; pushed to dev 19:21:15 UTC 15 Sep (`9760d0e..ac22b82 dev -> dev`); LANDED — accelerep.netlify.app served `index-DBD7v9I0.js` from 19:21:54 UTC (the hash poll, 39 s after the push). NOT yet observed by Jeff on dev.** Docs: state §0.141 (with the observation), the §0.140 correction, the header and §9, in the code commit; this handoff last. The session's earlier entry follows.

**Eighteenth session (15 September 2026, Jeff: "please do our normal start ritual" — then, with two screenshots: "I dont think the AI report builder is working correctly. No matter what I type in it seems to want to solve for days stuck for more than 14 days (its sample statement)" and "On scheduling reports can we make the AT a selectable time where users can include any time they want") — CLOSE.** The ritual passed (tree: `deno.lock` only, the CLI's two lines; `dev` == `origin/dev` == `052d3df`, `origin/master` `a4f4589` behind by the ship-record docs, ONE code commit `e955801` and the handoffs — exactly the §5 expectation; every fingerprint present, copies identical; dev serving `index-DIxtA4Km.js`, prod `index-zv28zhMt.js`, both as recorded). **Read against the code before building:** (1) the "AI-generated" create mode was ONE hard-coded stuck-deals picture — an inline computation, seven fixed chips, a fixed header, and a banner that said "The report builder does not interpret prompts yet … your prompt becomes the report's name"; Jeff's "Quota Performance By Rep" became that chart's title; and the engine (§0.133) had NO row filters, so the one report the stub drew could not be built by hand either. (2) The delivery dialog's At was a `<select>` of 24 hours, the schedule stored `hour` only, the job ran hourly and fired on `clock.hour === d.hour`. → **§0.139 `e302346`** (functions AND bundle; no schema change): `src/utils/reportPrompt.js` (pure — `interpretPrompt(text, { today, fiscalStart, stages, people })`: the source from its nouns; what cannot be done — win rate, forecast accuracy, deal tier, competitor, quota, weighted, a span of quarters — NAMED in `notes` and consumed; the period in the org's fiscal calendar (this / last quarter, this / last fiscal year, Q1–Q4, last N days / weeks / months, this / last month / week, today — the custom ones as from / to); the chart; the row filters (stuck / stale / silent … N days, 14 when none, a stuck deal is open; in stage N days; over / under $N; won / lost / open; converted / dead / unassigned / hot leads; calls and meetings as a type list); the measures; the dimensions in sentence order; the org's stage names and roster names as equality filters only when handed in), `PROMPT_STARTERS` the interpreter reads in full; `reportQuery.js` gains `where` — `filtersFor(source)` the allowlist (status, days since activity, days in stage, deal size, every text dimension; leads add score), `resolveWhere()` drops and names the rest, `whereLabel()` in words, `from` / `to` for the existing custom period; `ReportsTab`: the fake mode deleted (176 lines), `applyPrompt()` seeds the ONE builder and runs the engine, `PromptBanner` and `WhereEditor` at module scope, the Filters tab edits the filters and a custom range, saved at `filters.where/from/to`, reopened with them, dropped on a source change; Home and `report-deliveries.mjs` run the whole definition. +1 test file (12), three §0.133 pins and one §0.135 pin repointed, the history-feed scan pins the admission's absence; 11 mutants; one §0.133 anchor STALE after a rename, repointed. Guide **§18b41** (a box that says AI reads its prompt; its header lagged at §18b38 — bumped), state §0.139 and §9 (a Claude-backed reading as a next step, Jeff's call). Six gates, **838/838 unit, 170/170 integration, 529/529 mutations after a printed green baseline**, build guard OK 2,545 kB `index-GQgZt6NE.js`. → **§0.140 `a83be59`** (functions AND bundle AND `netlify.toml`; no schema change): `minute` on the allowlisted schedule (a stored one reads as :00), `localClock` to the minute, `deliveryDue` due from the scheduled minute for a FIFTEEN-minute window — not before, not the rest of the day (a schedule saved at 15:00 for 08:30 does not fire at 15:05; the hourly rule had that by accident) — then the day and the cadence gap as before; the job at `*/5 * * * *` (the Jobs registry the same, 300 000 ms); the dialog's At an `<input type="time">` set as hour and minute together, "at the time you pick". Tests widened (+1 due test: the window's edges, the midnight edge, the reason in words), the integration allowlist pin carries the minute (its first failure cascaded into the Send-now test through a report left renamed — one fix, both green), 5 mutants. Six gates, **839/839 unit, 170/170 integration, 534/534 mutations after a printed green baseline**, build guard OK 2,546 kB `index-B5zf14oE.js`. **OBSERVED in the pane (localhost:8888, signed in as Jeff — Admin; the Accelerep Test org `org_3BDQ…`; the viewport set to 1400×900 for the pass):** Reports → Saved reports → + Create report → the starter chip "Closed won revenue by industry this year" → GENERATE → the banner "Your prompt … Read as the report below" with SOURCE Opportunities · PERIOD This fiscal year · FILTER Closed won · GROUP Industry · MEASURE Revenue · CHART Bar, the breadcrumb AI-GENERATED, the REAL query drawn (None $650K, Transportation $210K, Food & Bev $95K; "4 of 20 opportunities in the period and the filters · 3 groups"); the Filters tab: "Only rows where · Closed won ×", the add row (Status / Days since last activity / Days in current stage / Deal size / Owner / Industry / Territory / Forecast category / Loss reason / Product line, the choice select Open deals / Closed won / Closed lost, + Add), the period select with "Custom range…"; the rail's Ask AI → "Deals stuck more than 14 days, by rep" → GO → FILTER Days since last activity ≥ 14 · FILTER Open deals · GROUP Owner · MEASURE # of deals · MEASURE Revenue · CHART Bar · PERIOD All time, drawn: Karen Russell 5, Jeff Russell 1, ZZFX Other Rep 1 — "7 of 20"; Cancel → the library → Deliver on "Untitled report" → the dialog with a time input (08:00 AM shown), "Any time, to the minute — it goes out within about 5 minutes of it." → set 08:30 → Save schedule → "Schedule saved, delivery off." → the PUT 200 whose row reads `hour: 8, minute: 30, enabled: false`, the `lastDeliveredAt` stamp kept (delivery was OFF before and is OFF after — prod's job runs against the same database and must not mail Jeff from a pane test). **A finding on the way → the follow-up `5ea58b8`** (bundle only): the picker's box and the rail's box share `aiPrompt`, so the second prompt typed in the rail landed INSIDE the first sentence ("Closed won revenue by iDeals stuck more than 14 days, by repndustry this year" — read, faithfully, as won + silent + revenue, a KPI card); now `applyPrompt` clears the box and "Edit prompt" restores the sentence; two scans; six gates, 839/839, 534/534 after a green baseline, build guard OK 2,546 kB `index-B0ZTf3LY.js`, `dist/` cleared. NOT observed in the pane: a filter ADDED through the editor and Update preview (the chips and the allowlist were read; the add row's inputs were not driven — the scan and the engine test are the proof); the custom range's date inputs; the dialog's "Save & send now" (would mail Jeff); the rep path as Karen (the pane holds Jeff's session). Pane side-notes, not this batch: the first load's twelve 401s (contacts, tasks, activities, leads) came while `netlify dev` was still warming up — its log read "Auth verification error:" four times at that moment — and the next load was clean; the pane's screenshots timed out repeatedly (text tools used instead); the Task Due Today reminder pops on the Reports tab and takes the first click. **NOT shipped; pushed to dev 18:30:57 UTC 15 Sep (`052d3df..5ea58b8 dev -> dev`); LANDED — accelerep.netlify.app served `index-B0ZTf3LY.js` from 18:31:36 UTC (the hash poll, 39 s after the push). NOT yet observed by Jeff on dev.** Docs: state §0.139 (with the follow-up's paragraph), §0.140, the header and §9, guide §18b41 and its header, in the code commits; this handoff last. **Jeff's §5 checks from the last session are HELD, not done (his ask: "Hold those last checks and remind me")** — they are the first block of §5 below.

**Seventeenth session, after the ship (15 September 2026, Jeff: "are the 140 off-brand literals in 18 other files from the new design I proposed" — no, the original Tailwind styling — then "fix the 140 off-brand literals before I start a new conversation then") — CLOSE.** **Read first:** the audit's remaining table (140 in 18 files) and the files themselves: `useSettings.js`'s sixteen are stored DEFAULTS (the pipeline colour, the KPI tolerance colours, the block types) — data, not styling — and the other files carry, beside the Tailwind values, the warm palette's OWN literals and array palettes (chart series, stage maps) that were not the ask. → **§0.138 `e955801`** (bundle only): the same script, fifteen more files, three rules added and kept — a literal inside an array is a palette and is never rewritten; in these files the warm palette's own values are left (a white → surface swap in a report card is a visible change nobody asked for); any binding named `T` counts as the import (the documents surfaces import it from `./atoms`; the first run added a second import and the script's own re-parse refused it — fixed, re-run) — and four Tailwind values the map lacked (the purple `#7c3aed`/`#6b21a8` by role, the wash `#f3e8ff`, the greens `#4ade80`/`#bbf7d0`). 126 replacements; imports added to ViewingBar, QuotaRepCard, QuickLogFab, ErrorBoundary and the two merge modals. The audit regenerated: **off-brand 140 → 31 tree-wide, 22 → 3 files, and the 31 are the three data palettes** (useSettings 16, App.jsx 11, ModalLayer 4) — from the audit's first count on 13 Sep, 542 → 31. +1 scan (`tests/blue-era-sweep.test.mjs`: the fifteen files bind `T` and carry no Tailwind value outside a palette array), 2 mutants. Six gates, **826/826 unit, 518/518 mutations after a printed green baseline**, build guard OK 2,533 kB `index-DIxtA4Km.js`, `dist/` cleared; the integration suite not re-run (nothing server-side changed). **NOT observed in a browser** — the pane's session is signed out (signing in is Jeff's); the scan and the audit are the proof until he looks. **NOT shipped; pushed to dev 16:49:31 UTC 15 Sep (`04b33e1..ecb535a dev -> dev`); LANDED — accelerep.netlify.app served `index-DIxtA4Km.js` from 16:50:02 UTC (the hash poll, 31 s after the push). NOT yet observed by Jeff on dev.** Docs: state §0.138, its header and §9, the audit regenerated, in the code commit; this handoff last.
...
**Seventeenth session, the last fix (15 September 2026, Jeff, with a screenshot of Home: "one small last fix before changing conversations … make adjustments so all sales stages can be displayed in this on your plate section without bleeding in to the activity") — CLOSE.** **Read first:** `PlateRow` in HomeTab — a 68px label column with `{amount} · {stage}` in one unguarded div; "Negotiation/Review" wrapped under the amount and, one unbreakable word, drew across the title column. → **§0.137 `9bce063`** (bundle only): the column is 136px (the longest stage fits on one line), the amount and the stage are two lines, the stage in its stage colour, both clipped with an ellipsis and the full text on hover, the title column still shrinking. +1 scan (`tests/plate-row.test.mjs`), no mutant, the harness not re-run. Six gates, **825/825 unit**, build guard OK 2,533 kB `index-oOwpiwjs.js`, `dist/` cleared. The state doc's §0.137 and header rode a docs commit after it (`f618eb5`) — the code commit went in before the doc script's last anchor failed (it named a sentence that lives in THIS file, not the state doc); recorded, not hidden. **NOT observed in a browser:** the pane's session had signed out overnight and the sign-in screen showed instead (which itself read as §0.136 says — the mark `#2a2622`, its line `#4d6b3d`, the subtitle in inkMuted, the page on `T.bg`); signing in is Jeff's. **NOT shipped; pushed to dev 14:03:42 UTC 15 Sep (`e2dde30..5cee871 dev -> dev`); LANDED — accelerep.netlify.app served `index-oOwpiwjs.js` from 14:04:35 UTC (the hash poll, 53 s after the push). NOT yet observed by Jeff on dev.** Docs: state §0.137 and its header; this handoff last.

**Seventeenth session, continued again (14 September 2026, Jeff: "this is another item on our fix list … The blue-era modals. Yesterday's audit put numbers on this: ModalLayer, LeadImportModal, App.jsx, OutlookImportModal, ContactModal and UserModal carry roughly 400 off-brand Tailwind colours … one job. It is independent of the paused signal migration.") — CLOSE.** **Read against the code before building:** the audit's §4 table (542 off-brand uses in 22 files; 417 in the six) and the literals themselves — slate greys, `#2563eb` on buttons AND on links/borders (the same hex, two meanings), Tailwind washes, and three DATA palettes that are not styling (a pipeline's stored default colour, the stage pill map `getStageColor` keys on, the avatar initials map); the guide's allowed `#1c1917` header; `CsvImportModal`'s chrome as the target; the legacy `.modal` CSS class still painting white under the confirm dialogs. → **§0.136 `f0705c0`** (bundle only; no function, no schema change): **`scripts/migrate-blue-era.mjs`** (kept as the record — @babel/parser over each file; every hex literal, exact / in a string / a template quasi / a JSX attribute / an 8-digit hex with its own alpha, rewritten to the token it means; the BLUES by ROLE from the literal's context — a background is the ink button, a text/border/outline is `T.info`; the washes to a token with an alpha; the import added; the data palettes kept BY WHAT THE LINE SAYS) — 549 literals across the six files; by hand the four named whites, `.modal`'s background, `OutlookImportModal`'s one CSS var, and `LeadImportModal`'s shell onto `CsvImportModal`'s (the dark drag-handle band, radius 12, a border, "Import Leads from CSV"). The audit regenerated: 542 → 140 tree-wide, the six 417 → 15 (all kept palettes). +1 unit file (5), 3 mutants; `tests/fetch-status.test.mjs`'s banner pin repointed. Six gates (169 files; 93 function files), **824/824 unit, 516/516 mutations after a printed green baseline (no restore residue)**, build guard OK 2,532 kB `index-5_oMz9NV.js`, `dist/` cleared; the integration suite NOT re-run (nothing server-side changed; last 170/170). **A finding on the way, recorded in state §0.136:** the script's first exclusion list was BY LINE NUMBER; the second pass (for the 8-digit hexes, after the import had shifted every line by one) converted half the avatar palette and a stored pipeline colour — caught in the dry run, both files restored from git, the exclusions rewritten by content, re-run clean. **OBSERVED in the pane (localhost:8888, Jeff, Admin; computed styles read through the console — the pane's canvas is narrower than these modals, so screenshots clipped):** Leads → Import → "Import Leads from CSV" on the dark band (rgb 28,25,23) with the title in the surface colour, Cancel on surface2 with mid ink; a lead's trash icon → the confirm dialog: `.modal` on the surface (251,248,243), the Delete button `T.danger` (156,58,46), its icon wash the 8% danger tint, Cancel on the surface with the border token — Cancelled, nothing deleted; Contacts → New contact → "Create Contact" on the ink button (42,38,34), no inline hex left in the DOM. NOT observed: UserModal and OutlookImportModal opened (the same map, scan-tested), the sign-in and no-org screens (the pane holds a session). **Then Jeff: "push to dev" → pushed 22:25:11 UTC (`367cc3d..2f93068 dev -> dev`); LANDED — accelerep.netlify.app served `index-5_oMz9NV.js` from 22:25:42 UTC (the hash poll, 31 s after the push). OBSERVED by Jeff on dev (14 Sep): "everything testing correctly" — §0.135 and §0.136 both. NOT shipped** — `dev` ahead of `origin/master` by FOUR code commits `9fb744a`, `5bbd813` (§0.134), `62dc20a` (§0.135) and `f0705c0` (§0.136) and the docs. Docs: state §0.136, its header and §9, guide §16, the audit regenerated, in the code commit; this handoff last.

**Seventeenth session, continued (14 September 2026, Jeff: "Lets tackle this task next. Saved reports: sharing, scheduled email or Slack delivery, pin to dashboard." and "when I was in the Tasks tab and I had selected all and all time - I navigated away and came back and it had reset to all and today. Aren't the tabs supposed to maintain your last selections until they are changed? Same thing is happening on the Dispatch tab.") — CLOSE.** **Read against the code before building:** `saved_reports.is_shared` existed, was stored, and gated NOTHING — the GET returned every row in the org (its comment said "own + shared"); the PUT was the CLAUDE.md sanitize shape a THIRD time (`sanitize({ ...data, orgId, ownerId: userId })` — a partial body would have wiped the definition and handed the report to the caller); nothing scheduled anything (`export_schedules` has no runner; the four cron jobs are the digest, alerts, reminders, scoring); the library's "Pinned" is four hard-coded tiles; and the tabs persist only what opts in by name (`tab:<tab>:<key>` — scope and sub-view), so the Tasks range pill and the Dispatch board range were plain state and reset on every return. → **§0.135 `62dc20a`** (functions AND bundle, netlify.toml; NO schema change): `src/utils/reportDelivery.js` (pure — the ALLOWLISTED schedule at `config.delivery`, due-ness in the schedule's own zone with a per-cadence dedup window, the table / text / Block Kit / escaped html a delivered report becomes); **`netlify/functions/report-deliveries.mjs`** (NEW, hourly, heartbeat-wrapped and `JOBS_ENABLED`-gated like the other four, in `SCHEDULED_JOBS` and netlify.toml — every due report run on the server with the tab's own `runReport` over the OWNER's read of the org (canSeeAll → the org; else their rows), mailed as a table to the roster members the schedule names (resolved in THAT org) and/or posted to the workspace's Slack, then stamped); `saved-reports.mjs` (GET own + shared, an Admin the org; PUT read-then-merge, the owner kept, `delivery` validated and its recipients limited to this org's roster, the job's stamps never from a body; `POST ?action=deliver&id=` = Send now through the job's own path); `_selfProfile.mjs` + `users.mjs` (`pinnedReports` on my profile — the §18b34 allowlist and the packing); `App.jsx` (`setMyProfile` into the context); `ReportsTab.jsx` (`LibraryCard` and `DeliveryDialog` at module scope; "Your reports" and "Shared with you"; Pin · Share · Deliver · × gated as the endpoint gates; a partial PUT adopts the server's row; the open request from Home); `HomeTab.jsx` (a "Pinned reports" section — the library loaded per active org, each pinned report run over what the viewer can see and drawn by `ReportChart`, "Open →" into the library); `TasksTab.jsx` `tab:tasks:range` and `DispatchTab.jsx` `tab:dispatch:boardRange` (validated on read; the anchor date deliberately not persisted). +1 unit file (11), +1 integration suite (6, mocked mailer), 13 mutants and one older anchor repointed (the card is a component now); the job-health test's four-job pins are five; §9's report-builder line — missed at §0.133 — struck; CLAUDE.md's sanitize count is three. Six gates (169 files; 93 function files), **819/819 unit, 170/170 integration, 513/513 mutations after a printed green baseline (the first run reported one STALE anchor — the old card markup — repointed, then a clean run)**, build guard OK 2,532 kB `index-ODWcjiOD.js`, `dist/` cleared. **OBSERVED in the pane (localhost:8888, signed in as Jeff — Admin; the Accelerep Test org, which held NO saved reports — a read-only query confirmed 0 rows for it; 21:05–21:15 UTC 14 Sep):** + Create report → Blank → Update preview → Save to library (POST 201) → the card under "Your reports" with **Pin · Share · Deliver · ×**; Pin → "Remove from your Home" (PUT users?me=true 200); Share → the SHARED mark and "Stop sharing" (PUT saved-reports 200); Deliver → the dialog (Delivery on, Every week, Monday, 8:00 AM, America/Chicago prefilled from the browser, four roster members with Jeff ticked, Post to the workspace's Slack channel enabled); Slack ticked → **Save & send now** → the green line "Sent to jeffrussell1@yahoo.com, Slack — 51 rows in the period." (the POST ?action=deliver 200: email [Jeff's address], slack true, errors [], the schedule stamped lastDeliveredAt 21:09:34 UTC); the card's footer "⏱ Every week · Monday 08:00 America/Chicago · email to 1 · Slack"; Home → a "Pinned reports" section with the report, its chart and "Open →" → the library with the report open in the builder (Update preview / Save to library present — it showed the "Changes are drafted" note rather than the rendered preview; flagged); Tasks → All time → Home → Tasks → All time still the active pill (`tab:tasks:range` = all); Dispatch → This week → Home → Dispatch → This week still active (`tab:dispatch:boardRange` = week, the drag strip present). Reopening Deliver showed "Last sent 9/14/2026, 4:09:34 PM"; **the schedule was then turned OFF and saved ("Schedule saved, delivery off.") so prod's hourly job — which runs against the SAME database after a ship — does not mail Jeff every Monday from a pane test.** The test report "Untitled report" (shared, pinned, delivery off) is left in the Accelerep Test org for Jeff's own checks. **Then Jeff's two screenshots (14 Sep): the EMAIL in his inbox (From "Salespipelinetracker Noreply", 4:09 PM — "Untitled report", the line "Opportunities · All time · saved by Jeff Russell · 51 rows in the period — sent now from Accelerep.", the Owner / Stage / Revenue table with the bold Total $3.8M, "… 14 more rows in the app.", the Open in Accelerep button) and the SLACK post in #sales-alerts (Accelerep Linker, 4:09 PM — the header "Untitled report", the context line, the table in a code block with Total and "… 14 more rows in the app", the Open in Accelerep button). Both rendered as designed.** NOT observed: the rep path as Karen (the pane holds Jeff's session). **Then Jeff (14 Sep): "i am ant accelerep.netlify.app - is this still local only" — it was (the site served `index-BULS4hTf.js`); "push to dev" → pushed 21:20:18 UTC (`f655023..b1ec203 dev -> dev`); LANDED — accelerep.netlify.app served `index-ODWcjiOD.js` from 21:20:59 UTC (the hash poll, 41 s after the push). NOT yet observed by Jeff on dev after the landing. NOT shipped** — `dev` ahead of `origin/master` by THREE code commits `9fb744a`, `5bbd813` (§0.134) and `62dc20a` (§0.135) and the docs; a ship is `git push origin dev:master`, his call — **and the first ship of §0.135 puts a NEW scheduled function on prod: `report-deliveries` appears in the Jobs tile as "never" until its first hour.** Docs: state §0.135 and its header and §9, CLAUDE.md, in the code commit; this handoff last.

**Seventeenth session (14 September 2026, Jeff: "claude, please start with the ritual then I would like to add the following enhancement · Drag-to-reschedule on the dispatch week grid.") — CLOSE.** The ritual passed (tree clean, `dev` == `origin/dev` == `cc42c98`, `origin/master` `c17e2e8` behind by ONLY the ship-record docs commit, both fingerprints present, copies identical). **Read against the code before building:** the "week grid" is `WeekBoardView` (the board's This week range: technician rows × seven day columns, plus a "Needs a crew" row for dated jobs with no crew); the Work Schedules grid (`ScheduleView`) is a different week and was not the ask. Every card's only gesture was a click; the Kanban has dragged since the first sessions. The dispatcher PUT merges per field (`if (f in data)`), so a partial body is safe — and `status` on it has teeth (the history, the §0.111 confirmation, the §0.116 equipment release), which set the design: the drop sends the date and/or the crew and NEVER status. → **§0.134 `9fb744a`** (bundle only; no function, no schema change): `src/utils/weekDrop.js` (pure — the crew after a drop: a seat swaps, the lead stays the lead, a crew-mate's row is a date move, the Needs-a-crew row seats a held crew; the crew builder's gates in the same words over an injected `lookup(techId)`, the WHOLE crew checked when the day moves; the partial payload), `WeekBoardView` (HTML5 drag on every card, every cell a target, a strip above the grid for the hint / Moving… / the refusal with Dismiss / the result), `handleWeekMove` in DispatchTab (the plan; the equipment gate on a new day and held units re-picked; the builder's soft blockers through the app-wide `showConfirm`; the PUT; the client row AND `jobsRaw` adopt the server's row; audit `dispatch.reschedule[.override]`). +1 unit file (12), 8 mutants; the crew-next-step pin narrowed to the builder's own segment (its "one refusal banner" counted the whole file); the state header's Verified line bumped (it had stopped at §0.128's counts through four batches — a finding). Guide §18b40 (a drop is a write path: the form's gates, only what changed, never status). Six gates (168 files; 92 function files), **808/808 unit, 164/164 integration, 500/500 mutations after a printed green baseline (no restore residue)**, build guard OK 2,515 kB `index-B4AZRwho.js`, `dist/` cleared. **OBSERVED in the pane (localhost:8888, signed in as Jeff — Admin; the Accelerep Test org `org_3BDQ…`; 19:43 UTC 14 Sep):** the strip's hint on This week; the Tue 15 card on Savannah Miller's row (JOB-2026-0007, 08:00–10:00, scheduled) dragged to Wed 16 → ONE PUT, 200, the server row `scheduledDate 2026-09-16` with the start, end, status and crew UNCHANGED (and, per §0.111, a confirmation email to the test customer's address — Jeff's own — appended to the trail); dragged back → "Moved Dispatch Customer Test to 2026-09-15." in the strip, a second PUT, the row restored (a second confirmation, the rule); onto Jax Miller's Sunday → "Jax Miller is not rostered on Sunday." with Dismiss and NO PUT; dropped on its own cell → nothing said, no PUT; onto Jax's Tuesday → the app-wide confirm "Jax Miller does not meet this job's requirements: License not set · job needs Apprentice. Move it anyway?" — whose button read **Delete** (the danger variant); Cancelled, no write. The hand-over was NOT completed in the pane; the rep path as Karen was NOT run (the pane holds Jeff's session; signing in is his — §5). → **the follow-up `5bbd813`** (bundle only): the move's confirm passes `showConfirm`'s INFO variant (`false`) — its button reads Confirm; the danger variant's reads Delete; observed reading Confirm after HMR; the scan pins the flag; state §0.134 carries the whole observation. Six gates again, 808/808 unit, build guard OK 2,515 kB `index-BULS4hTf.js`, `dist/` cleared; the harness re-run after the server was stopped: 500/500 mutations caught, printed green baseline, no restore residue. `deno.lock` carries the same two-line CLI change the twenty-second ship left uncommitted (Jeff's call) — NOT staged. **Then Jeff (14 Sep): "I am unable to drag a job" — he was on accelerep.netlify.app, which still served `index-BNgFKZx-.js`: nothing had been pushed. Jeff: "push to dev" → pushed 20:01:59 UTC (`cc42c98..67afde6 dev -> dev`); LANDED — accelerep.netlify.app served `index-BULS4hTf.js` from 20:02:28 UTC (the hash poll, 29 s after the push). **OBSERVED by Jeff on dev (14 Sep): "it works now very well. Thank you."** NOT shipped** — `dev` ahead of `origin/master` by TWO code commits `9fb744a` and `5bbd813` and the docs; a ship is `git push origin dev:master`, his call. Docs: state §0.134 and its header and §9, guide §18b40, in the code commits; this handoff last.
...
**Sixteenth session, continued once more (14 September 2026 — Jeff: "lets fix this from your list. Report builder running real queries. The preview is illustrative today.") — CLOSE.** Read against the code first: the blank builder's Update preview drew ONE fixed picture (`openPipelineByRep`) whatever the source, chips, chart or period; Period, "+ Add filter", "Compare to" and the Format tab were labels; a saved builder report could not be opened; Quotes was offered though the tab holds none; heatmap and scatter were offered though nothing drew them. The server side was already complete (`saved_reports` has `filters` and `config`; PUT exists). → **§0.133 `ed34ca9`** (bundle only; no function, no schema change): `src/utils/reportQuery.js` (pure) — `fieldsFor` (the fields each of the four sources really has), `resolveField` (id / label / legacy ids — a report saved before today opens), `runReport` (group by the dimensions, every metric per group and in total, the period through the tab's `reportPeriod` on the source's own day, rows in month / stage / metric order, a named limit), `chartData` (six charts with a NAMED fallback); `src/components/ReportChart.jsx` draws it; the builder runs it on Update, shows it as the preview, stores the engine's ids in its chips, has a real Period select and a name input, opens a saved report into itself and PUTs it back in place; the labels with nothing behind them are gone. Three things caught before the pane: the open-deal Close date through `closeDayOf` (the engine test), a form control inside the inline-declared header (`check:inline`), a UTC-truncated today (the date-local scan) — each fixed. +1 unit file `report-query.test.mjs` (12), three older scans updated, 7 mutants. Six gates (167 files), **796/796 unit, 164/164 integration, 492/492 mutations after a printed green baseline**, build guard OK 2,508 kB `index-BNgFKZx-.js`, `dist/` cleared. Pushed 18:32:13 UTC; **LANDED on dev** at 18:32:42 (the hash poll). **Observed in the pane (Jeff, Admin, `index-BNgFKZx-.js`), every number checked against a read-only query of the org's `opportunities`:** a blank report named "Pane test — revenue by owner and stage", Owner + Stage, Revenue, Stacked bar → Update preview → "20 of 20 opportunities in the period · 9 groups" — the 20 are the default pipeline's deals (the org holds 51; 31 sit in `new-biz`); the builder's Period → Q3 → "0 of 20 · 0 groups", Q4 → "5 of 20 · 3 groups", FY → "5 of 20" — the org's fiscal year starts in October, so Q4 is Jul–Sep 2026 and the db holds exactly five default-pipeline deals with a forecasted close in those months (Karen Won ×1 Jul, Jeff Lost ×1 and Won ×2 Aug, Karen Won ×1 Sep = three owner-and-stage groups) and none Apr–Jun; the TAB's own Period select (the one with Custom) is a different control and does not enter the builder. Line with Owner first → the note "A line needs a date dimension first (Close date, Created date, Date) — showing a bar." and a bar. Save to library → POST 201, ONE card; click the card → the builder reopens with the breadcrumb "Saved report", the name in the input, the same footer and note; Table → Update → the nine rows Owner / Stage / Revenue — Jeff Russell Closed Won $650K, Jeff Russell Closed Lost $464K, Karen Russell Closed Won $305K, Karen Russell Proposal $144K, Karen Russell Negotiation/Review $142K, ZZFX Other Rep Closed Won $73K, ZZFX Other Rep Evaluation (Demo) $48K, Jeff Russell Proposal $44K, Karen Russell Qualification $26K — every sum and the order identical to `sum(arr) group by sales_rep, stage`; Save → PUT 200, still ONE card; reopen → "Chart: Table" persisted; Cancel → the library. No console errors. The test card "Pane test — revenue by owner and stage" is LEFT in the dev org's library (no deletes by Claude) — Jeff's to remove. **NOT shipped** — `dev` ahead of `origin/master` (`e2cba64`) by THREE code commits `9893ddc` (§0.132, functions AND bundle), `88148ab` (the modal's error, bundle), `ed34ca9` (§0.133, bundle) and the docs; the ship his call. Docs: state §0.133 and the header in the code commit; this handoff last.

**Sixteenth session, continued further (14 September 2026 — Jeff: "lets work on this item": the three sub-items left of §9's enhancement bullet — `task.overdue`, Slack blocks on the automation action, a "Train now" for lead scoring) — CLOSE.** Read against the code first: `task.overdue` had been struck from the vocabulary in §0.124 because nothing computed it (the webhooks list still names it); the `send_slack` action posted text alone where every pipeline alert posts Block Kit; the nightly batch trains only above the org's `minClosedRecords`, so an Admin under it could never see a model. → **§0.132 `9893ddc`** (functions AND bundle; no schema change): (1) the hourly job's new Signal 7 — for every org with an active `task.overdue` rule (one read; an org with none writes nothing), each open task past its due date whose assignee is an active user of THAT org fires once per task per week at the assignee's alert hour, deduped through the task-reminders ledger, with no email/SMS/Slack of its own (the rule is the switch); the payload carries `due_date`, `priority`, `days_overdue`; the panel's Tasks group says how its two triggers fire. (2) `automationSlackMessage` (pure): the rendered message as the section (mrkdwn-escaped), a context line naming the rule, the trigger and the record, an "Open Accelerep" button; `send-slack.mjs` wraps it as `slackTemplates.automation`; the engine's `send_slack` case posts it with the rule's name and the trigger's label — a ONE-LINE call, so the §0.127 mutant that adds an alert type has one exact anchor (the first harness run reported it STALE against the two-line form; repointed, re-run). (3) `scoreOrg(orgId, { force })` extracted from the nightly batch — the gate order predictive-off → the engine's 20-row floor → the org threshold unless forced; the batch never forces — and a new Admin-only `train-lead-model.mjs` (POST, verify → Admin gate → force for the token's org, no body read) whose answer carries the gate that refused and the org's saved config; the Predictive card's "Train now" (disabled while dirty — training reads the SAVED config) adopts that config and prints `trainNowMessage`. +1 unit file `train-now.test.mjs` (4), +4 tests in `automation-events.test.mjs`, +1 integration file `train-lead-model.itest.mjs` (4; namespace `itest_train_*` — its first cut seeded a PARTIAL config and scored nothing; a saved config is the full object), +1 case in the automations itest (a `task.overdue` rule creates a follow-up linked to the overdue task's deal), 9 mutants, SUITES and `test:int` extended. Six gates (165 files; 92 function files), **782/782 unit, 164/164 integration, 484/484 mutations after a printed green baseline**, build guard OK 2,492 kB `index-DL6xNwD_.js`, `dist/` cleared. Pushed 16:45:34 UTC; **LANDED on dev** at 16:46:14 (the hash poll). **OBSERVED in the pane as Jeff (Admin), on `index-DL6xNwD_.js`:** Settings → Sales process → Lead scoring → the Predictive card carries **Train now** (enabled, its title "Train the model now on every decided lead, whatever the threshold"); clicked → `POST /.netlify/functions/train-lead-model` answered **200** and the panel printed "Needs at least 20 decided (Converted / Dead) leads to train — this workspace has 1." — the engine's floor, named; nothing stored (the dev org has one decided lead). Settings → Integrations → Automations → + New automation → the four groups, the Tasks group's note ("Task completed fires the moment a task is ticked. Task overdue is checked every hour: a rule fires once per task per week, at the assignee's alert hour, for an open task whose due date has passed.") and both triggers — "Task completed [task.completed]", "Task overdue (past its due date) [task.overdue]"; the modal closed without saving. Unauthenticated, the new function answers 401 on dev. No console error on either page. NOT observed in the pane: a Block Kit post itself (dev's Slack connection was not exercised — the integration test asserts the three blocks through the engine). **OBSERVED by Jeff on dev (14 Sep, his Slack screenshot: "created new rule, saw no name warning, used slack, and posted the bottom message"):** a new rule "Test #3" on Stage changed with a Post to Slack action; the blank-name warning on step 1; a deal moved → #sales-alerts received the Block Kit post — the section "sales rep Karen Russell:ZZFX Harborline Service moved to Negotiation/Review" (his merge fields rendered), the context line "Automation: Test #3 · Stage changed · ZZFX Harborline Service", the "Open Accelerep →" button — beneath the same event's company-switch "Stage changed" alert from the pipeline events. **NOT observed: the hourly fire itself** — `JOBS_ENABLED` is on prod alone, and a rule must exist there; a task.overdue rule on prod is Jeff's to create after a ship, and the first hourly run at the assignee's alert hour is the proof (§5). **Then Jeff: "I can't move a new automation from trigger stage to conditions" — the follow-up `88148ab` (bundle only):** reproduced in the pane first (Name blank → Next did nothing visible; with a name → step 2); the cause read against the code: the modal's only `{error && …}` render sat inside the step-4 block, so "Name is required" on step 1 never showed and Next looked dead — pre-existing since §0.124, surfaced by the new trigger; the render moved to the modal body after the step blocks (the app's callout), +1 scan, 1 mutant; 783/783 unit, 164/164 integration, 485/485 mutations, build guard OK `index-DoNj0dG6.js`; pushed 17:43:37 UTC, **LANDED on dev** 17:44:09. **OBSERVED in the pane as Jeff on `index-DoNj0dG6.js`:** New automation → Next with the Name blank → the modal stays on step 1 and "Name is required" is on screen, in the danger callout (a `2.67px rgb(156, 58, 46)` left rule measured), no console error; closed without saving. **NOT shipped** — `dev` ahead of `origin/master` (`e2cba64`) by TWO code commits `9893ddc` and `88148ab` (five function files in the first: `pipeline-alerts`, `dispatch-automations`, `send-slack`, `score-leads-batch`, `train-lead-model` new — Netlify rebuilds every function; and the bundle) and the docs; the ship his call. Docs: state §0.132 and the header in the code commit; this handoff last.

**PROD SHIPPED — the TWENTY-SECOND ship (Jeff: "push master", 14 Sep): `git push origin dev:master` at 16:01:27 UTC, `master` `ae0cae8` → `e2cba64`; salespipelinetracker.com serving `index-D0HaH5AU.js` from 16:02:06 UTC with the live key (`pk_live_` ×2 — the key and the SDK constant; `pk_test_` ×1, the SDK constant), 2,549,159 B; §0.130 (one token object), §0.131 (every tab announces itself the same way) and its gutter follow-up on prod — three code commits `10a78bc`, `4bb37ef`, `c9bf692`, all bundle only; no function, no schema change (`git diff --stat origin/master..dev -- db/schema.ts netlify/` EMPTY at pre-flight); the batch's literals in the served bundle ("Today is ", "techs available", "Workspace admin console", "My jobs", the Reports caption). Pre-flight: `dev` == `origin/dev`, `origin/master` an ancestor.** **OBSERVED on prod by Jeff (14 Sep): "All tabs look great on production."** Every "NOT shipped" in the paragraphs below for §0.130 and §0.131 is superseded by this line.

**Sixteenth session, continued further (14 September 2026 — Jeff, with a screenshot of every tab: "All the tabs have the correct look for announcing the tab ... The tabs done differently are (Dispatch, Documents, Reports (located too close to the title bar not where the rest are), Settings wrong font and script"; and Tasks: the date "moved under the Tasks label and above the 0 due today") — CLOSE.** Read first: seven tabs share one title block (28px serif italic, weight 300, then a 12px `inkMuted` caption, inside `.tab-page`); Tasks had an uppercase eyebrow above it; Dispatch put the VIEW's label as a 24px title behind a gold rule in its own bordered bar; Documents was bold sans in a 1280px centred column; Reports had the block but NO page padding (against the header band, 28px left of the others); Settings bold sans behind a gold rule. → **§0.131 `4bb37ef`** (bundle only, presentation only, five files): Tasks — title → "Today is {date}" → the counts; Dispatch — "Dispatch" is the title, the view label folds into the caption ("Mon, Sep 14 · 2 techs available · 7 jobs"; the technician's "My jobs"), the bar, rule and eyebrow gone; Documents — the shared block, "0 files · 0 KB" caption, full width; Reports — the page gutter, the title reading `T`; Settings — the shared block (the gold rule stays on the panel headings). The first cut failed three gates on a JSX comment placed between `return (` and the root element in ReportsTab — a parse error caught before any test ran; moved inside. **Then the follow-up `c9bf692`:** the first cut gave Documents and Reports `padding: 1.5rem` inline, but `.tab-page` is 0.75rem under 640px, so at the pane's phone width those two titles sat at x 32 / y 80 against every other tab's 20 / 68 (measured); both wrappers now carry `className="tab-page"` with `gap: 0`. Both commits: six gates, **774/774 unit, 159/159 integration, 475/475 mutations after a printed green baseline (twice, no restore residue)**, build guard OK 2,489 kB — `index-B_6zu6fm.js`, then `index-CyIfr4JI.js`. Pushed 15:38:42 and 15:51:46 UTC; **LANDED on dev** at 15:39:09 and 15:52:24 (the hash polls). **OBSERVED in the pane as Jeff (Admin), on `index-CyIfr4JI.js`:** a probe on each tab found the title element (the italic 28px div holding the tab's name) and read its computed style and box — Pipeline, Tasks, Dispatch, Documents, Reports, Settings ALL at x 20 / y 68, `Georgia, serif`, 28px, weight 300, the caption 12px under each; Tasks with nothing above the title and "Today is Mon, Sep 14" then "0 due today · 66 done this week" under it; Dispatch and Settings with a 0px left border on the block (the gold rule gone); Documents' page the viewport's width; no console error on any tab. Found in passing, NOT this batch's: at the pane's 279px width the document scrolls sideways on Pipeline (534px) and Reports (571px) — Pipeline was untouched, so it predates this; Documents does not (279). **OBSERVED by Jeff on dev, desktop (14 Sep): "updates to tabs looks great."** (The pane had measured a phone width only.) **NOT shipped** — `dev` ahead of `origin/master` (`ae0cae8`) by THREE code commits `10a78bc`, `4bb37ef`, `c9bf692` (all bundle only; no function, no schema change) and the docs. Docs: state §0.131 in `4bb37ef`, its follow-up paragraph in `c9bf692`; this handoff last.

**Sixteenth session, continued (14 September 2026, Jeff: "lets do that single token fix") — CLOSE.** The "single token fix" is the finding the signal-migration audit surfaced: `settings/shared/tokens.js` reached the Settings panels only; 24 files declared their own copy of `T`, `ReportsTab.jsx` EIGHT (`T`, three `T2`, `T2b`, `T2c`, `T3`, `T4`, `TS`, `T_ACTIVITY`) plus per-template `const T = TS` aliases. **Read against the code with the parser before writing:** the 14 palette colours identical in every copy; five spellings of `serif` (`"Source Serif 4"` / `"Tiempos"` first in some — neither loaded by `index.html`/`index.css` nor installed here, so every screen rendered Georgia already); `r: 4` in Dispatch alone (100 reads); four alias keys (`ink2`/`ink3`/`surface3`/`radiusMd`); and THREE reads of keys a copy never had, silently `undefined` — `T.r` in HomeTab (a bordered box, the Connect-calendar button: no radius), `T.info` inside the report templates whose `TS` had no `info` (a Call's colour falling to grey; a chip's `borderLeft: 3px solid undefined`). → **`10a78bc`** (bundle only; no function, no schema change): `src/tokens.js` is the ONE token literal (the union: `stages`, `mono`, `tint`, `surfaceInk`/`Fg`, `rSm`/`rMd`/`rLg` included); 30 files import it — Settings through `settings/shared/tokens.js`, Documents through `documents/atoms.jsx`, both re-exports; `DispatchTab` derives `const T = { ...TOKENS, r: 4 }` (the one recorded divergence); the aliases renamed to canonical names across `src/` (the seven Documents files that import from atoms included — missed by the first pass, caught by the resolve-every-read check); ReportsTab's copies and aliases gone, `T2.`/`T3.`/`TS.` → `T.`. A scratchpad script resolved every `<Name>.<key>` read to the innermost declaration in scope and compared its value with the canonical one before writing; its own first cut reported false differences (exports cut twice, `TYPE_COLORS` matched, ReportsTab scopes conflated) — script defects, fixed before a byte was written; the three `undefined` reads were the only real differences, accepted by hand and named in state §0.130. +1 test file `tests/single-token-file.test.mjs` (5: the values and exact key set by runtime import; exactly one literal in the tree; every reader imports; every read names a real key; one override), 4 mutants, SUITES; guide §18b39 (new — "One Token Object — Import It, Never Declare It") and §16 rewritten. Six gates (165 files; 91 function files), **774/774 unit, 159/159 integration, 475/475 mutations after a printed green baseline (no restore residue — the four targets checked back in place)**, build guard OK 2,489 kB `index-D0WIMRUE.js` (was 2,504 — the duplicates), `dist/` cleared. Pushed 15:02:02 UTC (with the three unpushed docs commits of the session); **LANDED on dev** — accelerep.netlify.app served `index-D0WIMRUE.js` at 15:02:45 (the hash poll). **OBSERVED in the pane as Jeff (Admin), 15:05–15:09 UTC:** Home, Reports, Dispatch, Documents, Leads, Settings each rendered on the new bundle — stone background `rgb(240,236,228)`, the ink band `rgb(42,38,34)`, Georgia on the Reports display numbers, Dispatch's radius 4 on 16 elements — and on every one of those pages `[...document.querySelectorAll('[style]')].filter(e => /undefined|NaN/.test(e.getAttribute('style')))` was EMPTY (845 styled elements on Leads, 786 on Settings); no console error on any page. NOT observed as Karen — a token refactor has no ownership or role branch; `netlify dev` would not start locally (the Neon build-hooks plugin refuses the CLI token: "Token requires extension_site_config:read permission" — unrelated, unread further). NOT observed: the three accepted fixes themselves (HomeTab's Connect-calendar button shows only with no calendar connected; the report-template Call colour and the metric chip border need that template open) — Jeff's, §5. **NOT shipped** — `dev` ahead of `origin/master` (`ae0cae8`) by ONE code commit `10a78bc` and the docs; the ship his call. Docs: state §0.130 and the header, guide §18b39/§16, in the code commit; this handoff last.

**Sixteenth session (13 September 2026, Jeff: "claude, do the ritual. I had claude design update the look of the application. I have attached the design files for your reference. I would like to make these changes.") — CLOSE, at the brief's own first gate.** The ritual passed (tree clean, `dev` == `origin/dev` == `7aec59d`, `origin/master` `ae0cae8` behind by only the two ship-record docs commits, both fingerprints present, copies identical). The attachment is Claude Design's `handoff-signal-tonal/` — the "Tonal & Form" signal system: NOT a palette swap but a FORM migration (solid deep-fill pills at radius 2, a 3px left spine on a flagged row, 1/2/3 severity dots, bare trend deltas, sparkline area fills, deep-bordered metric cards; categorical pills stay tinted and rounded on purpose), with a phased prompt of its own (README §4) whose first two phases each end in "stop for my approval". **Read against the code before building — and the brief's file list is the mock's, not this repo's:** four SELECTION spines in three colours (`ListView.jsx` 252 gold, `ContactsTab.jsx` 464 ink, `LeadsTab.jsx` 758 score band, `DispatchTab.jsx` 3126 plan tier), and the Dispatch customer list renders overdue/renewal in the selected row — the collision is live today; one severity spine already in the design's form (`DispatchTab.jsx` 809); ~60 notice boxes already tint + spine; ~20 categorical/ordinal spines, two of them severity by another name (job priority, rep health). **The token object is not shared** — `settings/shared/tokens.js` reaches the Settings panels only; 24 files declare their own `T`, `ReportsTab.jsx` eight — so the brief's "ship primitives, not tokens" is the only route to every screen. Guide §16 line 740 ("Pills: `borderRadius: 999`") contradicts the design for signal pills — flagged, NOT edited. Signals on the ink header band (`AppHeader.jsx` badges) where the ramp's deep steps would vanish — the contrast gate needs a third background. A Tailwind-family off-brand palette still in `ModalLayer`, `LeadImportModal`, `App.jsx` and five more — outside this migration (brief rule 8), recorded. → **`9f4d35c`** (docs and one script; NO src/, netlify/ or schema change; the bundle hash UNCHANGED `index-BIIXAgGT.js`): the brief copied to `docs/design/handoff-signal-tonal/`; **Phase 0 `docs/design/SPINE_COLLISION.md`** — every left spine by meaning, recommendation **A** (severity owns the 3px spine in `deep`; selection = `surface2` + a 5px INK spine — `ContactsTab` already does this), the priority mapping, the brief's §7 questions answered from the code, §7 what Jeff must decide; **Phase 1 `scripts/audit-colors.mjs` → `docs/design/color-audit.md` + `.csv`** — 5,514 colour uses in 164 files, 1,741 signal sites (ok 435 · warn 328 · danger 664 · info 314), form and a heuristic classification per line (the token names stripped before matching — the first cut counted every `T.warn` line as an event), 316 `inkMuted`-as-status, the off-brand map, the token-copy map; state §0.129 and the header. Six gates, **769/769 unit**, build guard OK, `dist/` cleared; the integration suite and the mutation harness NOT run (nothing they cover changed). **NOTHING migrated: no token, primitive or call site changed. Phase 2 waits on Jeff's answers — §5.** Not pushed at the time of writing. Docs: state §0.129 in the commit; this handoff last. **Then (14 Sep) — Jeff, shown the specimens page (https://claude.ai/code/artifact/ffafc210-7db4-4cd6-81d8-b6d69b94d5e1 — seven real surfaces, today beside the proposal, the five recommended decisions applied): "leave things as are for now. I am not sure that is the direction I want to go." The migration is ON HOLD at Phase 0/1; nothing in src/ changed; the docs, the script and the audit stay committed as the record. Do NOT start Phase 2 unless Jeff says so.**

**PROD SHIPPED — the TWENTY-FIRST ship (Jeff: "ship to master", 13 Sep): `git push origin dev:master` at 21:47:59 UTC, `master` `7fd83de` → `ae0cae8`; salespipelinetracker.com serving `index-BnV7qKA1.js` from 21:48:30 UTC with the live key; §0.126, §0.127 and §0.128 on prod (four code commits; three function files rebuilt; no schema change); every literal of the batch in the served bundle; the automations function 401 on prod. Every "NOT shipped" in the paragraphs below is superseded by this line. NOT yet observed on prod — the prod checks are the first item of §5.**

**Fifteenth session — the §0.128 follow-up (13 Sep — Jeff: "when i click on the lead score nothing pops up except a scroll bar on the right ... on dev after hard refresh in brave or in edge"; then "oddly the list appears and the scroll bar disappears if I move the cursor off the lead after clicking") — CLOSE.** Read against the code: the Triage card lifts on hover — `transform: translateY(-1px)` — and a transformed ancestor is the containing block for a `position: fixed` descendant; with a real pointer over the card the fixed popover sat inside the card, clipped by the strip, and the strip grew a scrollbar; the cursor leaving dropped the transform and the box snapped to the viewport (his second observation). The pane's clicks are synthetic and never hover, which is how the first pass measured a correct box; reproduced with a `hover` action first (the card's transform `matrix(1, 0, 0, 1, 0, -1)`). The guide already had the rule (§16, the menu rule: portal + fixed, "any ancestor with a CSS transform ... traps position:fixed"); CLAUDE.md's index line had shortened it to fixed alone — corrected in the same commit. → **`eec5ae2`** (bundle only): the popover renders through `createPortal(…, document.body)`; the outside-click check covers the wrapper AND the portal's box. +6 scans, 1 mutant; six gates, **769/769 unit, 159/159 integration, 471/471 mutations after a printed green baseline**, build guard OK 2,504 kB `index-BIIXAgGT.js`. **LANDED on dev** 21:43:24 UTC. **OBSERVED in the pane under his conditions** — the pointer hovering the card (transform active) through a real click on the badge: the popover's parent `document.body`, fixed directly under the badge, painted in full, the strip's scroll height unchanged. **OBSERVED by Jeff on dev (13 Sep): "working perfectly."** **Flagged:** the three integration row menus (`useRowMenu`) are fixed without a portal — right today, the same latent trap if a row ever transforms. **NOT shipped** — `dev` ahead of `origin/master` (`7fd83de`) by FOUR code commits: `324c04e` (§0.126), `ac50a91` (§0.127), `e2dceca` and `eec5ae2` (§0.128); no schema change; the ship his call.

**Fifteenth session, continued further (13 Sep — Jeff: "lets just keep fixing those here", then his Leads screenshot) — CLOSE.** Two more batches. **§0.127 `ac50a91`** (functions AND bundle; no schema change): **Post to Slack as an automation action** — the engine renders `params.message` with the event's merge fields and posts through `sendSlackToOrg(orgId, { text })` with NO alert type (the rule is the switch; the org's webhook and master switch still gate; an unconnected Slack is a skipped action that says so); the panel offers "💬 Post to Slack" (message, merge hint, a note on the webhook; Create refuses a blank message; the review names it) — and **deal momentum and AI score drop fire the engine**: `opportunity.momentum` (stage_count, days_since_created) and `opportunity.score_drop` (score, verdict) join the vocabulary, the hourly job fires each right after its Slack post inside the same dedup block, so all five deal-health signals now drive rules. +1 unit, the silent-deal integration test extended (one Slack post to A, merge fields rendered, no alert type, 3 executed), 4 mutants; six gates, **767/767 unit, 159/159 integration, 466/466 mutations after a printed green baseline**, build guard OK 2,503 kB `index-CiwV447T.js`. **LANDED on dev** 21:17:42 UTC. NOT observed in a browser (§5). **§0.128 `e2dceca`** (bundle only — Jeff's screenshot: the lead score's "Why this score" card "gets truncated ... and a side slider is added ... only ... in these single line categories (call today, needs first touch, and working)"): the card was `position: absolute` under the badge, and the Triage strips scroll sideways (`overflow: auto`), so they clipped it and grew a scrollbar; now the popover rule — `src/utils/popoverPlacement.js` (pure, new: below when it fits, above when that fits, else pinned on screen; both edges clamped; viewport injectable, unit-tested) places it `position: fixed` from the badge's rect, and an outside click, a scroll or a resize closes it. +1 test file (2), 4 mutants; six gates, **769/769 unit, 159/159 integration, 470/470 mutations after a printed green baseline**, build guard OK 2,504 kB `index-838pOcA_.js`. **LANDED on dev** 21:26:43 UTC. **OBSERVED in the pane as Jeff:** Leads → Triage → the Working strip's 57 badge → the card fixed at the viewport directly under the badge, painted in full over the panel beside the strip, the strip's scroll height unchanged, an outside click closing it. **NOT shipped** — `dev` ahead of `origin/master` (`7fd83de`) by THREE code commits: `324c04e` (§0.126, functions AND bundle), `ac50a91` (§0.127, functions AND bundle), `e2dceca` (§0.128, bundle only); no schema change in any; the ship his call. Docs: state §0.127 and §0.128 with the header, in their code commits; the landing lines after; this handoff last.

**Fifteenth session, continued (13 Sep — Jeff: "lets just keep fixing those here") — CLOSE.** Read against the code first: §9's "client skips `GET /users` for non-admin roles (eats an expected 403)" has been moot since §0.109's directory read (a non-admin GET returns 200 rows) — struck, nothing built. → **§0.126 `324c04e`** (functions AND bundle; no schema change): the automation's **Assign to is a roster picker** — `assigneeOptions` / `assigneeParams` (pure, `automationEvents.js`) drive a SELECT over the org's active users that stores BOTH `assignedTo` (the name) and `assignedToId`; `_lib.mjs` exports `rosterUserById(id, orgId)` (this org's roster only); the engine owns the task by the id when it is in the event's org and refreshes the name from the roster (rename-proof, never ambiguous), falling back to the §0.124 name path; a rule saved before the picker shows its name "by name" and is never blanked; the review line names the assignee. And the calendar strip clears the previous org's meetings on a switch (§0.125's flag). +2 unit, +1 integration (A's Karen by id with a stale name → owned, the name refreshed; B's Karen's id in A's rule → a miss in A, the name resolves in A), 5 mutants. Six gates (163 files; 91 function files), **766/766 unit, 159/159 integration, 462/462 mutations after a printed green baseline (no restore errors)**, build guard OK 2,502 kB `index-qwmC8w1K.js`, `dist/` cleared. Pushed 21:02:44 UTC; **LANDED on dev** at 21:03:25 (the hash poll). **OBSERVED in the pane as Jeff (Admin):** + New automation → Actions → "Assign to" a SELECT — "The deal's rep (default)", Jeff Russell, Karen Russell, Ryan Algie, Savannah Miller; closed without saving. **NOT observed:** a rule saved through the picker and its task owned by the id — Jeff's, §5. **NOT shipped** — `dev` ahead of `origin/master` (`7fd83de`) by this ONE code commit (functions AND bundle) and the docs; the ship his call. Docs: state §0.126 and the header, §9's item struck, guide §18b37's rule extended (no new section; the header stays through §18b38), in the code commit; the landing line after; this handoff last.

**PROD SHIPPED — the TWENTIETH ship (Jeff: "ship to master", 13 Sep): `git push origin dev:master` at 20:43:16 UTC, `master` `b2218b2` → `7fd83de`; salespipelinetracker.com serving `index-DdSH6fY8.js` from 20:43:41 UTC with the live key; §0.125 and §0.124's Edit follow-up on prod (two bundle-only code commits; no function, no schema change); the give-up literal and "Edit automation" in the served bundle; the automations function 401 on prod. Every "NOT shipped" in the fifteenth-session paragraph below is superseded by this line. **OBSERVED on prod by Jeff (13 Sep): "tests passed - edit and change org" — Edit on an automation rule and the org switch reloading Home, both on salespipelinetracker.com. The rep sign-in in a private window on prod is not stated (observed on dev the same hour).****

**Fifteenth session (13 September 2026, Jeff: "calude, lets continue working" — then, offered the flagged list, "Fix the 401-then-200 load race") — CLOSE.** The ritual passed (tree clean, `dev` == `origin/dev` == `00bd9ce`, `origin/master` `b2218b2` behind by ONE code commit `b88ee9d` and two docs commits, both fingerprints present, copies identical). **Read against the code before building:** all three call sites (the spiff-claims effect, `useCoachingNotes`, the calendar strip) already awaited `waitForToken()`; what set them apart from every other load was the GATE — the main load runs only when `clerkUser && organization?.id`, the three ran from mount on `waitForToken` alone, and `waitForToken` gives up after 8 s and resolves ANYWAY (its own comment: "dbFetch will send without token and get 401"). So the in-app sign-in with its MFA step, or the no-org page, sent all three unauthenticated once and nothing retried; the "once after (200)" §9 had recorded was the NEXT page load — the pane's `read_network_requests` spans page loads (three loads read this session, each request once per load; saved to memory). A warm reload as Jeff (Admin) on `index-De3J8tLC.js` showed no 401 at all. Second finding, the same read: the header's `OrganizationSwitcher` (rendered with more than one membership) re-runs the main load and NONE of the three — the previous org's spiff claims, coaching notes and calendar stayed on the client until a refresh. → **§0.125 `83646ca`** (bundle only; no function, no schema change; guide §18b38 new): `App.jsx` derives `activeOrgId = (clerkUser && organization?.id) || null`; the spiff effect keys on it (clear, gate, cancel, `r.ok`), `useCoachingNotes` takes `orgId` (reset, gate, per-org deps), the calendar auto-fetch gates and re-arms per org (`calendarOrgRef`), `waitForToken`'s give-up warns before it resolves. +1 unit file (6, in `SUITES`), the coaching scan re-pinned, 5 mutants. Six gates (163 files; 91 function files), **764/764 unit, 158/158 integration, 457/457 mutations after a printed green baseline (no restore errors)**, build guard OK 2,500 kB `index-s-O_Wnog.js`, `dist/` cleared. Pushed 20:26:20 UTC; **LANDED on dev** — accelerep.netlify.app served `index-s-O_Wnog.js` at 20:26:50 UTC (the hash poll). **OBSERVED in the pane (Jeff signed in as himself, Admin; three orgs in the switcher — Jeff: "it has the org change"):** the reload fired the three once each, all 200, no 401, no give-up line; the avatar strip's switcher → Accelerep (`org_3B8Lh…`) re-ran the main load AND the three in place (no page load, the token's org changed, the panel showed that org's blank profile); → back to Accelerep Test (`org_3BDQ…`), the same once more, his profile back. The landing line `52ec1b2`. **OBSERVED by Jeff as Karen (role User) — his screenshot, 13 Sep, accelerep.netlify.app:** Home after signing in as Karen shows "Notes from your manager" with Jeff's 3 Sep note (coaching-notes loaded), Spiffs → My claims with two APPROVED claims (spiff-claims loaded) and the "Connect your calendar" prompt (calendar-events answered, no calendar connected) — the three loads populated on a rep's Home with no refresh; the console is not in the screenshot, and whether the sign-in went through the in-app form (the 8-second path) or an existing session is not stated. (recorded in state §0.125, its own commit). **Then the slow path itself, OBSERVED by Jeff (13 Sep): "logged in through private window and received no errors" — a fresh sign-in as Karen through the in-app form, no 401 in the console. Every check of §0.125 is observed.** **Was, before that: NOT observed, the slow path itself, strictly** — a sign-in through the in-app form (the pane holds Jeff's session; signing him out is his) — the code is the proof of it, and the check is in §5. **NOT shipped** — `dev` ahead of `origin/master` by TWO code commits, `b88ee9d` (the Edit follow-up) and `83646ca` (§0.125), both bundle only, and the docs; a ship is `git push origin dev:master`, his call. Docs: state §0.125 and its header, §9's entry struck, guide §18b38 (the header through §18b38, the auth-architecture line), all in the code commit; the landing line after; this handoff last.
...
hash poll), 2 min 40 s after the push. **OBSERVED in part by Jeff within the
hour** — his screenshot: "Edit job →" beside a saved job's id, "Create job →"
and "crew and duration not set" on every won-opportunity card, the header grid
populated — **and a finding:** "I clicked on this scheduled job on the job
board. It opened it in Queue and now has added it to the jobs to schedule
list. It did not go away until a hard refresh. I think the flow should be
when a person clicks on a scheduled job in the job board it either opens a
rail with the job for editing or it opens in the job tab under dispatch." The
first cut had listed a scheduled job opened from the board while it was
selected (so it could be re-crewed) — that read as the bug it looked like.
**Follow-up `0deb6b6`** (bundle only): the queue lists exactly the jobs still
to schedule, a selection pointing at a scheduled job is ignored, a scheduled
job clicked on the Job Board opens its record in Jobs (`handleJobClick` →
`openJobRecord`, the editor "Edit job →" already reaches — the Jobs tab, the
second of his two options; a rail would be new UI), Service Due's "Open job"
on a scheduled visit goes there too, and the "✓ Scheduled" card badge went
with the last scheduled card. 1 scan rewritten, 1 mutant added; six gates,
711/711 unit, 147/147 integration, **418/418 mutations after a printed green
baseline**, `index-BYQZhHpB.js`; **LANDED on dev** at 17:03:24 UTC (the hash
...
**§0.119 OBSERVED by Jeff's next screenshot** (the Closed Won list: Q4 2026 ·
4 deals, Q1 2027 · 2 deals in the quarter rail, two rows in the table) —
**with the next finding on it: "This is for this quarter and the time window
selected in the filter. I keep trying to select all time and it allows me to
select it but it does not actually use that selection and when I reopen the
filter it still is closed won but has switched back to this quarter for
time."** Read: the popover's Apply mapped BOTH "All time" and "This quarter"
to the empty quarter filter (`c25a043`, 21 Apr), and seeded its draft with
'thisQuarter' whenever the filter was empty — so "This quarter" had NEVER
filtered (his "this quarter" screenshot listed Q4 2026 and Q1 2027 deals side
by side; the match rule in `timeFilterOpts` was never reached) and "All
time" was applied invisibly, reading back as "This quarter". → **§0.120
`e5d574f`** (bundle only): only "All time" is the empty filter; every other
window is stored and filters; the empty filter reads back as "All time" on
mount, reopen and Reset; the default view is unchanged. +1 test file
(`pipeline-time-window`, 3 scans, registered in `SUITES`), 1 mutant; six
gates, 716/716 unit, 147/147 integration, **420/420 mutations after a printed
green baseline**, `index-DtQQ5W86.js`; **LANDED on dev** at 18:48:40 UTC (the
...
(the hash poll). **Jeff, on dev (12 Sep): "email templates will not open in settings > sales
process"** — the Settings page hands a card a click ONLY when its id is in
`AdminView`'s `DETAIL_PANELS` map; the batch added the route and the
catalogue row but not the map entry, so the card was dead. → **fix
`51675be`**: the entry, a scan pinning it, a mutant removing it; a Settings
detail is FOUR edits (catalogue row, card detail, `DETAIL_PANELS`, route)
and the scan now names all four. Along the way the harness file failed to
parse twice on the new mutant's label (an apostrophe in a single-quoted
string, then a `\n` that became a real line break through a shell edit) —
`node --check scripts/mutate-import.mjs` before a run is the cheap guard;
the label was rewritten and the string repaired with a direct edit. Six
gates, 735/735, 153/153, **430/430 mutations after a printed green
baseline**, `index-CD8TO61e.js`; **LANDED on dev** at 16:44:59 UTC. **Jeff
opened the app in the pane signed in as himself; OBSERVED in the pane after the fix (16:50 UTC 12 Sep): Settings → Sales process → Email templates opened (breadcrumb, '0 templates', + New template); a template 'Follow-up after a call' with subject 'Following up, {{firstName}}' and a body using {{company}}, {{repName}}, {{companyName}}, {{repPhone}} saved (PUT /settings 200, the panel reading '✓ 1 template'); Contacts → Sunniva (ZZFX Fenwick Group) → the rail's button read '✉ Email ▾' → the picker listed 'Blank email' and the template with 'Following up, Sunniva' under it, and the row's href decoded to mailto:sunniva@… ?subject=Following up, Sunniva&body=Hi Sunniva, … for ZZFX Fenwick Group. Jeff Russell / Accelerep Test · (his profile has no phone, so {{repPhone}} rendered empty). NOT clicked — a mail client from the pane; the mail-client hand-off and the logged activity are Jeff's to see. The template is left on dev for him.** **NOT shipped** —
`dev` is ahead of `master` by FOUR code commits, `04ad57b`, `d936baf`
(§0.121), `59cd6c3` and `51675be` (§0.122), plus docs.
**Then Jeff's screenshot as Karen: "Emailed from template as karen and the
process worked fine"** — the "Jeff Russelltest" contact's Activity history
carries "Sep 12 · Email · Template: Follow-up after a call — Following up,
Jeff" (§0.122 OBSERVED end to end by his hand) — **and: "I have not received
an email. It may be that I need to setup an email path in Accelerep."** No:
by design NOTHING is sent by the app — the template opens in the rep's own
mail client and goes out only when Send is pressed there; the activity was
logged on the click. **And: "we better make the email template selector a
drop down list … if someone has a lot that will be a lot of screen real
estate"** → **`f422a92`**: the picker is a `<select>` (placeholder, Blank
email, each template with its rendered subject); choosing sets
`window.location.href` to the rendered `mailto:` and logs as before; a line
under it says "Opens in your own mail client with the fields filled in —
nothing is sent until you press Send there." Six gates, 735/735, 153/153,
**430/430 mutations**, `index-s255BQ_M.js`; **LANDED on dev** at 19:25:14 UTC.
...
mutations, `index-SFEVnpmt.js`, served by dev at 16:44 UTC; OBSERVED in part by
Jeff (his screenshot), whose finding — a scheduled job clicked on the board
joined the list — is the **follow-up `0deb6b6`**: a scheduled board click
opens the record in Jobs, the list is strictly unscheduled; 418/418
mutations, `index-BYQZhHpB.js`, served by dev at 17:03:24 UTC. **The follow-up
NOT observed. NOT shipped.**
...
`### 0.149` with a paragraph beginning **"The pure half (`src/utils/invoices.js`; no React, no db)."** and one beginning **"Product & service types — the Admin’s"** (16 Sep — quote → job → invoice) and `docs/ACCELEREP_CODING_GUIDE.md` `## 18b44. A Mirror Column Has One Writer — The Endpoint That Owns The Truth It Mirrors (hard rule)`,
`### 0.148` with a paragraph beginning **"(1) The Price book."** (16 Sep — three small things after the twenty-seventh ship),
`### 0.147` with a paragraph beginning **"(1) The notes."** (16 Sep — the builder’s notes on the customer preview),
`### 0.146` with a paragraph beginning **"What it was."** and one beginning **"Now."** (15 Sep — money on the Quotes tab in the app’s face),
`### 0.145` with a paragraph beginning **"Now."** and the words "itest-targets-test-db" (15 Sep — the heartbeat suite and the application database) and `docs/ACCELEREP_CODING_GUIDE.md` `## 18b43. An Integration Suite’s First Statement Points The Client At The Test Database`, `### 0.144` with a paragraph beginning **"Now."** and the words "fetch-depth: 0" (15 Sep — CI’s unit job and the shallow clone), `### 0.143` with a paragraph beginning **"The guard (`tests/audit-coverage.test.mjs`, new)."** (15 Sep — every write path writes the audit log) and `docs/ACCELEREP_CODING_GUIDE.md` `## 18b42. Every Write Path Writes The Audit Log — Through `auditAs`, Named, With The Word For What Happened (hard rule)`,
`### 0.142` with a paragraph beginning **"Now (`src/utils/quoteTemplates.js`, pure)."** (15 Sep — the Start-quote picker), and under `### 0.140` a paragraph beginning **"A Send now is not the scheduled delivery"**,
`### 0.141` with a paragraph beginning **"The reader (`netlify/functions/report-prompt.mjs`; the pure half in `_reportPromptShape.mjs` so the tests run it)."** (15 Sep — Claude reads report prompts when an Admin turns it on),
`### 0.140` with a paragraph beginning **"The schedule (`src/utils/reportDelivery.js`)."** (15 Sep — a delivery at any minute) and one beginning **"The picker, corrected"**,
`### 0.139` with a paragraph beginning **"The interpreter (`src/utils/reportPrompt.js`, pure)."** (15 Sep — the report builder reads its prompt) and `docs/ACCELEREP_CODING_GUIDE.md` `## 18b41. A Box That Says "AI" Reads Its Prompt — A Fixed Answer Is A Stub, And A Stub Is Labelled, Not Dressed (hard rule)`,
`### 0.138` with a paragraph beginning **"The pass."** (15 Sep — the rest of the off-brand table),
`### 0.137` with a paragraph beginning **"The fix."** (15 Sep — the plate row’s stage on its own line),
`### 0.136` with a paragraph beginning **"The script (`scripts/migrate-blue-era.mjs`, kept as the record; the §0.130 method)."** (14 Sep — the blue-era modals on the tokens),
`### 0.135` with a paragraph beginning **"Scheduled delivery — the shape (`src/utils/reportDelivery.js`, pure)."** (14 Sep — saved reports: sharing, delivery, pins; the two tab preferences),
`### 0.134` with a paragraph beginning **"What a drop means (`src/utils/weekDrop.js`, pure — no React, no db; the board’s reads are injected)."** (14 Sep — drag-to-reschedule on the week board) and `docs/ACCELEREP_CODING_GUIDE.md` `## 18b40. A Drop Is A Write Path — It Runs The Form’s Gates, Sends Only What Changed, Never Sets Status (hard rule)`,
`### 0.133` with a paragraph beginning **"The engine — `src/utils/reportQuery.js` (pure; no React, no db)."** (14 Sep — the report builder runs real queries),
`### 0.132` with a paragraph beginning **"(1) `task.overdue` — computed, not merely offered."** (14 Sep — the enhancement thread's last three),
`### 0.131` with a paragraph beginning **"What changed (five files, presentation only)."** (14 Sep — every tab announces itself the same way),
`### 0.130` with a paragraph beginning **"What was found, before writing anything (the parser, not grep)."** (14 Sep — one token object),
`### 0.129` with a paragraph beginning **"NOT done, by the brief's own gates:"** (13 Sep — the signal migration's Phase 0 and 1, nothing migrated),
...
## 1. What shipped — the NINETEENTH ship put §0.124 and its first three follow-ups on `master` (13 Sep 19:59 UTC, `b2218b2`; six functions, `package.json`, the bundle; no schema change); the Edit follow-up is NOT shipped

**PROD SHIPPED — the NINETEENTH ship (Jeff: "ship to master", 13 Sep).** Pre-flight: `dev` == `origin/dev` == `b2218b2`, `origin/master` (`b54a2a7`) an ancestor of `dev`, 14 commits after it (three code: `427fd65` §0.124, `892c978` the menu follow-up, `ba9aefb` the third follow-up), `git diff origin/master..dev -- db/schema.ts` EMPTY — no schema change; seven files under `netlify/` and `package.json` (the six functions of §0.124 and the integration-test list), `netlify.toml` untouched; the working tree carried the UNCOMMITTED fourth follow-up (Edit an existing automation, its harness still running) — a push of `dev:master` ships the committed branch only, and that follow-up is NOT in this ship. `git push origin dev:master` at 19:59:53 UTC: `master` `b54a2a7` → `b2218b2`. salespipelinetracker.com served `index-BfMnj4VB.js` from 20:00:24 UTC (the hash poll, 31 s after the push; was `index-COkIzXNn.js`). Landing proof by the served bundle's literals — "Deal gone silent (no activity for 14 days)", "Merge fields:", "Update field: choose the field to set", "Choose a field…", "no field chosen", "never fires" all present; `pk_live_` inlined (the one `pk_test_` string is Clerk's SDK constant `s4="pk_test_"`, not a key) — and by the function: GET `/.netlify/functions/automations` on prod answers 401 `{"error":"Unauthorized: no token"}` (the function is deployed and behind Clerk). The Netlify deploy record was not read. **What prod does differently from this moment:** the hourly job fires `opportunity.silent` / `.stuck` / `.close_lapsed` rules at each rep's alert hour (prod alone runs the jobs — `JOBS_ENABLED`); a rule's task is linked to the deal and owned by the assignee; `task.completed` fires; the panel offers the shared vocabulary, a field select, merge fields, notes; `update_field` is allowlisted (an `orgId` write refused); the row menus are fixed popovers in three panels; the run history names the record; the Update field action saves what it shows. **The three ships' rules on prod are Jeff's to create** — prod holds no automation rule until an Admin makes one. Every "NOT shipped" above for §0.124 and its first three follow-ups is superseded by this paragraph; the fourth follow-up (Edit) is NOT shipped. **NOT yet observed on prod.**

**Before the ship — §0.124 (light workflow automation, `427fd65`, 13 Sep) was on `dev` only: functions (`dispatch-automations.mjs`, `pipeline-alerts.mjs`, `opportunities.mjs`, `leads.mjs`, `lead-intake.mjs`, `tasks.mjs`), `package.json` (the integration list), the bundle; no schema change. A ship would be `git push origin dev:master` after Jeff's dev observation.**
...
## 4. Verified state at close (15 Sep — the TWENTY-FIFTH ship, then §0.142, the §0.140 corrections and §0.143 on dev, NOT shipped)

**Close (15 Sep, after §0.143): `origin/master` == `930e276` (the TWENTY-FIFTH ship, 21:10:08 UTC); `dev` == `origin/dev` == this handoff commit — ahead of `origin/master` by THREE CODE commits `4d6b0f5` (§0.142 + the Send-now correction; functions AND bundle), `ddac313` (the custom-range label on a delivered report; functions AND bundle) and `d48b3f6` (§0.143 — 31 function files, `_lib.mjs`, `AuditDetail.jsx`, the harness, two test files; functions AND bundle; no schema change), the ship-record docs commit and this handoff. Counts: six gates (171 files; 96 function files), 854/854 unit, 170/170 integration, 552/552 mutations (printed green baseline, no restore residue), build guard OK 2,551 kB `index-D5O2AGMO.js`, `dist/` cleared; pushed to dev 22:42:35 UTC 15 Sep (`ddac313..d48b3f6 dev -> dev`); LANDED — accelerep.netlify.app served `index-D5O2AGMO.js` from 22:43:23 UTC 15 Sep (the hash poll). Prod unchanged: `index-DfRbwLhy.js` = `930e276`, the live key; 86 functions; `report-deliveries` every five minutes — its first SCHEDULED send observed at 21:20:14 UTC (16:20 Chicago). `deno.lock` (two lines, the CLI’s) uncommitted.** The state before it:

**Close (15 Sep, after the TWENTY-FIFTH ship): `origin/master` == `930e276` (the TWENTY-FIFTH ship, 21:10:08 UTC); `dev` == `origin/dev` == this handoff commit — ahead of `origin/master` by ONE CODE commit `4d6b0f5` (§0.142 the quote picker, bundle; the §0.140 correction — `report-deliveries.mjs`, `saved-reports.mjs`, `reportDelivery.js` — functions AND bundle; no schema change) and this handoff. Counts: six gates (171 files; 96 function files), 848/848 unit, 170/170 integration, 546/546 mutations (printed green baseline, no restore residue), build guard OK 2,550 kB `index-uCWPULHc.js`, `dist/` cleared; dev serving `index-uCWPULHc.js` from 21:38:31 UTC 15 Sep. Prod: `index-DfRbwLhy.js` = `930e276`, the live key, from 21:10:47 UTC; 86 functions; `report-deliveries` every five minutes (heartbeat ok at 21:25 UTC). `deno.lock` (two lines, the CLI's) uncommitted. The Accelerep Test org: `aiReportPromptsEnabled` ON with Jeff's BYOK key.** The state before it:

**AT THE TWENTY-EIGHTH SHIP (19:39:50 UTC 16 Sep): `master` == `dev` == `a38e0c6`; prod serving `index-BkLKFS3b.js` from 19:40:22 UTC; 86 functions, none rebuilt (bundle only); `dev` then ahead only by this handoff. CI green on master. `deno.lock` (two lines) uncommitted at the ship.** The state before it:

**AT THE TWENTY-SEVENTH SHIP (16:39:20 UTC 16 Sep): `master` == `dev` == `8e2d137`; prod serving `index-D8uZRWUK.js` from 16:39:52 UTC; 86 functions, none rebuilt (bundle only); `dev` then ahead only by this handoff. `deno.lock` (two lines) uncommitted at the ship.** The state before it:

**AT THE TWENTY-SIXTH SHIP (13:31:49 UTC 16 Sep): `master` == `dev` == `459c321`; prod serving `index-BqHYVvqM.js` from 13:32:44 UTC; 86 functions (32 rebuilt); `dev` then ahead only by this handoff. CI green on master. `deno.lock` (two lines) uncommitted at the ship.** The state before it:

**AT THE TWENTY-FIFTH SHIP (21:10:08 UTC 15 Sep): `master` == `dev` == `930e276`; prod serving `index-DfRbwLhy.js` from 21:10:47 UTC; `dev` then ahead only by this handoff.** The state before the ship:

**Close (15 Sep, eighteenth session, continued further): `origin/master` == `a4f4589` (the TWENTY-FOURTH ship); `dev` == `origin/dev` == this handoff commit — ahead of `origin/master` by the ship-record docs commits, SIX CODE commits `e955801` (§0.138; bundle only), `e302346` (§0.139; functions AND bundle), `a83be59` (§0.140; functions AND bundle AND `netlify.toml`), `5ea58b8` (the §0.139 follow-up; bundle only), `ac22b82` (§0.141; functions AND bundle), `1482fc5` (the §0.141 fixes: the stage filter in the engine, the tool call's `none`, the log line; functions AND bundle) and the handoffs. Counts: six gates (170 files; 96 function files), 845/845 unit, 170/170 integration, 542/542 mutations (printed green baseline, no restore residue), build guard OK 2,549 kB `index-Be4l2aWO.js`, `dist/` cleared; dev serving `index-Be4l2aWO.js` from 21:02:54 UTC 15 Sep (before it `index-DBD7v9I0.js`). Prod: `index-zv28zhMt.js` = `a4f4589`. `deno.lock` (two lines, the CLI's) uncommitted. The Accelerep Test org: `aiReportPromptsEnabled` ON and a BYOK key installed (Jeff's, through the local instance — the shared database, so on dev too).** The state before it:

**Close (15 Sep, eighteenth session, continued): `origin/master` == `a4f4589` (the TWENTY-FOURTH ship); `dev` == `origin/dev` == this handoff commit — ahead of `origin/master` by the ship-record docs commits, FIVE CODE commits `e955801` (§0.138; bundle only), `e302346` (§0.139; functions AND bundle), `a83be59` (§0.140; functions AND bundle AND `netlify.toml`), `5ea58b8` (the §0.139 follow-up; bundle only), `ac22b82` (§0.141; functions AND bundle — `report-prompt.mjs`, `_aiKey.mjs`, `_reportPromptShape.mjs` new; `settings.mjs`, `ai-score.mjs`) and the handoffs. Counts: six gates (170 files; 96 function files), 845/845 unit, 170/170 integration, 541/541 mutations (printed green baseline, no restore residue), build guard OK 2,549 kB `index-DBD7v9I0.js`, `dist/` cleared; dev serving `index-DBD7v9I0.js` from 19:21:54 UTC 15 Sep (before it `index-B0ZTf3LY.js`). Prod: `index-zv28zhMt.js` = `a4f4589`. `deno.lock` (two lines, the CLI's) uncommitted. The Accelerep Test org's `aiReportPromptsEnabled` is ON (left on from the pane check; no BYOK key there).** The state before it:

**Close (15 Sep, eighteenth session): `origin/master` == `a4f4589` (the TWENTY-FOURTH ship); `dev` == `origin/dev` == this handoff commit — ahead of `origin/master` by the ship-record docs commits, FOUR CODE commits `e955801` (§0.138; bundle only), `e302346` (§0.139; functions AND bundle — `report-deliveries.mjs` runs the filters), `a83be59` (§0.140; functions AND bundle AND `netlify.toml` — the cron `*/5 * * * *`), `5ea58b8` (the §0.139 follow-up; bundle only) and the handoffs. Counts: six gates (170 files; 93 function files), 839/839 unit, 170/170 integration, 534/534 mutations (printed green baseline, no restore residue), build guard OK 2,546 kB `index-B0ZTf3LY.js`, `dist/` cleared; dev serving `index-B0ZTf3LY.js` from 18:31:36 UTC 15 Sep (before it `index-DIxtA4Km.js`). Prod: `index-zv28zhMt.js` = `a4f4589`. `deno.lock` (two lines, the CLI's) uncommitted.** The state before it:

**Close (15 Sep): `origin/master` == `a4f4589` (the TWENTY-FOURTH ship); `dev` == this handoff commit — ahead of `origin/master` by the ship-record docs commits, ONE CODE commit `e955801` (§0.138; bundle only) and this handoff. Counts: six gates (169 files; 93 function files), 826/826 unit, 170/170 integration (at §0.135), 518/518 mutations (printed green baseline, no restore residue), build guard OK 2,533 kB `index-DIxtA4Km.js`, `dist/` cleared. Prod: `index-zv28zhMt.js` = `a4f4589`. `deno.lock` (two lines, the CLI's) uncommitted.** The state before it:

**AT THE TWENTY-FOURTH SHIP (14:50:55 UTC 15 Sep): `master` == `dev` == `a4f4589`; prod serving `index-zv28zhMt.js` from 14:51:57 UTC; `dev` then ahead only by the ship-record docs commit (this handoff). `deno.lock` (two lines) uncommitted at the ship.** The state before the ship:

**Close (15 Sep): `origin/master` == `c17e2e8`; `dev` == this handoff commit — ahead of `origin/master` by FIVE CODE commits `9fb744a`, `5bbd813` (§0.134), `62dc20a` (§0.135; functions AND bundle, netlify.toml), `f0705c0` (§0.136) and `9bce063` (§0.137; both bundle only) and the docs. Counts: six gates (169 files; 93 function files), 825/825 unit, 170/170 integration (at §0.135), 516/516 mutations (at §0.136, printed green baseline), build guard OK 2,533 kB `index-oOwpiwjs.js`, `dist/` cleared. Prod: `index-Ct7x96Jv.js` = `c17e2e8`. `deno.lock` (two lines, the CLI’s) uncommitted.** The state before it:

**Close (14 Sep): `origin/master` == `c17e2e8`; `dev` == `f0705c0` + this handoff commit — ahead of `origin/master` by FOUR CODE commits `9fb744a`, `5bbd813` (§0.134), `62dc20a` (§0.135; functions AND bundle, netlify.toml) and `f0705c0` (§0.136; bundle only) and the docs. Counts: six gates (169 files; 93 function files), 824/824 unit, 170/170 integration (at §0.135; not re-run), 516/516 mutations (printed green baseline, no restore residue), build guard OK 2,532 kB `index-5_oMz9NV.js`, `dist/` cleared. Dev site: `index-ODWcjiOD.js` (§0.135) until the next push. Prod: `index-Ct7x96Jv.js` = `c17e2e8`. `deno.lock` (two lines, the CLI's) uncommitted.** The state before it:

**Seventeenth session's close, continued (14 Sep): `origin/master` == `c17e2e8` (the TWENTY-THIRD ship); `dev` == `62dc20a` + this handoff commit — ahead of `origin/master` by the ship-record docs commit, THREE CODE commits `9fb744a`, `5bbd813` (§0.134, bundle only) and `62dc20a` (§0.135: `report-deliveries.mjs` new, `saved-reports.mjs`, `send-email.mjs`, `_selfProfile.mjs`, `users.mjs`, `netlify.toml`, `src/utils/reportDelivery.js` new, `jobHealth.js`, `App.jsx`, `ReportsTab.jsx`, `HomeTab.jsx`, `TasksTab.jsx`, `DispatchTab.jsx`, tests, the harness, `package.json`; functions AND bundle; NO schema change) and the docs. Counts: six gates (169 files; 93 function files), 819/819 unit, 170/170 integration, 513/513 mutations (printed green baseline, no restore residue), build guard OK 2,532 kB `index-ODWcjiOD.js`, `dist/` cleared. Dev site: `index-BULS4hTf.js` (§0.134) until the next push. Prod: `index-Ct7x96Jv.js` = `c17e2e8`. `deno.lock` (two lines, the CLI's) uncommitted.** The state before it:

**Seventeenth session's close (14 Sep): `origin/master` == `c17e2e8` (the TWENTY-THIRD ship); `dev` == `5bbd813` + this handoff commit — ahead of `origin/master` by the ship-record docs commit `cc42c98`, TWO CODE commits `9fb744a` (§0.134: `src/utils/weekDrop.js` new, `src/Tabs/DispatchTab.jsx`, two test files, the harness; bundle only — no function, no schema change) and `5bbd813` (its confirm-variant follow-up; bundle only) and this handoff. Counts: six gates (168 files; 92 function files), 808/808 unit, 164/164 integration, 500/500 mutations caught, printed green baseline, no restore residue (printed green baseline, no restore residue), build guard OK 2,515 kB `index-BULS4hTf.js`, `dist/` cleared. Prod: `index-Ct7x96Jv.js` = `c17e2e8`. Not pushed at the time of writing; `deno.lock` (two lines, the CLI's) uncommitted.** The state before it:

**AT THE TWENTY-THIRD SHIP (18:53:25 UTC 14 Sep): `master` == `dev` == `c17e2e8`; prod serving `index-Ct7x96Jv.js` from 18:54:02 UTC (82 functions, nine re-uploaded); `dev` then ahead only by the ship-record docs commit (this handoff). The tree was clean at the ship.** The state before the ship:

**After §0.133 (14 Sep): `dev` == `origin/dev` == the handoff commit after `ed34ca9` — ahead of `origin/master` (`e2cba64`) by THREE CODE commits `9893ddc` (§0.132, functions AND bundle), `88148ab` (bundle only), `ed34ca9` (§0.133, bundle only) and the docs. Counts: six gates (167 files; 92 function files), 796/796 unit, 164/164 integration, 492/492 mutations (printed green baseline), build guard OK 2,508 kB `index-BNgFKZx-.js`, `dist/` cleared; dev serving `index-BNgFKZx-.js` from 18:32:42 UTC (before it `index-DoNj0dG6.js` from 17:44:09).** The state before it:


**After the follow-up (14 Sep): `dev` == `origin/dev` == the handoff commit after `88148ab` — ahead of `origin/master` (`e2cba64`) by TWO CODE commits `9893ddc` (§0.132, functions AND bundle) and `88148ab` (the modal's error, bundle only) and the docs. Counts: six gates, 783/783 unit, 164/164 integration, 485/485 mutations (printed green baseline), build guard OK 2,492 kB `index-DoNj0dG6.js`, `dist/` cleared; dev serving `index-DoNj0dG6.js` from 17:44:09 UTC (before it `index-DL6xNwD_.js` from 16:46:14).** The state before the follow-up:

**Sixteenth session, continued further — close (14 Sep): `origin/master` == `e2cba64` (the TWENTY-SECOND ship); `dev` == `origin/dev` == the handoff commit after `9893ddc` — ahead of `origin/master` by ONE CODE commit `9893ddc` (§0.132: functions AND bundle; no schema change) and the docs commits (`391d364`, the observation commit, this handoff). Counts: six gates (165 files; 92 function files), 782/782 unit, 164/164 integration, 484/484 mutations (printed green baseline, no restore residue; one anchor repointed after a STALE report on the first run), build guard OK 2,492 kB `index-DL6xNwD_.js`, `dist/` cleared; dev serving `index-DL6xNwD_.js` from 16:46:14 UTC 14 Sep (before it `index-CyIfr4JI.js`). Prod: `index-D0HaH5AU.js` = `e2cba64`.**

**AT THE TWENTY-SECOND SHIP (16:01:27 UTC 14 Sep): `master` == `dev` == `e2cba64`; prod serving `index-D0HaH5AU.js` from 16:02:06 UTC; `dev` then ahead only by the ship-record docs commit (this handoff). The `deno.lock` two-line change from the CLI upgrade was UNCOMMITTED at the ship (Jeff's call).** The state before the ship:

**Sixteenth session, continued further — close (14 Sep): `origin/master` == `ae0cae8` (the TWENTY-FIRST ship); `dev` == `origin/dev` == the handoff commit after `c9bf692` — ahead of `origin/master` by THREE CODE commits `10a78bc` (§0.130), `4bb37ef` (§0.131), `c9bf692` (its follow-up) — all bundle only; no function, no schema change — and the docs commits (`9f4d35c`, `37d665f`, `1b55199`, `59b78bc`, this handoff). Counts: six gates (165 files; 91 function files), 774/774 unit, 159/159 integration, 475/475 mutations (printed green baseline, no restore residue), build guard OK 2,489 kB `index-CyIfr4JI.js`, `dist/` cleared; dev serving `index-CyIfr4JI.js` from 15:52:24 UTC 14 Sep (before it `index-B_6zu6fm.js` from 15:39:09, `index-D0WIMRUE.js` from 15:02:45). Prod: `index-BnV7qKA1.js` = `ae0cae8`.**

**Sixteenth session, continued — close (14 Sep): `origin/master` == `ae0cae8` (the TWENTY-FIRST ship); `dev` == `origin/dev` == the handoff commit after `10a78bc` — ahead of `origin/master` by ONE CODE commit `10a78bc` (§0.130, bundle only; no function, no schema change) and the docs commits (`9f4d35c` design docs + audit script, `37d665f`, `1b55199`, this handoff). Counts: six gates (165 files; 91 function files), 774/774 unit, 159/159 integration, 475/475 mutations (printed green baseline, no restore residue), build guard OK 2,489 kB `index-D0WIMRUE.js`, `dist/` cleared; dev serving `index-D0WIMRUE.js` from 15:02:45 UTC 14 Sep (before it `index-BIIXAgGT.js`). Prod: `index-BnV7qKA1.js` = `ae0cae8`.**

**Sixteenth session's close (13 Sep): `origin/master` == `ae0cae8` (the TWENTY-FIRST ship); `dev` == `9f4d35c` + this handoff commit — ahead of `origin/master` by the two ship-record docs commits, `9f4d35c` (docs/design + `scripts/audit-colors.mjs` + state §0.129 — NOT a code commit: no src/, netlify/ or schema change, the bundle hash unchanged) and this handoff. Counts: six gates (164 files; 91 function files), 769/769 unit, build guard OK 2,504 kB `index-BIIXAgGT.js` (unchanged), `dist/` cleared; integration and mutations NOT run this session (last: 159/159, 471/471 at the fifteenth close). Prod: `index-BnV7qKA1.js` = `ae0cae8`. Not pushed at the time of writing.**

**AT THE TWENTY-FIRST SHIP (21:47:59 UTC 13 Sep): `master` == `dev` == `ae0cae8`; prod serving `index-BnV7qKA1.js` from 21:48:30 UTC; `dev` then ahead only by the ship-record docs commits (the state commit, then this handoff).** The state before the ship: **The §0.128 follow-up's close (13 Sep): `dev` == `origin/dev` == the landing-line commit before this handoff — ahead of `origin/master` (`7fd83de`) by FOUR CODE commits (`324c04e`, `ac50a91`, `e2dceca`, `eec5ae2`; NO schema change) and the docs. Counts: six gates (164 files; 91 function files), 769/769 unit, 159/159 integration, 471/471 mutations (printed green baseline, no restore errors), build guard OK 2,504 kB `index-BIIXAgGT.js`, `dist/` cleared; dev serving `index-BIIXAgGT.js` from 21:43:24 UTC 13 Sep (before it `index-838pOcA_.js` from 21:26:43). Prod: `index-DdSH6fY8.js` = `7fd83de`.** The state before the follow-up: **Fifteenth session, continued further — close (13 Sep): `origin/master` == `7fd83de` (the TWENTIETH ship); `dev` == `origin/dev` == the landing-lines commit before this handoff — ahead by THREE CODE commits (`324c04e` §0.126, `ac50a91` §0.127 — both functions AND bundle; `e2dceca` §0.128 — bundle only; NO schema change) and the docs. Counts: six gates (check:fnscope 91 function files; the five on 164 files), 769/769 unit, 159/159 integration, 470/470 mutations (printed green baseline, no restore errors), build guard OK 2,504 kB `index-838pOcA_.js`, `dist/` cleared; dev serving `index-838pOcA_.js` from 21:26:43 UTC 13 Sep (before it `index-CiwV447T.js` from 21:17:42, `index-qwmC8w1K.js` from 21:03:25). Prod: `index-DdSH6fY8.js` = `7fd83de`.**

**Fifteenth session, continued — close (13 Sep): `origin/master` == `7fd83de` (the TWENTIETH ship); `dev` == `origin/dev` == the landing-line commit before this handoff — ahead by ONE CODE commit (`324c04e` §0.126 — functions `_lib.mjs` and `dispatch-automations.mjs` AND the bundle; NO schema change) and the docs. Counts: six gates (check:fnscope 91 function files; the five on 163 files), 766/766 unit, 159/159 integration, 462/462 mutations (printed green baseline, no restore errors), build guard OK 2,502 kB `index-qwmC8w1K.js`, `dist/` cleared; dev serving `index-qwmC8w1K.js` from 21:03:25 UTC 13 Sep (before it `index-s-O_Wnog.js` from 20:26:50). Prod: `index-DdSH6fY8.js` = `7fd83de`.**

**AT THE TWENTIETH SHIP (20:43:16 UTC 13 Sep): `master` == `dev` == `7fd83de`; prod serving `index-DdSH6fY8.js` from 20:43:41 UTC; `dev` then ahead only by the ship-record docs commits (the state commit, then this handoff).** The state before the ship: **Fifteenth session's close (13 Sep): `origin/master` == `b2218b2` (the NINETEENTH ship); `dev` == `origin/dev` == `52ec1b2` before this handoff commit — ahead by TWO CODE commits (`b88ee9d` the Edit follow-up; `83646ca` §0.125 — both bundle only, NO function, NO schema change) and the docs commits around them. Counts: six gates (check:fnscope 91 function files; the five on 163 files), 764/764 unit, 158/158 integration, 457/457 mutations (printed green baseline, no restore errors), build guard OK 2,500 kB `index-s-O_Wnog.js`, `dist/` cleared; dev serving `index-s-O_Wnog.js` from 20:26:50 UTC 13 Sep (before it `index-De3J8tLC.js` from 20:03:21). Prod unchanged: `index-BfMnj4VB.js` = `b2218b2`.**

**Fourteenth session's close (13 Sep): `origin/master` == `b54a2a7` (the EIGHTEENTH ship); `dev` == `origin/dev` == `427fd65` before this handoff commit — ahead by ONE CODE commit (§0.124: six function files, `package.json`, the bundle, a new pure module; NO schema change) and this handoff. Then the menu follow-up `892c978` (bundle only) — `dev` ahead by TWO code commits. Counts: six gates (check:fnscope 91 function files; the five on 163 files), 755/755 unit, 158/158 integration, 445/445 mutations (printed green baseline, no restore errors), build guard OK 2,499 kB `index-DqfNZYvt.js`, `dist/` cleared; Then the third follow-up `ba9aefb` (bundle only — one menu hook for three panels, run-history names, the Update field action saving what it shows) — `dev` ahead by THREE code commits: 757/757 unit, 450/450 mutations, build guard OK 2,499 kB `index-DpgxUDFK.js`; Then the fourth follow-up `b88ee9d` (bundle only — Edit an existing automation) — `dev` ahead by FOUR code commits: 758/758 unit, 452/452 mutations, build guard OK 2,500 kB `index-De3J8tLC.js`; dev serving `index-De3J8tLC.js` from 20:03:21 UTC 13 Sep (before it `index-DpgxUDFK.js` from 19:42:53, `index-DqfNZYvt.js` from 19:19:41 UTC 13 Sep, `index-CDir98BZ.js` from 13:47:02, `index-DS-lL6jZ.js`). Prod unchanged: `index-COkIzXNn.js` = `b54a2a7`.**
...
**Twentieth-session prep — first (§0.149, the newest — quote → job → invoice, NOT shipped):**
- **Ritual** (item 1), then `git log --oneline origin/master..dev` — expect, after `a38e0c6`: the ship-record docs commit `9d49c00`, **`4fe1efc` (CODE — §0.149: functions, bundle, a schema change already applied to BOTH databases)**, the handoff `6559166`, **`ffb3de1` (CODE — the price book is one; bundle only)**, the handoff `27bf80c` and the landing-line docs commit; anything else is a finding. **`dev` == `origin/dev` since the push at 02:15 UTC 17 Sep — on another machine `git clone` (or `git pull`) has everything; `.env` is rebuilt by hand, never sent.** `deno.lock` dirty (two lines) is the CLI’s, known.
- **§0.149 on dev — Jeff’s look (accelerep.netlify.app, hard-refresh to `index-BycitA9a.js` — LANDED 02:16:07 UTC 17 Sep):** Quotes → a deal → a Sent / Approved quote → "Customer accepted this quote →" → the Dispatch job card → "Create dispatch job →" → the job named on the card; Dispatch → Jobs → that job → the Invoice block → Create invoice → Edit (add a line, a tax rate) → Save → Issue → Mark paid → the Paid line; Void on a fresh one; the Invoices sub-tab; Settings → Quoting → Product & service types → add a type, retype a built-in’s kind → Quotes → **Price Book** (Catalog, renamed) → a product’s Type select; Settings → Quoting → Price book → the card opens Quotes → Price Book (no panel); then a quote whose product carries the new type → job → the line’s kind on the invoice. **On the test org the pane left `JOB-2026-0008` (unscheduled) and `INV-2026-0001` ISSUED on it, and a "Pacific Rim Logistics" dispatch customer, and an "Equipment → Material" type — his to keep or clear.**
- **The rep path as Karen (role User):** the Customer accepted button and Create dispatch job on HER quote; a ReadOnly user sees neither button. Not run this session (the pane holds Jeff’s session).
- **The push / the ship — his call.** `git push origin dev` lands it on accelerep.netlify.app (functions AND bundle — `db/schema.ts` changed, so every function rebuilds); a ship is `git push origin dev:master`. Pre-flight as before: `git diff origin/master..dev -- db/schema.ts` is NOT empty this time — `dispatch_jobs.quoteId` and the `invoices` table — and BOTH databases already carry them (read back by `db/apply-invoices.mjs` on 16 Sep), which is the §18c order.
- **QuickBooks — what Jeff does before that half is built, none of it in chat:** in the Intuit developer portal, an app with the two redirect URIs (`https://accelerep.netlify.app/.netlify/functions/quickbooks-oauth-callback` and the same path on `https://salespipelinetracker.com`) and a sandbox company; in Netlify’s UI on BOTH sites, the app’s Client ID and Client Secret as environment variables (suggested names `INTUIT_CLIENT_ID` / `INTUIT_CLIENT_SECRET`); then tell Claude the variable NAMES. The `invoices.quickbooks_*` columns are waiting; the export is one endpoint pair (start / callback, the calendar-oauth shape) plus a per-invoice "Send to QuickBooks" that writes an Invoice to the sandbox and stamps the id.
- **Flagged, carried (state §9):** the dead `priceBookProducts` key in both settings.mjs halves and in stored extras (one line to drop in a sweep); an invoice PDF and email; a deep link from the quote card into the job; per-org default tax rate and terms; the settings autosave mirrors every list panel’s save back as a second PUT (pre-existing; seen on the types panel); the five native time inputs; the interpreter’s gaps.
- **Memory-worthy, saved this session:** never run the mutation harness while `netlify dev` is up; `rm -rf dist node_modules/.vite` after any gate build before a pane pass; the pane may open signed out — Jeff signs in.

**Nineteenth-session prep — first (§0.148, the newest — after the TWENTY-SEVENTH ship):**
- **Ritual** (item 1), then `git log --oneline origin/master..dev` — expect, after `8e2d137`: the ship-record docs commit `9f9aa7a`, **`334a079` (CODE — §0.148, bundle)** and this handoff; anything else is a finding. `deno.lock` dirty (two lines) is the CLI’s, known.
- **§0.148 on dev — Jeff’s look (accelerep.netlify.app, hard-refresh to `index-M4Pa72MW.js`):** Settings → Quoting → Price book → list price, cost, a bundle’s price and the editor’s big price in the app’s face (SKUs still typewriter); Settings landing → Recently changed → the job’s row reads "Report delivery job report delivery sent"; **switch orgs in the Clerk menu while on Settings** → the feed and the health tile change WITH the cards (no refresh) — the §0.148 (3) proof.
- ~~**The ship — his call, ONE code commit** (`334a079`, bundle only; pre-flight as before).~~ **DONE — the TWENTY-EIGHTH ship, 19:39:50 UTC 16 Sep (above). `master` == `dev` at the ship; the next `git log origin/master..dev` should show ONLY this handoff.**
- **Closed by evidence and by his look:** prod’s scheduled deliveries (both orgs; the Send-now case delivered at its noon slot); the "Scheduled jobs running" health line green on prod. **Open, small (state §9):** the landing page’s cards show zeros for the second before `settings` arrives after a hard refresh.

**Nineteenth-session prep — first (§0.147, the newest — after the TWENTY-SIXTH ship):**
- **Ritual** (item 1), then `git log --oneline origin/master..dev` — expect, after `459c321`: the ship-record docs commit `d9cc563`, **`0bccf4b` (CODE — §0.147, bundle)** and this handoff; anything else is a finding. `deno.lock` dirty (two lines) is the CLI’s, known.
- **§0.147 on dev — Jeff’s look (accelerep.netlify.app, hard-refresh to `index-BZQXq9Av.js`):** Quotes → a quote with notes in the builder → Customer preview → a NOTES block above the signature lines; Preview PDF → the notes in the file (or in the print page, if that is what opens — tell me which, it names the surface he saw); the saved card → "Edit in builder →" a sand bar; List total / Discount / Net in the app’s face.
- ~~**The ship — his call, ONE code commit** (`0bccf4b`, bundle only; pre-flight as before).~~ **DONE — the TWENTY-SEVENTH ship, 16:39:20 UTC 16 Sep (above). `master` == `dev` at the ship; the next `git log origin/master..dev` should show ONLY this handoff.**
- **Open questions for Jeff:** the Price book panel’s monospace money (match it, or leave it); prod’s Jobs tile (Settings → the Jobs tile → Report delivery "ok") — the delivery itself is proven by his inbox, the tile is unobserved.

**Nineteenth-session prep — first (§0.146, the newest — the Quotes font; and the CI now green):**
- **Ritual** (item 1), then `git log --oneline origin/master..dev` — expect, after `930e276`: `4d6b0f5`, the handoff, `ddac313`, `d48b3f6` (§0.143), handoffs, `5df4736` (§0.144: the workflow + one test), `b809237` (§0.145: one suite + one scan test + the harness), the handoff, **`fffbfb8` (CODE — §0.146, bundle)** and this handoff; anything else is a finding. `deno.lock` dirty (two lines) is the CLI’s, known.
- **§0.146 on dev — Jeff’s look (accelerep.netlify.app, hard-refresh to `index-leQ6sUwD.js`):** Quotes → Catalog → the List price column in the app’s face (no longer typewriter digits); the preview’s prices and totals the same; SKUs still monospace.
- ~~**The ship — his call, FOUR code commits on dev beyond master**~~ **DONE — the TWENTY-SIXTH ship, 13:31:49 UTC 16 Sep (above). Prod observations, Jeff’s (salespipelinetracker.com, hard-refresh to `index-BqHYVvqM.js`): Settings → Security → Audit log after one action of each kind — his name as Actor, the new words; Quotes → Catalog → list price in the app’s face; Start quote → the one placeholder; the Jobs tile → Report delivery ok.** The old bullet, kept: FOUR code commits on dev beyond master (`4d6b0f5`, `ddac313`, `d48b3f6` functions AND bundle; `fffbfb8` bundle; `5df4736` and `b809237` are tests and the workflow). Pre-flight as before; the schema diff EMPTY. **CI is green on dev — a red run on the master push is a NEW cause** (read its first `not ok` the same way).
- **Open questions for Jeff:** the Price book panel’s monospace money (match it, or leave it); prod’s Jobs tile (Settings → the Jobs tile → Report delivery "ok" within five minutes) — the one held check left.

**Nineteenth-session prep — first (§0.143, the newest — the audit log):**
- **Ritual** (item 1), then `git log --oneline origin/master..dev` — expect, after `930e276`: **`4d6b0f5` (CODE — §0.142 + the §0.140 correction)**, the handoff, **`ddac313` (CODE — the range label)**, **`d48b3f6` (CODE — §0.143)** and this handoff; anything else is a finding. `deno.lock` dirty (two lines) is the CLI’s, known.
- **§0.143 on dev — Jeff’s check (accelerep.netlify.app, hard-refresh to `index-D5O2AGMO.js`):** do ONE thing of each kind he cares about — save a report, Deliver → Save schedule, Send now, start a quote, edit a dispatch job’s status, dismiss a recommendation — then Settings → Security → Audit log → a row per action, **Actor = his name** (not "System", not an id), the action word (`saved_report.scheduled` with the schedule in words; `quote.created`; `dispatch_job.updated` with `from → to`), the category filter placing saved reports and dispatch under Data. The job’s own row (`report_delivery.sent`, actor "Report delivery job") appears on prod only, at the next scheduled send — after the ship. A row missing for an action he took is a finding: name the action.
- **The ship — his call, THREE code commits now** (`git push origin dev:master`; functions AND bundle; pre-flight as before: `git diff origin/master..dev -- db/schema.ts` EMPTY, `origin/master` an ancestor). Before noon Chicago if he means to Send now on the other org’s 12:00 report first (the Send-now flaw runs on prod until then).
- **CI — GREEN: `unit` since `5df4736` (§0.144), `integration` since `b809237` (§0.145) — the run for `b809237`: unit SUCCESS, gates SUCCESS, integration SUCCESS — the first fully green run since 13 Sep.** The next red run is a NEW cause; read its first `not ok` the same way (the run → the job in the left column → the log search). Nothing else in this bullet stands.
- **Flagged, carried:** the thirteen files that audited before §0.143 write rows for their mass paths only — a plain PUT on a deal writes none (state §9, a batch of its own); rows carry no caller role (§9); the log endpoint’s 500-row window wants paging now that every path writes (§9); the site key (dev’s answers 401; prod’s unknown; `ai-score.mjs` shares it); the Quote templates panel (state §9: two jobs); AI deal scoring has no switch; the five native time inputs; the interpreter’s gaps.

**Nineteenth-session prep — first (after the TWENTY-FIFTH ship; §0.142 and the Send-now correction on dev):**
- **Ritual** (item 1), then `git log --oneline origin/master..dev` — expect, after `930e276`: **`4d6b0f5` (CODE — §0.142 + the §0.140 correction)** and this handoff; anything else is a finding. `deno.lock` dirty (two lines) is the CLI's, known.
- **The scheduled reports — what to expect on prod (Jeff: "they do not seem to be being sent"):** Karen's daily 13:50 and Jeff's daily 15:50 (America/Chicago, the Accelerep Test org) fire TOMORROW within five minutes of their time — the first sends. The other org's daily 12:00 fires at 12:00 tomorrow UNLESS a Send now is pressed within the 20 hours before it — that is the flaw `4d6b0f5` fixes, and prod runs the old rule until the next ship. His call: **ship `4d6b0f5`** (`git push origin dev:master`; functions AND bundle; pre-flight as before, the schema diff EMPTY) — before noon Chicago tomorrow if he means to Send now on that report first. After the ship, the dialog's "Last sent" reads any send and names the last scheduled one when it differs.
- **§0.142 on dev — Jeff's check (accelerep.netlify.app, hard-refresh to `index-uCWPULHc.js`):** Quotes → Start quote on any deal → one placeholder card ("No quote templates yet …") and Start from scratch; no SMB Starter / Growth Package / Enterprise / Trial → Paid. Create quote → the blank quote opens as before.
- **After the TWENTY-FIFTH ship — prod observations, Jeff's (salespipelinetracker.com, hard-refresh to `index-DfRbwLhy.js`; prod's rows are his):** Settings → Features & AI → the "Claude reads report prompts" switch (OFF on every prod org until he turns it on); with it on and a BYOK key installed on a prod org → Reports → Saved reports → + Create report → a prompt → READ BY · Claude; without a key the note names the site key's rejection (if prod's `ANTHROPIC_API_KEY` is as bad as dev's); the Reports prompt builder, the Filters tab and the custom range (§0.139); Deliver → the house time dropdown at five-minute steps (§0.140); Settings → the Jobs tile → Report delivery refreshing every five minutes.
- **Flagged, carried:** the site key (dev's answers 401; prod's unknown; `ai-score.mjs` shares it); the Quote templates panel — no line items on a template, and four invented defaults an Admin's Save persists (state §9: two jobs); AI deal scoring has no switch (state §9); the five native time inputs (state §9); the interpreter's gaps; no per-user reader choice by design.

**HELD from the eighteenth session — Jeff: "Hold those last checks and remind me." These were his; the ship item is DONE (the twenty-fifth ship, 21:10 UTC 15 Sep); the two observations remain:**
- **§0.138 on dev — Jeff's check** (accelerep.netlify.app, now `index-B0ZTf3LY.js`): nothing should LOOK different except the last Tailwind traces going warm — Quotes (the status pills and the quote table), the Viewing bar's slice chips and the filter pill, a rep's quota card (Sales Manager), the Quick log fab, an account's or contact's do-not-contact notice (amber) and merge notices (red wash), a deal's modal (the ink header, the health line), the merge review dialogs, the document upload rail's error line; the error boundary's Retry button is ink now.
- **Prod's hourly report delivery and the Jobs tile** (salespipelinetracker.com): the first fire of a scheduled delivery at its own hour, and Settings → the Jobs tile's Report delivery row "ok" — unobserved since the twenty-fourth ship. **After the NEXT ship this changes:** the job runs every five minutes (§0.140), so the row refreshes every five minutes and a schedule fires within about five minutes of its time.
- **Then the ship** — his call: `git push origin dev:master` — now FOUR code commits (§0.138 bundle only; §0.139 functions AND bundle; §0.140 functions AND bundle AND the cron; the follow-up bundle only). Pre-flight as before: `git diff origin/master..dev -- db/schema.ts` EMPTY (no schema change in any of them); `git diff --stat origin/master..dev` names `netlify.toml`, `netlify/functions/report-deliveries.mjs`, `src/`, `tests/`, `scripts/`, `docs/`. **Prod's cron for `report-deliveries` changes from hourly to every five minutes at the ship** — expect it in Netlify's deploy record and, from the first five minutes, in the Jobs tile.

**Nineteenth-session prep — first (§0.141, the newest):**
- **The site key — a finding that stands:** the DEV site's `ANTHROPIC_API_KEY` answers 401 `invalid x-api-key` (observed through `netlify dev`, which injects the site's env). Prod's is unknown, and `ai-score.mjs` has used the same site key for as long as it has existed — if prod's is the same value, AI deal scoring has been failing quietly too. The Accelerep Test org now has Jeff's BYOK key (which wins over the site key) — any OTHER org, and any org on prod, still falls to the site key. His to check in Netlify's env (dev and prod).
- **§0.141 on dev — Jeff's check (accelerep.netlify.app, hard-refresh to `index-Be4l2aWO.js`; the switch is ON and his BYOK key installed on the Accelerep Test org):** Reports → Saved reports → + Create report → a prompt → "ASKING CLAUDE…" (10–20 s) → READ BY · Claude and the report the sentence asked — observed on local three times (§0.141), his to see on the dev site once: a paraphrase ("who is closing the most this quarter"), a stage in words ("deals in Negotiation/Review nobody has touched in 3 weeks" → Stage is Negotiation/Review · ≥ 21 days · Open deals), and a "cannot do" ("… and our win rate" → the note). Then the switch OFF (Settings → Features & AI) → the same prompt → READ BY · Built-in interpreter and the note that AI assistance is available; back ON. Also: Deliver on any report → At is the house dropdown, five-minute steps, type "915" to jump.
- **The stage filter, corrected (§0.141 fix b):** the built-in reader's "Karen's deals in Proposal" now filters — before `1482fc5` the stage filter was silently dropped by the engine. Any report saved with a stage filter before today (none known) runs it now.
- **Ritual** (item 1), then `git log --oneline origin/master..dev` — expect, after `a4f4589`: the ship-record docs commits, **`e955801` (CODE — §0.138)**, the handoffs, **`e302346` (CODE — §0.139)**, **`a83be59` (CODE — §0.140)**, **`5ea58b8` (CODE — the §0.139 follow-up)**, **`ac22b82` (CODE — §0.141)** and this handoff; anything else is a finding. `deno.lock` dirty (two lines) is the CLI's, known.
- **Flagged, carried (§0.141):** AI deal scoring has no switch anywhere in Settings (state §9); the five native time inputs left in AppHeader and DispatchTab (state §9 — a sweep of its own); no per-user reader choice by design; the system prompt is not cached.

**Nineteenth-session prep (§0.139 / §0.140; kept):**
- **Ritual** (item 1) as above.
- **§0.139 on dev — Jeff's check (accelerep.netlify.app, hard-refresh to `index-B0ZTf3LY.js`):** Reports → Saved reports → + Create report → type HIS prompt ("Quota Performance By Rep") → GENERATE → the banner reads it (Opportunities · grouped by Owner · Revenue · All time · Bar) AND says "Quota attainment is the Performance tab's; the builder has no quota field" — the report is revenue by rep, not the stuck-deals chart, and its title is his prompt. Then a starter chip → GENERATE → each one a different report. Then any of: "Open deals over $50k by owner", "Calls per rep last 30 days", "How many leads by source this quarter", "Karen's deals in Proposal" (the org's stage and roster names become filters), "Win rate by rep" (the note says it cannot). Filters tab → the chips with ×, "+ Add" a filter (Deal size ≥ 100000) → Update preview → the count drops; Period → Custom range… → two dates. Save to library → the card → open it → the filters are still there. Pin it → Home draws it with the filters. Deliver → Save & send now → the email's table matches the builder's picture (the filters ran on the server).
- **§0.140 on dev — Jeff's check:** Deliver on a report → At is a time picker → pick 8:37 → Save schedule → the card's footer reads "… 08:37 America/Chicago …" → reopen → 08:37 still. **The send at that minute is prod's** (the dev site does not run the jobs): after a ship, a schedule for a few minutes ahead on a prod report → the email or Slack post within about five minutes of it; the Jobs tile's Report delivery row refreshing every five minutes.
- **The rep path as Karen (role User)** — the prompt builder as a rep: her prompts run over HER rows only (the tab's scope), a saved report of hers with filters reopens with them. Not run this session (the pane holds Jeff's session).
- **Flagged, carried:** a Claude-backed reading of the prompt (state §9 — the shape is written; a key, a cost and a gating decision are Jeff's); the interpreter's gaps (no "top N", no team filter, no compare-to, a quarter grouping reads as months with a note); the WhereEditor's add row and the custom range's date inputs not driven in the pane; the email names the cadence, not the time; a "next send" instant is not shown on the card; the reminder modal takes the first click on the Reports tab.

**Eighteenth-session prep — first (§0.138; kept — the check is HELD above):**
- **Ritual** (item 1), then `git log --oneline origin/master..dev` — expect, after `a4f4589`: the ship-record docs commits, **`e955801` (CODE — §0.138)** and this handoff; anything else is a finding. `deno.lock` dirty (two lines) is the CLI's, known.
- **§0.138 on dev — Jeff's check (accelerep.netlify.app, hard-refresh to `index-DIxtA4Km.js`):** nothing should LOOK different except the last Tailwind traces going warm — Quotes (the status pills and the quote table), the Viewing bar's slice chips and the filter pill, a rep's quota card (Sales Manager), the Quick log fab, an account's or contact's do-not-contact notice (amber) and merge notices (red wash), a deal's modal (the ink header, the health line), the merge review dialogs, the document upload rail's error line. The error boundary's Retry button is ink now (seen only if something throws).
- **Then, his call: ship** (`git push origin dev:master` — ONE code commit, bundle only; no function, no schema change).
- **Flagged, carried:** the three dialogs on the `.modal` CSS class; the warm palette's own literals (~200 in these files) and OpportunityModal's forecast-category badge palette — a value-identical pass by token name, if wanted; `useSettings`' sixteen stored defaults are data and stay; the hourly report delivery's first fire on prod and the Jobs tile row.

**Eighteenth-session prep — first (before §0.138, kept):**
- ~~**After the TWENTY-FOURTH ship — prod observations, Jeff's**~~ **DONE — Jeff (15 Sep): the plate row "clear", "Dispatch worked", "Reports worked", "Leads worked". The hourly delivery at its own hour and the Jobs tile's Report delivery row were not named — unobserved, the next session's first look.** Was (salespipelinetracker.com, hard-refresh to `index-zv28zhMt.js`; prod's rows are his — read before assuming anything exists):** Home → On your plate → a row's stage on its own line, nothing crossing the title (§0.137); Dispatch → Job Board → This week → drag a job a day and back (§0.134); Reports → Saved reports → a card's Pin · Share · Deliver; Deliver on a report of his → Save & send now → the email and the Slack post (§0.135) — then a schedule for the NEXT hour, and at that hour the send arrives on its own (the hourly job runs on prod only) and Settings → the Jobs tile shows Report delivery "ok"; Leads → Import and a lead's trash icon → the warm dialogs (§0.136).
- **Ritual** (item 1), then `git log --oneline origin/master..dev` — expect ONLY the ship-record docs commit(s) after `a4f4589`; any CODE commit is a finding.
- **Flagged, carried:** §0.136's remaining 140 off-brand literals in 18 files (the same script, more files); the three dialogs on the `.modal` class; the plate row not measured live in the pane.

**Eighteenth-session prep (before the ship, kept):**
- **Ritual** (item 1), then `git log --oneline origin/master..dev` — expect, after `c17e2e8`: the ship-record docs commit, **`9fb744a`, `5bbd813` (CODE — §0.134)**, **`62dc20a` (CODE — §0.135)**, **`f0705c0` (CODE — §0.136)**, **`9bce063` (CODE — §0.137)**, the docs commits and this handoff; anything else is a finding. `deno.lock` dirty (two lines) is the CLI’s, known.
- **§0.137 on dev — Jeff’s check (accelerep.netlify.app, hard-refresh to `index-oOwpiwjs.js`):** Home → On your plate → the row whose deal is in Negotiation/Review (his screenshot’s "Send proposal to ZZFX Ashgrove Holdings") reads "TODAY", the amount, then "Negotiation/Review" on its own line in the stage colour, nothing crossing into "Follow-up · ZZFX Ashgrove Holdings"; the Open deal button where it was.
- **Then, his call: ship** (`git push origin dev:master` — FIVE code commits; `report-deliveries` (§0.135) is a NEW scheduled function; pre-flight as before: `git diff origin/master..dev -- db/schema.ts` EMPTY).
- **The rep path as Karen — DONE, Jeff (15 Sep), signed in as Karen (role User) on accelerep.netlify.app:** (1) the week-board drag of a scheduled job worked both ways; (2) a card dragged to a day the technician is off was refused with the warning; (3) "Untitled report" appears under Shared with you, marked SHARED, with only the Pin button; (4) pinned, it shows on Karen's Home; (5) Karen's own report is visible to Jeff as Admin. §0.134's rep path and §0.135's sharing are both observed.
- **Flagged, not done:** the plate row was not measured live (the pane had signed out) — the scan is the proof until he looks; §0.136’s remaining 140 off-brand literals in 18 files.

**Eighteenth-session prep — first (§0.136, kept):**
- **Ritual** (item 1), then `git log --oneline origin/master..dev` — expect, after `c17e2e8`: the ship-record docs commit, **`9fb744a`, `5bbd813` (CODE — §0.134)**, **`62dc20a` (CODE — §0.135)**, **`f0705c0` (CODE — §0.136)**, the docs commits and this handoff; anything else is a finding. `deno.lock` dirty (two lines) is the CLI's, known.
- ~~**Push to dev if not pushed**~~ **DONE — pushed 22:25:11 UTC 14 Sep; accelerep.netlify.app served `index-5_oMz9NV.js` from 22:25:42 UTC.**
- **§0.136 on dev — DONE, Jeff (14 Sep): "everything testing correctly". The list, for the record (accelerep.netlify.app, `index-5_oMz9NV.js`):** every dialog these six files draw now reads in the warm palette — no blue button, no white panel, no slate grey: Leads → Import (the dark band "Import Leads from CSV", the ink step badge, the drop zone); a lead's trash icon → the confirm dialog (the surface panel, the red Delete, Cancel on the surface) → Cancel; Contacts → New contact (the ink "Create Contact"); Settings → Users → add or edit a user (UserModal); Contacts → Import from Outlook (OutlookImportModal); the offline banner if it ever shows (amber for an auth problem, red for an outage); sign out and back in — the sign-in screen's mark is ink with a green line, the "No Organization Found" screen warm. The stage pills and the avatar initials keep their colours on purpose (data palettes).
- **Then, his call: ship** (`git push origin dev:master` — FOUR code commits; `report-deliveries` (§0.135) is a NEW scheduled function; pre-flight as before: `git diff origin/master..dev -- db/schema.ts` EMPTY).
- **Flagged, not done (state §0.136):** 140 off-brand literals remain in 18 files (QuotesTab 20, ViewingBar 18, useSettings 16 — data, QuotaRepCard 14, QuickLogFab 10, the Account and Contact rails 9 each, ErrorBoundary 7, OpportunityModal 7, the two merge modals 5 each, five single-line files) — the same script with more files; the confirm / prompt / blocked-delete dialogs still wear the `.modal` CSS class (its background is warm now; the guide wants inline chrome in new work); the audit's regex misses 8-digit hexes; the stage pill palette and the avatar palette are Jeff's call.

**Eighteenth-session prep — first (§0.135, kept):**
- **Ritual** (item 1), then `git log --oneline origin/master..dev` — expect, after `c17e2e8`: the ship-record docs commit, **`9fb744a`, `5bbd813` (CODE — §0.134)**, the docs commits, **`62dc20a` (CODE — §0.135)** and this handoff; anything else is a finding. Compare against `origin/master`, never local `master`. A dirty `deno.lock` (two lines) is the CLI's, known.
- ~~**Push to dev if not pushed**~~ **DONE — pushed 21:20:18 UTC 14 Sep; accelerep.netlify.app served `index-ODWcjiOD.js` from 21:20:59 UTC.** **The dev site does NOT run scheduled jobs** (`JOBS_ENABLED` is prod's) — on dev the schedule is stored and Send now works; the hourly send is a prod behaviour.
- **§0.135 on dev — DONE, Jeff (14 Sep): "everything testing correctly" (the email and the Slack post already seen in his screenshots). The list, for the record (accelerep.netlify.app; as Karen for the rep path still open):** Reports → Saved reports (the "custom" sub-tab) → a card of yours now carries **Pin · Share · Deliver · ×** in its footer. **Pin** → "Pinned" → Home → a "Pinned reports" section draws the report's chart with "Open →" (it opens the report in the builder) and "Manage in Reports →"; refresh → still pinned (it is on your profile); Pin again → "Pin" → gone from Home. **Share** → a SHARED mark on the card; as Karen the report appears under "Shared with you" with Pin only (no Share / Deliver / ×); as Karen a report of hers that is NOT shared must not appear for you unless you are Admin (you are — it appears under "Shared with you" with no mark). **Deliver** → the dialog: Delivery on, Every week, Monday, 8:00 AM, your zone prefilled, Email to (tick yourself), Post to Slack (enabled — your workspace has Slack) → **Save & send now** → the green line "Sent to <your email>, Slack — N rows in the period." → the email (a table: the report's groups and totals, an Open in Accelerep button) and the #sales-alerts post (a header, a context line, the table in a code block, a button); the card's footer now reads "⏱ Every week · Monday 08:00 <zone> · email to 1 · Slack". Untick everything → Save & send now is disabled ("Pick a recipient, or turn Slack on, first"). **The two preferences:** Tasks → All time → another tab → back → still All time; Dispatch → Job Board → This week → another tab → back → still This week (anchored on today).
- **Then, his call: ship** (`git push origin dev:master` — THREE code commits; `report-deliveries` is a NEW scheduled function: expect it in Netlify's deploy record and, after the first hour, in the Jobs tile; pre-flight as before: `git diff origin/master..dev -- db/schema.ts` EMPTY; `git diff --stat origin/master..dev` names `netlify/functions/`, `netlify.toml`, `src/`, `tests/`, `scripts/`, `package.json`, `CLAUDE.md` and `docs/`). Prod check after: a schedule set for the NEXT hour on a prod report, then the email or the Slack post at that hour and the card's "Last sent"; the Jobs tile's Report delivery row "ok".
- **Flagged, not done (state §0.135):** recipients are roster members only; one hour slot per schedule; the email is a table, not the chart; Home draws a pinned report over the app's visibility (`visibleOpportunities`) while the tab scopes by name (a Manager may see a difference); a report whose owner leaves the roster stops delivering with a named error on the card; the four hard-coded "Pinned" live tiles in the library are unchanged; the rep path as Karen for §0.134 is still open.

**Eighteenth-session prep — first (§0.134, kept):**
- **Ritual** (item 1), then `git log --oneline origin/master..dev` — expect, after `c17e2e8`: `cc42c98` (the ship-record docs commit), **`9fb744a` (CODE — §0.134)**, **`5bbd813` (CODE — its follow-up)** and this handoff; anything else is a finding. Compare against `origin/master`, never local `master`. A dirty `deno.lock` (two lines) is the CLI's, known.
- ~~**Push first if not pushed**~~ **DONE — pushed 20:01:59 UTC 14 Sep; accelerep.netlify.app served `index-BULS4hTf.js` from 20:02:28 UTC.**
- **§0.134 on dev — DONE, Jeff (14 Sep): "it works now very well." The rep path as Karen (role User) is still open — the pane's pass and his were Admin. The list, for the record (accelerep.netlify.app, `index-BULS4hTf.js`):** Dispatch → Job Board → **This week** → a strip under the range bar reads "Drag a job to another day to reschedule it, or onto another technician's row to hand it over." → drag a scheduled job's card one column right on its own row → the card fades while in hand, the cell under it takes a dashed gold outline, and on drop the strip reads "Moved <job> to <date> · <tech>." for 8 s, the card sits in the new column with the SAME time, and the job's record (click it) shows the new date with the start time unchanged → drag it back. Then drag a card from one technician's row onto ANOTHER technician's row on the same day → the card moves to that row (the seat handed over; on a two-person crew the other member's row keeps it) → the record's crew shows the new technician. Then a refusal: drag a card onto a technician whose Work Schedules row is not rostered that day (or who is marked out) → the strip turns red with the reason in words ("<name> is not rostered on <day>." / "<name> is out (<type>) on <date>.") and NOTHING moves; Dismiss clears it. Then a no-op: drag a card and drop it back on its own cell → nothing happens, nothing is said. If Customer notifications are ON for the org (Settings → Dispatch → Customer notifications), a day move of a SCHEDULED job sends the confirmation again — the job's notification trail shows it; that is §0.111's rule, not new. Each move writes an audit line `dispatch.reschedule` (Settings → Audit).
- **Then, his call: ship** (`git push origin dev:master` — TWO code commits `9fb744a` and `5bbd813`, bundle only; no function files change; no schema change; pre-flight as the twenty-third ship's: `git diff origin/master..dev -- db/schema.ts` EMPTY, `git diff --stat origin/master..dev` names only `src/`, `tests/`, `scripts/` and `docs/`). Prod check after: salespipelinetracker.com hard-refreshed to a NEW hash; the week board's strip present.
- **Flagged, not done (state §0.134):** the day board (hour lanes) and the month grid do not drag; a drop cannot change the TIME (row = technician, column = day); dropping a crewed job on the Needs-a-crew row is refused rather than an unassign; `hoursThisWeek` on the technician rows is a load-time figure (the over-hours soft blocker reads it until a refresh — same after `onScheduled`); `onScheduled` and `onHeld` still leave `jobsRaw` stale after a crew-builder write (the Jobs view shows the old crew until a refresh) — the drop handler updates both, they should too; touch is not supported.

**Seventeenth-session prep — first (the sixteenth's list, kept):**
- ~~**After the TWENTY-SECOND ship — prod observations, Jeff's**~~ **DONE — Jeff (14 Sep): "All tabs look great on production."** Was (salespipelinetracker.com, `index-D0HaH5AU.js`): every tab announces itself the way Pipeline does (Tasks with the date under the title; Dispatch with "Dispatch" and the view in the caption; Documents full width; Reports with the gutter; Settings in the serif); the app looks otherwise exactly as before (the one token object — nothing visible changed except the three fixes named in state §0.130). Prod's rows are his; read before assuming anything exists there.
- ~~**After the TWENTY-THIRD ship — prod observations, Jeff's**~~ **DONE — Jeff (14 Sep): "All tests passed."** Was (salespipelinetracker.com, hard-refresh to `index-Ct7x96Jv.js`): the §0.133 walk in the next bullet, on prod's OWN deals (prod's rows are his; read before assuming anything exists there — the fiscal year there is whatever prod's Settings say); Settings → Integrations → Automations → + New automation → the Tasks group offers "Task overdue" with its note, and a rule with a Slack action posts Block Kit in his channel (the section, the context line, the button); Settings → Sales process → Lead scoring → Train now answers with the floor or a model. **The hourly fire is live on prod from 18:54 UTC** (`JOBS_ENABLED`): a task.overdue rule there is his to create; nothing fires until one exists (the bullet below).
- **Ritual** (item 1), then `git log --oneline origin/master..dev` — expect ONLY the ship-record docs commit after `c17e2e8` (this handoff); any CODE commit is a finding. Compare against `origin/master`, never local `master`.
- **§0.133 on dev — Jeff's checks (accelerep.netlify.app, hard-refresh to `index-BNgFKZx-.js`):** Reports → Saved reports → + New report → Blank canvas → name it → Data: pick a source (the chips change per source), add Owner and Stage, keep Revenue → Filters: Period → Q3 → Chart: Stacked bar → Update preview → the real numbers, grouped, with the footer "N of M opportunities in the period · K groups" → Save to library → the card names it → click the card → it opens INTO the builder with the same picture; change the chart, Save → the card updates in place, no second card. Pick Line with Owner first → the note "A line needs a date dimension first…" and a bar. Table shows every group with every measure. Any report saved before today opens with its fields resolved by label (a dropped field is named in a warning).
- **§0.132 on dev — Jeff's checks: DONE for the rule flow, the blank-name warning and the Block Kit post (his Slack screenshot, 14 Sep); the Train now answer was observed in the pane. The list, for the record** (Admin, accelerep.netlify.app): Settings → Integrations → Automations → + New automation → the Tasks group offers **Task overdue (past its due date)** with the note on how it fires; pick it → Conditions offers Days overdue, Due date, Priority; Actions → Create task with "Chase: {{title}} ({{days_overdue}}d late)" → Create. Settings → Sales process → Lead scoring → Predictive scoring → **Train now** → on dev the answer is "Needs at least 20 decided (Converted / Dead) leads to train — this workspace has N." (the dev org had 1 decided lead of 25 at §0.123; nothing is stored). A Post to Slack rule firing on dev (Slack connected there?) shows the message as a section with "Automation: *rule* · trigger · record" under it and an Open Accelerep button — else the run history still reads "send_slack: skipped — Slack is not connected".
- **The hourly fire is prod-only** (`JOBS_ENABLED`): after a ship, a task.overdue rule on prod is Jeff's to create; an open task past due, assigned to him, fires at his alert hour once — the run history names the task; the second hour does not fire it again (the week's dedup). Prod's rows are his; read before assuming any task or rule exists there.
- ~~**Then, his call: ship**~~ **DONE — the TWENTY-THIRD ship, 18:53:25 UTC 14 Sep (Jeff: "ship master"; the record at the top of this file and in state §0.133's last paragraph). Netlify re-uploaded NINE function bundles, not every function — the five changed and the four endpoints importing the automation module.**
- **Flagged, not done (state §0.132):** the webhooks vocabulary still lists `task.overdue` and nothing fires a webhook for it; `days_overdue` counts UTC days like every deal signal; Train now trains on the SAVED config (the button says so while dirty); a large org's Train now runs the nightly engine inside one function invocation.
- **Ritual** (item 1), then `git log --oneline origin/master..dev` — expect, after `ae0cae8`: the two ship-record docs commits, `9f4d35c` (design docs + `scripts/audit-colors.mjs`, NOT code), `37d665f`, `1b55199`, **`10a78bc` (CODE — §0.130)**, `59b78bc` (handoff), **`4bb37ef` (CODE — §0.131)**, **`c9bf692` (CODE — the §0.131 follow-up)**, then the handoff commit after it; anything else is a finding. Compare against `origin/master`, never local `master`.
- ~~**§0.131 on dev — Jeff's check**~~ **DONE — Jeff (14 Sep): "updates to tabs looks great."** Was (accelerep.netlify.app, `index-CyIfr4JI.js`, desktop): every tab now announces itself the way Pipeline does — Tasks with the date under the title; Dispatch with "Dispatch" and the view in the caption (Job Board shows the date range there, Customers "2 dispatch customers"); Documents full width; Reports with the gutter; Settings in the serif. His screenshots were the desktop view; the pane measured a phone width only.
- **§0.130 on dev — Jeff's checks (accelerep.netlify.app, hard-refresh to `index-D0WIMRUE.js`):** the app looks exactly as before everywhere — that is the whole claim. The three deliberate fixes, if he wants to see them: Home with NO calendar connected → the "Connect calendar" button now has the 3px radius every other button has; Reports → the sales-manager template (the one with the `TS` tokens — Performance/Activity cards) → a Call activity's swatch is slate, not grey, and the metric chips carry a slate left border. Dispatch still radius 4 (by design, recorded).
- **Then, his call: ship** (`git push origin dev:master` — THREE code commits `10a78bc`, `4bb37ef`, `c9bf692`, all bundle only; no function files change; no schema change; pre-flight as the twenty-first ship's: `git diff origin/master..dev -- db/schema.ts` EMPTY, `git diff --stat origin/master..dev -- netlify/` EMPTY).
- **Flagged, not done (state §0.130):** the `KPIThresholdsDetail` "Color palette" card prints `#4d6b3d` etc. as TEXT labels beside `T.ok`/`T.warn`/`T.danger` swatches — it reads wrong the day a value moves (it should print the token's value); `docs/design/color-audit.md` §5 (the token-copy map) now describes the tree BEFORE this batch — regenerate with `node scripts/audit-colors.mjs` if the audit is ever resumed; ~~`netlify dev` does not start on this machine (the Neon build-hooks plugin's token permission)~~ **RESOLVED 14 Sep (Jeff: "did the npm and logged out and back in to netlify")** — the CLI's login token predated the `extension_site_config:read` scope the Neon extension demands; `npm install -g netlify-cli@latest` (24.3.0 → 27.6.0), `netlify logout`, `netlify login`; the server then started (every function loaded, no extension error, localhost:8888 HTTP 200, the Clerk sign-in page in the pane — the local session is separate from the dev site's). The `netlify-dev` launch config works again; the pane can verify locally before a push.
- **The signal migration is ON HOLD (Jeff, 14 Sep: "leave things as are for now. I am not sure that is the direction I want to go") — build nothing from it unless he says so.** If he resumes: the brief's FIRST gate — Jeff decides, then Phase 2. Read `docs/design/SPINE_COLLISION.md` §7: (1) resolution **A** (severity owns the 3px left spine; selection = `surface2` + 5px ink) or B / C; (2) job priority → severity (emergency → critical, high → attend) or dots; (3) signal pills at radius 2 everywhere or only in dense lists (guide §16 line 740 changes either way); (4) destructive stays `T.danger` (recommended); (5) the header badges' rule (`AppHeader.jsx` 325/330/443/448 on the ink band — the deep ramp is invisible there). Then his read of `docs/design/color-audit.md` §7 — every `?` and every `destructive`/`event` split of `T.danger` is a decision. **With the answers: Phase 2** — `RAMP` / `SIGNAL` / `FORM` in a plain `.js` (so `node --test` can import them for the contrast gate, both directions AND the ink band), the four primitives at module scope (`SignalPill`, `SignalRow`, `SeverityDots`, `TrendCard`, matching the `look === 'tonal'` branches of `docs/design/handoff-signal-tonal/src/signal-palette-shared.jsx`), a visual test page, NO call sites; the new test file added to `SUITES` in `scripts/mutate-import.mjs`. Then Phase 3 in the brief's order (trends → events → status pills → info → the rest), one commit per batch, `ReportsTab.jsx` and `DispatchTab.jsx` one file per commit. `MIGRATION-NOTES.md` at the end. Do NOT touch the exclusions (gold, the ink text ramp, the header band, file types, document categories, tiers, stages) or the off-brand Tailwind literals (a separate cleanup).
- **After the TWENTY-FIRST ship — prod observations, Jeff's (salespipelinetracker.com, hard-refresh to `index-BnV7qKA1.js`), in order:** Leads → Triage → with the mouse on a card, click its score badge → the card opens in full, nothing clipped, no scrollbar on the strip. Settings → Integrations → Automations → a rule's ⋯ → Edit → Actions → Assign to is a SELECT over prod's roster; + New automation → Deal health shows five triggers; Actions offers Post to Slack (a blank message refused at Create). **The two new triggers fire on prod's hourly job** at the rep's alert hour for a deal that meets the signal, once per week — a rule on them is his to create; prod's rows are his, read before assuming any rule, lead or claim exists there.
- **Ritual** (item 1), then `git log --oneline origin/master..dev` — expect ONLY the two ship-record docs commits after `ae0cae8` (the state commit, then this handoff); any CODE commit is a finding. Compare against `origin/master`, never local `master`.
- **§0.127 on dev — Jeff's checks (Admin, accelerep.netlify.app, hard-refresh to `index-838pOcA_.js`):** Settings → Integrations → Automations → + New automation → the trigger step's Deal health group now has FIVE — Deal gone silent, Deal stuck, Close date lapsed, **Deal gaining momentum (2+ stages in 14 days)**, **AI score dropped below 40**; Actions → the type select offers **💬 Post to Slack** → a Message box with the merge-field hint and the note on the webhook; leave it blank → Create says "Post to Slack: a message is required"; type "{{sales_rep}}: {{opportunity_name}} moved to {{to_stage}}" on a Stage changed rule → Review reads → Slack: "…" → Create; move a deal → the org's Slack (Settings → Connected apps → Slack, if connected on dev) shows the rendered line; with Slack NOT connected the run history reads "send_slack: skipped — Slack is not connected for this workspace". **The two new triggers fire on prod only** (`JOBS_ENABLED`), after a ship, at the rep's alert hour, once per deal per week.
- **§0.128 on dev — DONE, Jeff: "working perfectly" (kept for the record):** Leads → Triage (All) → with the mouse resting on the card, click the 100 badge in Call today → the card opens in full below the badge, nothing clipped, no scrollbar on the strip, and it stays put as the mouse moves; click anywhere else → it closes; the All leads table's badge behaves the same.
- ~~**Then, his call: ship**~~ **DONE — the TWENTY-FIRST ship, 21:47:59 UTC 13 Sep.** Was: (`git push origin dev:master` — FOUR code commits `324c04e`, `ac50a91`, `e2dceca`, `eec5ae2`; two touch functions (`_lib.mjs`, `dispatch-automations.mjs`, `pipeline-alerts.mjs`) — Netlify rebuilds every function; no schema change; pre-flight as the twentieth ship's).
- **Flagged, not done (state §0.127 / §0.128):** `task.overdue` (nothing computes it); the Slack action posts text only; the three integration row menus (`useRowMenu`) are fixed WITHOUT a portal — the same latent trap if a row ever transforms; fold `menuPlacement` into `popoverPlacement`; the card's height allowance is a constant (300); the other absolute popovers (RepPickerPopover, the status picker) not audited.
- **Ritual** (item 1), then `git log --oneline origin/master..dev` — expect, after `7fd83de`: the four ship-record / observation docs commits, `324c04e` (CODE), `c032da0` and `fc6a428` (docs), `ac50a91` (CODE), `e2dceca` (CODE), `209d883` and `af7ef45` (docs), `eec5ae2` (CODE), the landing-line docs commit, then this handoff; anything else is a finding.

**Sixteenth-session prep (§0.126 — kept for the record):**
- **§0.126 on dev — Jeff's checks (Admin, accelerep.netlify.app, hard-refresh to `index-qwmC8w1K.js`):** Settings → Integrations → Automations → the "Proposal > Task" rule's ⋯ → Edit → Actions → "Assign to" reads "The deal's rep (default)" (the rule stored no assignee) → pick **Karen Russell** → Review shows "→ Karen Russell" → Save changes. Pipeline → move any deal into Proposal → Tasks (as Karen, Mine): the new task is hers even for a deal that is not hers (owned by her id). Then Edit → Assign to back to "The deal's rep (default)" → Save changes, so the rule behaves as before. A rule saved before the picker with a typed name shows that name "(by name — not in the roster)" or, if it matches one user, that user.
- **Then, his call: ship** (`git push origin dev:master` — ONE code commit `324c04e`, functions AND bundle; no schema change; pre-flight as the twentieth ship's; two function files change (`_lib.mjs`, `dispatch-automations.mjs`) — Netlify rebuilds every function).
- **Flagged, not done (state §0.126):** the picker offers every active user, Technicians included; Update field and Send email keep typed inputs; the rest of §0.124's list — Slack as an action, `task.overdue`, momentum and score-drop triggers.
- **Ritual** (item 1), then `git log --oneline origin/master..dev` — expect, after `7fd83de`: the four ship-record / observation docs commits (`6998ece`, `b0b6323`, `2c3e67b`, `4d1bfe2`), `324c04e` (CODE), the landing-line docs commit, then this handoff; anything else is a finding.

**Sixteenth-session prep (the TWENTIETH ship's list — Edit and the org switch OBSERVED on prod by Jeff; kept for the record):**
- **After the TWENTIETH ship — prod observations: DONE for Edit and the org switch (Jeff: "tests passed - edit and change org"); the private-window rep sign-in on prod not stated, observed on dev. The list, for the record (salespipelinetracker.com, `index-DdSH6fY8.js`):** sign in as a rep in a private window, taking the MFA step slowly → Home shows the manager's notes, My claims and the calendar strip (or its Connect prompt) at once, and the console has no "DB error 401" and no "waitForToken: no Clerk token" line. As an Admin with more than one org: the avatar strip's switcher → another org → Home reflects it at once → back. Settings → Integrations → Automations → a rule's ⋯ → **Edit** → "Edit automation" seeded from the rule → Save changes → the row updates in place. Prod's rows are his — read before assuming any rule or claim exists there.
- **Ritual** (item 1), then `git log --oneline origin/master..dev` — expect ONLY the two ship-record docs commits after `7fd83de` (the state commit, then this handoff); any CODE commit is a finding. Compare against `origin/master`, never local `master`.
- **§0.125 on dev — DONE, every check OBSERVED by Jeff as Karen (the private-window sign-in: "received no errors"; kept for the record). Was: OBSERVED IN PART by Jeff as Karen (his screenshot: the manager's note, My claims and the calendar prompt on Home after sign-in). Left, if he wants the strict slow-path proof (Karen, role User, accelerep.netlify.app, hard-refresh to `index-s-O_Wnog.js`):** open the site signed OUT (a private window), sign in through the in-app form including the MFA step, taking longer than 8 seconds → Home shows the calendar strip and, as Karen, any coaching note addressed to her, and the console has NO "DB error 401" and NO "waitForToken: no Clerk token" line — before §0.125 that path left the three empty until a refresh. Then, as himself: the avatar strip → the org switcher → another org → Home reflects that org at once (seen from the pane's request log this session; the eyes-on is his) → back to Accelerep Test.
- ~~**Then, his call: ship**~~ **DONE — the TWENTIETH ship, 20:43 UTC 13 Sep.** Was: (`git push origin dev:master` — TWO bundle-only code commits, `b88ee9d` and `83646ca`; no function, no schema change; pre-flight as the nineteenth ship's: `dev` == `origin/dev`, `origin/master` an ancestor of `dev`, `git diff origin/master..dev -- db/schema.ts` EMPTY, nothing under `netlify/` in `git diff --stat origin/master..dev`).
- **Flagged, not done (state §0.125):** §9's neighbour — the `GET /users` 403 noise for non-admin roles (re-check whether anything still 403s: reps receive a directory read now); the calendar strip keeps the previous org's events on screen for the length of the new fetch; `waitForToken` resolves rather than rejects on give-up (loud now; a rejection would be unhandled in `loadData`).
- **Flagged, not done (state §0.124):** unchanged — the list under "Fifteenth-session prep" below.
- (Superseded by the ship — the Ritual line at the top of this list.) Was: **Ritual** (item 1), then `git log --oneline origin/master..dev` — expect, after `b2218b2`: `2aae1bd` (docs), `b88ee9d` (CODE), `b94e2b2` and `00bd9ce` (docs), `83646ca` (CODE), `52ec1b2` (docs), then this handoff; anything else is a finding. Compare against `origin/master`, never local `master`.

**Fifteenth-session prep (DONE — every check below observed by Jeff on dev in the fourteenth session; kept for the record):**
...
- **The menu follow-up (`892c978`), his check after a hard-refresh to `index-DqfNZYvt.js`:** Automations → the row's ⋯ → the menu opens fully BELOW the button (Pause, View run history, Delete), nothing clipped; scroll the page → it closes. With a long list, the last row's menu opens ABOVE its button.
- **The third follow-up (`ba9aefb`), his checks after a hard-refresh to `index-DpgxUDFK.js`:** (a) Automations → ⋯ → View run history → each row reads "Triggered by: <deal name>" (the id on hover). (b) **Delete his empty "Field Test" rule** (⋯ → Delete) and build it again: + New automation → Stage changed → no conditions → action Update field → the Field is now a dropdown → Forecast category, Value `Commit` → Create; move a deal one stage; **hard-refresh** (the write is server-side after the save — the open page holds the old value until it reloads) → the deal's Forecast category reads Commit; run history: Success · 1 action. Then the review step of a new rule with Update field and no field chosen → Create says "Update field: choose the field to set". (c) Settings → Integrations → Webhooks and → API keys: a row's ⋯ menu opens fully, below the button, nothing clipped.
- **The fourth follow-up (`b88ee9d`), his check after a hard-refresh to `index-De3J8tLC.js`:** Automations → a rule's ⋯ → **Edit** → the modal opens as "Edit automation" with the rule's name, trigger, conditions and actions filled in; click any step in the stepper directly; change the task title (or fix a rule's field) → Review → **Save changes** → the row updates in place, still one row, RUNS unchanged; move a deal → the run uses the new title. Then delete Field Test (and Test2) so stage moves stop rewriting the forecast category.
...
- **Flagged, not done (state §0.124):** `task.overdue` (nothing computes it); momentum and score-drop as triggers; a roster picker for "Assign to"; Slack as an action; a rule fires only when the job raises the signal (a deal with no rep, or whose rep has no email or is inactive, raises none — the job's own rule).
...
- **§0.121 on dev — OBSERVED by Jeff** (Turn on, the hosted form, the
  ...
  `org_id` the dev org.
- **§0.122 on dev — OBSERVED by Jeff as Karen** ("Emailed from template as
  karen and the process worked fine"; the logged activity in his screenshot).
  **Remaining, his (bundle `index-s255BQ_M.js`, hard-refresh):** the picker is
  now a DROPDOWN — Contacts → a contact with an email → "✉ Email ▾" → a select
  reading "Choose a template…" with Blank email and each template (name —
  rendered subject) → choose one → his mail client opens filled in and the
  Log-activity dialog opens with "Template: …"; the line under the select says
  nothing is sent until Send is pressed in the client. **On "I have not
  received an email":** the app sends nothing by design — check the browser's
  default mail handler on Karen's machine and her client's Sent folder; if he
  wants Accelerep itself to send, that is the per-org sending-domain project
  in the flagged list, his call. The original checks, for the record: Settings → Sales
...
- **§0.120 on dev — OBSERVED by Jeff ("all works as it is supposed to")
  ...
  1 – Sep 30 by the org's fiscal year), the badge counting it ("Filter · 2"
  with a stage chosen); reopen → still "This quarter". Pick "All time" →
  Apply → every quarter again; reopen → "All time". Reset → "All time".
- **§0.119 on dev — OBSERVED by Jeff's screenshot** (Closed Won → the won
  deals listed by quarter: Q4 2026 · 4, Q1 2027 · 2). Still his call on the
  two flagged items: the header's "0 open deals" beside a won-only list, and
  whether Funnel/Kanban should grow a Closed Lost column.
