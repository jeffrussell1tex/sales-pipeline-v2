...
- **Push to dev if not pushed** (`git push origin dev`), then the hash poll: accelerep.netlify.app serves `index-ODWcjiOD.js` when §0.135 has landed. **The dev site does NOT run scheduled jobs** (`JOBS_ENABLED` is prod's) — on dev the schedule is stored and Send now works; the hourly send is a prod behaviour.
