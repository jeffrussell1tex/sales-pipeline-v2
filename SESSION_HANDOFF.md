...
- **Ritual** (item 1), then `git log --oneline origin/master..dev` — expect, after `a38e0c6`: the ship-record docs commit `9d49c00`, **`4fe1efc` (CODE — §0.149: functions, bundle, a schema change already applied to BOTH databases)** and this handoff; anything else is a finding. `deno.lock` dirty (two lines) is the CLI’s, known.
- **§0.149 on dev — Jeff’s look (accelerep.netlify.app, hard-refresh to the hash the push lands — `index-DlCzV-dZ.js` from a local gate build; the CDN’s may differ):** Quotes → a deal → a Sent / Approved quote → "Customer accepted this quote →" → the Dispatch job card → "Create dispatch job →" → the job named on the card; Dispatch → Jobs → that job → the Invoice block → Create invoice → Edit (add a line, a tax rate) → Save → Issue → Mark paid → the Paid line; Void on a fresh one; the Invoices sub-tab; Settings → Quoting → Product & service types → add a type, retype a built-in’s kind → Quotes → Catalog → a product’s Type select; then a quote whose product carries the new type → job → the line’s kind on the invoice. **On the test org the pane left `JOB-2026-0008` (unscheduled) and `INV-2026-0001` ISSUED on it, and a "Pacific Rim Logistics" dispatch customer, and an "Equipment → Material" type — his to keep or clear.**
...
- **Flagged, carried (state §9):** two price books (Settings → Quoting → Price book edits a list nothing reads; the catalog quotes use is Quotes → Catalog) — reconcile or retire one; an invoice PDF and email; a deep link from the quote card into the job; per-org default tax rate and terms; the settings autosave mirrors every list panel’s save back as a second PUT (pre-existing; seen on the types panel); the five native time inputs; the interpreter’s gaps.
