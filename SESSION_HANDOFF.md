...
- **Push to dev if not pushed** (`git push origin dev`), then the hash poll: accelerep.netlify.app serves `index-5_oMz9NV.js` when §0.136 has landed.
- **§0.136 on dev — Jeff's checks (accelerep.netlify.app, hard-refresh to `index-5_oMz9NV.js`):** every dialog these six files draw now reads in the warm palette — no blue button, no white panel, no slate grey: Leads → Import (the dark band "Import Leads from CSV", the ink step badge, the drop zone); a lead's trash icon → the confirm dialog (the surface panel, the red Delete, Cancel on the surface) → Cancel; Contacts → New contact (the ink "Create Contact"); Settings → Users → add or edit a user (UserModal); Contacts → Import from Outlook (OutlookImportModal); the offline banner if it ever shows (amber for an auth problem, red for an outage); sign out and back in — the sign-in screen's mark is ink with a green line, the "No Organization Found" screen warm. The stage pills and the avatar initials keep their colours on purpose (data palettes).
