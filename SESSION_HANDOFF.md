...
`### 0.143` with a paragraph beginning **"The guard (`tests/audit-coverage.test.mjs`, new)."** (15 Sep — every write path writes the audit log) and `docs/ACCELEREP_CODING_GUIDE.md` `## 18b42. Every Write Path Writes The Audit Log — Through `auditAs`, Named, With The Word For What Happened (hard rule)`,
...
- **CI — still red, still unread:** the `Tests` workflow fails `unit` and `integration` on every run since ≥13 Sep; the logs are admin-only (API 403 from here) and it does not reproduce locally (TZ=UTC, Node 22.19). **Ask Jeff for the first `not ok` line of the `unit` job** — one line names the suite.
