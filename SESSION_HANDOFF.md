...
- **CI — `unit` GREEN since `5df4736` (§0.144: the shallow-clone fixture); `integration` still red, its cause unread.** Get its first `not ok` line the same way (the run → **integration** in the left column → the log search → `not ok`). Read it against the suite it names before touching anything: a schema the CI database lacks, a namespace collision, or a secret pointing at a different branch are all possible and all look alike from here. `test:int` is 170/170 on this machine.
