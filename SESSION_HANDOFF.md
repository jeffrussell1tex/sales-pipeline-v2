...
- **The Anthropic key — a finding for Jeff, before anything else:** the DEV site's `ANTHROPIC_API_KEY` answers 401 `invalid x-api-key` (observed through `netlify dev`, which injects the site's env). Prod's is unknown, and `ai-score.mjs` has used the same site key for as long as it has existed — if prod's is the same value, AI deal scoring has been failing quietly too. His to check in Netlify's env (dev and prod) or to install a BYOK key on the workspace (Settings → Features → AI → BYOK, which wins over the site key).
- **§0.141 on dev — Jeff's check (accelerep.netlify.app, hard-refresh to `index-DBD7v9I0.js`):** Settings → Features & AI → "Claude reads report prompts" is ON for the Accelerep Test org (left on from the pane). With a WORKING key installed: Reports → Saved reports → + Create report → a prompt → GENERATE reads "ASKING CLAUDE…" then the banner's first chip is READ BY · Claude and the report is what the sentence asked; a paraphrase the built-in reader cannot do ("who is closing the most this quarter", "what's slipping") reads sensibly; something the vocabulary cannot express ("win rate by rep") comes back with a note. With the switch OFF: the same prompt → READ BY · Built-in interpreter and the note "AI assistance is available: an Admin can turn "Claude reads report prompts" on under Settings → Features → AI". With the switch on and NO working key: the note names the rejected key. Also: Deliver on any report → At is the house dropdown, five-minute steps, type "915" to jump.
