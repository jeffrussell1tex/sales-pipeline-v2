...
- **After the TWENTY-SECOND ship — prod observations, Jeff's (salespipelinetracker.com, hard-refresh to `index-D0HaH5AU.js`):** every tab announces itself the way Pipeline does (Tasks with the date under the title; Dispatch with "Dispatch" and the view in the caption; Documents full width; Reports with the gutter; Settings in the serif); the app looks otherwise exactly as before (the one token object — nothing visible changed except the three fixes named in state §0.130). Prod's rows are his; read before assuming anything exists there.
