...
test('ReportsTab: leads sliced, the sub-tabs handed the gated sets, rep lists scoped', () => {
    ...
    assert.match(src, /<SavedReportsTab\s+showConfirm=\{showConfirm\}\s+accounts=\{accounts\}\s+reportsOpps=\{reportsOpps\}\s+reportsTimedActivities=\{reportsTimedActivities\}\s+activities=\{reportsActivities\}\s+scopedRepNames=\{scopedRepNames\}/);
