#!/usr/bin/env node
/**
 * v2 — guide + state doc ONLY. docs/SESSION_HANDOFF.md is replaced wholesale by
 * the separately delivered file (the repo held the 26-Aug handoff; the 27-Aug
 * one was never committed), so it is not patched here.
 *
 * Edits: the six owed since 77e119c (handoff §4), the §22 back-record of that
 * commit, the tasks-GET horizon closure, the NETLIFY_DATABASE_URL correction,
 * and the new §0.48–§0.50 batch entry carrying this batch's verified numbers.
 *
 *   node patch-docs-v2.mjs        # dry run
 *   node patch-docs-v2.mjs --apply # writes
 *
