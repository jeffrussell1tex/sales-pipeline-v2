...
function QuotaBar({ closedArr, commitArr, quota, label }) {
    ...
    const remaining  = quota - closedArr - commitArr;

    return (
        <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: T.r+1, padding: '1rem 1.25rem' }}>
...
export default function HomeTab() {
    ...
    // Overdue tasks first, then today's calendar events, then today's tasks
    const plate = [];
    ...
    todayCalEvents.forEach(ev => {
        ...
        plate.push({
            ...
            onClick:   linkedOpp ? () => { setEditingOpp(linkedOpp); setShowModal(true); } : null,
            item:      ev,
        });
    });
    ...
    const recentFeed = [...(activities || [])]
        .filter(a => !canSeeAll || a.salesRep === currentUser || a.author === currentUser)
    ...
    const myActivities     = (activities||[]).filter(a => !canSeeAll || a.salesRep === currentUser || a.author === currentUser);
    ...
    // Deals advanced this week (stage changed this week)
    ...
    const worthAttention = (() => {
        ...
        activeOpps.forEach(opp => {
            ...
            const daysInStage = opp.stageChangedDate ? Math.floor((now - new Date(opp.stageChangedDate + 'T12:00:00'))/86400000) : null;
            ...
            if (daysSinceAct !== null && daysSinceAct >= 14) {
                items.push({
                    ...
                    body: `$${(arr/1000).toFixed(0)}K in ${opp.stage} stage with no activity since ${lastActDate ? new Date(lastActDate+'T12:00:00').toLocaleDateString('en-US',{month:'short',day:'numeric'}) : '—'}. Last ${oppActs[0]?.type?.toLowerCase() || 'email'} unanswered.`,
            ...
            if (contactNames.length >= 2 && engagedContacts.size < 2 && arr >= 20000) {
                items.push({
                    ...
                    body: `${contactNames.filter(n => !engagedContacts.has(n.split(' (')[0]))[0]?.split(' (')[0] || 'Key contact'} not engaged. Ask ${oppActs[0]?.contactName?.split(' ')[0] || 'your champion'} for intro before next meeting.`,
                    onClick: () => { setEditingOpp(opp); setShowModal(true); },
                    borderColor: T.warn,
                });
            }
            ...
            <div style={{
                ...
                <div style={{ overflowY: 'auto', padding: '1.5rem 1.75rem 2rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    ...
                    <div>
                        ...
                        {plate.length === 0 ? (
                            <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: T.r+1, padding: '2rem', textAlign: 'center', color: T.inkMuted, fontSize: '0.875rem', fontFamily: T.sans }}>
                                ...
                                {plate.map((item, idx) => {
                                    ...
                                    return (
                                        <div key={idx} style={{
                                            ...
                                            borderBottom: idx < plate.length - 1 ? `1px solid ${T.border}` : 'none',
                    ...
                    <div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.75rem' }}>
                            {/* Activities */}
                            <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: T.r+1, padding: '0.875rem 1rem' }}>
                                <div style={{ ...eyebrow(), marginBottom: '0.3rem' }}>Activities this week</div>
                                <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem' }}>
                                    <span style={{ fontSize: '1.75rem', fontWeight: '700', color: T.ink, fontFamily: T.sans, lineHeight: 1 }}>{weekActs.length}</span>
                                    {actDelta !== 0 && (
                                        <span style={{ fontSize: '0.8125rem', color: actDelta > 0 ? T.ok : T.danger, fontWeight: '600', fontFamily: T.sans }}>
                            ...
                            {/* Tasks completed */}
                            <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: T.r+1, padding: '0.875rem 1rem' }}>
                                <div style={{ ...eyebrow(), marginBottom: '0.3rem' }}>Tasks completed</div>
                                <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem' }}>
                                    <span style={{ fontSize: '1.75rem', fontWeight: '700', color: T.ink, fontFamily: T.sans, lineHeight: 1 }}>{weekCompletedTasks.length}</span>
                                    {stillOpenTasks > 0 && (
                                        <span style={{ fontSize: '0.8125rem', color: T.inkMid, fontFamily: T.sans }}>
                                            {stillOpenTasks} still open
                                        </span>
                                    )}
                                </div>
                            </div>
                            ...
                            <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: T.r+1, padding: '0.875rem 1rem' }}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem', flexWrap: 'wrap' }}>
                                    ...
                                    {advancedNames && (
                                        <span style={{ fontSize: '0.8125rem', color: T.inkMid, fontFamily: T.sans }}>{advancedNames}</span>
                                    )}
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: '0.625rem' }}>
                                ...
                                {worthAttention.length >= 3 && (
                                    <button onClick={() => {}} style={{ fontSize: '0.75rem', color: T.goldInk, fontWeight: '600', background: 'none', border: 'none', cursor: 'pointer', fontFamily: T.sans }}>See all 7 →</button>
                                )}
                            </div>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.75rem' }}>
                                {worthAttention.map(item => (
                                    <div key={item.id} onClick={item.onClick}
                                        style={{ background: T.surface, border: `1px solid ${T.border}`, borderLeft: `3px solid ${item.borderColor}`, borderRadius: `0 ${T.r}px ${T.r}px 0`, padding: '0.875rem 1rem', cursor: 'pointer', transition: 'background 0.1s' }}
