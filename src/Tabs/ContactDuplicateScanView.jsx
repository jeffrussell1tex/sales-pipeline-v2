...
const REL_LABEL = {
    'email-mismatch': 'Same email, different name — likely a data-entry error',
};

const displayName = (c) =>
    (c?.name || [c?.firstName, c?.lastName].filter(Boolean).join(' ') || c?.email || 'Contact').toString().trim();
const fmtLoc = (c) => [c.city, c.state].filter(Boolean).join(', ');

// ── One contact's quick facts inside a compare card ─────────────────────────
function ContactFacts({ c }) {
    const rows = [
...
function PairCard({ pair, tier, onMerge }) {
    ...
    const isMismatch = pair.relationship === 'email-mismatch';
    return (
        <div style={{ background: T.surface, border: `1px solid ${isMismatch ? 'rgba(156,58,46,0.4)' : T.border}`, borderRadius: 6, padding: 16, marginBottom: 12 }}>
            <div style={{ display: 'flex', alignItems: 'stretch', gap: 16 }}>
                <ContactFacts c={pair.a} />
                <div style={{ width: 1, background: T.border, flexShrink: 0 }} />
                <ContactFacts c={pair.b} />
            </div>

            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 14, paddingTop: 12, borderTop: `1px solid ${T.border}`, flexWrap: 'wrap' }}>
                {reasons && (
                    <span style={{ fontSize: 11, fontWeight: 600, color: isMismatch ? T.danger : T.goldInk, background: isMismatch ? 'rgba(156,58,46,0.08)' : 'rgba(200,185,154,0.22)', border: `1px solid ${isMismatch ? 'rgba(156,58,46,0.35)' : 'rgba(200,185,154,0.5)'}`, borderRadius: 999, padding: '2px 10px', fontFamily: T.sans }}>
                        {reasons}
                    </span>
                )}
                {relLabel && (
...
// ── Main panel ──────────────────────────────────────────────────────────────
export default function ContactDuplicateScanView({ onBack }) {
    const { findContactDuplicates, setContactMergeModal, contacts } = useApp();
    ...
    // findContactDuplicates is recreated each App render; hold it in a ref so the
    // scan effect stays stable and doesn't loop.
    const fdRef = useRef(findContactDuplicates);
    useEffect(() => { fdRef.current = findContactDuplicates; });
    ...
    // Re-scan after a merge: the contacts list shrinks when a row is archived.
    useEffect(() => {
        const n = (contacts || []).length;
        if (countRef.current === null) { countRef.current = n; return; }
        if (n !== countRef.current) { countRef.current = n; runScan(tier); }
    }, [contacts, tier, runScan]);

    const counts = data?.counts || { duplicate: 0, related: 0 };
    const pairs  = data?.pairs || [];

    const openMerge = (pair) => setContactMergeModal?.({ aId: pair.a.id, bId: pair.b.id });
    ...
    return (
        <div>
            ...
            <div style={{ marginBottom: 14 }}>
                <div style={{ fontSize: 20, fontWeight: 700, color: T.ink, marginBottom: 4, fontFamily: T.sans }}>Find &amp; merge duplicate contacts</div>
                <div style={{ fontSize: 13, color: T.inkMid, fontFamily: T.sans }}>
                    Scan contacts for likely duplicates and merge them into one record. Every merge is fully reversible.
                </div>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
                ...
                <span style={{ fontSize: 12, color: T.inkMuted, fontFamily: T.sans }}>
                    {data ? `${data.scanned} contacts scanned` : ''}
                </span>
            ...
            {loading && !data && (
                <div style={{ color: T.inkMuted, fontSize: 13, fontStyle: 'italic', fontFamily: T.sans, padding: '20px 0' }}>Scanning contacts…</div>
            )}
            ...
            {!loading && data && pairs.length === 0 && (
                <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 6, padding: 24, textAlign: 'center', color: T.inkMid, fontSize: 13, fontFamily: T.sans }}>
                    {tier === 'duplicate'
                        ? 'No likely duplicates found — your contacts look clean.'
                        : 'No possibly-related contacts flagged.'}
                </div>
            )}
