...
dbFetch('/.netlify/functions/users?me=true')
    ...
    // ── Feature flag helper ──────────────────────────────────────────────────
    // Default is TRUE — flags only disable when explicitly set to false in settings.
    ...
    // Aggregates deal health across all open opportunities for an account.
    const calculateAccountHealth = (accountId) => {
        if (!isFeatureEnabled('account-health-scores')) return null;
        const acctOpps = (opportunities || []).filter(o => o.accountId === accountId && o.stage !== 'Closed Won' && o.stage !== 'Closed Lost');
        if (!acctOpps.length) return null;
        const scores = acctOpps.map(o => calculateDealHealth(o).score);
        const avg = Math.round(scores.reduce((a, b) => a + b, 0) / scores.length);
        const status = avg >= 70 ? 'Healthy' : avg >= 45 ? 'At Risk' : 'Critical';
        return { score: avg, status, oppCount: acctOpps.length };
    };
