            ...
            {activeTab === 'settings' && isAdmin && (
                ...
                <>
                    ...
                    {settingsView === 'team-builder' && (() => {
                        ...
                        const reps = allUsers.filter(u => u.userType === 'User');
                        const teams = settings.teams || [];

                        const [editingTeam, setEditingTeam] = React.useState(null);
                        const [showTeamForm, setShowTeamForm] = React.useState(false);
                        const [teamForm, setTeamForm] = React.useState({ name: '', territory: '', managerId: '', repIds: [] });
                        ...
                        const saveTeam = () => {
                            ...
                            const updatedTeams = editingTeam
                                ? teams.map(t => t.id === editingTeam.id ? saved : t)
                                : [...teams, saved];
                            // Sync team/territory onto each user record
                            const updatedUsers = (settings.users || []).map(u => {
                                // Clear this team from user if they were removed
                        ...
                        const labelStyle = { display: 'block', fontSize: '0.75rem', fontWeight: '600', color: '#374151', marginBottom: '0.375rem', textTransform: 'uppercase', letterSpacing: '0.04em' };
                                    ...
                                    <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '10px', padding: '1.5rem', marginBottom: '1.5rem' }}>
                                        ...
                                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
                                            ...
                                            <div>
                                                ...
                                                <input style={inputStyle} value={teamForm.territory} onChange={e => setTeamForm(p => ({ ...p, territory: e.target.value }))} placeholder="e.g. Northeast" />
                                            </div>
                                        </div>
                                        ...
                                        <div style={{ marginBottom: '1.25rem' }}>
                                            <label style={labelStyle}>Sales Reps ({teamForm.repIds.length} selected)</label>
                                            {reps.length === 0 ? (
