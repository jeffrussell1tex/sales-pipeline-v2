                ...
                <div style={{ flex: 1, overflowY: 'auto', minHeight: 0, padding: '16px 18px' }}>
                    ...
                    {!file ? (
                        <div
                            ...
                            <div style={{ fontSize: 11, color: T.inkMuted, marginTop: 4 }}>PDF, Word, PowerPoint, Excel, Images — up to 50&nbsp;MB</div>
                        </div>
                    ) : (
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                {progress != null ? (
                                    <div style={{ marginTop: 5 }}>
                                        ...
                                        <div style={{ fontSize: 10, color: T.inkMuted, marginTop: 3 }}>{progress}%</div>
                                    </div>
                                ) : (
                                    <div style={{ fontSize: 11, color: T.inkMuted, marginTop: 1 }}>{m?.label} · {fmtSize(Math.round(file.size / 1024))}</div>
                                )}
                            </div>
                            {!uploading && <button onClick={() => { setFile(null); setProgress(null); }} style={{ background: 'none', border: 'none', color: T.inkMuted, fontSize: 16, cursor: 'pointer', lineHeight: 1, flexShrink: 0 }}>×</button>}
                        </div>
                    )}
                    ...
                    {isVersion && (
                        <div style={{ marginTop: 16 }}>
                            <div style={{ fontSize: 10, fontWeight: 700, color: T.inkMuted, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 5 }}>What changed?</div>
                    ...
                    {!isVersion && (
                        <>
                            <div style={{ marginTop: 18 }}>
                                <div style={{ fontSize: 10, fontWeight: 700, color: T.inkMuted, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 5 }}>Document name</div>
                            ...
                            <div style={{ marginTop: 16 }}>
                                <div style={{ fontSize: 10, fontWeight: 700, color: T.inkMuted, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 8 }}>Category</div>
                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
                                    {CATEGORIES.map((c) => {
                                        const on = category === c;
                                        return (
                                            <button key={c} onClick={() => setCategory(c)}
                                                style={{ borderRadius: 999, padding: '5px 12px', fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: T.sans, letterSpacing: '0.03em', textTransform: 'uppercase', border: `1px solid ${on ? T.ink : T.border}`, background: on ? T.ink : T.surface, color: on ? '#f5f1eb' : T.inkMid }}>
                                                {c}
                                            </button>
                                        );
                                    })}
                            ...
                            <div style={{ marginTop: 16 }}>
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                                    <div style={{ fontSize: 10, fontWeight: 700, color: T.inkMuted, textTransform: 'uppercase', letterSpacing: '0.07em' }}>Link to records <span style={{ color: T.inkMuted, textTransform: 'none', fontWeight: 500 }}>· attach one or more</span></div>
                                </div>
                                <button onClick={openLinkPicker}
                                    style={{ width: '100%', textAlign: 'left', padding: '8px 12px', border: `1px solid ${T.border}`, borderRadius: T.r, fontSize: 13, background: T.surface, color: T.inkMuted, fontFamily: T.sans, cursor: 'pointer' }}>
