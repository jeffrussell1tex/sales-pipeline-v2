...
export default function ContactModal({
    ...
    const handleSelectCompany = (companyName) => {
        ...
        setFormData(prev => ({
            ...
            address2: prev.address2 || matchedAccount?.address2 || '',
    ...
    return (
                        ...
                        <div className="form-grid">
                            ...
                            <div className="form-group" style={{ position: 'relative' }}>
                                ...
                                <input type="text" value={contactRepSearch}
                                    onChange={e => { setContactRepSearch(e.target.value); setShowContactRepSugg(true); }}
                                    onFocus={() => setShowContactRepSugg(true)}
                    ...
                    {activeContactTab === 'additional' && (
                        <div className="form-grid">
                            ...
                            <div className="form-group full">
                                ...
                                <input type="text" value={formData.homeAddress || ''} onChange={e => handleChange('homeAddress', e.target.value)} />
                            </div>
                            <div className="form-group full">
                                <label>Notes</label>
                                <textarea
        ...
        {nestedModal && (
            <div className="modal-overlay" style={{ zIndex: 20000, position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
...
export function NestedNewAccountForm({ name, onSave, onCancel }) {
    const [fd, setFd] = useState({ name: name || '', phone: '', website: '', address: '', address2: '', city: '', state: '', zip: '' });
    const hc = (f, v) => setFd(prev => ({ ...prev, [f]: v }));
    return (
        <form onSubmit={e => { e.preventDefault(); if (fd.name.trim()) onSave(fd); }}>
            <div className="form-grid">
                ...
                <div className="form-group full"><label>Address Line 2</label><input type="text" value={fd.address2 || ''} onChange={e => hc('address2', e.target.value)} placeholder="Suite, floor, unit..." /></div>
