...
export default function ViewingBar({
    ...
    return (
        <div style={{ background: T.surface2, border: `1px solid ${T.border}`, borderRadius: '10px', padding: '0.65rem 1.25rem', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.625rem', flexWrap: 'wrap' }}>
            <span style={{ fontSize: '0.75rem', fontWeight: '700', color: T.inkMid, textTransform: 'uppercase', letterSpacing: '0.05em', flexShrink: 0, marginRight: '0.125rem' }}>Viewing:</span>
            ...
            {hasPipelineChoice && hasSlicers && (
                <span style={{ color: T.border, fontSize: '1rem', userSelect: 'none' }}>│</span>
            )}
            ...
            {isFiltered && (
                <>
                    <span style={{ fontSize: '0.6875rem', color: T.inkMuted, marginLeft: '0.25rem' }}>
                        {visibleCount} of {totalCount} {countLabel || 'items'}
                    </span>
                    <button onClick={clearAll}
                        style={{ marginLeft: 'auto', padding: '0.2rem 0.5rem', borderRadius: '4px', border: `1px solid ${T.border}`, background: '#fff', color: T.inkMuted, fontSize: '0.625rem', fontWeight: '600', cursor: 'pointer', fontFamily: 'inherit' }}>
                        ✕ Clear
                    </button>
                </>
            )}
...
export function SliceDropdown({ label, icon, options, selected, onSelect, alwaysActive, activeColor, colorMap }) {
    ...
    const btnColor = activeColor || T.info;
    const btnStyle = {
        ...
        border: '1px solid ' + (isActive ? btnColor : T.border),
        background: isActive ? btnColor : '#fff',
        color: isActive ? '#fff' : T.inkMid,
    };
    const dropStyle = {
        position: 'absolute', top: 'calc(100% + 6px)', left: 0, zIndex: 200,
        background: '#fff', border: `1px solid ${T.border}`, borderRadius: '10px',
    ...
    const rowStyle = (isSelected) => ({
        ...
        color: isSelected ? T.info : T.ink, fontWeight: isSelected ? '700' : '400',
        background: isSelected ? `${T.info}14` : 'transparent',
            ...
            {open && (
                <div style={dropStyle}>
                    ...
                    <div
                        ...
                        style={{ ...rowStyle(!selected), borderBottom: `1px solid ${T.surface2}` }}
                    >
                    ...
                    {options.map(opt => (
                        <div key={opt}
                            onMouseEnter={e => e.currentTarget.style.background = T.surface2}
                            onMouseLeave={e => e.currentTarget.style.background = selected === opt ? `${T.info}14` : 'transparent'}
