...
- **§0.131 on dev — Jeff's check (accelerep.netlify.app, hard-refresh to `index-CyIfr4JI.js`, desktop):** every tab now announces itself the way Pipeline does — Tasks with the date under the title; Dispatch with "Dispatch" and the view in the caption (Job Board shows the date range there, Customers "2 dispatch customers"); Documents full width; Reports with the gutter; Settings in the serif. His screenshots were the desktop view; the pane measured a phone width only.
