...
- **The ship — his call, FOUR code commits on dev beyond master** (`4d6b0f5`, `ddac313`, `d48b3f6` functions AND bundle; `fffbfb8` bundle; `5df4736` and `b809237` are tests and the workflow). Pre-flight as before; the schema diff EMPTY. **CI is green on dev — a red run on the master push is a NEW cause** (read its first `not ok` the same way).
