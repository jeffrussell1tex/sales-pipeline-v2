...
- **§0.125 on dev — OBSERVED IN PART by Jeff as Karen (his screenshot: the manager's note, My claims and the calendar prompt on Home after sign-in). Left, if he wants the strict slow-path proof (Karen, role User, accelerep.netlify.app, hard-refresh to `index-s-O_Wnog.js`):** open the site signed OUT (a private window), sign in through the in-app form including the MFA step, taking longer than 8 seconds → Home shows the calendar strip and, as Karen, any coaching note addressed to her, and the console has NO "DB error 401" and NO "waitForToken: no Clerk token" line — before §0.125 that path left the three empty until a refresh. Then, as himself: the avatar strip → the org switcher → another org → Home reflects that org at once (seen from the pane's request log this session; the eyes-on is his) → back to Accelerep Test.
