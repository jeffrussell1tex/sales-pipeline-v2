...
- **§0.128 on dev — DONE in the pane; his eyes-on if he likes:** Leads → Triage (All) → click the 100 badge in Call today → the card opens in full below the badge, nothing clipped, no scrollbar on the strip; click anywhere else → it closes; the All leads table's badge behaves the same.
- **Then, his call: ship** (`git push origin dev:master` — THREE code commits `324c04e`, `ac50a91`, `e2dceca`; two touch functions (`_lib.mjs`, `dispatch-automations.mjs`, `pipeline-alerts.mjs`) — Netlify rebuilds every function; no schema change; pre-flight as the twentieth ship's).
- **Flagged, not done (state §0.127 / §0.128):** `task.overdue` (nothing computes it); the Slack action posts text only; fold `menuPlacement` into `popoverPlacement`; the card's height allowance is a constant (300); the other absolute popovers (RepPickerPopover, the status picker) not audited.
- **Ritual** (item 1), then `git log --oneline origin/master..dev` — expect, after `7fd83de`: the four ship-record / observation docs commits, `324c04e` (CODE), `c032da0` and `fc6a428` (docs), `ac50a91` (CODE), `e2dceca` (CODE), the landing-lines docs commit, then this handoff; anything else is a finding.
