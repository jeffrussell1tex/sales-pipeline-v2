...
## 4. Verified state at close (13 Sep, sixteenth session — the signal migration's Phase 0 and 1 as docs, NOTHING migrated; §0.126–§0.128 SHIPPED, the TWENTY-FIRST ship)
...
- **Ritual** (item 1), then `git log --oneline origin/master..dev` — expect the two ship-record docs commits, `9f4d35c` (design docs + the audit script, NOT code) and the handoff commit after it; anything under `src/` or `netlify/` is a finding. `git status` — the tree was clean at close; `dist/` cleared.
