...
**Verified:** six gates, **825/825 unit**, build guard OK 2,533 kB `index-oOwpiwjs.js`, `dist/` cleared. Browser: NOT observed — the pane’s session had signed out overnight (the sign-in screen showed instead, and read as §0.136 says: the mark `#2a2622`, its line `#4d6b3d`, the subtitle in inkMuted, the page on `T.bg`); signing in is Jeff’s. **NOT shipped.** Bundle only.
