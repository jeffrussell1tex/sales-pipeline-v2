...
**And a second probe (`2e6a9c4`):** with the path-segment rewrite live, the function called directly at `/.netlify/functions/dispatch-status/<token>` answered 200 while `/status/<token>` STILL 404'd — a 200 rewrite hands the function the BROWSER'S path (`event.path` is `/status/<token>`), not the rewritten target, so a match on `/dispatch-status/` alone missed it. The function now reads the last segment of EITHER path; the scan pins the two-form regex and the integration suite calls the handler with `/status/<token>` as well. READ on dev at 15:28:36 UTC 10 Sep, after the deploy of `2e6a9c4`: `GET /status/oSP8_…` → **200** — "Your service provider" (the dev org has no company name or display name set — Settings → Company profile fills that line), "Filter Change", status **Scheduled**, "Friday, September 11, 2026 between 9:00 AM and 11:00 AM", technician "Jax", "400 Westheimer, Houston, TX", reference JOB-2026-0005; no id and no contact detail on the page. **OBSERVED by Jeff's screenshot (10 Sep, ~15:35 UTC): the Job Board shows Jax Miller 2/50 hrs with "Dispatch Customer Test · 9a · 2h" on Fri 11 — the schedule the pane sent, rendered where he had looked for it.** Then "it is still showing it as needing a crew above the Jax Miller schedule" — read against the rows: a SECOND "Filter Change", JOB-2026-0006, created 17:05:19 UTC during his attempts, unscheduled on the same date; the board was right to show both. Cancelling it is Jeff's, not a script's. **And, Jeff: "please add that banner. Great idea" — `55e219b`:** the crew builder's refusal (no start time, a shift or time-off clash, equipment, a won-opportunity placeholder) is now a `role="alert"` banner above the action bar reading "Not scheduled — <why>", not an 11.5px line among the controls (§18b32); pinned by a scan in `tests/dispatch-stubs.test.mjs`, one mutant (the line comes back). **Verified:** six gates, 686/686 unit, 391/391 mutations, build guard OK `index-DgjASrGN.js` (a NEW hash), `dist/` cleared. **Verified:** six gates, 685/685 unit, 390/390 mutations, 145/145 integration, `index-D8PIckNZ.js` unchanged.
