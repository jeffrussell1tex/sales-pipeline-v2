...
**Verified:** six gates (84 function files now — `_selfProfile.mjs` is scanned), **652/652 unit** (10 new), **368/368 mutations, printed green baseline** (7 new), **133/133 integration** (5 new), build guard OK `index-CRtPA764.js` (2,447 kB; a NEW hash — two buttons gone, two banners added), `dist/` cleared. **Then, as its own commit (the §9 entry asked for that):** `prioColor2` collapsed into `prioColor` in DispatchTab (three call sites), one assertion added to `tests/dispatch-stubs.test.mjs`, one mutant (the alias returns); gates, **652/652 unit**, **369/369 mutations**, build guard OK `index-BlthEjPa.js` (a NEW hash), `dist/` cleared. NOT observed in a browser: the pane holds no session, and the new banners need a REFUSED delete (a rep cannot reach Settings; an Admin's delete succeeds) — the shape is the banner LeadConversionDetail already renders. Landed on dev, NOT shipped.
