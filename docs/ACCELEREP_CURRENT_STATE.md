...
**And a second probe (`2e6a9c4`):** with the path-segment rewrite live, the function called directly at `/.netlify/functions/dispatch-status/<token>` answered 200 while `/status/<token>` STILL 404'd — a 200 rewrite hands the function the BROWSER'S path (`event.path` is `/status/<token>`), not the rewritten target, so a match on `/dispatch-status/` alone missed it. The function now reads the last segment of EITHER path; the scan pins the two-form regex and the integration suite calls the handler with `/status/<token>` as well. Read on dev after the deploy: the pretty path 200 with the real token (see the handoff). **Verified:** six gates, 685/685 unit, 390/390 mutations, 145/145 integration, `index-D8PIckNZ.js` unchanged.
