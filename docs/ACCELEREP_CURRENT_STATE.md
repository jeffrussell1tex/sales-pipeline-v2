...
**Tests:** +1 file `tests/email-templates.test.mjs` (7: the pure module RUN — normalisation, the merge context, rendering with an unknown field kept, the mailto encoding; scans of both settings halves, the catalogue row, the card count, the AdminView route, the panel's module-scope editor and rethrown save, the rail's picker, links and logged note), registered in `SUITES`. **Mutants (4):** an unknown merge field vanishes; the mailto body unencoded; the settings PUT loses the key; a duplicate id kept. **Verified:** six gates (161 files; 91 function files), 735/735 unit, 153/153 integration, 429/429 mutations after a printed green baseline, build guard OK 2,494 kB `index-Bk9klMuV.js`, `dist/` cleared. No schema change. NOT observed on dev at the time of writing; NOT shipped.
