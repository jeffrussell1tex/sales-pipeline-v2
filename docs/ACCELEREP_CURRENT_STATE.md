...
- **Customer-facing notifications** — email now through Resend (appointment confirmation on schedule, "tech on the way" on `en_route`), SMS behind the SAME switch for when Twilio A2P clears, and a public job-status link — a NEW pattern: a signed, org-scoped, unauthenticated read that must be designed org-scoped from the first line. Customers carry `contactPhone`/`contactEmail`; nothing sends to them today.
