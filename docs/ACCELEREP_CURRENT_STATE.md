...
**Follow-up, the same hour — the gutter is the class, not a number (`the follow-up commit`).** The first cut gave Documents and Reports `padding: 1.5rem` inline. The other tabs get their gutter from `.tab-page`, which is 1.5rem on a desktop and 0.75rem under 640px (`index.css` 698) — so at the pane's phone width the two titles sat 12px right of and below every other tab's (x 32 / y 80 against 20 / 68, measured), and on a desktop they would have matched by coincidence. Both wrappers now carry `className="tab-page"` with `gap: 0` inline (the class's column gap would have opened 1.25rem between the header, the toolbar or sub-tab nav, and the table — each page spaces itself). Same six gates, 774/774, 159/159, 475/475 mutations, build guard OK `index-CyIfr4JI.js`.
