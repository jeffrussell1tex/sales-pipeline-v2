...
**Updated:** September 9, 2026 (eleventh session — §0.101 the digest job fixed, §0.102 the mutation harness restores on any exit)
**Verified at:** five gates green on 154 files · **619 tests** · **340/340 mutations, printed green baseline** · **124/124 integration** · build guard OK 2,446 kB `index-B-DxPVK_.js` (dev — the SAME hash as §0.99 and §0.100: §0.101 is functions-only, §0.102 tooling-only) · **prod `5a306e3` serving `index-JJdhJmyd.js` (tenth ship, 8 Sep 22:36 UTC — §0.92 through §0.99)** · dev ahead of `master` by the ship-record docs commits, §0.100, §0.101 and §0.102. **The Resend inbound webhook targets PROD — Jeff's screenshot after the tenth ship; not re-read this session.** The app database and the test database both hold `audit_stream_destinations`, `job_heartbeats` (legacy, dropped by hand after the eleventh ship) and `site_job_heartbeats`. (This header and the per-batch lines under it were refreshed at the ninth session's open, at the tenth ship, at §0.100, §0.101 and §0.102 — the sections are the record.)
