...
**Fix (bundle only).** (1) `isUnscheduled(j)` (no placed start, or no crew — a held crew and a won opportunity both count) and `sortedQueue` lists those PLUS the selected job when it is scheduled — a job opened from the Job Board or Service Due (`handleJobClick`, the "Open job" button) still lands in the builder and can be re-crewed, and drops out of the list once deselected; the default selection is the first job to schedule, `null` (the existing "Select a job from the queue" state) when there is none; the parent no longer passes `jobsWithBridge[0]?.id` as a default, and the held-crew/preferred-start effect keys on `selectedJob?.id` so the builder's own default gets its held crew back. (2) The header button reads **"Edit job →"** for a saved job (was "Open job record →"; same handler, the Jobs editor) and, for a won opportunity, a near-black **"Create job →"** in the same slot (`onCreateBridgeJob`, the prefilled new-job form where its details are entered); each card gets "Create job →" beside where a saved job's "Open →" sits; the banner keeps its explanation and loses its duplicate button. (3) `crewLine(j, compact)` (module scope) reads "2 techs · 4h" / "2p × 4h", "crew and duration not set", "crew not set · 4h", "2 techs · duration not set"; the header grid reads "Not set" for crew size, duration and min license. Not done: an in-place editor in the queue — the Jobs editor stays the one editor; the button takes the dispatcher there. Seen and left: `unscheduledJobs` and `scheduledJobs` inside `CrewBuilderView` are computed and never read (dead since some earlier batch — not touched here).

**Tests:** +2 scans in `crew-scoring` (the list rule and default selection; the two buttons, the card link, one Create action on the selected row, the crew line and the header labels), 1 pin updated in `dispatch-stubs` (the relabel). **Mutants (3):** every job listed again; the bridge header button gone; the both-null line gone. **Verified:** six gates (check:fnscope 90 function files; the five on 158 files — the header's 89/157 were the previous print, the difference not traced), 711/711 unit, 147/147 integration, 417/417 mutations after a printed green baseline (no restore errors this run), build guard OK 2,479 kB `index-SFEVnpmt.js`, `dist/` cleared. NOT observed on dev at the time of writing; NOT shipped.
