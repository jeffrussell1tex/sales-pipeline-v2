...
**Updated:** September 15, 2026 (§0.138 — the rest of the audit’s off-brand table: 126 Tailwind literals in 15 more files onto the tokens, the tree-wide count 140 → 31, every one of the 31 a stored data palette — on dev, NOT shipped; TWENTY-FOUR ships; §0.134–§0.137 on prod at `a4f4589`, 14:52 UTC 15 Sep — the TWENTY-FOURTH ship, functions AND bundle, `report-deliveries` new on prod; seventeenth session, its last fix — §0.137 (Home’s plate row: the stage name on its own guarded line in a 136px column, no longer bleeding across the title); September 14 — §0.136 (the blue-era modals on the tokens: 549 Tailwind literals in six files rewritten by a parser-driven script, by role for the blues; LeadImportModal on CsvImportModal’s chrome; the off-brand count 542 → 140 tree-wide, the six files 417 → 15 kept data palettes) on dev, NOT shipped; §0.135 (saved reports: sharing (own + shared, an Admin the org; PUT read-then-merge, the owner kept), scheduled email / Slack delivery (an allowlisted schedule on config.delivery, the hourly `report-deliveries` job runs the report server-side over the owner’s read and sends; "Send now" the same path), pin to Home (my profile’s `pinnedReports`, drawn on Home by the tab’s own engine and chart); and the Tasks range / Dispatch board range persisted) on dev, NOT shipped; §0.134 (drag-to-reschedule on the dispatch week board: a card drags to another day or another technician’s row; the crew builder’s gates, the equipment gate, the builder’s override confirm; a partial PUT that never sets status) on dev, NOT shipped; TWENTY-THREE ships; §0.132 (task.overdue computed by the hourly job, Block Kit on the Slack action, an Admin's Train now — the last three of the enhancement thread; with its modal follow-up) and §0.133 (the report builder runs real queries — a pure engine over the tab's scoped sets, a chart component, saved reports open and save in place, a real period) on prod at `c17e2e8`, 18:54 UTC 14 Sep — the TWENTY-THIRD ship, functions AND bundle; TWENTY-TWO ships; §0.130 (one token object) and §0.131 (every tab announces itself the same way, with its gutter follow-up) on prod at `e2cba64`, 16:02 UTC 14 Sep — the TWENTY-SECOND ship, bundle only; every tab announces itself the same way, §0.131 (Tasks, Dispatch, Documents, Reports, Settings brought onto the one title block); ONE design-token object, §0.130: `src/tokens.js` replaces 32 local copies across 30 files, three silently-undefined reads fixed, a test and four mutants pin it; NOT shipped; the "Tonal & Form" signal-system migration: Phase 0 (the spine collision) and Phase 1 (the colour audit) delivered as docs and one read-only script, §0.129 — NOTHING migrated, no token, primitive or call site changed, no bundle change; fifteenth session — TWENTY-ONE ships; §0.126 (the Assign-to roster picker; the calendar strip cleared on a switch) and §0.127 (Post to Slack; momentum and score-drop triggers) and §0.128 (the lead score popover fixed, not clipped) on prod at `ae0cae8`, 21:48 UTC 13 Sep — the TWENTY-FIRST ship; §0.125 (mount-time loads keyed on an active org) and §0.124's Edit follow-up on prod at `7fd83de`, 20:43 UTC 13 Sep — the TWENTIETH ship; §0.124 and its first three follow-ups on prod at `b2218b2`, 19:59 UTC 13 Sep — the NINETEENTH ship; §0.117–§0.123 on prod at `b54a2a7`, 13:14 UTC 13 Sep — the EIGHTEENTH ship; `master` == `dev` at the ship)
**Verified at:** SIX gates green (check:fnscope — 93 function files, no unbound reads; the five on 169 files) · **826 tests** · **518/518 mutations, printed green baseline** · **170/170 integration** · build guard OK 2,533 kB `index-DIxtA4Km.js` (dev — §0.138, NOT shipped; before it §0.137 at 2,533 kB `index-oOwpiwjs.js`, SHIPPED as the TWENTY-FOURTH ship; prod serves `index-zv28zhMt.js` = `a4f4589`, the live key, from 14:51:57 UTC 15 Sep; before it §0.136 at 2,532 kB `index-5_oMz9NV.js`, pushed and observed; before it §0.135 at `index-ODWcjiOD.js`, NOT shipped; before it §0.134 with its confirm-variant follow-up `5bbd813` at 2,515 kB `index-BULS4hTf.js`, NOT shipped; before the follow-up `index-B4AZRwho.js`; before it `index-BNgFKZx-.js` = `c17e2e8` = prod `index-Ct7x96Jv.js` the TWENTY-THIRD ship; the line as it stood before, kept for the record: SIX gates green (check:fnscope — 91 function files, no unbound reads; the five on 163 files) · **769 tests** · **471/471 mutations, printed green baseline** · **159/159 integration** · build guard OK 2,504 kB `index-BIIXAgGT.js` (dev — §0.126–§0.128, SHIPPED as the TWENTY-FIRST ship; prod serves `index-BnV7qKA1.js` = `ae0cae8`, the live key, from 21:48:30 UTC 13 Sep; was `index-838pOcA_.js`, `index-CiwV447T.js`, `index-qwmC8w1K.js` on dev; was `index-s-O_Wnog.js`, §0.125, SHIPPED with the Edit follow-up as the TWENTIETH ship; prod serves `index-DdSH6fY8.js` = `7fd83de`, the live key, from 20:43:41 UTC 13 Sep; was `index-De3J8tLC.js`, the Edit follow-up; §0.124 and its first three follow-ups SHIPPED as the NINETEENTH ship; was `index-DpgxUDFK.js`; was `index-DqfNZYvt.js`; was `index-CDir98BZ.js`; was `index-DS-lL6jZ.js`, §0.122 with its fix and the dropdown; prod serves `index-BfMnj4VB.js` = `b2218b2`, the live key — the NINETEENTH ship, 20:00:24 UTC 13 Sep; was `index-COkIzXNn.js` = `b54a2a7`, the EIGHTEENTH, 13:15:31 UTC 13 Sep) · **two schema changes, additive, in BOTH databases, read back: `dispatch_jobs.public_token` + `customer_notifications` (§0.111), and `dispatch_service_plans.renewal_lead_days` and the `dispatch_plan_visits` table, additive, in BOTH databases, read back** · **prod `0726934` serving `index-DaJE9sq9.js` (the SIXTEENTH ship, 10 Sep 22:51 UTC — §0.112–§0.116, 80 functions, 2 redirect rules; was `0a242c6` / `index-C23CrCCK.js`, the FIFTEENTH ship, 10 Sep 17:47 UTC — §0.109–§0.111, 80 functions, 2 redirect rules; was `59116be` / `index-DKAHLlej.js`, the FOURTEENTH ship, 9 Sep 21:17 UTC — §0.108, functions only; the eleventh, twelfth and thirteenth ships were 18:51, 19:44 and 20:45 UTC the same day)** · `master` == `dev` == `b2218b2` at the ship; `dev` then ahead by the ship-record docs and the Edit follow-up once it lands. **The Resend inbound webhook targets PROD — re-read from Jeff's screenshot of resend.com/webhooks after the eleventh ship (9 Sep): the prod endpoint Enabled (3mo old), the dev endpoint Disabled (1d old).** **`JOBS_ENABLED` = `true` is on the prod site (Jeff's hand, 9 Sep) and on no other — prod alone runs the scheduled jobs; its first per-site row read back within a minute of the ship.** The app database and the test database both hold `audit_stream_destinations`, `job_heartbeats` (legacy, writer-less since the eleventh ship — Jeff drops it by hand once prod has all four per-site rows) and `site_job_heartbeats`, and since §0.105 the four `activities.email_*` columns (applied to both, read back). (This header and the per-batch lines under it were refreshed at the ninth session's open, at the tenth ship, at §0.100, §0.101, §0.102 and §0.103 — the sections are the record.)
**Batch:** **the rest of the off-brand table on the tokens (§0.138 — Jeff: "fix the 140 off-brand literals before I start a new conversation then"; and "are the 140 off-brand literals … from the new design I proposed" — no: the original Tailwind styling, the same class as §0.136)** — the same script with fifteen more files (QuotesTab, ViewingBar, QuotaRepCard, QuickLogFab, the Account and Contact rails, ErrorBoundary, OpportunityModal, the two merge modals, ReportsTab, the two document surfaces, the Activity and Task rails): 126 Tailwind literals to the tokens they mean, by role for the blues and purples, an import added where a file had none, EVERY literal inside an array (a chart palette, a stage map) and every value of the warm palette itself left as it was (the ask was the off-brand set; a white → surface swap in a report card is a visible change nobody asked for); `useSettings.js` not touched — its sixteen are stored defaults. The audit regenerated: off-brand 140 → 31 tree-wide, 22 → 3 files, and the 31 are the three data palettes (useSettings 16, App.jsx 11, ModalLayer 4). +1 scan, 2 mutants; bundle only. **NOT shipped; the handoff says what was observed.**
**Prior batch:** **SHIPPED — the TWENTY-FOURTH ship, 14:52 UTC 15 Sep (§0.134, §0.135, §0.136, §0.137; functions AND bundle, netlify.toml; `report-deliveries` new on prod; see §0.137’s last paragraph).** **Home’s plate row shows the whole stage (§0.137 — Jeff, with a screenshot, 15 Sep: "make adjustments so all sales stages can be displayed in this on your plate section without bleeding in to the activity")** — the label column was 68px, so "$84K · Negotiation/Review" wrapped and the stage, one unbreakable word, ran across the title column; the column is 136px (the longest stage name fits), the amount and the stage are two lines, the stage in its stage colour, both guarded with an ellipsis and the full text on hover. +1 scan (1). Bundle only. **NOT shipped; NOT observed in a browser (the pane’s session had signed out overnight); the handoff says so.**
**Prior batch:** **the blue-era modals on the tokens (§0.136 — Jeff: "The blue-era modals. Yesterday’s audit put numbers on this: ModalLayer, LeadImportModal, App.jsx, OutlookImportModal, ContactModal and UserModal carry roughly 400 off-brand Tailwind colours … one job. It is independent of the paused signal migration")** — `scripts/migrate-blue-era.mjs` (parser-driven, the §0.130 method: a dry run lists every decision by line, the apply re-parses and counts what is left) rewrote 549 hex literals in the six files to the token each means — the blues by ROLE (a blue background was a primary button → `T.ink`; a blue text, border or outline → `T.info`), the Tailwind washes to a token with an alpha (`${T.danger}14`), an 8-digit hex keeping its own alpha, a string carrying a hex to a template literal, a JSX attribute to braces — added the import to each, and KEPT three data palettes by what the line says (the pipeline default colour, the stage pill map, the avatar initials map) and the allowed `#1c1917` header; four named whites and the legacy `.modal` CSS class’s white went to the surface; LeadImportModal wears CsvImportModal’s chrome (the dark drag-handle header band, radius 12, a border). The audit regenerated: off-brand uses 542 → 140 tree-wide (22 → 18 files); the six files 417 → 15, all of them the kept palettes. +1 unit file (5), 3 mutants; bundle only, no function, no schema change. **NOT shipped; the handoff says what was observed.**
**Prior batch:** **saved reports: sharing, scheduled email or Slack delivery, pin to Home (§0.135 — Jeff: "Saved reports: sharing, scheduled email or Slack delivery, pin to dashboard"; and "when I was in the Tasks tab and I had selected all and all time - I navigated away and came back and it had reset")** — `saved-reports` GET is own + shared (an Admin the org) where it returned every row; PUT is read-then-merge and keeps the owner (it rebuilt the row from the body and re-stamped ownerId with the caller — the CLAUDE.md sanitize rule, a third time); a delivery schedule lives at `config.delivery` in an allowlisted shape (`src/utils/reportDelivery.js`: cadence, hour, weekday / day-of-month, IANA zone, roster ids, Slack) with recipients limited to THIS org’s roster; the hourly `report-deliveries` job (heartbeat-wrapped, `JOBS_ENABLED`-gated, in the Jobs registry and netlify.toml) runs each due report on the server with the tab’s own `runReport` over the OWNER’s read (the whole org when the owner’s role can see all, else their rows), mails a table to the chosen members and/or posts Block Kit to the workspace’s Slack, and stamps `lastDeliveredAt` / `lastError`; `POST ?action=deliver` is "Send now" through the same path; a pin is `pinnedReports` on my own profile (the §18b34 allowlist) and Home draws each pinned report with `ReportChart` over what I can see; the library card carries Pin / Share / Deliver / delete gated as the endpoint gates, a Shared mark, the schedule’s summary, and a "Shared with you" section; the Tasks date range and the Dispatch board range persist like scope. +1 unit file (11), +1 integration suite (6), 13 mutants; functions AND bundle, netlify.toml; NO schema change. **NOT shipped; the handoff says what was observed — and as Karen (15 Sep): the shared report under Shared with you with only Pin, pinned onto her Home; her own report visible to the Admin.**
**Prior batch:** **drag-to-reschedule on the dispatch week board (§0.134 — Jeff: "Drag-to-reschedule on the dispatch week grid")** — a job card on the week board drags to another cell: another day on its own row moves the date (the whole crew is checked on the new day), another technician’s row hands that seat of the crew over (the lead seat stays the lead), a card from the Needs-a-crew row takes the technician it lands on as a held crew. `src/utils/weekDrop.js` (pure; the board’s reads injected) decides the crew, runs the crew builder’s gates in words (not rostered, out all day, outside the shift, a partial block, double-booked), and builds a PARTIAL body — the date and/or the crew, never `status`, so a held crew stays held and a scheduled job keeps its time; the parent runs the equipment gate for a new day and re-picks held units, confirms the builder’s soft blockers through the app-wide confirm, PUTs, adopts the server’s row (the customer confirmation is the server’s, §0.111), audits `dispatch.reschedule[.override]`, and shows the refusal or the result in a strip above the grid. +1 unit file (12), 8 mutants; bundle only, no function, no schema change. **NOT shipped; NOT observed in a browser at the time of writing — the handoff says what was.**
**Prior batch:** **SHIPPED — the TWENTY-THIRD ship, 18:54 UTC 14 Sep (§0.132, its modal follow-up and §0.133; functions AND bundle; see §0.133's last paragraph).** **the report builder runs real queries (§0.133 — Jeff: "Report builder running real queries. The preview is illustrative today.")** — `src/utils/reportQuery.js` (pure): `fieldsFor(source)` (the dimensions and metrics each of Opportunities / Accounts / Leads / Activity really has), `runReport(def, data)` (group by the dimensions, every metric per group and in total, the period through the tab's `reportPeriod`), `chartData(result, chart)` (bar / stacked / line / funnel / table / kpi, with a NAMED fallback when the definition cannot support the chart), `resolveField` (by id or by label — a report saved before today opens); `src/components/ReportChart.jsx` draws it; the builder's Update preview runs it over the tab's scoped sets, the chips store the engine's ids, Period is a real select, a saved builder report OPENS into the builder and Save PUTs it in place, a name input names it. Gone: the fixed "open pipeline by owner" picture, the Format tab, the add-filter button and compare-to picker (labels), Quotes (the tab holds none), heatmap and scatter (never drawn). Found while building: "Close date" of an OPEN deal through `closeDayOf` read its last stage change (fixed in the engine); a name input placed inside the inline-declared header would have lost focus per keystroke (check:inline caught it, moved out); a UTC-truncated "today" (the date-local scan caught it; `isoLocal`). +1 unit file (12), 7 mutants, three older scans updated. Bundle only. 492/492 mutations, 796/796 unit, 164/164 integration, build guard OK `index-BNgFKZx-.js`. **SHIPPED — the twenty-third ship.**
**Prior batch:** **the enhancement thread's last three (§0.132 — Jeff: "lets work on this item")** — (1) **`task.overdue`** is back in the vocabulary as an hourly Tasks trigger and the hourly job computes it: for every org with an active rule on it, each open task past its due date fires once per task per week at its assignee's alert hour, deduped through the task-reminders ledger, cross-tenant-checked, with no email/SMS/Slack of its own (the rule is the switch); the payload carries `due_date`, `priority`, `days_overdue`. (2) **The Slack action posts Block Kit** — the rendered message as the section, a context line naming the rule, the trigger and the record, an app button — built by a pure helper the pipeline-alert templates' module wraps. (3) **Train now** — `scoreOrg(orgId, { force })` extracted from the nightly batch, a new Admin-only `train-lead-model.mjs` calls it with force for the caller's org, the Predictive card gets the button and prints which gate stopped a training (the engine's 20-row floor still applies). +1 unit file (4), +1 integration file (4), +1 integration case, 9 mutants. Functions AND bundle; no schema change. 484/484 mutations, 782/782 unit, 164/164 integration, build guard OK `index-DL6xNwD_.js`, then `index-DoNj0dG6.js` after the follow-up (the modal's error visible on every step — Jeff's "can't move … to conditions"). **NOT shipped.**
**Prior batch:** **SHIPPED — the TWENTY-SECOND ship, 16:02 UTC 14 Sep (§0.130 + §0.131, bundle only; see §0.131's last paragraph).** **every tab announces itself the same way (§0.131 — Jeff, with a screenshot of every tab)** — the title block Pipeline, Accounts, Contacts, Leads, Quotes and Sales Manager share (the tab's name in the serif italic at 28, then a 12px caption) is now on Tasks (the date moved from an eyebrow above the title to the line under it — his ask), Dispatch (both the dispatcher's and the technician's header: "Dispatch" as the title, the view's own label — the board's date range, "Jobs to schedule", "N technicians", "N dispatch customers" — folded into the caption; the gold rule and the DISPATCH eyebrow gone), Documents (the bold sans title and the eyebrow replaced; the page full width like the rest, its 1280px centred column gone), Reports (the page gains the 1.5rem gutter every other tab has — the title had sat against the header band and left of the others') and Settings (the bold sans title and the gold rule replaced). Bundle only; no test change (presentation). 475/475 mutations, 774/774 unit, 159/159 integration, build guard OK `index-B_6zu6fm.js`, then `index-CyIfr4JI.js` after the follow-up (the gutter from `.tab-page`, not a fixed 1.5rem). **NOT shipped.**
**Prior batch:** **ONE design-token object (§0.130 — Jeff: "lets do that single token fix")** — `src/tokens.js` is now the only token literal in the tree; 30 files import it (the Settings panels through `settings/shared/tokens.js`, the Documents surfaces through `documents/atoms.jsx`, both re-exports), ReportsTab's eight copies and its per-template `const T = TS` aliases are gone, the alias keys (`ink2`/`ink3`/`surface3`/`radiusMd`) are the canonical names, Dispatch keeps its radius 4 as a one-line derived object. Value identity was proved per read by a parser-driven script before a byte was written — the only differences were THREE reads of keys a copy never had (`T.r` in HomeTab; `T.info` inside the report templates), each `undefined` before and a real value now, accepted by hand. +1 test file (5), 4 mutants, guide §18b39 (new) and §16. 475/475 mutations after a printed green baseline, 159/159 integration, 774/774 unit, build guard OK 2,489 kB `index-D0WIMRUE.js` (15 kB smaller — the duplicates). **NOT shipped.**
**Prior batch:** **the "Tonal & Form" signal-system migration — Phase 0 and Phase 1 delivered, NOTHING migrated (§0.129 — Jeff: "I had claude design update the look of the application ... I would like to make these changes")** — Claude Design's brief (`docs/design/handoff-signal-tonal/`, the deliverable `src/tokens-signal-tonal.jsx`: a tint/base/deep RAMP per semantic, SIGNAL aliases, FORM constants, the spine collision, the migration map, the exclusions) is a FORM migration with two stop-for-approval gates of its own; read against `src/`, the spine collision is live today (the Dispatch customer list selects with a tier-coloured 3px spine and renders overdue/renewal in the same row), the token object is NOT shared (24 files declare their own `T`, ReportsTab eight — the brief's "ship primitives, not tokens" is the only route to every screen), and guide §16's "Pills: borderRadius 999" contradicts the design for signal pills (flagged, not edited). Delivered: `docs/design/SPINE_COLLISION.md` (every left spine by meaning; recommendation A; the brief's §7 questions answered from the code; what Jeff must decide) and `scripts/audit-colors.mjs` → `docs/design/color-audit.md` + `.csv` (5,514 colour uses in 164 files; 1,741 signal sites; 316 `inkMuted`-as-status; the off-brand map; the token-copy map). No src/, netlify/ or schema change; the bundle hash unchanged. **Phase 2 waits on Jeff's answers (SPINE_COLLISION §7).**
**Prior batch:** **the lead score's "Why this score" popover is a fixed popover (§0.128 — Jeff: "the review gets truncated ... and a side slider is added")** — absolute under the badge, the Triage rows (horizontal strips, overflow auto) clipped the card and grew a scrollbar; now placed from the badge's rect by a pure `popoverPlacement` (below when it fits, above when that fits, else pinned on screen; both edges clamped), closed by an outside click, a scroll or a resize. +1 test file (2), 4 mutants; bundle only. **LANDED on dev 21:26:43 UTC 13 Sep; then Jeff: "nothing pops up except a scroll bar" — the Triage card's hover lift (a transform) was the containing block for the fixed box; the follow-up renders the popover through a portal to document.body (+6 scans, 1 mutant) — LANDED on dev 21:43:24 UTC, OBSERVED in the pane with the pointer over the card and by Jeff ("working perfectly"). **SHIPPED — the TWENTY-FIRST ship, 21:48 UTC 13 Sep (§0.128, last paragraph).**
**Prior batch:** **Post to Slack as an automation action; deal momentum and AI score drop fire the engine (§0.127 — the next two of §0.124's flagged list)** — the hourly job posted five signals to Slack and fired rules for three; now all five fire, and a rule can post its own message to the workspace's Slack (no alert-type gate — the rule is the switch; an unconnected Slack is a skip that says so). +1 unit, the silent-deal integration test extended, 4 mutants; functions AND bundle, no schema change. **LANDED on dev 21:17:42 UTC 13 Sep; NOT observed in a browser; SHIPPED — the TWENTY-FIRST ship.**
**Prior batch:** **the automation's Assign-to is a roster picker stored by id and name (§0.126 — Jeff: "lets just keep fixing those here")** — a typed name that matched nobody, two users of one name, or a rep renamed later left the rule's task unowned; now a SELECT over the org's active users stores the id and the name, the engine owns by the id in the event's org first (rename-proof, never ambiguous) and by name otherwise; the calendar strip clears on an org switch; §9's users-403 item struck by code. +2 unit, +1 integration, 5 mutants; functions AND bundle, no schema change. **LANDED on dev 21:03:25 UTC 13 Sep; OBSERVED in the pane (the Assign-to select over the roster); SHIPPED — the TWENTY-FIRST ship.**
**Prior batch:** **every mount-time load keys on an active org (§0.125 — Jeff: "Fix the 401-then-200 load race")** — spiff claims, coaching notes and the calendar strip fired from mount on `waitForToken` alone, whose 8-second give-up sent them out unauthenticated (the §9 "401 then 200"; the 200 was the next page load); now they key on `activeOrgId` — nothing before an org, again per org, the previous org's rows cleared first (the header's OrganizationSwitcher had left them on screen) — and the give-up warns. +1 test file (6), 5 mutants; bundle only, no schema change. **LANDED on dev 20:26:50 UTC 13 Sep; OBSERVED in the pane (the reload once-and-200; the org switch reloading all three, both directions) and by Jeff as Karen (his screenshot: the manager's note, My claims and the calendar prompt on Home; then a private-window sign-in, "received no errors"); **SHIPPED — the TWENTIETH ship, 20:43 UTC 13 Sep (§0.125, last paragraph).**
**Prior batch:** **light workflow automation (§0.124 — the enhancement Claude recommended: "when deal enters Proposal, create task X; alert if silent 14 days")** — the hourly job's silent / stuck / close-lapsed signals now fire the rules engine (once per deal per signal per week, at the rep's alert hour); a rule's task is linked to the deal and owned by its assignee, resolved in the org's roster; one trigger vocabulary shared by the engine and the panel (task.overdue, never fired, dropped; task.completed now fired); the condition field is a select over the event's fields; merge fields ({{account}}) render in titles, notes and emails; **update_field takes an allowlist — it had written any column the rule named, orgId included (guide §18b37, new)**. +1 unit file (13), +1 integration file (5), 9 mutants; no schema change. **OBSERVED by Jeff on dev (every walkthrough check); SHIPPED with its first three follow-ups — the NINETEENTH ship, 19:59 UTC 13 Sep (§0.124, last paragraph).**
**Prior batch:** **lead scoring, read against the code (§0.123 — Jeff: "finish Lead Scoring v1.5")** — v1.5 AND Phase 2 were already live (the horizon entries were stale); fixed what was not right: the panel's drifted mirror of the defaults (five engagement rules where the engine has ten — a Reset or a new org's first Save undid v1.5) replaced by ONE shared copy, "Web Form" made an inbound source, the engine's 18 tests registered in the harness with three engine mutants, a readiness line toward the predictive threshold. +1 test file (6), 5 mutants; no schema change. **SHIPPED — the EIGHTEENTH ship, 13:14 UTC 13 Sep (§0.123, last paragraph).**
**Prior batch:** **email templates for reps, mailto-based (§0.122 — the second half of the agreed recommendation)** — an org library in settings (both halves), nine merge fields, rendered for the contact in view and opened in the rep's own mail client from a picker under ✉ Email, the activity logged with the template named; a Sales process → Email templates panel. +1 test file (7), 4 mutants; no schema change. **SHIPPED — the EIGHTEENTH ship, 13:14 UTC 13 Sep (§0.123, last paragraph).**
**Prior batch:** **web-to-lead — an embeddable form that posts into Leads (§0.121 — Jeff: "SMBs ask for this constantly" → "I agree with your recommendations")** — one hosted form per org behind a server-minted token (guide §18b36, new), `/lead-form/<token>` framable on the customer's site, a submission an UNASSIGNED scored lead in the token's org with source "Web Form", a Slack post under a new `webLead` switch, a Connected apps card with the link and the embed code. +1 unit file (12), +1 integration file (6), 5 mutants; no schema change (a settings.extra key). **SHIPPED — the EIGHTEENTH ship, 13:14 UTC 13 Sep (§0.123, last paragraph).**
**Prior batch:** **the Pipeline filter's time window is applied and read back honestly (§0.120 — Jeff: "I keep trying to select all time … it does not actually use that selection")** — Apply had mapped both "This quarter" and "All time" to the empty filter since April, so "This quarter" never filtered and "All time" read back as "This quarter"; now only "All time" is empty, and empty reads back as "All time". +1 test file (3), 1 mutant. **SHIPPED — the SEVENTEENTH ship, 20:59 UTC 11 Sep (§0.120, last paragraph).**
**Prior batch:** **the Pipeline list shows the closed deals its filter asked for (§0.119 — Jeff: "Filtered by won deals only and it returned nothing in the list")** — the tab's filter kept the six won deals and the header counted them, then `ListView` dropped every closed deal on its own; now the list renders the set it is handed, the "All open" default unchanged. +1 test file (2), 1 mutant. **SHIPPED — the SEVENTEENTH ship.**
**Prior batch:** **the queue lists only jobs to schedule; Create job / Edit job in the queue header (§0.118 — Jeff: "I can't edit the selected job beyond scheduling"; "only show unscheduled jobs under that list")** — the left list held every job, scheduled ones included; a won opportunity had no button where a saved job has one, and its header read "null techs"/"nullh". Now unscheduled only, "Edit job →" / "Create job →" beside the name, unset template values read "Not set"; and per Jeff's finding on dev the same hour, a scheduled job clicked on the Job Board opens its record in Jobs instead of joining the queue. +2 tests, 4 mutants. **SHIPPED — the SEVENTEENTH ship.**
**Prior batch:** **two small things seen while shipping (§0.117)** — a queue card read "◷ 08:00" on an unscheduled job with a preferred time (now the date, "· prefers 8:00 AM"); the document still scrolled 8 px (the shell's bottom padding now counted). +1 test, 2 mutants. **SHIPPED — the SEVENTEENTH ship.**
**Prior batch:** **a job's equipment requirement can be seen and changed (§0.116 — Jeff: "I do not see where an equipment requirement is listed")** — JOB-2026-0004's August-seed `equipment_ids` held a unit id where the scheduler expects equipment kinds, so it blocked as "no units exist", and the Jobs editor had no equipment field. Jeff's final word: "I want to require a kind of equipment … I don't care which one I get as long as there is one assigned to me and the job at the day and time." So: a requirement is a KIND (the fleet's Category), the editor and new-job form pick kinds, a stale entry is a removable chip, and SCHEDULING RESERVES one in-service unit per kind that no overlapping job holds — stored in the new nullable `assigned_equipment_ids` (both DBs), shown on the job, released when it leaves the schedule. +5 tests, +1 integration, 3 mutants. SHIPPED in the SIXTEENTH ship (`0726934`, 22:51 UTC 10 Sep).
**Prior batch:** **the next step after choosing a crew, and a page that fits the screen (§0.115 — Jeff: "the schedule crew buttons are so low I can't see them on a 40" ultrawide")** — the Dispatch page's height resolved to auto, so the Queue's action bar sat below the fold: the root of §0.111's and §0.114's "still Needs a crew" reports. The page is now viewport-bound; the moment a slot is filled a card at the top asks Schedule now / Wait for group schedule / Clear; a held crew is persisted without a time, survives leaving, shows as TBD on the week board, and Mass-schedule honours it; a preferred start on an unscheduled job no longer reads as a placement. +5 scans, 5 mutants. SHIPPED in the SIXTEENTH ship (`0726934`, 22:51 UTC 10 Sep).
**Prior batch:** **a filled crew slot is not a saved schedule (§0.114 — Jeff: "assigned Savannah … it still shows it needs to build a crew")** — the row was never written; "+ Add" read as done while only "Schedule crew" persists, and a success said nothing. The header now says "not saved yet: click Schedule crew to assign"; a success shows a status banner. The write path reproduced end to end in the pane. +1 scan, 2 mutants. SHIPPED in the SIXTEENTH ship (`0726934`, 22:51 UTC 10 Sep).
**Prior batch:** **saving a job with no category 500ed; a new technician starts rostered Mon–Fri 8–5 (§0.113 — Jeff: "I set a preferred start time to 11AM and it gave me the server error")** — the start time was innocent: `trade`/`jobType` are NOT NULL and the Jobs editor sent null for "— None —"; the PUT now coerces to `''` and the editor sends `''`. `src/utils/workWeek.js` seeds a new technician with Mon–Fri 08:00–17:00 (client draft and server fallback). +5 unit, +1 integration, 4 mutants. SHIPPED in the SIXTEENTH ship (`0726934`, 22:51 UTC 10 Sep).
**Prior batch:** **the crew builder's phantom clash, and a job's preferred start time (§0.112 — Jeff: "where is the 9AM assumption coming from?")** — `scoreTech` scored an unscheduled job as if it began at 9 AM and compared start hours against the technician's jobs on ANY date; now same-day only, and only when a start is known (the builder's chosen Start, else the job's new **Preferred start time**, saved as `scheduledStart`/`scheduledEnd`/`timeSlot` and seeding the builder). +3 scans, 2 mutants. SHIPPED in the SIXTEENTH ship (`0726934`, 22:51 UTC 10 Sep).
**Prior batch:** **customer-facing notifications — appointment confirmation, "technician on the way", and a private job-status link (§0.111 — Jeff's second pick; email now, SMS behind the same switch)** — per-org switches (`settings.extra.customerNotifications`, OFF by default, a new Settings → Dispatch panel) decide; a pure module decides what a job change means and how it reads; `_customerNotify.mjs` runs after every dispatch-jobs write, emails through Resend, texts through Twilio only when the site has credentials (otherwise the job's trail records why not), issues a random per-job token once, and appends every attempt to `dispatch_jobs.customer_notifications`; `/status/<token>` is the app's first unauthenticated read — token-only, escaped, no-store; the job detail shows the trail and copies the link. +13 unit, +6 integration, 10 mutants. SHIPPED in the FIFTEENTH ship (`0a242c6`, 17:47 UTC 10 Sep).
**Prior batch:** **maintenance agreements — skip or defer a plan occurrence, agreement renewals in the Service Due queue and by email/Slack (§0.110 — Jeff's first pick of four dispatch items)** — the recurrence arithmetic moves to `src/utils/planVisits.js` and is shared with the hourly job; `dispatch_plan_visits` records a skipped (retired) or deferred (new due date, same grid date) occurrence per customer per plan; plans carry `renewalLeadDays` (null → 60); the queue shows Skip/Defer/undo and an Agreement renewals list with "Renew 12 months" (anniversary kept); `pipeline-alerts` Signal 6 emails the org's Admins/Managers behind an `agreementRenewal` preference and posts to Slack under a company switch, weekly per customer through the alert ledger, which the Reports read now excludes. +20 unit, +6 integration, 10 mutants. SHIPPED in the FIFTEENTH ship (`0a242c6`, 17:47 UTC 10 Sep).
**Prior batch:** **the self-profile save takes an allowlist, the last three console-only settings catches are shown, two inert dispatch buttons go, and the §9 backlog is read against the code (§0.109 — Jeff: "Let's get these all resolved")** — `PUT /users?me=true` had its role column pinned but merged the WHOLE body, and the profile panel sends the whole roster row it holds, so every preference toggle rewrote quota, team, territory, `active` and the quarterly quotas from the caller's own copy; now `_selfProfile.mjs`'s frozen `SELF_EDITABLE_KEYS` is all that reaches the merge (guide §18b34), proven against the test database. The team and territory delete cascades and the lead-conversion benchmark save report a refused write in a banner instead of the console. "Manual pick" and "Test auto-create" (rendered, did nothing) removed. Read, not built: the dispatch "smaller gaps", the template name bug, the Technician whitelist and the legacy tech fields were already done; the "ten @test.com users" are one, and it is a linked Clerk account. SHIPPED in the FIFTEENTH ship (`0a242c6`, 17:47 UTC 10 Sep).
**Prior batch:** **a first sign-in provisions the roster row (§0.108 — Jeff: "Please fix the clerk name issue for integration requests")** — the production workspace had no `users` rows at all (only an Admin's Sync ever created them), so its Admin's request recorded no name and everything they created was unowned; now `ensureRosterRow` creates the caller's row from Clerk in the sync's shape with a VALIDATED role, idempotently, from the self-profile GET when nothing matches and from the request function for an unrostered requester. Landed on dev at 21:08:04 UTC 9 Sep (Netlify record `6aa1ca8b…` = `8134313`, functions only); **SHIPPED in the fourteenth ship (`59116be`, 21:17 UTC).**
**Prior batch:** **`check:fnscope` — every name a function file reads is bound (§0.107, the guard §0.95 and §0.101 asked for)** — a lexical-scope walk over every `.mjs` under `netlify/functions/` and `db/` that reports an identifier read with no binding in any enclosing scope and no standard global of that name; run against `bf4a3c5` it flags every line of §0.95 and §0.101; a sixth gate in the verification chain. Tooling only; SHIPPED with §0.106 in the thirteenth ship (`f546a03`).
**Prior batch:** **a bulk stage move posts one summary line, and three small closes (§0.106 — Jeff: "lets resolve these from your list")** — the CSV import that moves deals now posts ONE Slack line under the org's "Deal stage changed" switch (how many moved, into which stages, wins, who imported), never one per deal; item 31 struck (Jeff agreed); the Connected-apps line non-Admins could never reach removed; `INTEGRATION_REQUESTS_TO` is Jeff's hand on the Netlify sites. Landed on dev at 20:26:54 UTC 9 Sep (`index--5aHcCr_.js`, Netlify record `6aa1c0ea…` = `50a6cf7`); **SHIPPED in the thirteenth ship (`f546a03`, `index-DKAHLlej.js`, 20:46:52 UTC).**
**Prior batch:** **owned underneath, named on screen (§0.104, handoff item 28) and a logged email keeps its envelope (§0.105, item 27) — one verified tree, two sections** — `stampOwnerId` now stamps the owner id AND the display name from one roster row when the caller becomes owner by default (a contact created without naming a rep had read as nobody's and could vanish from a Manager's name-keyed scope); the contact rail names the owner of an older row; `db/backfill-owner-names.mjs` fills the four legacy rows by Jeff's hand (dry run printed). Four nullable columns on `activities` (applied to both databases first, §18c) carry From, To, Cc and Message-ID from `envelopeOf` (the header To/Cc winning over the envelope), and the viewer shows them when present. Landed on dev at 19:26:03 UTC 9 Sep (`index-De0DMnrV.js`, Netlify record `6aa1b288…` = `dfcdee4`); §0.104 OBSERVED by Jeff as Karen; **SHIPPED in the twelfth ship (`901ad12`, `index-DowJRBZt.js`, 19:45:52 UTC)**; the four legacy rows left as they are (demo data, Jeff's call).
**Prior batch:** **one site runs the scheduled jobs (§0.103, handoff item 36 — Jeff: "add the per site env flag")** — dev and prod share one database and both had run every job since the tenth ship, so every digest, every task reminder and the lead-scoring batch ran twice, one per site, and each digest email would have gone out twice (the five hourly signals alone dedupe through `recommendation_log` — the first hourly alert ever, 13:00 UTC 9 Sep, posted once with both sites running; Jeff's screenshot); now `jobsEnabled(env)` is true only where `JOBS_ENABLED` is exactly "true" in that site's Netlify env, the heartbeat wrapper is the one gate (no run, no stamp, a 200 that says skipped anywhere else), `job-status` carries `enabled`, and the tile reads "Scheduled jobs not enabled on this site (JOBS_ENABLED)" as a FAILING check so a forgotten flag on prod can never read as healthy. Unset means off. Set on the prod site by Jeff's hand (the tool write was refused by the permission classifier); dev stays off. Landed on dev at 17:31:33 UTC 9 Sep (`index-CghUEQTb.js`, Netlify record `6aa197ca…` = `67d5554`) — PROVEN: dev's `task-reminders` row stopped at 17:31:02 and had not moved at 17:33:01 while prod's legacy row ticked on. **SHIPPED in the eleventh ship (`a9d2d08`, `index-DcLtbkSJ.js`, 18:52:48 UTC) — PROVEN on prod: the `salespipelinetracker.com` / `task-reminders` row appeared within a minute (ok at 18:54:03, `ok_count` 2), the legacy row stopped at 18:52:02.**
**Prior batch:** **a mutant cannot survive the harness's death (§0.102, handoff item 35)** — twice the mutation harness died mid-mutant (a transient Windows file lock on the mutant write) and left a mutated source file on disk for `git status` to catch; now every mutant goes through `scripts/_mutant.mjs`, which registers the original BEFORE the write, restores in a finally and again from a process exit hook (exit, signals, uncaught exception), refuses an async run, and rethrows a failed restore so the harness stops rather than grading over a stuck mutant; the harness has no `writeFileSync` of its own. Eight tests against a temp file, the crash class proven with a child process that calls `process.exit()` inside the run; 3 mutants on the sidecar itself. Tooling-only; nothing deploys. ON DEV.
**Prior batch:** **the digest job had thrown twice a day since 7 April — `bf4a3c5`'s other file (§0.101, eleventh session — found in the per-site heartbeat rows: `digest` `error_count` 6 on each site)** — the same rename §0.95 fixed in `pipeline-alerts.mjs` had left `wantsDigest` and `wantsSms` in `digest.mjs` reading a name bound only inside the user loop; every roster member is 08:00 local, so at 08:00 and 13:00 UTC the first matching user threw, the outer catch answered 500 and Netlify retried twice — no digest email or SMS to anyone for five months, and the Monday manager digest had selected managers by `userType`, a key a `db.select()` row never carries, so none had ever matched. Six lines fixed; the helpers lifted out of the source and RUN by a new suite (4 mutants). Functions-only; landed on dev at 16:51:23 UTC 9 Sep (Netlify record `6aa18e68…` = `cab9b92`, only the `digest` function digest changed); the proof is tomorrow's 08:00 and 13:00 UTC runs — `error_count` staying 6 while `ok_count` climbs. NOT shipped. Item 31 found not to be a bug by the same reading.
...
**Jeff's two decisions, asked with the audit above and DONE in the second
commit:** the totals are **quarter-to-date**, and the three inert header
buttons are **removed**.

**The totals (second commit).** `buildRepStats(rep, opportunities,
activities, tasks, period)` takes the current fiscal quarter: `wonInQ` is
the rep's Closed Won deals with a close day inside `period.from..to`
(`closeDayInRange` from pipelineReport.js — wonDate, else the stage-change
day, else the forecast; §0.75's rule), `closedArr` sums those, `quota` is
`userQuotaFor(rep, 'Qn')` for the current quarter's n (a quarterly plan's own figure, else
annual ÷ 4 — the Reports helper, so the Performance leaderboard and this
tab now agree). `attainPct`, the health score and its label, "Team to
quota", the Team cards, the Forecast ledger and the Today tab's
Gap-to-Quota (now a quarter's quota less commit, over the quarter's weeks —
the "/wk needed" is finally a real number) all follow. `wonOpps` stays
all-time and is still returned: the Team tab's coaching flags compute a
win rate from it; `wonInQ` is returned beside it. The quarter block moved
ABOVE the `repStats` memo (it is an input now) and the memo's deps carry
`curQ.key`, so the stats re-run when the quarter turns. **The
Administration board** keeps the annual quota in its column and its bar is
fiscal-year-to-date: both `rWon` sums (territory rows and unassigned rows)
take `closeDayInRange(o, fyRange.from, fyRange.to)` with `fyRange =
fiscalRange(curQ.fiscalYear, 'FY', fiscalStart)` from reportPeriod.js,
passed to `AdminTab` as a prop; the column header reads "FY attainment".
The header's three buttons (This quarter / All reps / Export) are gone
with their wrapper div. **Nothing was labelled "Export" that worked:**
`exportToCSV` is destructured from the context in this tab and never
called, so the tab has no export (carried, handoff item 19).

**What a user sees change (second commit):** Closed, Attainment and the
health labels on the Forecast, Team and Today tabs are this fiscal
quarter's; a rep whose wins were all in earlier quarters drops to 0% /
"AT RISK" here (the win rate flag on the Team tab is unchanged —
all-time); Quota on those tabs is the quarter's figure; the Gap-to-Quota
tile's gap and "/wk needed" are quarter numbers; the Administration
board's bar counts this fiscal year's wins against the annual quota; the
three header buttons are gone.

Tests: `tests/current-quarter.test.mjs` grows to 13 — three source
assertions pin the quarter window, the quarter quota, the memo deps and
the curQ-above-repStats order (a TDZ otherwise), both FY-range sums and
the column label, and the absence of the three buttons with the header
closing straight onto `<SubTabs />`. Gates green on 142 files,
**459/459**, **204/204 mutations, printed green baseline** (four new:
Closed every deal ever again, Quota the annual figure again, the
Administration bar all-time again — a two-line anchor so it hits the
territory rows and not the one-line unassigned variant — and the memo deps
dropping the quarter key), **79/79 integration** (no function changed),
build guard OK (2,479 kB, `index-ByAdikgx.js`), `dist/` cleared. **Not
browser-checked here** (Admin/Manager-only tab); Jeff eyeballs on deployed
...
unauthenticated. `master` stays at `eec3948`.
...
"Lost deals —" and "no closed deals" present in the served bundle. `master`
stays at `eec3948`.
...
configured", "14 industries" and "Q1 starts Feb 1" absent. `master` stays at `ad76a38` (the eighth ship).
...
**Pre-ship read (8 Sep, ninth session — the handoff's "read prod's stored
Slack webhook" step, done before any ship is asked for):** one read-only
SELECT of `extra->'slackConfig'->>'webhookUrl'` (with `channel` and
`enabled`) per `settings` row on the shared app database — a throwaway script
at the repo root importing `validateSlackWebhookUrl`, printing host, length
and validity and never the URL, deleted after the run, tree clean. Five
settings rows; ONE webhook stored — the dev org's, host `hooks.slack.com`,
`valid=true`, channel `#sales-alerts`, enabled — and the other four orgs
store none. So after this ships no org's alerts start failing silently
against a non-Slack host; nothing to raise with Jeff. The 403/400 itself is
still not observed.

**OBSERVED by Jeff (8 Sep, ninth session — his screenshot), and a follow-up
the same hour.** He typed `hooks.slack.com/services/` — no scheme — into the
Incoming Webhook URL field and clicked Save configuration. The server
refused it (`settings` PUT 400), nothing was saved, and the Slack card behind
the dialog still read "Connected · #sales-alerts" with the real webhook: the
gate, seen. Two things in the screenshot were wrong, both circled: the
refusal was rendered as the PANEL's banner — "Slack settings not saved —
Webhook URL is not a valid URL." at the top of Connected apps, behind the
still-open modal (the modal's own message box served only Send test
message; `handleSaveSlack` restored its snapshot and wrote `setError`, which
the dialog covers); and "is not a valid URL" — `new URL()` refusing a
scheme-less string — said nothing about what to do. Now: the modal has one
message slot for both buttons, `handleSave` catches what `onSave` throws and
shows "Not saved — <reason>" under the field, `handleSaveSlack` restores its
snapshot and RETHROWS (no banner; on success it closes the modal as before),
and `validateSlackWebhookUrl` checks for a scheme before parsing and answers
"Webhook URL must start with https:// (https://hooks.slack.com/services/…)."
— every other refusal is unchanged. Guide §18b32. Verified: five gates,
build guard OK 2,437 kB `index-BRrOppU7.js`, **572/572 unit** (2 new: the
scheme-less cases in `slack-webhook.test.mjs`; the modal-side scan in
`connected-apps.test.mjs`), **116/116 integration** (unchanged;
`send-slack.itest.mjs` matches `/Webhook URL/`, which the new message still
carries), **302/302 mutations, printed green baseline** (3 new: the rethrow
becomes the banner again; the modal swallows the refusal; the scheme check
is removed). Not browser-checked here (the pane holds no session); Jeff
...
### 0.96 An org chooses which pipeline alerts post to Slack (8 Sep, ninth session — handoff item 29; Jeff: "lets do 29, 30 and 32")

**Jeff: "How do I choose what gets posted to slack (or other connected apps)?
Can we add an option that enables me to select what actions get posted" —
then "lets do 29, 30 and 32 while I reset the webhook."** What existed (§0.95,
last paragraph): each of the five signals fired when the deal's rep had that
alert on in their own preferences, and `sendSlackToOrg` posted every fired
signal to the org's one webhook, gated only by `slackConfig.enabled`. No
org-level choice; the modal's info box listed the five as a static sentence.

**Design.** One pure module shared by both sides the way
`integrationCatalog.js` is — `src/utils/slackAlerts.js`: `SLACK_ALERT_TYPES`
(five `{ key, label }`, the keys being the rep-side `DEFAULT_PREFS` keys —
`dealSilent`, `dealStuck`, `closeLapsed`, `dealMomentum`, `scoreDropAlert` —
so an Admin and a rep name the same alert the same way, pinned by test),
`cleanSlackAlerts(raw)` (every key → boolean, absent → true, unknown dropped,
garbage → all on), `slackAlertEnabled(slackConfig, alertType)` (no type → not
gated, so the digest and the test message are untouched; unknown type →
false, fail closed on a typo; no `alerts` saved → true, so **a config saved
before this key existed posts everything, as it always did**; otherwise only
an explicit false is off), `slackAlertsOnCount`. `sendSlackToOrg(orgId, msg,
alertType)` asks it after the enabled check and before the post; the five
call sites in `pipeline-alerts.mjs` pass their key; `settings.mjs` PUT stores
`slackConfig.alerts` as five booleans whatever the client sent (inside the
existing webhook guard, so Disconnect's `slackConfig: {}` is untouched); the
Configure Slack modal replaces the static sentence with five checkboxes
seeded from the stored config (absent = ticked) and saves them with the
webhook; the Slack card's status reads "Connected · #channel · n of 5
...
all four touching files), **118/118 integration** (2 new in
`send-slack.itest.mjs`, against the real row: an org with `dealSilent: false`
posts nothing for that type and still posts `dealStuck` and the untyped
digest, an unknown type posts nothing; an org saved before the key posts
every type), **314/314 mutations, printed green baseline** (5 new: the
unticked alert posts anyway; a typo posts instead of failing closed; the org
path stops asking; a call site drops its type; settings stores the raw
shape). A harness run earlier in this batch died mid-mutant and left one of
its own mutants on disk (`integration-requests.mjs`, `if (!to && false)`);
`git status` showed a file this batch never touched, the file was restored
from the index, the harness re-run in full — **a harness run is not over
until `git status` shows only the batch's own files.** Not browser-checked
...
**PROD SHIPPED (Jeff: "ship prod") — the tenth ship, §0.92 through §0.99.**
Ancestor check (tree clean, `dev` == `origin/dev` at `5a306e3`,
`origin/master` an ancestor of `dev`), then `git push origin dev:master`:
`master` `cf72f99` → `5a306e3` (49 commits), pushed 22:36:01 UTC.
salespipelinetracker.com served `index-JJdhJmyd.js` at 22:36:51 UTC, 50
seconds after the push, 2,504,539 bytes (dev's size), `pk_live_` inlined (the
one `pk_test_` string is Clerk's SDK prefix check, as on every ship); "Not
saved — ", "Not requested — ", `title:"Open"`, "Posted the moment it
happens", "Scheduled jobs running" and "was not connected — " in the served
bundle, "Slack settings not saved" absent. Netlify's deploy record for the
prod site (`sales-pipeline-v2`, `ef1f2af9…`): deploy `6aa08dd3…`, commit
`5a306e3`, branch `master`, ready, published 22:36:50 UTC, **76 functions**
(`_heartbeat` and `job-status` among them, every function rebuilt), secret
scan 0 matches over 375 files. Unauthenticated: `send-slack` POST 401,
`job-status` GET 401 / POST 405, `email-inbound` 401,
`calendar-connections` 401. **Prod's first heartbeat:** a read-only SELECT
at 22:38:51 UTC — `task-reminders` `ok_count` 40 where dev alone had ticked
38 times since 22:01, so prod's 22:37 and 22:38 runs stamped the row: the
...
and it says the job ran SOMEWHERE, not where. A prod job that stalled while
...
stays at `5a306e3`.
### 0.101 The digest job had thrown twice a day since 7 April — `bf4a3c5`'s other file (9 Sep, eleventh session — found in the per-site heartbeat rows)

**Found at the session's open**, reading `site_job_heartbeats` as the handoff asked: one site, four jobs, every `last_status` `ok` — and `digest` with `error_count` 6 on dev's row (`ok_count` 15) and 6 on the legacy row (17), `last_error` null because the next ok hour overwrites it. The counts are the arithmetic of the bug: dev's digest had 17 hourly slots since §0.100 landed (00:00–16:00 UTC on the 9th), 15 ok and two failing, each failure retried twice by Netlify — six stamps; the legacy row is prod's from 23:00 on the 8th, 19 slots, 17 ok, the same two hours. `task-reminders` (1,030 ticks in 1,029 minutes) and `pipeline-alerts` (17 in 17) match their slots exactly; only digest's total exceeds its slots, which is the retry.

**What it is:** `bf4a3c5` (7 Apr, "text bug fix 2") touched TWO files — `pipeline-alerts.mjs`, fixed in §0.95, and `digest.mjs`, where the same rename left `wantsDigest(resolvedProfile, alertType)` reading `getPref(profile, alertType)` and `wantsSms(resolvedProfile, smsKey)` reading `profile?.smsNotifications`; the only `profile` in the file is the `const` inside the user loop, invisible to a module-scope function. The run walks every active user, converts their digest hour to UTC, and on the first user whose hour matches calls `wantsDigest` — a ReferenceError outside every inner try, the outer catch's 500, stamped `error` by `withHeartbeat`. Every roster member's `digestTime` is 08:00 (read from `profile`, read-only) in `America/Chicago` or with no timezone (UTC), so the throw is at 13:00 and 08:00 UTC daily, on both sites: no task digest, overdue nudge, opportunity digest or digest SMS has reached anyone since April, and the hours between returned 200 having done nothing. Two more in the Monday manager block, older than the rename: managers were selected by `u.userType === 'Manager' || u.userType === 'Admin'` and the reps they see by `u.userType === 'User'` — the users table has `role`; `userType` is a key inside the `profile` jsonb (`users.mjs` `sanitize()` writes it there and the GET spreads it flat), so on a `db.select()` row it is `undefined` and no manager ever matched; and had one matched, the block read `resolvedProfile` — the rep loop's const, out of scope — for the manager's preference. The comment at the loop head said the prefs were "flat on the user row"; the table has no such columns and the `profile.*` fallbacks are the values that were ever read.

**Fix:** both helper bodies read `resolvedProfile`; the manager block reads `u.role` and `mgr.role`, and the manager's own `profile` for `managerTeamDigest`; the comment now says where the fields live. Nothing else changed — the emails are as designed and the profile panel's Notifications rows turn them off. **Test:** `tests/digest-prefs.test.mjs` (4) lifts `DEFAULT_PREFS`, `getPref`, `wantsDigest` and `wantsSms` out of the source by regex and runs them with nothing but the globals in scope (a disabled preference honoured, instant is not digest, an unset one falls back to the default, no profile is the default and not a throw; the SMS helper the same), then scans: neither body names `profile`, five rep-side `wantsDigest(resolvedProfile, …)`, two `wantsSms(resolvedProfile, 'digest')`, the manager check reads the `profile` declared the line above and `resolvedProfile` appears nowhere after the rep loop, and `.userType` is read nowhere on a row — that scan caught a third read the hand had missed, `mgr.userType === 'Admin'` choosing the visible reps. **Mutants (4):** each helper reading `profile` again, the manager check reading `resolvedProfile`, managers by `userType`. **Verified:** five gates, **611/611 unit** (4 new), **337/337 mutations, printed green baseline**, **124/124 integration**, build guard OK `index-B-DxPVK_.js` (UNCHANGED — no `src/` change; functions-only, the proof is Netlify's record). Guide §18b33 carries two new bullets: a rename lands in every file the commit touched — `git show --stat` the origin commit before the rule is written; and a `db.select()` row has the schema's columns and nothing else.

**What changes when it lands:** at 13:00 UTC (08:00 Chicago) dev's digest reaches the users whose digest preferences are on and who have tasks due today, overdue tasks, or deals updated in the last 24 hours; at 08:00 UTC the ones with no timezone. Both sites run the job against the shared database, so after the ship a user can receive the same digest from each site — the shape `pipeline-alerts` already has, not new to this batch; an open item. The first Monday manager digest ever would go on 14 Sep at those hours to Admins and Managers with `managerTeamDigest` on (the default), listing the reps of their org (an Admin) or team. **Item 31, reconsidered by this reading:** the four manager copies in `pipeline-alerts.mjs` read `manager.profile || {}` — which IS where a row's preferences live (the profile panel's save goes through `users.mjs` `sanitize()` into the jsonb); the rep path's top-level-first read is the dead half, harmless. Item 31 is not a bug.

**Landed on dev (`cab9b92`, pushed 16:50:47 UTC 9 Sep) — read from Netlify's
deploy record, since nothing the client bundles changed:** deploy `6aa18e68…`,
commit `cab9b92`, branch `dev`, created 16:50:48, published 16:51:23 UTC (36
seconds after the push), 76 functions, secret scan clean; the `digest`
function's digest changed (`fa43f084…` → `a6ed6870…`, built 16:51:23) and
every other function's digest is the one `6aa09a36…` (the 8 Sep handoff
commit) carried. **Not yet proven by a row:** the 17:00 UTC run cannot reach
the bug (no roster member's digest hour is 17:00); the proof is the 08:00 and
13:00 UTC runs on 10 Sep — a read-only SELECT of `site_job_heartbeats` for
`accelerep.netlify.app` / `digest` showing `error_count` still 6 and
`ok_count` up by one after each, with `last_summary` `{ body: "Digest
complete" }` — and, for a user with tasks due, overdue tasks or a deal updated
in the last 24 hours, the email itself. Prod's legacy row keeps counting six a
day until the ship. `master` stays at `5a306e3`.
### 0.102 A mutant cannot survive the harness's death — restore in a finally and from the exit hook (9 Sep, eleventh session — handoff item 35)

**What it was:** `scripts/mutate-import.mjs` applied each mutant as three statements — `writeFileSync(file, mutant)`, `execSync` the suites, `writeFileSync(file, original)` — with nothing between a throw and a dirty tree. Twice it died mid-mutant: §0.96, and §0.100 when the mutant write itself threw `UNKNOWN errno -4094` (a transient Windows file lock) and left `forecastCall.js` mutated on disk. `git status` caught it both times; nothing else would have.

**Fix:** a sidecar, `scripts/_mutant.mjs` — `withMutant(file, mutated, run)` reads the original, registers it in a module-level map BEFORE the mutant write, writes the mutant, calls `run()` (synchronous; an async run is refused, because the finally would restore before it settled), and in a `finally` writes the original back and unregisters it; a restore that fails stays registered and is rethrown so the harness stops rather than grading the next mutant over a stuck one. `armRestoreOnExit()` (idempotent) registers a process-level restore of everything still in the map on `exit`, on SIGINT/SIGTERM/SIGHUP and on an uncaught exception. The harness imports both, arms the hook at top level before the first mutant, and its loop is now `const failed = withMutant(file, mutated, () => { try { execSync(…); return false; } catch { return true; } })`; `writeFileSync` no longer appears in the harness at all. **Test:** `tests/mutant-restore.test.mjs` (8) against a temp file — a run that returns (the mutant seen during the run, the original after, the result passed through), a run that throws (restored, the throw the caller's), an async run refused, bad arguments refused before any write; **the crash class itself:** a child `node` that arms the hook and calls `process.exit(7)` from inside the run — every `finally` skipped — exits 7 with "mutant: restored … (process exit)" on stderr and the file back to its original; a child whose run throws uncaught dies non-zero with the file restored by the finally; the hook armed twice adds one listener; and a source scan of the harness — it imports the sidecar, arms it at top level, applies every mutant through `withMutant`, and has no `writeFileSync`. **Mutants (3, on the sidecar itself):** the original not registered before the write (the crash test fails — the hook has nothing to restore), the finally no longer restoring, an async run graded. **Verified:** five gates, **619/619 unit** (8 new), **340/340 mutations, printed green baseline** (the harness graded its own sidecar through the sidecar — the running process holds the unmutated module, the spawned children read the mutant from disk), **124/124 integration**, build guard OK `index-B-DxPVK_.js` (UNCHANGED — nothing under `src/` or `netlify/` changed; a tooling-only batch that no deploy shows). Guide §18b23 has a third part.

### 0.103 One site runs the scheduled jobs — `JOBS_ENABLED`, per site, unset means off (9 Sep, eleventh session — handoff item 36; Jeff: "add the per site env flag")

**What it was:** dev and prod share one Neon database, and since the tenth ship both sites have run every scheduled job — `pipeline-alerts` and `digest` hourly, `task-reminders` every minute, `score-leads-batch` daily — against the same orgs, deals, preferences and Slack webhook. Every task reminder and the daily lead-scoring batch ran twice, one per site; with §0.101 shipped every digest email would have gone out twice, and the first manager digest on Monday 14 Sep would have arrived in duplicate. **A correction to what this batch first claimed:** the five hourly pipeline signals did NOT double — `wasRecentlyAlerted` in `pipeline-alerts.mjs` reads `recommendation_log` for the same org, rep, deal and signal within seven days (`DEDUP_DAYS`), so the second site's run at the same hour finds the first's record and skips. OBSERVED (Jeff's screenshot of #sales-alerts, 9 Sep): the first hourly pipeline alert ever — "🔴 Close date lapsed — test (Test) — close date 2026-04-30 passed 132 days ago · Rep: Jeff Russell · Stage: Qualification · ARR: $100K", 8:00 AM Chicago = 13:00 UTC, while BOTH sites were still running — posted ONCE, the "Update close date →" button beneath it. Five months after §0.95's bug began, the first hourly signal reached a channel. That dedupe is per signal and fails OPEN on a read error; the digests, reminders and lead scoring have no such record — the flag is for them, and stands.

**Fix (option a from the handoff):** `jobsEnabled(env)` in `src/utils/jobHealth.js` — true only when `JOBS_ENABLED` is exactly `"true"` (trimmed, any case); unset, `"1"`, `"yes"`, a boolean are all OFF, so a new site, a branch deploy or a preview never double-sends by default. The wrapper `_heartbeat.mjs` is the one gate: after naming the site and before any stamp or the run, a disabled site logs one line and answers 200 `{ skipped: true, reason: 'JOBS_ENABLED is not "true" on this site' }` — the handler is never called and no row is written. `job-status` adds `enabled: jobsEnabled(process.env)` to its body; `jobHealth(rows, now, { enabled })` makes every verdict `'disabled'` when it is false, whatever the rows say (old rows from before the flag would otherwise read ok or stalled); `jobLine` reads "Not enabled on this site (JOBS_ENABLED)"; `jobsCheck` reads "Scheduled jobs not enabled on this site (JOBS_ENABLED)" and is **deliberately NOT ok** — on dev it is the truth, and on prod a forgotten flag must never read as healthy, which would be the §0.95 silence again. AdminView and the Connected-apps Slack card pass `{ enabled: d.enabled !== false }` (an older server that does not say is treated as enabled). **The flag is set on the production site alone** (`sales-pipeline-v2`, Site configuration → Environment variables, `JOBS_ENABLED` = `true`, all contexts and scopes) — by Jeff's hand: Claude's write through the Netlify tool was refused by the permission classifier. Dev (`accelerep`) stays unset: dev's jobs stop the moment this lands, and dev's tile reads the failing check by design.

**Test:** `tests/job-heartbeat.test.mjs` +2 — `jobsEnabled` over ten shapes (the host alone enables nothing; unset is OFF), `jobHealth` with `enabled:false` over fresh ok rows is `'disabled'` for all four with the counts riding along, the tile check not ok, the card line, and the option defaulting to enabled; the wrapper scan pins the gate line, its 200 body, and that it sits before the start stamp and before the run; the job-status scan pins `enabled` in the body; the client scans pin both call sites. `tests/integration/job-heartbeat.itest.mjs` +1 against the real test database: a wrapped handler under an unset flag is never called, stamps no row and answers the skipped body; `job-status` on that site says `enabled: false`; the same wrapper with the flag on runs and stamps — and every existing test now runs with the flag on around its call. **Mutants (5):** unset enabling the jobs, the wrapper ignoring the flag, a disabled site reading as a healthy tile, `enabled:false` no longer overriding the rows, job-status dropping `enabled`. **Verified:** five gates, **621/621 unit**, **345/345 mutations, printed green baseline**, **125/125 integration**, build guard OK **`index-CghUEQTb.js`** (a new hash — `jobHealth.js`, AdminView and the Slack card are in the bundle). Guide §18b33 has a new bullet.

**What changes when it lands on dev:** dev's four jobs stop; `site_job_heartbeats` for `accelerep.netlify.app` freezes at its last stamps; Settings → Workspace Health reads "Scheduled jobs not enabled on this site (JOBS_ENABLED)" as a failing check and the Slack card "Alerts job: Not enabled on this site (JOBS_ENABLED)". §0.101's proof moves to PROD after the ship: prod's `salespipelinetracker.com` / `digest` row at 08:00 and 13:00 UTC with `error_count` 0. **The ship waits for the flag:** shipping `master` without `JOBS_ENABLED` on the prod site stops prod's jobs too — no alerts, no digests, no task reminders, no lead scoring — until it is set, and the tile would say so in red.

**Landed on dev (`67d5554`, pushed 17:30:46 UTC 9 Sep) — and PROVEN by the
rows going quiet:** accelerep.netlify.app served `index-CghUEQTb.js` — the
local gate build's hash — at 17:31:33 UTC (47 seconds after the push);
Netlify's record: deploy `6aa197ca…`, commit `67d5554`, branch `dev`,
published 17:31:29 UTC, 76 functions, secret scan clean, the `_heartbeat`,
`job-status`, `digest`, `pipeline-alerts`, `score-leads-batch` and
`task-reminders` digests changed (every scheduled function imports the
wrapper). The served bundle carries "Scheduled jobs not enabled on this site
(JOBS_ENABLED)" and "Not enabled on this site (JOBS_ENABLED)" once each, and
`enabled!==!1` three times; the deployed `job-status` answers 401
unauthenticated. **The proof:** a read-only SELECT of `site_job_heartbeats`
at 17:33:01 UTC — dev's `task-reminders` row last finished 17:31:02.312, the
last minute before the publish, and had not stamped 17:32 or 17:33
(`ok_count` 1,086, frozen); `digest` and `pipeline-alerts` at 17:00:41
...
**PROD SHIPPED — the ELEVENTH ship (Jeff: "ship the manager digest / add the
per site env flag", then "done" — the flag set on `sales-pipeline-v2` by his
hand, after a first attempt on the dev site's page was caught by the URL:
"this site is the production site - my original was dev").** Ancestor check
(tree clean, `dev` == `origin/dev`, `origin/master` an ancestor of `dev`),
then `git push origin dev:master`: `master` `5a306e3` → `a9d2d08` (16
commits — §0.100 through §0.103 and their docs), pushed 18:51:40 UTC 9 Sep.
salespipelinetracker.com served `index-DcLtbkSJ.js` at 18:52:48 UTC (68
seconds), 2,504,807 bytes — dev's byte size — with `pk_live_` inlined and
"Scheduled jobs not enabled on this site (JOBS_ENABLED)" present once (the one
`pk_test_` is Clerk's SDK prefix check, as on every ship); the deployed
`job-status` answers 401 unauthenticated. Netlify's record: deploy
`6aa1aabe…`, commit `a9d2d08`, branch `master`, published 18:52:43 UTC, 76
functions, secret scan clean. **THE PROOF, read at 18:54:17 UTC — a
read-only SELECT of `site_job_heartbeats`:** a NEW row,
`salespipelinetracker.com` / `task-reminders`, started 18:54:03.654, finished
18:54:03.850, `ok`, `{ sent: 0, skipped: 0 }`, `ok_count` 2 — prod's
wrapper had stamped 18:53 and 18:54, so `JOBS_ENABLED` IS on the prod site
and prod alone runs the jobs; dev's four rows unchanged since 17:31 (jobs
off); and the LEGACY `job_heartbeats.task-reminders` row finished
18:52:02.738 (`ok_count` 1,301) and never again — the old code's last
minute. From here the legacy table has no writer on either site. Still to
come on prod: `pipeline-alerts` and `digest` at 19:00 UTC (their first
per-site rows), `score-leads-batch` at 06:00 UTC on the 10th (then prod has
all four rows and Jeff drops the legacy table by hand), the digest at 08:00
and 13:00 UTC on the 10th with `error_count` 0 (§0.101's proof), and Monday
14 Sep 13:00 UTC the first manager digest ever, from prod alone. `master`
== `dev` == `a9d2d08` at the ship.

### 0.104 Owned underneath, named on screen — the display name is stamped with the owner (9 Sep, eleventh session — handoff item 28; Jeff: "Lets do these items")

**What it was:** `stampOwnerId` (`_lib.mjs`) made the caller the owner of anything they created without naming a rep — `ownerId` = their `users.id` — and left the display-name column (`assignedRep` on contacts, `salesRep`, `accountOwner`, `assignedTo`, `author` on the others) exactly as sent: null. Karen's contact from §0.93's step 6 was the observed case: owned by her, "Assigned Rep" blank in the rail, hidden from Jeff's "Mine" scope. Worse than a blank label: a Manager's only scoping is a set of rep NAMES (`isRepVisible` in App.jsx, keyed on `assignedRep`), so a row the server granted a Manager could vanish from their view. Read-only count at the session: four such rows in the app database — two contacts of Karen's, two accounts of Jeff's, all in the dev org, all resolvable.

**Fix (server):** `stampOwnerId` resolves a supplied name as before and KEEPS it; when nothing was named it takes ONE roster row (`resolveCaller`) and stamps both `ownerId` and the display-name column from it — the two cannot disagree at creation. An unresolvable caller stamps null for both, unassigned as before. Every single-create endpoint (contacts, accounts, opportunities, leads, tasks, activities) gets this through the one helper; the bulk import path (`stampOwnerIds`) is unchanged — a blank owner on an imported row stays unassigned by design. **Fix (client):** the contact rail's read-only "Assigned Rep" names the owner of an older row — from the roster when the caller holds it (Admins and Managers), else from the caller's own profile when they are the owner (`myProfile`, now in the app context). **The old rows:** `db/backfill-owner-names.mjs` — dry run by default, prints every row it would change, `--apply` fills a BLANK display name from the owner's roster name joined on (id, org), never overwrites, never touches `ownerId`, never crosses an org; run by Jeff's hand, not run this session. **Test:** `contacts.itest.mjs` +1 — a rep's create without a name has `ownerId` AND `assignedRep` = the rep; a supplied name is kept; an Admin naming another rep stamps that rep's id and name; an unresolvable caller stamps null for both. `ownership-registry.test.mjs` +1 pins the stamp's shape by source scan. **Mutants (1):** the name half dropped. Guide §18b22 has a new part.

### 0.105 A logged email keeps its envelope — From, To, Cc and Message-ID on the row (9 Sep, eleventh session — handoff item 27, option 3 of §0.93)

**What it was:** `email-inbound.mjs` stored an email as `subject` and `notes` ("<subject> — <body>", then the attachment names); who it was from, who it was to and copied to, and its Message-ID were used for matching and for the row's deterministic id and then dropped. The viewer (§0.93) could show only the body.

**Schema (§18c, database first):** four nullable, additive columns on `activities` — `email_from` text, `email_to` jsonb, `email_cc` jsonb, `email_message_id` text — applied to the APP database and the TEST database by `db/apply-email-headers.mjs` (ADD COLUMN IF NOT EXISTS, read back: all four present, nullable) BEFORE any code that names them; `_schema-guard.mjs` names `activities.email_message_id`. Prod's function until the ship never names them — unaffected. `activities.mjs` `sanitize()` does not name them either, so a PUT from the editor cannot blank them (the full-row-builder trap of §18b13 does not reach columns the builder never lists).

**Code:** `envelopeOf({ full, mail })` in `_inboundText.mjs` (pure): `from` as the provider gave it ("Name <addr>" when both are known), `to` and `cc` as lower-cased address lists — the fetched message's HEADER To/Cc when its headers map has them (Resend's structured `to` is the SMTP envelope, our own dropbox), else the structured fields of whichever payload there is — and the Message-ID from the message, the payload, or the headers map; every field present, empty when unknown, bounded (500 chars, 50 addresses). `headerMapOf` and `emailAddressesIn` are the shared halves. `email-inbound.mjs` stores the four beside `ownerId` and `author`. **Viewer:** `emailEnvelopeOf(activity)` in `activityView.js` normalises the row (`any` false for a row logged before §0.105, which renders exactly as before); `ActivityDetailDialog` shows From, To and Cc under the subject and the Message-ID in a monospace footer, only when present. **Test:** `inbound-text.test.mjs` +5 (both header shapes, address extraction, header To winning over the envelope, the flat payload, every field present when unknown; a scan of the function, the schema, the apply script — additive only, four ADD COLUMNs — the guard and `activities.mjs`), `activity-view.test.mjs` +2 (the normaliser; the dialog's lines), `email-inbound.itest.mjs` +1 against the real test database (From, To — the flat payload's dropbox as given — Cc and Message-ID on the row; no Message-ID stores null). **Mutants (5):** Cc dropped, the header To no longer winning, the Message-ID not stored, the footer gone, `any` always true. **Verified (both batches together, one tree):** five gates, **629/629 unit**, **351/351 mutations, printed green baseline**, **127/127 integration**, build guard OK **`index-De0DMnrV.js`**. Not observed: a real email's envelope in the viewer is the next email Jeff sends through prod's dropbox after the ship (prod's function writes every row — §18b31).

**Landed on dev (`dfcdee4`, pushed 19:24:54 UTC 9 Sep — both sections, one
tree):** accelerep.netlify.app served `index-De0DMnrV.js` — the local gate
build's hash — at 19:26:03 UTC (69 seconds after the push); Netlify's record:
deploy `6aa1b288…`, commit `dfcdee4`, branch `dev`, published 19:25:59 UTC,
...
dev if the webhook is flipped for a test. `master` stays at `a9d2d08`.
...
### 0.106 A bulk stage move posts one summary line; three small closes (9 Sep, eleventh session — Jeff: "lets resolve these from your list")

**The bulk move (was: "Jeff's call on a summary line" since §0.99).** The CSV import (`opportunities.mjs`'s array POST) moves deals through `applyStageChanges` and writes one audit row for the batch; since §0.99 a single deal save posts to Slack the moment its stage changes, but the batch posted nothing — one post per deal would flood a channel. Now the batch posts ONE line, under the org's "Deal stage changed" switch like the single save: `bulkStageSummary(rows, priors)` in `src/utils/slackAlerts.js` (pure) counts the rows that moved — the file supplied a stage, the stored stage is known, they differ, the same rule as `resolveStageChange` — the wins among them, and a tally by destination (most first, at most eight); `postBulkStageMove(orgId, { summary, total, mover })` in `send-slack.mjs` posts `slackTemplates.bulkStageMoved` ("➡️ Stages moved in bulk — *3* of 200 imported deals changed stage — *2* → Negotiation · *1* → Closed Won / 🏆 *1* closed won / Imported by *Jeff Russell*", a "View pipeline →" button) through `sendSlackToOrg(…, 'stageChanged')`, and posts nothing when nothing moved; the batch branch calls it inside the same `if (staged.changedCount > 0)` block as its audit, with the caller's roster name. The per-deal event post stays on the single save only. **Test:** `slack-alerts.test.mjs` +2 (the summary over moved, unchanged, unmapped, new, blank rows; the cap; the poster's shape, the template naming the importer, the batch branch's call beside the audit, one `postDealEvents` call in the file). **Mutants (4):** a win not counted, an unchanged row counted, the batch branch not posting, the poster posting on nothing.

**Item 31 — struck.** Jeff agreed: the four manager copies in `pipeline-alerts.mjs` read `manager.profile || {}`, which is where a row's preferences live (§0.101); not a bug.
...
**Landed on dev (`50a6cf7`, pushed 20:26:16 UTC 9 Sep):** accelerep.netlify.app
served `index--5aHcCr_.js` — the local gate build's hash — at 20:26:54 UTC (38
seconds after the push); Netlify's record: deploy `6aa1c0ea…`, commit
`50a6cf7`, branch `dev`, published 20:26:51 UTC, 76 functions, secret scan
...
sites read the shared database — so the post lands in the same channel). `INTEGRATION_REQUESTS_TO`: set on prod by Jeff after the landing. **OBSERVED end to end on prod (Jeff's screenshots, 21:14 UTC 9 Sep, after the thirteenth ship): Sync from Clerk on the Users page ("Synced: 1 added" — his roster row, `usr_68dd…`, Admin, 21:14:26), then Request on HubSpot → "Requested · 9/9/2026" (the record's `byName` "Jeff Russell", 21:14:53), then the mail in accelerep@outlook.com at 9:14 PM: "Integration request: HubSpot — org_3Dwny…", "Requested by: Jeff Russell <jeffrussell1@yahoo.com>". The workspace line reads "(no company name)" because prod's settings row has none. And the prod Slack card: Live, "Connected · #sales-alerts · 7 of 7 alerts".**

### 0.107 `check:fnscope` — every name a function file reads is bound (9 Sep, eleventh session — the guard §0.95 and §0.101 asked for)

**What it was:** `bf4a3c5` renamed two helpers' parameter and left their bodies reading the old name, which still existed in the file inside another function's loop. It parsed, bundled and imported, and threw on the first call — a 500 every hour for five months in `pipeline-alerts.mjs` (§0.95), and for a day longer in `digest.mjs` (§0.101). `check-tdz`'s undefined pass inspects Capitalised components under `src/` and JSX; a lowercase helper in a Netlify function had no scanner, and guide §18b33 said so.

**The scanner:** `scripts/fnscope.mjs` (pure — `undeclaredIn(source)` → `[{ name, line, column, enclosing }]`) walks the AST with an explicit scope stack: module scope (every import's local name, every top-level declaration), function scope (params and their destructuring, the function's own name, `arguments`, `var` and function declarations hoisted from anywhere in the body but not from nested functions), block scope (`let`/`const`/`class` of that block, a catch parameter, a for-loop's own declarations, a switch's cases), class scope (the class name). An `Identifier` that is a READ — not a member property, not a property or method key, not a label, not an import's remote name, not `import.meta`, not a binding position — with no binding in any enclosing scope and no standard Node/ES global of that name is reported. A shorthand property, a computed key, a default value, a spread and a template expression are reads; a re-export with a source (`export { a } from './m'`) binds nothing and reads nothing — the one false-positive class the live tree showed (`_stage.mjs`'s re-export of `MAX_DAYS_IN_STAGE`), handled and pinned. `scripts/check-fnscope.mjs` runs it over every `.mjs` under `netlify/functions/` and `db/` (76 + the apply scripts), prints `UNBOUND file:line:col "name" read in <function>`, exits 1 on a finding; `npm run check:fnscope` is in the verification chain (CLAUDE.md, guide §18b33's bullet rewritten). **Run against `bf4a3c5`:** `profile@43 in wantsAlert, profile@61 in wantsSms` in `pipeline-alerts.mjs`; `profile@38 in wantsDigest, profile@43 in wantsSms, resolvedProfile@292 in handler` in `digest.mjs` — every line of §0.95 and §0.101, on the day. On the current tree: no unbound reads in 83 files. **Test:** `tests/check-fnscope.test.mjs` (6): the §0.95 shape; the April commit's two files flagged (read from git) and today's clean; every way a name is bound, in one fixture, with nothing flagged; every kind of read flagged when unbound and keys/members/labels/`import.meta` not; the globals (a browser-only `window` is flagged — a function file that reads it dies on Netlify); the CLI wired into package.json and CLAUDE.md. **Mutants (3):** the scanner stops reporting, every name counts as a global, a shorthand property stops being a read. **Verified:** five gates plus the sixth, **640/640 unit**, **358/358 mutations, printed green baseline**, **127/127 integration**, build guard OK `index--5aHcCr_.js` (unchanged — tooling only).
...
### 0.108 A first sign-in provisions the roster row — the prod workspace had none (9 Sep, eleventh session — Jeff: "Please fix the clerk name issue for integration requests")

**What it was:** the DocuSign request Jeff clicked on prod recorded `byName: null`. Read against the database: the production workspace (`org_3Dwny…`) had NO rows in `users` at all. The roster is populated only by an Admin pressing Sync in Settings (`users-sync.mjs`), by an invite, or by an Admin creating a user; the self-profile GET (`users?me=true`) looked a caller up by Clerk id, then email, then display name, LINKED a pending row when it found one, and answered `null` when it found nothing. So in a fresh workspace nothing ever created the first row: the Admin's own requests carried no name, `resolveCaller` returned null for them — an unresolvable caller stamps null (§18b20) — and everything they created was UNASSIGNED with a blank display name; "Mine" was empty. Not a bug in the request function; a missing step in onboarding, visible for the first time because prod's org is the first one nobody synced by hand.

**Fix:** `ensureRosterRow({ clerkUserId, orgId, userRole, clerkUser })` in `_lib.mjs` — a signed-in member of an org with no roster row gets one from Clerk's own record, in exactly the shape `users-sync.mjs` creates: our `usr_` id, the Clerk id in its own column, the role VALIDATED (Clerk's `publicMetadata.role` when it is one of ours, else the verified role, else `User` — never copied raw), `profile { status: 'Active', userType }`, `active`. Idempotent: an existing row by Clerk id, else by email in this org, is returned untouched — linking an invited row to its Clerk identity stays the self-profile GET's job; `onConflictDoNothing` on the per-org unique index makes a race harmless; the caller cache is invalidated so the new row is seen at once; never throws. `isAppRole` is imported lazily, so a test that mocks `auth.mjs` without it still loads `_lib`. **Two callers:** the self-profile GET, when its three lookups matched nothing, hands over the Clerk user it already fetched — the app's first request after sign-in, so a fresh workspace's first member has a row before their first click; and `integration-requests.mjs`'s `requesterOf`, when the requester has no row, provisions it so the record and the mail carry a name. **Test:** `tests/roster-provision.test.mjs` (2 scans: the helper's shape and the validated role; both call sites, the self-profile one after the link attempt and before the answer), `integration-requests.itest.mjs` +1 against the real test database (a member provisioned from a Clerk record with role Admin; idempotent by Clerk id; a role Clerk holds that is not ours falls to the verified role; no name in Clerk makes the email the name; an invited row by email is returned, not duplicated, not linked; org A untouched; a request from the provisioned member records `byName`). **Mutants (3):** the self-profile GET no longer provisions, the request function no longer provisions, the role copied unvalidated. **Verified:** six gates, **642/642 unit**, **361/361 mutations, printed green baseline**, **128/128 integration**, build guard OK `index--5aHcCr_.js` (unchanged — functions only). **Landed on dev (`8134313`, pushed 21:07:21 UTC 9 Sep) — read from Netlify's record, since nothing the client bundles changed:** deploy `6aa1ca8b…`, commit `8134313`, branch `dev`, published 21:08:04 UTC (43 seconds after the push), 76 functions, secret scan clean, the `_lib`, `users` and `integration-requests` digests changed along with every function that imports `_lib`. **On prod, Jeff pressed Sync from Clerk at 21:14 UTC ("1 added") before this ships, so his row exists by the Admin path; the provisioning itself will show on prod only for a NEW member or a fresh workspace — the integration suite is its proof.** Was to observe: on prod, after the ship, Jeff's next page load provisions his row (a read-only SELECT of `users` for `org_3Dwny…` shows it), then a Request on a not-yet-requested row records his name and the mail reads it.

**Observed meanwhile (Jeff, 9 Sep): "production is connected to slack"** — the prod workspace's Slack card configured; the bulk-import post (§0.106) is now observable on prod as well as dev.
...
### 0.109 The self-profile save takes an allowlist; the last three console-only settings catches; two inert dispatch buttons; the backlog read against the code (9 Sep, twelfth session — Jeff: "Let's get these all resolved")

**What it was:** four backlog phrases carried at the bottom of this doc — "dispatch gaps, settings panels that swallow save errors, seed-data cleanup, and a security follow-up". Read against the tree before any code, most of it had been done by earlier batches and never struck; what was still live is below, and every entry verified by reading is struck in place in §9 with its reason.

**Security — `PUT /users?me=true` (guide §18b34).** The named defect (a self-supplied `role`) was already closed: the column is pinned to the stored value by `withRole(…, roleOf(id))`, and the client's gates read Clerk `publicMetadata.role` in App.jsx, not the roster. But it was closed by a denylist of one. The self-save merged the WHOLE body, and the profile panel sends the whole roster row it holds (`{ ...myDbUser, ...updates }` in AppHeader's `saveProfile`), so every preference toggle rewrote quota, team, territory, `active`, the quarterly quotas and the forecast calls from the caller's own copy — and a body written by hand could set any of them; `profile.userType` was taken from the body too (harmless to reads, since `flatten()` overrides it from the column, but two answers to one question in the row again). Now `netlify/functions/_selfProfile.mjs` (pure, no imports) holds `SELF_EDITABLE_KEYS` — the personal fields, contact details, `email`, `emailSignature` and the four preference keys — and `pickSelfEditable(data)` keeps only those that are PRESENT (field-present semantics, an explicit '' or null included; a dropped key keeps its stored value through `mergeForUpdate`; keys are dropped, not 400'd, because the panel has always sent them). The me branch merges `{ id, ...pickSelfEditable(data) }` and pins `userType` to the stored role; the Admin PUT pins the blob copy as well. `email` stays self-editable by design: the panel offers it, the per-org unique index refuses a collision, and Clerk is the identity. **Test:** `tests/self-profile.test.mjs` (4 — the frozen list carries the personal keys and none of 22 administrative ones; pickSelfEditable's field-present contract; the two users.mjs call sites by scan; and every top-level key the profile panel passes to `saveProfile`, found by brace-depth parsing of AppHeader, is on the list — a panel save can never be dropped silently), `tests/integration/users-self.itest.mjs` (5, the real test database — one body carrying role, userType, quota, annualQuota, q1Quota, quotaType, team, teamId, territory, vertical, manager, active, status, a foreign clerkUserId and forecastCalls changes NONE of them and does change firstName and title; preferences round-trip and an omitted key keeps its value; another member's id is 403 with their row unchanged; a Manager saving `role: 'User'` stays Manager; the same Clerk identity's row in org B untouched). **Mutants (3):** the allowlist drops and the whole body reaches the merge; the blob copy comes from the body again; `quota` added to the list.

**Settings — the last three console-only catches.** The backlog item described a class already closed for nearly every panel (`putSettings`, guide §18b1); an audit of every `console.error` under `src/Tabs/settings/` found three live WRITES still reporting to the console alone: `TeamsDetail.handleDelete` and `TerritoriesDetail.handleDelete` (`if (res.ok)` did nothing on failure, the member/rep cascade's failures were logged, and the catch swallowed — a refused settings PUT looked like a delete that simply had not happened, with nothing on screen), and `LeadConversionDetail.handleSave` (it had `saveError` state and rendered the banner, but its catch never set it). Both deletes now go through `putSettings` (throws on non-2xx), report "Team/Territory not deleted — <why>" in a banner above the table, then cascade and report "deleted, but N member record(s) did not update: <names>", updating local state only for the rows that did; the benchmark save sets its banner. Every other `console.error` under settings is a read (calendars, approval stats) or is paired with an alert (CSV parse). **Test:** `tests/settings-cascade-errors.test.mjs` (4). **Mutants (3):** each catch back to console-only.

**Dispatch — read, not built.** Of the "smaller gaps": `jobNumber` IS issued server-side (`nextJobNumber` inside `withNumberRetry`, immutable on PUT); `preferredTechId` IS settable (the customer form's select); `dispatch_equipment` HAS a UI (Vehicles & equipment) and availability checks in the planner; mass-schedule honours `crewSize` (partial crews are skipped with a reason) — all four struck. The `name`/`ctype` template bug is fixed and templates are selectable in New Job (`templateId`). The Technician role exists with a per-field write whitelist (`TECH_WRITABLE`, `TECH_STATUS_ALLOWED` in dispatch-jobs.mjs), and the legacy `dispatchTechProfiles` key, `DispatchTechDetail` and `u.dispatchEnabled` are gone. "Save draft" no longer exists; SMS notify is disabled on purpose pending Twilio. Built here: the two inert buttons removed — "Manual pick" (crew builder; the copy that said "Override by clicking Manual pick" now ends at "customer preference.") and "Test auto-create" (job templates). Still open, restated in §9: drag-to-reschedule on the week grid; the New Job flow's three client-side writes (customer, location, job) are not one transaction. **Test:** `tests/dispatch-stubs.test.mjs` (2). **Mutant (1):** the Manual-pick copy returns.

**Seed data — struck by a read-only query.** The "ten `@test.com` users" are ONE: `smiller@test.com` (Savannah Miller, role Technician, the dev org `org_3BDQ…`), and that row is LINKED to a Clerk identity — a real test account, not orphaned seed. No other org has one (four orgs read: 4, 4, 6, 1 rows). Nothing to delete.

**Verified:** six gates (84 function files now — `_selfProfile.mjs` is scanned), **652/652 unit** (10 new), **368/368 mutations, printed green baseline** (7 new), **133/133 integration** (5 new), build guard OK `index-CRtPA764.js` (2,447 kB; a NEW hash — two buttons gone, two banners added), `dist/` cleared. **Then, as its own commit (the §9 entry asked for that):** `prioColor2` collapsed into `prioColor` in DispatchTab (three call sites), one assertion added to `tests/dispatch-stubs.test.mjs`, one mutant (the alias returns); gates, **652/652 unit**, **369/369 mutations**, build guard OK `index-BlthEjPa.js` (a NEW hash), `dist/` cleared. NOT observed in a browser: the pane holds no session, and the new banners need a REFUSED delete (a rep cannot reach Settings; an Admin's delete succeeds) — the shape is the banner LeadConversionDetail already renders. **Landed on dev (`8a02cba`, the handoff commit after `29a6e7e` and `9996c1c`; pushed 21:51 UTC 9 Sep) — read from Netlify's record:** deploy `6aa1d50a…`, commit `8a02cba`, branch `dev`, published 21:52:43 UTC, **77 functions** (`_selfProfile` new, created 21:52:42; the `users` digest changed; every other function's digest unchanged), secret scan clean over 392 files, "2 new files uploaded — index.html and 1 asset"; accelerep.netlify.app serving `index-BlthEjPa.js` at 21:53:01 UTC (curl). NOT shipped. **OBSERVED by Jeff on dev as Karen (role User), 9 Sep ~22:00 UTC — "all test passed": the profile save and a notification toggle persist across a hard refresh. Read-only, 22:02 UTC: her roster row `usr_e7e09733…` (the dev org) has `updated_at` 22:00:19 UTC, title "Account Executive", `notificationPrefs` populated, and role User, team Enterprise West, territory West, `annualQuota` 1000000, `active` true, the blob `userType` User — the administrative fields intact after a save whose body carried the whole row. The Admin team-delete check is Jeff's report too.** **Then the pane pass, Jeff signed in as Karen (22:03–22:07 UTC): the nav had no Settings tab (role User); Profile title "Account Executive" → "Senior Account Executive", Save → `PUT users?me=true` 200 whose response read role User, team Enterprise West, territory West, the four quarterly quotas and `annualQuota` 1000000, `clerkUserId` pinned; Notifications → Enable on "Opportunity updated" → a second PUT 200, `opportunityUpdated.enabled` true and nothing else changed; hard refresh → the title textbox read the new value and the toggle read Disable; the row read back read-only at 22:05:17 UTC with every administrative column intact; then both restored to what Jeff left (title "Account Executive", the toggle off — 22:06:52 UTC). One pre-existing thing seen on every load, not this batch's: `coaching-notes`, `calendar-events` and `spiff-claims` each GET once BEFORE the Clerk token is attached (401, "DB error 401" in the console) and once after (200) — a load-order race, three console errors per page load, recorded in §9.**

### 0.110 Maintenance agreements — skip or defer a plan occurrence, and agreement renewals surfaced and alerted (9 Sep, twelfth session — Jeff: "I would like to do these", maintenance agreements first)

**What it was:** Jeff brought four strategic dispatch items from another conversation (tech mobile — deferred to a single mobile pass by his call; quote → job → invoice → QuickBooks; recurring jobs / maintenance agreements; customer-facing notifications). Read against the tree first: service plans were already a first-class table with cadence, interval, lead days and fixed-vs-rolling anchors, visits were computed (never generated) into a Service Due queue with mass-scheduling, and customers carried a plan, a start date and an `agreementExpiry`. What was missing was narrow: nothing read `agreementExpiry` except the coverage-end check (no renewal reminder anywhere), a single occurrence could not be skipped or deferred (§9 had carried "needs a `dispatch_plan_visits` table"), and the recurrence arithmetic lived inside DispatchTab where nothing else could run it. Jeff chose this first, renewals told to the dispatcher in the queue AND by email/Slack.

**The arithmetic moves out.** `src/utils/planVisits.js` (pure, no imports) now holds `planVisitState`, `buildVisitQueue`, the date helpers, and the new `renewalState` / `buildRenewalQueue` / `renewedExpiry` — imported by DispatchTab and by the hourly job the way `forecastCall.js` is by users.mjs, so the queue, the customer list's overdue counts and the alert can never disagree. Two recorded exceptions per occurrence, honoured everywhere at once: **skipped** retires the occurrence without a job (fixed grid: it joins the completed set; rolling: it counts as serviced on its date), and **deferred** keeps the occurrence's grid date — a job created for it still stamps the ORIGINAL date as `planDueDate`, so completing it retires the right occurrence — while it falls due on `deferredTo`; on the fixed grid the outstanding occurrences are led by their EFFECTIVE date, so a deferral can drop behind a later visit. `renewalState` reads the plan's new `renewalLeadDays` (null → 60) against `agreementExpiry`: expiring, expired, upcoming, or none (no end date, not covered, a malformed date, do-not-service); a legacy free-text agreement with no plan row still renews on the default. `renewedExpiry` keeps the anniversary — twelve months from the CURRENT end date, from today only when already expired.

**Schema (§18c, database first — both databases before any code read them, read back and verified):** `dispatch_service_plans.renewal_lead_days integer NULL`, and the new `dispatch_plan_visits` (id `pv_…`, org_id, customer_id, plan_id, due_date, action 'skipped'|'deferred', deferred_to, reason, by_user_id, by_name, created_at; UNIQUE (org_id, customer_id, plan_id, due_date); org_id index) — `db/apply-plan-visits.mjs`, additive and idempotent. The schema guard names both.

**Endpoint:** `dispatch-plan-visits.mjs` — GET the org's rows (optional `?customerId=`); POST `{ customerId, planId, dueDate, action, deferredTo?, reason? }` validates the action and both dates (`isYmd`; a deferral needs a different valid date), looks the customer up BY ORG (another org's id is a 404 and writes nothing), stamps the caller's roster id and name, and upserts on the occurrence key — a second decision about the same date replaces the first; DELETE `?id=` is org-scoped and 404s on a repeat. `requireWrite`: Technicians and ReadOnly read, dispatchers write.

**The queue.** Service Due rows carry Skip and Defer beside Create visit — an inline editor (a date for the deferral, an optional reason, Confirm), each writing first and adopting the server's row; a refusal is a banner above the queue (§18b32), never a console line. A deferred row wears "deferred from <date>" with an undo; skipped occurrences are listed under the queue ("Show N skipped occurrences", who, why, Undo skip). Below the visits, **Agreement renewals**: every customer inside the plan's renewal window or past it, expiring or expired, earliest end date first, with "Open customer" and **"Renew 12 months"** — a confirm naming the new end date, then the customer PUT the edit form already uses (`agreementExpiry` only). Settings → Dispatch → Service plans gains a **Renewal reminder** field (days before the agreement ends; default 60) beside the lead-days field, and the list shows it. The eighth dispatch fetch loads the exceptions with the rest and a failed load is reported like the others.

**The alert.** `pipeline-alerts.mjs` gains Signal 6, after the deal loop and inside its own try so it can never stop the deal signals: per org, `buildRenewalQueue` over that org's customers and plans; recipients are THAT org's active Admins and Managers, each at their own alert hour, emailed (`emailTemplates.agreementRenewal`) behind a new `agreementRenewal` preference (default on; the toggle shows only to Admins and Managers in avatar → Notifications), and posted once to the org's Slack (`slackTemplates.agreementRenewal`) under a new company switch of the same name in `SLACK_ALERT_TYPES` (eight types now). Deduped through the existing ledger keyed on the customer — one email per recipient per customer per seven days, one post per customer per seven days — so an unrenewed agreement nudges weekly, not hourly. Because those ledger rows are not recommendations and never resolve, `recommendation-log.mjs`'s GET excludes `actionType LIKE 'renewal%'` so the Reports page never lists them. Runs where `JOBS_ENABLED` is set — prod only (§0.103).

**Tests:** `tests/plan-visits.test.mjs` (13, the arithmetic run: fixed grid, skip, defer, a job found by the grid date and retiring the occurrence, exceptions scoped to one customer and plan, rolling anchor with skip and defer, the queue, the renewal window and its default, none-states, the renewal queue's order, the anniversary), `tests/agreement-renewals.test.mjs` (8 scans: the renewal pass's recipients, preference, dedupe and Slack type; the switch; the templates; the report's exclusion; the toggle; the tab's wiring and the grid-date stamp; the plans endpoint and panel; the exceptions endpoint's org check, upsert key, validation and gate), `tests/integration/dispatch-plan-visits.itest.mjs` (6, the real test database — a skip recorded with the caller's name; the replace-on-same-occurrence; five refusals; another org's customer 404 and org B sees nothing; a Technician reads and cannot write; DELETE org-scoped and idempotent). Two pinned counts updated (`slack-alerts`: eight types, six hourly; `slack-webhook` and `pipeline-alerts`: six `sendSlackToOrg`/`wantsAlert` sites). **Mutants (10):** a skip still counts as due; a deferral ignored; the renewal window never opens; renewing from today; the email stops; the Slack post bypasses the switch; the report lists the ledger rows; another org's customer accepted; a deferred visit stamps the deferred date; the queue ignores the exceptions. The org-scoping scan caught the endpoint's first upsert without `setWhere` — fixed before the commit.

**Verified:** six gates (155 files; 86 function files), **672/672 unit** (+20), **379/379 mutations, printed green baseline** (+10), **139/139 integration** (+6), build guard OK `index-5Gkq-AHb.js` (2,458 kB; a NEW hash), `dist/` cleared. Both databases carry the column and the table (read back by the apply script). **Landed on dev (`a3fa3e2`, pushed 22:46 UTC 9 Sep) — read from Netlify's record:** deploy `6aa1e1d5…`, commit `a3fa3e2`, branch `dev`, published 22:47:36 UTC, **78 functions** (`dispatch-plan-visits` new; every function rebuilt, since `db/schema.ts` changed), secret scan clean over 398 files; accelerep.netlify.app serving `index-5Gkq-AHb.js` at 22:47:38 UTC (curl). **OBSERVED in the pane, Jeff signed in as Karen (role User), 22:49–22:53 UTC:** Dispatch → Service Due showed the one overdue visit (Dispatch Customer Test, Air Filter Quarterly Change, due 2026-08-01, 39 days overdue) with Skip, Defer and Create visit; Defer to 2026-09-20 with a reason → POST 201 → the row read "Due · deferred from 2026-08-01 · due 2026-09-20 · in 11 days"; undo → DELETE 200 → the overdue row returned; Skip with "Pane test: unit replaced" → POST 201 → "0 to book … Nothing due" and "Show 1 skipped occurrence" listing "2026-08-01 · Pane test: unit replaced · by Karen Russell"; Undo skip → DELETE 200 → the row returned; Customers → Edit details → Coverage ends 2027-07-31 → 2026-10-15 → Save → PUT 200 → Service Due's **Agreement renewals** read "0 expired · 1 expiring" with the customer "ends 2026-10-15 · expires in 36 days" (the plan's `renewal_lead_days` is null — the default 60 applied); Renew 12 months → the confirm read "Renew the maintenance agreement for Dispatch Customer Test through 2027-10-15?" → Confirm → PUT 200 → "none due"; the end date restored to 2027-07-31 (PUT 200) and read back read-only at 22:52:47 UTC with zero `dispatch_plan_visits` rows in any org and zero renewal ledger rows. One pane note: a coordinate click on the narrow pane missed the Defer button while a DOM click landed — the pane's scaling, not the button (the button's rect was where the tree said). NOT observed: the plan panel's Renewal reminder field (Admin-only). NOT shipped.

**Jeff's finding, fixed the same hour (`1866158`): "well i deferred the test and it is now unviewable."** He deferred the test visit to 2026-09-30 — 21 days out, past the plan's 14-day lead window — so the occurrence became 'upcoming', which the queue excludes BY DESIGN (that is what leadDays is for), and the only undo lived on the queue row that had just disappeared. The recorded exception was correct and unreachable. Now the list under the queue holds every exception the queue does not itself show — every skip, and any deferral whose occurrence is not a queue row — "Show N skipped or deferred occurrences not in the queue", each naming the customer, plan, grid date, "deferred to <date> — back in the queue N days before" (the plan's lead), the reason and who, with "Undo deferral" or "Undo skip". A deferral still inside the window stays on its row with its own undo and is not listed twice. `tests/agreement-renewals.test.mjs` pins the filter and both undo labels; one mutant (the list drops the deferrals). **Verified:** six gates, 672/672 unit, 380/380 mutations, build guard OK `index-CxVhrH5t.js`, `dist/` cleared. Jeff's own deferral (2026-08-01 → 2026-09-30, "Customer pon vacation", by Karen Russell) is the row the fix is read against on dev. **OBSERVED by Jeff on dev ("tested and passed", 9 Sep ~23:50 UTC) after Claude read the same list in the pane at 23:46 UTC: his deferral listed under the queue with Undo deferral.**

**Not built, by Jeff's choices for the next sessions:** customer-facing notifications email-first with SMS behind the same switch (Twilio A2P still pending); quote → job → invoice, then QuickBooks export once Jeff creates the Intuit developer app; the tech mobile experience in one dedicated mobile pass.

### 0.111 Customer-facing notifications — appointment confirmation, "technician on the way", and a private job-status link (9 Sep, twelfth session — Jeff's second pick; email now, SMS behind the same switch)

**What it was:** nothing in the app had ever spoken to a service customer. Customers carried a contact email and phone; `send-email.mjs` (Resend) and `send-sms.mjs` (Twilio, credentials not yet on either site and the A2P 10DLC campaign not yet approved — Twilio's console showed Error 30034, "message from an unregistered number", against a brand approved 6 Apr) existed for the org's own users; and there was no unauthenticated read of anything anywhere. Jeff's answers: email first, SMS wired behind the SAME switch for when A2P clears, and a public status link.

**The company decides, per org.** `settings.extra.customerNotifications` — `{ enabled, confirmationEmail, confirmationSms, onTheWayEmail, onTheWaySms, statusLink }`, normalised by `src/utils/customerNotifications.js` `cleanCustomerNotifications` in BOTH halves of settings.mjs (the §18b rule) — **off by default: a new workspace never emails a customer until an Admin turns it on** in the new Settings → Dispatch → **Customer notifications** panel (`DispatchCustomerNotificationsDetail`, `putSettings`, catalogue `dsp-customer-notify`, routed in AdminView). The same pure module decides what a change MEANS: `notificationPlan({ before, after, cfg })` → `confirmation` when a job becomes scheduled with a date or its date/time changes while scheduled (a no-op save, an unrelated edit, or "scheduled" without a date says nothing), `on_the_way` when status becomes `en_route` (arriving says nothing); `whenText` renders the appointment ("Tuesday, September 15, 2026 between 8:00 AM and 10:00 AM" / "… in the morning" / "Date to be confirmed"); `smsText` puts the company name first and the link last; `CUSTOMER_STATUS_LABELS` keeps internal vocabulary internal ("Your technician is on the way", never `en_route`).

**Schema (§18c, both databases first, read back):** `dispatch_jobs.public_token text NULL` (unique index) and `dispatch_jobs.customer_notifications jsonb NULL` — `db/apply-customer-notifications.mjs`. The schema guard names the token column.

**The notifier — `_customerNotify.mjs`,** called by dispatch-jobs.mjs AFTER the row is written on all three paths (create, the technician's field update, the dispatcher's update — the create now selects the whole prior row, since an upsert may be a re-schedule), from the stored before/after rows, never changing the dispatcher's response and never throwing. It reads the org's switches, the customer, the technician and the settings row BY THE JOB'S ORG; issues the status token ONCE (24 random bytes, base64url — never an id) when the link switch is on; emails through `emailTemplates.customerAppointmentConfirmed` / `customerTechOnTheWay` (customer and company strings ESCAPED — the internal templates never were, and never had to be); texts through `sendSms` only when the site has all three Twilio variables, otherwise records "SMS not configured on this site"; and appends every attempt — sent or not, and why not — to the job's trail `[{ type, channel, to, at, ok, error?, by? }]`. The job's response carries `publicToken` and `customerNotifications`.

**The public page — `dispatch-status.mjs` (guide §18b35), the only unauthenticated read of dispatch data in the app.** `/status/<token>` (a netlify.toml rewrite placed ABOVE the SPA catch-all, or the sign-in wall swallows it) → `?t=`; the token is the only input, checked against `/^[A-Za-z0-9_-]{24,64}$/`, and the job is found by `public_token` alone — no org id, job id or customer id is accepted, rendered or distinguishable (a malformed and an unknown token return the same 404 body). Everything rendered is escaped; `Cache-Control: no-store`, `X-Robots-Tag: noindex`, no referrer, framing denied; the page shows the company, the job title, the customer-facing status, when, the technician's FIRST name, the service address and the job number — no contact details, nothing linking into the app.

**The dispatcher sees it.** The job detail pane gains **Customer notifications**: the trail (type, channel, recipient, sent or "not sent — why", when) and **Copy status link**. `handleSchedule` hands the server's row up so the trail and token show the moment the crew is scheduled, without a reload (`CustomerNotifyTrail` is module-scope, per the inline-component rule). The dispatch job mapping carries both fields.

**Tests:** `tests/customer-notifications.test.mjs` (13 — the defaults and the normaliser; the plan under every switch state, on create, on a date or time change, on an unrelated edit, on a no-op, on en_route and on arrival; the channels; `whenText`, `smsText`, `esc`, the path, the labels; and scans of settings.mjs both halves, the three dispatch-jobs call sites and the whole-row prior select, the notifier's random token / not-configured branch / org-scoped reads / no throw, the status page's token-only read, headers, escaping and org-scoped secondary reads, the toml order, the panel + catalogue + routing + the tab's trail and adoption), `tests/integration/customer-notify.itest.mjs` (6, the real test database with both senders mocked: scheduling confirms by email AND text with the link in both, issues a 32-char token and records two trail entries; a save that changes nothing sends nothing and keeps the token; without Twilio variables the email goes and the text is recorded "not configured"; en_route announces "Sam" once and arriving says nothing; an org with the switch off gets no message, no token, an empty trail; the public page renders "Acme &lt;HVAC&gt; &amp; Sons" and "Furnace &lt;tune-up&gt;" escaped, the customer-facing status, the technician's first name only, the address, and NO id or contact detail, while a short token, an unknown token and a job id are the same 404 and POST is 405). **Mutants (10):** the master switch ignored; an unchanged appointment re-confirmed; en_route re-announced; SMS attempted without credentials; the link carries the job id; the page found by id; the company unescaped; the settings key dropped from the PUT; the dispatcher path stops notifying; the toml rule renamed.

**Verified:** six gates (157 files; 89 function files), **685/685 unit** (+13), **390/390 mutations, printed green baseline** (+10), **145/145 integration** (+6), build guard OK `index-D8PIckNZ.js` (2,465 kB; a NEW hash), `dist/` cleared. **Landed on dev (`118ed31`, the docs commit carrying `7e6eed3`; pushed 00:08 UTC 10 Sep) — read from Netlify's record:** deploy `6aa1f500…`, branch `dev`, published 00:09:23 UTC, **80 functions** (`_customerNotify` and `dispatch-status` new; every function rebuilt, since `db/schema.ts` changed), **2 redirect rules processed**, secret scan clean over 405 files; accelerep.netlify.app serving `index-D8PIckNZ.js` at 00:09:26 UTC (curl). **The public route PROBED on dev:** `GET /status/<32 bogus chars>` → HTTP 404 from the FUNCTION (its own "We could not find that visit" page, `Cache-Control: no-store`, `X-Robots-Tag: noindex, nofollow`), and `/status/short` → 404 — the rewrite sits above the SPA catch-all. Not yet observed with a real token: the switch is Admin-only and the pane holds Karen. NOT shipped.

**Jeff's finding, and the first real token (10 Sep, `89e85b6`): "I assigned Jax Miller as tech — did override and schedule and it is not showing her scheduled in the job board."** Read against the row FIRST: JOB-2026-0005 "Filter Change" had been created at 14:56:09 UTC with a date of 2026-09-11 and was still `unscheduled`, no technician, and no status-history row for a schedule — the schedule write had never reached the server; Jeff had turned the org switch ON (read back: every switch true) and put his own email on the test customer. The server path was then proven from the pane with the exact request the crew builder sends (as Karen, the session's token): **200** — status scheduled, Jax assigned, 9:00–11:00 on the 11th, `public_token` issued (`oSP8_…`), and the trail `confirmation · email to jeffrussell1@yahoo.com · sent` (Resend, 15:10:28 UTC) plus `confirmation · sms · not sent — no customer phone on file`; the status-history row `unscheduled → scheduled` written. So the crew builder had refused BEFORE sending: its pre-flight gates (a date, a START TIME, the shift and time-off clashes, equipment) return with a red 11.5px line under the buttons — "Set a start time before scheduling." is the likeliest, since the job's card carried a date and no time. The board was showing the truth. **Then the public page 404'd on the real token** while the function called directly (`?t=`) answered 200: Netlify substitutes a redirect placeholder into a PATH but not into a query string, so `/status/<token>` had reached the function as the literal `:token` — the bogus-token probe could not tell. Fixed: the rewrite is `/.netlify/functions/dispatch-status/:token` and the function reads the last path segment (`?t=` still honoured); the scan pins the new rule and the function's path read; the integration suite adds the path-segment call and the unsubstituted-placeholder 404 (7 assertions more). **Verified:** six gates, 685/685 unit, 390/390 mutations, 145/145 integration, build guard OK `index-D8PIckNZ.js` (unchanged — functions and toml only), `dist/` cleared. Also SEEN in this read: every jsonb column written through `JSON.stringify` (the codebase's convention — `co_tech_ids`, `need_skills`, now `customer_notifications`) is stored double-encoded (a JSON string holding JSON) and comes back parsed through drizzle; raw SQL sees the string. Consistent, not a bug, worth knowing before any raw read.

**And a second probe (`2e6a9c4`):** with the path-segment rewrite live, the function called directly at `/.netlify/functions/dispatch-status/<token>` answered 200 while `/status/<token>` STILL 404'd — a 200 rewrite hands the function the BROWSER'S path (`event.path` is `/status/<token>`), not the rewritten target, so a match on `/dispatch-status/` alone missed it. The function now reads the last segment of EITHER path; the scan pins the two-form regex and the integration suite calls the handler with `/status/<token>` as well. READ on dev at 15:28:36 UTC 10 Sep, after the deploy of `2e6a9c4`: `GET /status/oSP8_…` → **200** — "Your service provider" (the dev org has no company name or display name set — Settings → Company profile fills that line), "Filter Change", status **Scheduled**, "Friday, September 11, 2026 between 9:00 AM and 11:00 AM", technician "Jax", "400 Westheimer, Houston, TX", reference JOB-2026-0005; no id and no contact detail on the page. **OBSERVED by Jeff's screenshot (10 Sep, ~15:35 UTC): the Job Board shows Jax Miller 2/50 hrs with "Dispatch Customer Test · 9a · 2h" on Fri 11 — the schedule the pane sent, rendered where he had looked for it.** Then "it is still showing it as needing a crew above the Jax Miller schedule" — read against the rows: a SECOND "Filter Change", JOB-2026-0006, created 17:05:19 UTC during his attempts, unscheduled on the same date; the board was right to show both. Cancelling it is Jeff's, not a script's. **And, Jeff: "please add that banner. Great idea" — `55e219b`:** the crew builder's refusal (no start time, a shift or time-off clash, equipment, a won-opportunity placeholder) is now a `role="alert"` banner above the action bar reading "Not scheduled — <why>", not an 11.5px line among the controls (§18b32); pinned by a scan in `tests/dispatch-stubs.test.mjs`, one mutant (the line comes back). **Verified:** six gates, 686/686 unit, 391/391 mutations, build guard OK `index-DgjASrGN.js` (a NEW hash), `dist/` cleared. **And Jeff (a screenshot with the queue's cards and the job header circled): "can we also make the circled areas click through to the job being listed? These are static right now" — `ddba08e`:** the queue's cards still SELECT a job for the crew builder (that is what a click did before), and each now carries an "Open →" link; the header's customer name and a new "Open job record →" button open the same. One handler in the parent remembers the request (`{ id, at }`, keyed on the object so the same job opens twice) and switches to the Jobs view, whose editor selects that job and widens its status filter so a hidden one still shows; a won-opportunity placeholder (a bridge row with no record) offers neither. +1 scan, 1 mutant (the click-through opens Jobs but selects nothing). **Verified:** six gates, 687/687 unit, 392/392 mutations, build guard OK `index-BHUmMfhB.js` (a NEW hash), `dist/` cleared. **READ in the pane as Karen (17:22 UTC 10 Sep, `index-BHUmMfhB.js` served): Dispatch → Queue showed six "Open →" links and the header's "Open job record →"; clicking a card's "Open →" switched to the Jobs view with that job's editor open — title input "Filter Change", JOB-2026-0006, Save present.**

**PROD SHIPPED — the FIFTEENTH ship (Jeff: "ship prod", 10 Sep).** Ancestor check (tree clean, `dev` == `origin/dev`, `origin/master` an ancestor of `dev`, 27 commits after `59116be`), then `git push origin dev:master`: `master` `59116be` → `0a242c6`, pushed 17:46:08 UTC. Netlify record `6aa2ece6…`: branch `master`, commit `0a242c6`, published 17:47:02 UTC (54 seconds after the push), **80 functions** (`_selfProfile`, `dispatch-plan-visits`, `_customerNotify`, `dispatch-status` all present), **2 redirect rules processed**, secret scan clean over 405 files; salespipelinetracker.com serving `index-C23CrCCK.js` at 17:47:03 UTC with the `pk_live_` key inlined; `/status/<32 bogus chars>` on prod → the function's own 404 with `Cache-Control: no-store` and `X-Robots-Tag: noindex` — the rewrite beats the SPA catch-all there too. The schema for §0.110 and §0.111 was already in the shared database. **What prod does differently from the moment of the ship:** every member's self-profile save goes through the allowlist (§0.109); a skipped or deferred plan occurrence and the Agreement renewals list exist in Service Due (§0.110); the hourly job's Signal 6 runs on prod (`JOBS_ENABLED` is set there) — a prod customer whose agreement ends within the plan's window would email its Admins/Managers at their alert hour and post once to #sales-alerts; and customer notifications are OFF for the prod org until an Admin turns them on (§0.111) — nothing is sent to any prod customer by this ship. **Verified:** six gates, 685/685 unit, 390/390 mutations, 145/145 integration, `index-D8PIckNZ.js` unchanged.

**To observe (Admin, either site once shipped; dev now):** Settings → Dispatch → Customer notifications → turn the master switch on and Save; give the test customer Jeff's own email; schedule its job → the job's Customer notifications trail reads "Appointment confirmation · email to … · sent" and "text … not sent — SMS not configured on this site"; the email arrives with "View visit status →"; the link opens the public page signed out; Start travel (or set En route) → the on-the-way email. SMS lights up the day the three Twilio variables land on a site AND the A2P campaign is approved — no code change.

### 0.112 The crew builder's phantom clash, and a job's preferred start time (10 Sep, twelfth session — Jeff: "It says double-booked at 9AM for Savannah. Nowhere in the job does it have a start time. Where is the 9AM assumption coming from?")

**What it was:** on JOB-2026-0006 (unscheduled, date 2026-09-11, no start time) the crew builder flagged BOTH technicians "Double-booked at 9a". Read: `scoreTech`'s availability check did `const ns = job.start || 9` — an unscheduled job was scored as if it began at 9 AM — and compared start HOURS against every job the technician had ever been assigned with a start, on ANY date (`assignedJobs` never looked at `scheduledDate`). Savannah's two August jobs at 09:00 therefore "clashed" with a September job; Jax's genuine 09:00 job on the same day was the only true clash in the list. The 15-point availability score, the ranking and the Override prompt all rode on it.

**Fix (`470b948`):** the check is SAME DAY only (`j.scheduledDate === availDate`, the builder's date or the job's), and it runs only when a start is actually known — the builder's chosen Start (`avail.start`, now passed and a dependency, so the list re-scores as the time changes), else the job's preferred start, else nothing to compare: the points are given ("no evidence of a clash") with a note "N other jobs that day — pick a start to check", and the time gate in `handleSchedule` still refuses a real overlap. **A job can now carry a preferred start:** Jobs → the editor gains **Preferred start time** (the shared `TimeDropdown`, 30-minute steps) and **Time window** (any time / morning / afternoon / evening; a start makes it "exact" and disables the select). Saving sends `scheduledStart`, `scheduledEnd` (start + duration, `addHoursHHMM`) and `timeSlot` — the columns the schema already had and the customer's confirmation already reads ("between 9:00 AM and 11:00 AM"). Selecting a job in the crew builder seeds its Start from the preferred time, so a dispatcher who set one is not asked again; the board's job shape carries `scheduledStart`, and a Jobs save updates the board's `start` too. **Test:** `tests/crew-scoring.test.mjs` (3 scans — the 9 AM default gone, same-day filter, no-start branch, the probe passed and depended on, the seed, the editor fields and payload). **Mutants (2):** the 9 AM default returns; any-date clashes return. **Verified:** six gates, 690/690 unit, 394/394 mutations, build guard OK `index-BPgC4Z9Q.js` (a NEW hash), `dist/` cleared. **READ in the pane as Karen (17:59–18:01 UTC 10 Sep, `index-BPgC4Z9Q.js` served): the queue's default job — unscheduled, JOB-2026-0007 on Mon 2026-09-14, no start — scored both technicians with NO "Double-booked" line (before the fix every unscheduled job carried Savannah's phantom 9 AM clash from her August jobs); "Open job record →" opened the editor with the Time window select ("Any time that day / Morning / Afternoon / Evening / Exact time (set a start)") beside the new Preferred start time. Also seen: both technicians read "Not rostered on Mon" — the dev technicians have no working hours set (Technicians → working hours), a data state, not a defect.** NOT shipped.

**Correction, the same hour (`a29cc1e`) — Jeff: "this is their set work schedule - is this not being referenced in the rostering process."** The line above ("the dev technicians have no working hours set") was WRONG: both rows carry Mon–Fri 07:00–17:00 in `working_hours`, and Work Schedules showed it. Read: `normaliseTech` — the mapping from the technician row to the board's shape — derived `hoursCap` from `workingHours` and then dropped the pattern itself, so every consumer of `shiftForDate` on the board shape (`scoreTech`'s "Not rostered on {Day}" blocker, the day board's lane shading, `handleSchedule`'s shift-bounds gate, the planner's `candidatesForDay` — which therefore returned NO candidate for any day) read every technician as unrostered on every day since the roster landed; only Work Schedules, which reads the raw rows, ever saw the pattern. Override was the only way any job got scheduled, which is how Jax's Fri 11 job exists. **Fix:** `workingHours: t.workingHours || {}` rides on the board technician. +1 scan (the pattern on the shape, the blocker still present), 1 mutant (drop it again). **Verified:** six gates, 691/691 unit, 395/395 mutations, 145/145 integration, `index-C3oKwuKE.js`, `dist/` cleared. **OBSERVED on dev (18:40 UTC, Karen):** the Fri 11 job — Savannah 92 with no roster blocker (Jax's only blocker the unset licence, plus "1 other job that day — pick a start to check"); Start 9:00 → Jax "Double-booked at 9a", 70 → 55, the REAL clash; Date moved to Sat 12 → both "Not rostered on Sat". Design left as documented (a missing day means not working): a technician with NO pattern set is now genuinely blocked every day until one is set under Work Schedules — before this fix the same was true of everyone. NOT shipped.

### 0.113 Saving a job with no category 500ed; a new technician starts rostered (10 Sep, twelfth session — Jeff: "I set a preferred start time to 11AM and it gave me the server error")

**What it was:** Jobs → JOB-2026-0007 → Preferred start time 11:00 AM → Save → "Internal server error". Bisected in the pane, field by field, against the same PUT: the start time was innocent — `jobType: null` and `trade: null` each 500 alone, every other field 200. Read: both columns are `NOT NULL` (defaults `'hvac'`/`'repair'`, no CHECK constraints); the create path writes `''` for "— None —" (`data.trade ?? ''`), but the Jobs editor's save sent `draft.trade || null` — so ANY job whose Job category / Job type read "— None —" could not be saved from the editor at all (JOB-2026-0004 and -0007 on dev; the earlier editor saves that passed were on jobs with a category). The generic 500 body hid the constraint; the function log shows no console output.

**Fix (`223c99a`):** the dispatcher PUT coerces a null `trade`/`jobType` to `''` right after the scalar copy (matching create), and the editor sends `''` in the first place. **And, per Jeff ("M-F 8AM to 5PM as default"), a new technician starts rostered:** `src/utils/workWeek.js` (pure, shared by client and server) — `DEFAULT_SHIFT_HOURS` 08:00–17:00, `WORK_WEEK_DAYS` mon–fri, `defaultWorkWeek()` a fresh pattern per call. The editor's new-technician draft carries it (so the POST body does), the technicians function falls back to it when the create carries NO pattern (`?? defaultWorkWeek()` — an explicit `{}` is still "no working days"), and the shift dialog's per-day toggle uses the same hours (was a second, disagreeing 07:00–17:00 constant). Existing technicians are untouched: Jax and Savannah keep their 07:00–17:00 rows. **Tests:** `tests/job-editor-save.test.mjs` (2 scans), `tests/work-week.test.mjs` (a unit test on the module, 2 scans), and an integration case in `customer-notify.itest.mjs` — a PUT with null trade/jobType → 200, `''` stored, the row otherwise unmoved. **Mutants (4):** the coercion gone; the editor's null back; the server's `{}` fallback back; the draft's default dropped. **Verified:** six gates, 696/696 unit, 146/146 integration, 399/399 mutations, build guard OK 2,468 kB `index-Bd046o9b.js`, `dist/` cleared. **OBSERVED on dev (20:17 UTC, Karen):** Jobs → JOB-2026-0007 → Preferred start time 11:00 AM → Save → "Saved"; the row (read-only) `scheduled_start` 11:00, `scheduled_end` 13:00, `time_slot` exact, `trade`/`job_type` `''`; Queue → that job → the builder's Start reads 11:00 AM unasked and the bar "11a – 1p" — §0.112's last unobserved item, closed. NOT observed: a technician created on dev arriving with the Mon–Fri pattern (Jeff's, when he next adds one — Work Schedules should show 08:00–17:00 Mon–Fri, Off Sat/Sun, without a click). NOT shipped.

### 0.114 A filled crew slot is not a saved schedule (10 Sep, twelfth session — Jeff: "I went to queue and assigned Savannah Miller to the job. Then I went to the Job Board … it still shows it needs to build a crew")

**What was read:** JOB-2026-0007's row after Jeff's report — `status` unscheduled, `assigned_tech_id` null, one status-history row ("Job created"), no customer-notification trail, no public token: NO write reached the server from his assignment. The persisted path was then reproduced end to end in the pane as Karen on the same job: Queue → + Add Savannah → Schedule crew → one PUT (`status: 'scheduled', assignedTechId, scheduledDate 2026-09-14, scheduledStart 11:00, scheduledEnd 13:00, timeSlot exact`) → 200 → the row assigned → Job Board, week Sep 13–19: the job under Savannah "11a · 2h", the "Needs a crew" row gone; after a reload the same (the from-server mapping `assignedTechIds: [j.assignedTechId, ...coTechIds]` is right). The write path is sound; what Jeff clicked cannot be read from here (Netlify's MCP has no function-log reader) — the two candidates are "+ Add" without "Schedule crew", and a refusal banner. Both are the same design fault: **"+ Add" turns the row green ("✓ Added") and the header reads "1 of 1 crew slots filled" — done, to any reader — while only "Schedule crew" persists; and a successful schedule clears the selection with no word at all** (the job simply leaves the list). This is the second report with this shape (§0.111, Jax).

**Fix (`9bc6b89`):** the slots header reads "N of M crew slots filled — not saved yet: click Schedule crew to assign" (warn tone) while any slot is filled; a successful write shows a 10-second `role="status"` banner above the action bar — "Scheduled — Savannah Miller on 2026-09-11 at 1:00 PM. It is on the Job Board now." — timed rather than tied to the selection (the selection moves to the next job on success), never shown over a refusal, cleared when the next schedule starts. `to12h` from `customerNotifications.js` formats the time. **Test:** one scan in `tests/dispatch-stubs.test.mjs` (header copy, the notice call, the status banner, the timer, the import). **Mutants (2):** the header's "not saved yet" gone; the notice silent. **Verified:** six gates, 697/697 unit, 146/146 integration, 401/401 mutations, build guard OK 2,468 kB `index-C99j1Gyv.js`, `dist/` cleared. **OBSERVED on dev (20:37 UTC, Karen), JOB-2026-0006 (Fri 11):** header "0 of 1 crew slots filled" → + Add → "1 of 1 crew slots filled — not saved yet: click Schedule crew to assign" → Start 1:00 PM → Schedule crew (1) → the status banner exactly as above, no alert. Side effect of the two reproductions, disclosed to Jeff: JOB-2026-0007 is now scheduled to Savannah Mon 14 11:00 and JOB-2026-0006 to Savannah Fri 11 13:00 on dev, and with customer notifications ON there the test customer's contact address received a confirmation for each. Still open: what Jeff saw when he clicked — asked. NOT shipped.

### 0.115 The next step after choosing a crew, and a page that fits the screen (10 Sep, twelfth session — Jeff: "the schedule crew buttons are so low I can't see them on a 40" ultrawide monitor … once I assign a rep/crew then it needs to automatically prompt me to schedule job or ask schedule now or wait for group schedule")

**What it was — the root of §0.111's and §0.114's reports:** Jeff's Queue screenshot showed the crew list with "✓ Added" and no action bar anywhere on a 40" screen. Read: the Dispatch page root was `height: '100%'` inside `.tab-page`, whose parent has no height, so it resolved to auto; the Queue's left rail (a dozen cards) set the row height, the centre column grew to match, and the action bar — Date, Start, **Schedule crew** — sat at the bottom of that column, below the fold (observed in the pane at 1800×1000: the button at y=1382). Every "I assigned her and the board still says Needs a crew" was a filled slot that was never scheduled because the button to schedule it was off screen. Confirmed the other way too: JOB-2026-0006's row shows Jeff's OWN schedule at 20:45 UTC (Savannah, 11:00–12:00, Fri 11) — his retest with the §0.114 hint on screen wrote.

**Fix (`360e4f7`, follow-up `54fb180`):** (1) **The page is bound to the viewport** below the fixed header — `calc(100vh - top)`, top measured from the page div (the ContactsTab pattern), on every render until measured (the mount-only version never measured: the loading state renders first, without the div — the first dev observation caught it, height still "100%"). Now the page ends at the viewport edge and the action bar is on screen (y=933 of 1000). (2) **The next step is asked, at the top.** The moment a crew slot is filled a `CrewNextStep` card (module scope, data as props) appears above "Suggested crew — ranked by match", where the eye already is: "Crew chosen — Savannah Miller · 1 of 1 · Nothing is saved yet." with Date and Start in the card and three actions — **Schedule now** (the existing `handleSchedule`), **Wait for group schedule**, **Clear crew**. The refusal (`role="alert"`) and success (`role="status"`) banners moved up beside the card. (3) **Wait for group schedule** persists the chosen crew on the job — `PUT { status: 'unscheduled', assignedTechId, coTechIds }`, no time, status unchanged, so no customer confirmation goes out — and the job stays in Jobs to schedule; selecting it again brings the crew back as the added crew, the header reads "held for group schedule", the card offers **Release crew** (a PUT that clears the assignment); the week board shows it on the crew's row as "TBD · 2h" (the day board's rail still lists it — no placed start); `planWeek` (Mass-schedule next week) honours a held crew — `candidatesForDay`/`crewForDay` take an `only` set, want = the held count, and an unplaceable held crew is skipped as "Held crew (names) not free together in this window", never swapped. Audit: `dispatch.crew.hold` / `dispatch.crew.release`. (4) **A preferred start is not a placement:** the board mapping turned any `scheduledStart` into a placed `start`, so an UNSCHEDULED job with a preferred time (§0.112) — and a crew held on it — read as booked at that hour; now only a job whose status is not 'unscheduled' has a placed start (the board mapping and the editor-save mapping), and the parent's held update clears `start`/status/window on the client copy. **Tests:** `tests/crew-next-step.test.mjs` (5 scans: the card and its order, the banners' order, the hold body and mappings, the planner's `only`, the viewport height), 3 scans updated in `dispatch-stubs`. **Mutants (5):** the card never renders; hold flips status to scheduled; the planner ignores a held crew; a preferred start reads as placed again; the viewport height gone. **Verified:** six gates, 702/702 unit, 146/146 integration, 406/406 mutations, build guard OK 2,473 kB `index-B8shyvit.js`, `dist/` cleared. **OBSERVED on dev (21:05 UTC, Karen, pane at 1800×1000):** page top 72 → bottom 1000 (`calc(-72px + 100vh)`), Schedule crew at y=933 in view; JOB-2026-0007 (crew held by the first observation) opens with "Crew held for group schedule — Savannah Miller · 1 of 1 · Saved without a time…", Schedule now / Release crew, header "1 of 1 crew slots filled — held for group schedule"; Job Board week Sep 13–19: Savannah's row "Dispatch Customer Test · TBD · 2h", no "Needs a crew" row. Earlier in the same pass: + Add → the card in view at y=533 with "Crew chosen — Savannah Miller · Nothing is saved yet", Wait for group schedule → "Crew held — Savannah Miller. The job stays in Jobs to schedule; Mass-schedule next week will place them, or come back and Schedule now." Still open: `document.documentElement.scrollHeight` exceeds the viewport by a little even with the page bound — the source (header margin or body padding) not read; the controls are on screen regardless. NOT shipped.

### 0.116 A job's equipment requirement can be seen and changed (10 Sep, twelfth session — Jeff: "Why would this one not schedule due to equipment unavailable? I do not see where an equipment requirement is listed")

**What it was:** JOB-2026-0004 (New HVAC System, 15 Aug, 2 techs × 4h) refused every schedule with "Equipment unavailable — eq_1786389760843 — no units exist in Vehicles & equipment", and the crew builder's Required line showed only "Equip: eq_1786389760843". Read: the row's `equipment_ids` is `["eq_1786389760843"]`, written when the job was created on 10 Aug (the August seed) — a unit ID, where the scheduler treats the list as equipment KINDS (`dispatch_equipment.category` values — the fleet on dev has one unit, "Pressure Tester", category "HVAC"); `equipmentConflicts` matched no category → `missing` → blocked. And the Jobs editor had NO equipment field: the requirement was invisible there and could not be removed. Not a scheduler defect — the check did what it should with a requirement nothing can satisfy — but a record the dispatcher could neither see nor fix.

**Fix (`e5970f3`):** Jobs → the editor gains **Required equipment**: the fleet's categories as toggles (the new-job form's pattern), and any stored requirement the fleet does not have as an amber chip "⚠ eq_… — not in Vehicles & equipment · remove ×" (`role="button"`, one click removes it). Saving writes `equipmentIds` (the dispatcher PUT already accepted it; no client ever sent it from the editor) and the board job takes the list back as `equipCategories`, so the crew builder re-checks against what was just saved. The builder's Required line marks an unmatched kind "(not in Vehicles & equipment — remove it under Jobs)". JobsView takes an `equipment` prop from the parent. **Tests:** `tests/job-equipment.test.mjs` (4 scans — the field, the chips, the save, the board take-back, the builder note, the server's JSON field). **Mutants (2):** the save drops the field; the unmatched chip hidden. **Verified:** six gates, 706/706 unit, 146/146 integration, 408/408 mutations, build guard OK 2,475 kB `index-3dtx4520.js`, `dist/` cleared. **OBSERVED on dev (21:45 UTC, Karen):** Jobs → New HVAC System → "REQUIRED EQUIPMENT · HVAC · ⚠ eq_1786389760843 — not in Vehicles & equipment · remove ×". Not observed: the builder's Required-line note (the pane's card pick landed on another job; scan-covered). The stale value itself is left for Jeff to remove — his record. NOT shipped.

**Jeff's follow-up, the same hour (`d7f0765`): "HVAC is not a type of equipment. The equip showing is in code, why does it not just show the name of the actual piece of equipment required."** He is right about the model as the editor first showed it: "HVAC" was the free-text `category` he had typed on the Pressure Tester, not a kind of equipment, and the stale entry was a UNIT id — the seed's first Pressure Tester record (`eq_1786389760843` decodes to 10 Aug 19:22 UTC; the surviving unit `eq_bfef5b6f…` was created 11 Aug 00:45), so the column meant specific units from the start and the kinds reading was later. **Now a requirement is a specific unit, shown by name:** the Jobs editor and the new-job form list the fleet's units by name (category as a muted tag) and store the unit id; a job template still contributes KINDS — the templates panel migrated to categories in an earlier session — shown as "any <kind>" chips, removable; an id whose unit no longer exists is the amber "no longer in Vehicles & equipment · remove ×" chip. `equipmentConflicts` handles both entry types: a unit must be in service and not required by an overlapping job the same day (the conflict names the unit — "Pressure Tester — committed to an overlapping job that day"); a kind keeps the capacity count; a lost id reads "no longer in Vehicles & equipment (remove it under Jobs)". `equipLabel(req, units)` renders an entry for readers (the builder's Required line). Trade-off, stated: a specific unit is stricter than a kind — if that one unit is committed the job blocks even when a twin is free; a template's kind stays the flexible form. **Tests:** `tests/job-equipment.test.mjs` rewritten (5 tests: unit pickers in both forms, kind and lost-id chips, the two-mode conflict check and its wording, `equipLabel`, the server field). **Mutant (+1):** a unit id no longer recognised as a unit. **Verified:** six gates, 707/707 unit, 146/146 integration, 409/409 mutations, build guard OK 2,477 kB `index-CLWA5fOk.js`, `dist/` cleared. **OBSERVED on dev (22:06 UTC, Karen):** Jobs → New HVAC System → "REQUIRED EQUIPMENT · Pressure Tester [HVAC] · ⚠ eq_1786389760843 — no longer in Vehicles & equipment · remove ×" — the unit by name, the lost id as the removable chip. Not observed: the new-job form's unit pickers and the builder's conflict wording (scan-covered). Superseded within the hour — next paragraph.

**Jeff's decision, the same hour (`1233de4`): "I don't want to require a specific unit. I want to require a kind of equipment. If I need a pressure tester and we have 10 of them I don't care which one I get as long as there is one assigned to me and the job at the day and time needed."** So the unit model above lasted one commit. **The model now:** a requirement is a KIND — the Category on the fleet's units, to be named the way a technician asks for it ("Pressure Tester", not "HVAC"; the editor says so) — and **scheduling reserves a unit**: `pickEquipmentFor(kinds, units, rivals, alsoTaken)` picks, per kind, the first in-service unit by name that no overlapping job that day holds (rivals = `overlappingRivals` — same day, not cancelled/completed, overlapping the probe, requiring or holding equipment); the crew builder's schedule PUT and the mass scheduler's per-proposal PUTs carry `assignedEquipmentIds` (the mass run keeps a `reservedThisRun` ledger so two proposals the same day never take the same unit); the job stores it in the NEW nullable jsonb `dispatch_jobs.assigned_equipment_ids` (`db/apply-assigned-equipment.mjs`, applied to the APP and TEST databases before the code — §18c; the schema guard requires it); the board job carries `assignedEquipment`; the builder's Required line reads "HVAC · reserved: Pressure Tester", the success banner "Reserved: Pressure Tester.", the Jobs editor "Reserved for this job: …" read-only. **Release:** the jobs function clears the reservation whenever a PUT sets status 'unscheduled' without saying otherwise — the hold, the release, a time-off unassign, the editor — and the client copies follow. **Capacity:** `committedOfKind` counts a rival's reserved units of the kind when it has a reservation, else its requirement, so the count stays right across old and new rows. Check-out (`checkedOutJobId`) is untouched — the physical "in the truck now" state, one job at a time, the wrong place for a Sep 18 reservation. Pickers in the editor and the new-job form are kinds again (in-service/total, unit names in the tooltip); a stored entry no kind matches is the amber "not a kind in Vehicles & equipment · remove ×" chip. **Tests:** `tests/job-equipment.test.mjs` rewritten (5), 2 scans updated, +1 integration case (stored with the schedule, untouched by an unrelated save, cleared on unscheduled). **Mutants:** 3 (a reservation ignores what rivals hold; the schedule write drops it; the server keeps it on unscheduled). **Verified:** six gates, 707/707 unit, 147/147 integration, 411/411 mutations, build guard OK 2,478 kB `index-DRfAzx7m.js`, `dist/` cleared. **OBSERVED on dev (22:25 UTC, Karen), end to end on JOB-2026-0004 (New HVAC System, Jeff had moved it to Thu 18, preferred 8:00):** Jobs → the field "HVAC 1/1 · ⚠ eq_1786389760843 — not a kind … remove ×" → chip removed, HVAC on → Saved → the row's `equipment_ids` `["HVAC"]`; Queue → the job (Required: Equip: HVAC) → + Add Savannah → Schedule now (Start seeded 8:00) → **"Scheduled — Savannah Miller on 2026-09-18 at 8:00 AM. Reserved: Pressure Tester. It is on the Job Board now."** — the row's `assigned_equipment_ids` holds the Pressure Tester's id. Side effects disclosed: JOB-2026-0004 is now scheduled (Savannah, Thu 18 08:00, Pressure Tester reserved) and JOB-2026-0007 was scheduled by a mis-aimed card click in the pane (Savannah, Tue 15 08:00 — Jeff had moved its date to the 15th) before the intended job was reached; both are dev test rows. Also seen, not fixed: a queue card for an UNSCHEDULED job with a preferred time shows "◷ 08:00" where the others show the date. NOT shipped.

**And the chips (`a7201b7`) — Jeff: "There is no way to add equipment under required equipment."** Read in the pane first: the pills DID toggle on click, in the Jobs editor and the new-job form (a click turned "Pressure Tester 2/2" dark; a second click turned it back) — but they looked like labels: an outline pill with a count, no checkbox, no "+", no caption. Same class as the Schedule crew button below the fold: a control nobody can tell is one. Now every such chip reads **"+ Kind"** when off and **"✓ Kind"** when on, carries `role="checkbox"` and `aria-checked`, the skill chips a title, and each picker a line: "Click a kind to require it; click again to drop it." Applied to skills in the technician editor, skills and kinds in the Jobs editor, skills and kinds in the new-job form (5 sites). Also read from the fleet: Jeff's two new units "Pressure Tester #1/#2" (category "Pressure Tester") arrived by the templates panel's equipment import (`imp_eq_1786389760843_u1/u2`) — the old blob's item became a kind with two units, exactly the shape the model wants. **Test:** +1 scan (5 checkbox roles, the prefixes in both forms, both captions). **Mutant (+1):** the kind chips lose their + / ✓. **Verified:** six gates, 708/708 unit, 147/147 integration, 412/412 mutations, build guard OK 2,479 kB `index-mSB6P0Q8.js`, `dist/` cleared. **OBSERVED on dev (22:48 UTC, Karen):** Jobs → New HVAC System → three `role="checkbox"` chips: "+ HVAC Install & Repair" (false), "✓ HVAC 1/1" (true), "+ Pressure Tester 2/2" (false), and the caption "Click a kind to require it; click again to drop it." NOT shipped.

**PROD SHIPPED — the SIXTEENTH ship (Jeff: "ship prod", 10 Sep).** Pre-flight: tree clean, `dev` == `origin/dev`, `origin/master` an ancestor of `dev`, 29 commits after `0a242c6` (ten of them code: `470b948`, `a29cc1e`, `223c99a`, `9bc6b89`, `360e4f7`, `54fb180`, `e5970f3`, `d7f0765`, `1233de4`, `a7201b7`). `git push origin dev:master` at 22:50:39 UTC: `master` `0a242c6` → `0726934`. Netlify record `6aa33443…`: branch `master`, commit `0726934`, created 22:50:43, published 22:51:30 UTC (51 seconds after the push, deploy_time 45 s), **80 functions**, **2 redirect rules**, 1 header rule, secret scan clean over 412 files; salespipelinetracker.com serving `index-DaJE9sq9.js` from 22:51:42 UTC with the `pk_live_` key inlined (prod's hash never equals dev's `index-mSB6P0Q8.js` — the key differs). The one schema change of the batch, `dispatch_jobs.assigned_equipment_ids`, was already in the shared database (applied 10 Sep, §18c) — nothing applied at ship time. **What prod does differently from this moment:** the roster is read on the board (a technician with a pattern is rostered, one without is "Not rostered" — §0.112); a job with no category saves from the editor, a new technician starts Mon–Fri 08:00–17:00 (§0.113); the Dispatch page fits the screen, the next-step card asks Schedule now / Wait for group schedule / Clear the moment a crew slot is filled, a held crew is persisted and honoured by Mass-schedule (§0.114–§0.115); equipment is required by KIND, scheduling reserves one in-service unit per kind and releases it on unschedule, and the kind/skill chips read as toggles (§0.116). Every "NOT shipped" in §0.112–§0.116 above is superseded by this line. **OBSERVED on prod by Jeff (Admin), 10 Sep: "everything tested out perfectly"** — the Queue's action bar and next-step card, a new technician's default week, a no-category save, equipment by kind with the reservation named on schedule.

### 0.117 Two small things seen while shipping (10 Sep, twelfth session — Jeff: "either way please do a fix on the two things you noticed")

**(1) A queue card read "◷ 08:00" on an UNSCHEDULED job.** The board mapping set `window` to `scheduledStart` whenever `timeSlot` was 'exact' — and since §0.112 an unscheduled job may carry a PREFERRED start with slot 'exact', so its card read like a booking beside the scheduled ones. Now `queueWindow(j)` (module scope): a time only for a job whose status is not 'unscheduled' with an exact slot; otherwise the date, and a preferred start as a note — "2026-09-18 · prefers 8:00 AM". The editor-save mapping sets `window` too, so saving a preferred time updates the card without a reload. **(2) The document still scrolled by 8 px** with the Dispatch page bound to the viewport (§0.115): the app shell's `.app-container` pads the page by 8 px, and the measured offset counted only the top. The layout effect now adds the parent's bottom padding — `calc(100vh - 64px)` on dev where it was `- 56px`. **Tests:** +1 (`crew-scoring`: the status guard, the "prefers" note, both mappings, the card), 1 scan updated (`crew-next-step`). **Mutants (2):** the status guard gone; the bottom padding uncounted. **Verified:** six gates, 709/709 unit, 147/147 integration, 414/414 mutations, build guard OK 2,479 kB `index-DlxqI1V6.js`, `dist/` cleared. **OBSERVED on dev (23:21 UTC, Karen):** `document.documentElement.scrollHeight` 1273 == `innerHeight` 1273 — no scroll; the page's height `calc(-64px + 100vh)`, bottom 1265 of 1273; queue clocks "◷ 09:00 / 08:00 / 11:00" on the scheduled jobs, "◷ 2026-08-13" on the unscheduled one, "◷ Not scheduled" on the bridge rows. NOT observed: the "prefers" note — no unscheduled job on dev carries a preferred time now (the two that did were scheduled by the §0.116 observations); scan-covered. **Environment finding, twice this hour:** the mutation harness ended red after a GREEN baseline with `UNKNOWN: unknown error, open '<file>'` (errno -4094) while RESTORING a mutated file — `DispatchTab.jsx` once, `ConnectedAppsDetail.jsx` once, the second left mutated on disk until `git checkout -- <file>`. Windows Defender (`MsMpEng.exe`) scanning fresh writes is the likely holder; the third run was clean. A red harness exit is therefore not always a survivor: read the log for "restore of"/"COULD NOT" and `git status` before believing either. NOT shipped.

### 0.118 The queue lists only jobs to schedule; a won opportunity gets Create job where a saved job gets Edit job (11 Sep, thirteenth session — Jeff: "I can't edit the selected job beyond scheduling. It would be helpful to edit the job's details from here via pushing a button" and "scheduled jobs still show in the list on the left under the title Jobs to Schedule. Doesn't it make more sense to only show unscheduled jobs")

**What it was.** (1) The Queue's left list, headed "Jobs to schedule", rendered EVERY job (`jobs.slice().sort(…)`) — the scheduled ones sat among the unscheduled with a "✓ Scheduled" badge, and the builder's default selection was `jobs[0]` after the first unscheduled, so with nothing left to schedule it opened a scheduled job. The §0.114 comment already said "the job leaves the unscheduled list" on a schedule; it never did. (2) Jeff's screenshot: the selected row was a WON OPPORTUNITY (`auto_…`, `isBridge`), which has no record to open — so the header carried no button where a saved job shows "Open job record →", the card no "Open →", and the only way into its details was the "Create job" button inside the explanatory banner lower down. The same row's header read **"null techs" and "nullh"** and its card "p × h": the bridge builder leaves crew size and duration null when no template matches (by design — the bridge builder's own comment: "left null so the UI can say 'not set' instead of inventing a figure"), and three renders printed the null.

**Fix (bundle only).** (1) `isUnscheduled(j)` (module scope: no placed start, or no crew — a held crew and a won opportunity both count) and `sortedQueue` lists exactly those; the default selection is the first job to schedule, `null` (the existing "Select a job from the queue" state) when there is none, and a selection pointing at a scheduled job is ignored; the parent no longer passes `jobsWithBridge[0]?.id` as a default, and the held-crew/preferred-start effect keys on `selectedJob?.id` so the builder's own default gets its held crew back. **A scheduled job clicked on the Job Board opens its record in Jobs** (`handleJobClick` → `openJobRecord`), as does Service Due's "Open job" on a scheduled visit; only a job still to schedule opens the crew builder. (The first cut, `889cc85`, listed a scheduled job opened from the board for as long as it was selected, so it could be re-crewed. Jeff, on dev within the hour: "I clicked on this scheduled job on the job board. It opened it in Queue and now has added it to the jobs to schedule list. It did not go away until a hard refresh. I think the flow should be when a person clicks on a scheduled job in the job board it either opens a rail with the job for editing or it opens in the job tab under dispatch" — the Jobs tab, the editor "Edit job →" already reaches; the "✓ Scheduled" card badge went with the last scheduled card. Why it "did not go away": by that design it stayed while selected; not re-read, moot now.) (2) The header button reads **"Edit job →"** for a saved job (was "Open job record →"; same handler, the Jobs editor) and, for a won opportunity, a near-black **"Create job →"** in the same slot (`onCreateBridgeJob`, the prefilled new-job form where its details are entered); each card gets "Create job →" beside where a saved job's "Open →" sits; the banner keeps its explanation and loses its duplicate button. (3) `crewLine(j, compact)` (module scope) reads "2 techs · 4h" / "2p × 4h", "crew and duration not set", "crew not set · 4h", "2 techs · duration not set"; the header grid reads "Not set" for crew size, duration and min license. Not done: an in-place editor in the queue — the Jobs editor stays the one editor; the button takes the dispatcher there. Seen and left: `unscheduledJobs` and `scheduledJobs` inside `CrewBuilderView` are computed and never read (dead since some earlier batch — not touched here).

**Tests:** +2 scans in `crew-scoring` (the list rule and default selection; the two buttons, the card link, one Create action on the selected row, the crew line and the header labels), 1 pin updated in `dispatch-stubs` (the relabel). **Mutants (4):** every job listed again; a scheduled board click lands in the queue again; the bridge header button gone; the both-null line gone. **Verified:** six gates (check:fnscope 90 function files; the five on 158 files — the header's 89/157 were the previous print, the difference not traced), 711/711 unit, 147/147 integration, 418/418 mutations after a printed green baseline (no restore errors, both runs), build guard OK 2,479 kB `index-BYQZhHpB.js` (the first cut: `index-SFEVnpmt.js`, 417/417), `dist/` cleared. **OBSERVED in part by Jeff on dev (the first cut, served 16:44 UTC 11 Sep):** his screenshot shows the list without scheduled cards except the one he had opened from the board, "Edit job →" beside a saved job's id, "Create job →" and "crew and duration not set" on every won-opportunity card, and the header grid populated for a saved job; the follow-up (the board click opening Jobs) NOT observed. NOT shipped.

### 0.119 The Pipeline list shows the closed deals its filter asked for (11 Sep, thirteenth session — Jeff: "I marked an opp won. Filtered by won deals only and it returned nothing in the list. I have all selected in all/mine")

**What it was.** Pipeline → Filter → Stage "Closed Won" → Apply, List view: the header counted the six won deals — "0 open deals · $1.0M total pipeline", "Showing 6 of 6" — and the list read "No deals match the current filter." The tab's own filter chain (`pipelineFilteredOpps`) had kept them: its "All open" exclusion of closed deals applies only when no stage filter is set, and the stage filter kept exactly the Closed Won rows. `ListView` then dropped every closed deal itself — `openOpps = pipelineFilteredOpps.filter(o => !['Closed Won','Closed Lost'].includes(o.stage))`, commented "Exclude closed deals (same as other views)", written in `740cac7` (27 Apr) — so the Filter popover offered two stages the List view could never show. Funnel and Kanban were not the same: both build their columns from `stages` minus Closed Lost, so a Closed Won filter shows the won deals there.

**Fix (bundle only).** The tab decides what the list shows: `ListView` renders the set it is handed (`listOpps = pipelineFilteredOpps`), grouped by close quarter as before (a won deal with no forecast date lands in the "No close date" group). The default is unchanged — "All open" with no stage filter still excludes closed deals upstream — so nothing changes until a closed stage is asked for. Not done, flagged: with a closed stage selected the header still reads "0 open deals" (accurate, since it counts open deals; odd beside a won-only list); a "Closed Lost" filter shows nothing in Funnel or Kanban, whose columns omit that stage by design — Jeff's call whether a lost column belongs there.

**Tests:** +1 file `tests/list-view-closed.test.mjs` (2 scans: the list has no closed-stage filter of its own; the tab still hides closed deals by default and only by default, the popover offers Closed Won, the list receives the smart-filtered set), registered in the harness's `SUITES` (§0.95's lesson). **Mutants (1):** the drop returns. **Verified:** six gates, 713/713 unit, 147/147 integration, 419/419 mutations after a printed green baseline, build guard OK 2,479 kB `index-jRTXPWFT.js`, `dist/` cleared. NOT observed on dev at the time of writing; NOT shipped.

### 0.120 The Pipeline filter's time window is applied and read back honestly (11 Sep, thirteenth session — Jeff: "I keep trying to select all time and it allows me to select it but it does not actually use that selection and when I reopen the filter it still is closed won but has switched back to this quarter for time")

**What it was.** Two faults in one line of the Filter popover's Apply, since `c25a043` (21 Apr): `setPipelineQuarterFilter(draftTimeWindow === 'allTime' || draftTimeWindow === 'thisQuarter' ? [] : [draftTimeWindow])`. An empty quarter filter means no time filtering at all (the tab's chain returns every row when it is empty), so (1) **"This quarter" never filtered anything** — the match rule for it in `timeFilterOpts` was never reached; Jeff's screenshot under "This quarter" listed Q4 2026 and Q1 2027 deals side by side — and (2) **"All time" read back as "This quarter"**: the popover's draft was seeded with `pipelineQuarterFilter[0] || 'thisQuarter'` on mount and on every reopen, and Reset put the draft back to 'thisQuarter' too. So "All time" WAS applied, invisibly, and looked as if it had been ignored; "This quarter" looked applied and was not.

**Fix (bundle only).** Only "All time" is the empty filter; every other window is stored and therefore filters (the match table was already right). The empty filter reads back as "All time" on mount, on reopen and on Reset. The default view is unchanged — the filter state starts empty and the pipeline shows every open deal — it just says so now; a chosen "This quarter" counts in the Filter badge like any other filter. Not touched: the ReportsTab's own period control (§0.69, a different mechanism, where 'All time' maps to null by design).

**Tests:** +1 file `tests/pipeline-time-window.test.mjs` (3 scans: only allTime maps to the empty filter; the empty filter reads back as allTime on mount, reopen and Reset; every offered window has a match rule), registered in `SUITES`. **Mutants (1):** the April mapping returns. **Verified:** six gates, 716/716 unit, 147/147 integration, 420/420 mutations after a printed green baseline, build guard OK 2,479 kB `index-DtQQ5W86.js`, `dist/` cleared. OBSERVED on dev by Jeff ("all works as it is supposed to").

**PROD SHIPPED — the SEVENTEENTH ship (Jeff: "all works as it is supposed to. Ship prod", 11 Sep).** Pre-flight: tree clean, `dev` == `origin/dev` == `dab2655`, `origin/master` (`0726934`) an ancestor of `dev`, 15 commits after it (five of them code: `e4d9ac3`, `889cc85`, `0deb6b6`, `074669d`, `e5d574f`), `git diff --stat origin/master..dev` over `netlify/`, `db/`, `package.json` and `netlify.toml` EMPTY — a bundle-only ship, no function, schema or config change. `git push origin dev:master` at 20:59:07 UTC: `master` `0726934` → `dab2655`. salespipelinetracker.com served `index-BDt4t8KW.js` from 20:59:42 UTC (the hash poll; 35 seconds after the push; the previous `index-DaJE9sq9.js` was still served at 20:59:32) with the `pk_live_` key inlined. Landing proof by the bundle's own strings (a minified bundle renames identifiers, so the literals): "Edit job →", "Create job →", "crew and duration not set" (§0.118), "No deals match the current filter." (§0.119, the list's empty state now reachable only when the filter truly matches nothing), `"allTime"?[]:[` present and `"thisQuarter"?[]` ABSENT (§0.120). The Netlify deploy record was not read this time — the deploy id was not to hand and the reader takes an id, not a site; the served hash is the proof (as the deploy-check memory says). **What prod does differently from this moment:** the Queue lists only jobs to schedule and a scheduled job clicked on the Job Board opens its record in Jobs; a saved job's header reads "Edit job →", a won opportunity's "Create job →", and unset template values read "Not set" (§0.118); the Pipeline List shows the closed deals its filter asked for (§0.119); the Filter popover's "This quarter" filters and "All time" reads back as itself (§0.120); the queue card's "prefers" note and the 8 px scroll fix (§0.117). Every "NOT shipped" in §0.117–§0.120 above is superseded by this paragraph. **§0.117–§0.120 OBSERVED on dev by Jeff before the ship** ("all works as it is supposed to"); NOT yet observed on prod.

### 0.121 Web-to-lead: an embeddable form that posts into Leads (11 Sep, thirteenth session — Jeff: "one of the enhancement recommendations that Claude has made is the following: email templates for reps; web-to-lead forms (an embeddable form that posts into your Leads module — SMBs ask for this constantly)" → "I agree with your recommendations": web-to-lead first, a new web lead UNASSIGNED)

**What it was.** Nothing outside the app could create a lead: `leads.mjs` POST is behind Clerk, and the public REST API is read-only by design (§0.56, `api-surface.test.mjs`). The app had ONE unauthenticated surface, the customer job-status page (§0.111, guide §18b35), and no unauthenticated write at all.

**The model.** One hosted form per org behind ONE unguessable token — the token is the whole authority, exactly the §18b35 rule, now applied to a WRITE (guide §18b36, new). `settings.extra.webToLead` = `{ enabled, token, source, thankYouUrl }`, normalised by `cleanWebToLead()` in `src/utils/webToLead.js` (pure) and carried in BOTH halves of `settings.mjs` (18b12): the GET normalises with no mint; the PUT passes `randomBytes(24).toString('base64url')` as the mint, called only when the form is turned on without a token or a rotate is asked for — the stored token survives every other save; the client can never mint. `lead-intake.mjs` (new, no `auth.mjs`, no `apiKeys`): GET `/lead-form/<token>` (netlify.toml rewrite ABOVE the SPA catch-all, like `/status/`) is the hosted form — the org's display name, six fields (`INTAKE_FIELDS`: first name, last name, company, email, phone, message), a honeypot the visitor never sees — no `X-Frame-Options` and NO `frame-ancestors` directive, so it can be framed anywhere (the first cut sent `frame-ancestors *`; Chrome refused a `data:` parent in the pane — "'*' matches only URLs with network schemes" — and would refuse a `file:` one, which is how a customer tests an embed locally; dropped the same hour); POST `/.netlify/functions/lead-intake?t=<token>` takes JSON (→ 201 `{ ok }`) or a form-encoded post (→ a thank-you page, or 303 to the org's CONFIGURED https `thankYouUrl` — never a URL from the payload). The token finds the settings row (`extra->'webToLead'->>'token'`), the row names the org, a turned-off form is the same 404 as an unknown or malformed token; the ONE write is `db.insert(leads)` with `orgId` from that row and `assignedTo: null, ownerId: null` — UNASSIGNED, the claim-request pool (§0.58) and the Manager's assignment apply unchanged; `source` is the org's (default "Web Form" — matched the same hour to the import vocabulary in LeadImportModal; Jeff's first test lead on dev, submitted before the rename, carries "Web form" with a lowercase f and shows as a second source in the Leads sources list — his row to edit or delete), `status` New, scored on create with the org's lead-scoring config, the message plus "Submitted via web form from <referer>" in notes; then the same fan-out `leads.mjs` POST does (webhook `lead.created`, automations) and a Slack post under a NEW event switch `webLead` ("New lead from the web form", `slackTemplates.webLead`, nine types now). Defences: the honeypot (a filled one is thanked and NOT stored), a per-token in-memory rate limit (30 per 15 min per instance, the `public-api.mjs` approach), a 16 kB body cap, every field trimmed and capped, every rendered value escaped, no-store, noindex. **The card:** Connected apps → "Web-to-lead form" (`WebFormCard`, module scope) — Off / Live, Turn on (Admin), the hosted link and Copy link / Copy embed code (an iframe), New link (rotate), Turn off; the panel saves through `putSettings` and RE-READS settings to learn the server-minted token (the PUT returns `{ success }`, not the row); counted in the panel's "n live". Not done, flagged: email to Admins/Managers on a new web lead (the hourly job's preference system is a different mechanism — Slack and the unassigned pool are v1); custom fields; a per-form thank-you URL field in the card (the key exists and is honoured; no UI yet). **Verification on dev is a header probe** — Netlify's site-wide `X-Frame-Options: DENY` in netlify.toml is a static-files rule and the function sets its own headers; whether a function response inherits it is read from the deployed 404 page's headers, not assumed.

**Tests:** +1 unit file `tests/lead-intake.test.mjs` (12: the pure module RUN — defaults, the stored token surviving a save, mint-only-by-the-PUT, rotate, the https-only thank-you URL, intake cleaning and refusals, the honeypot, the links; scans of the endpoint, the settings halves, the rewrite's place, the Slack type and template, the card), registered in `SUITES`; +1 integration file `tests/integration/lead-intake.itest.mjs` (6: the hosted form and its headers by query and by path; a turned-off, an unknown and a malformed token the same 404 on GET and POST with nothing reaching the other org; a JSON submission → an unassigned, unowned, scored lead in the token's org with its source, the referer in notes, the Slack post under `webLead`, the other org empty; a form-encoded post → the thank-you page, the honeypot thanked with NO row, a payload naming an owner/org stored unowned in the token's org; no contact → 400 and no row; the rate limit → 429), registered in `test:int`; `slack-alerts` pins raised to nine types / three events. **Mutants (5):** a turned-off form still accepts; the caller's payload names the owner; no-contact accepted; a save drops the stored token; the settings PUT loses the key. **Verified:** six gates (check:fnscope 91 function files; the five on 159 files), 728/728 unit, 153/153 integration, 425/425 mutations after a printed green baseline, build guard OK 2,483 kB `index-CPK391Dz.js`, `dist/` cleared. **OBSERVED END TO END on dev by Jeff (his four screenshots, ~22:05–22:10 UTC 11 Sep): the hosted form at `/lead-form/0PkiNR8Q…` under "ACCELEREP TEST" filled with his own details; the thank-you page at the intake URL; Leads → "Needs first touch — New leads not yet assigned": "45 · Jeff Russell · TestCo · Web form · not yet touched", the page header "1 unassigned"; #sales-alerts at 22:09 UTC: "📝 New lead from the web form — Jeff Russell — TestCo · Email: … · Phone: … · Unassigned — claim it in Leads", the message quoted, View leads →.** The token minted by his Turn on, the GET rewrite, the form-encoded POST, the insert, the scoring, the Slack switch — all proven by his hand. **Two follow-ups the same hour, one commit:** (1) the frame policy above (`frame-ancestors *` → none; found by loading the embed test page in the pane — the console said why); (2) the default source "Web form" → "Web Form" to match the import vocabulary — his screenshot's Lead sources list showed both spellings. Re-verified: six gates, 728/728 unit, 153/153 integration, 425/425 mutations, `index-CPK391Dz.js`. **The follow-up OBSERVED by Jeff (11 Sep, ~22:30 UTC): "All test confirmed passed. It all works as it should" — the embed from a local HTML file, New link, Turn off / Turn on.** **SHIPPED — the EIGHTEENTH ship.**

### 0.122 Email templates for reps, mailto-based (11 Sep, thirteenth session — the second half of the recommendation Jeff agreed to: "email templates for reps"; Jeff: "do the second half of the recommendation please")

**What it was.** Reps emailed a contact through the rail's ✉ Email button — a bare `mailto:` that opened their own mail client and prompted the Email activity — and typed every message from nothing. The app itself sends email only for quotes, alerts, digests and customer notifications, from one app-owned sender.

**The model.** An org keeps a template library — `settings.extra.emailTemplates`, `[{ id, name, subject, body }]`, normalised by `cleanEmailTemplates()` in `src/utils/emailTemplates.js` (pure; caps 80/200/5000, 100 templates, duplicate ids and unnamed rows dropped) and carried in BOTH halves of `settings.mjs` (18b12). Nine merge fields (`MERGE_FIELDS`): the contact's first name, last name, full name, company and title; the rep's name, email and phone (their roster profile, `myProfile`); the org's company name (display name, else legal). `renderTemplate()` replaces `{{key}}` (spaces tolerated) and leaves an UNKNOWN field as typed, visible — a field silently vanishing would be noticed by the customer, not the rep. **The template is sent by the rep's own mail client:** the rail renders it for the contact in view and opens a real `mailto:` link with the URL-encoded subject and body (`mailtoHref`). Nothing is sent by the app — no per-org sending domain, no deliverability to own, no copy anywhere but the rep's Sent folder — the honest v1 the assessment recommended; sending from the app is a different project. **Settings:** Sales process → **Email templates** (`EmailTemplatesDetail`, a catalogue row with no typed count, the card reading "n templates"): a list, an editor (name, subject, body, merge-field chips that insert at the cursor, a preview against a sample contact), Save through `putSettings` with the nav-guard hooks every list panel uses; an unnamed template blocks Save. **The rail:** with no templates the ✉ Email button is unchanged; with templates it reads "✉ Email ▾" and opens a `TemplatePicker` (module scope) under the action bar — "Blank email" and one row per template with its rendered subject — each a real `mailto:` link; picking one logs the Email activity with `notes: "Template: <name> — <subject>"` through the existing `openCommLog`, extended to take extra prefill (the activity modal spreads the initial context into its form). Not done, flagged: personal (per-rep) templates; templates on a LEAD (the Leads tab has no `mailto:` action to hang them on); a "sent" record beyond the logged activity — by design, the app never sees the message.

**Tests:** +1 file `tests/email-templates.test.mjs` (7: the pure module RUN — normalisation, the merge context, rendering with an unknown field kept, the mailto encoding; scans of both settings halves, the catalogue row, the card count, the AdminView route, the panel's module-scope editor and rethrown save, the rail's picker, links and logged note), registered in `SUITES`. **Jeff's finding on dev, the same evening (12 Sep, "email templates will not open in settings > sales process"):** the Settings page hands a card a click ONLY when its id is in `AdminView`'s `DETAIL_PANELS` map (`onOpen={DETAIL_PANELS[it.id] ? … : undefined}`); the batch added the route line and the catalogue row but not the map entry, so the card rendered dead. Fixed in the follow-up commit: the map entry, a scan pinning it ("the DETAIL_PANELS gate, not just the route"), a mutant that removes it. Lesson for the next panel: a Settings detail is FOUR edits — catalogue row, `settingsCards` detail, `DETAIL_PANELS` entry, the route — and the scan now names all four. Along the way the harness file itself failed to parse (an unescaped apostrophe in the new mutant's label — the harness printed a SyntaxError and "0 caught"; `node --check scripts/mutate-import.mjs` is the cheap guard before a run). **Mutants (5):** an unknown merge field vanishes; the mailto body unencoded; the settings PUT loses the key; a duplicate id kept; the Settings card has no click again. **Verified (after the fix):** six gates (161 files; 91 function files), 735/735 unit, 153/153 integration, 430/430 mutations after a printed green baseline, build guard OK 2,494 kB `index-CD8TO61e.js` (the first cut `index-Bk9klMuV.js`), `dist/` cleared. **OBSERVED in the pane signed in as Jeff (16:50 UTC 12 Sep):** the panel opened, "Follow-up after a call" saved (PUT /settings 200, "✓ 1 template"), Sunniva's rail read "✉ Email ▾", the picker's link decoded to `mailto:sunniva@…?subject=Following up, Sunniva&body=Hi Sunniva, … for ZZFX Fenwick Group. Jeff Russell / Accelerep Test ·`. **OBSERVED by Jeff as Karen (his screenshot, 12 Sep): "Emailed from template as karen and the process worked fine"** — the "Jeff Russelltest" contact's Activity history carries "Sep 12 · Email · Template: Follow-up after a call — Following up, Jeff". **And his question: "I have not received an email. It may be that I need to setup an email path in Accelerep."** No — by design NOTHING is sent by the app: the template opens in the rep's own mail client and goes out only when Send is pressed there; if the browser has no default mail handler, or the compose window is dismissed, nothing goes anywhere, and the activity was logged on the click, not on a send. There is no email path to configure for this; sending from the app is the per-org-sending-domain project flagged below. **Then Jeff: "we better make the email template selector a drop down list … if someone has a lot that will be a lot of screen real estate"** → the picker is a `<select>` (placeholder "Choose a template…", "Blank email", then each template with its rendered subject); choosing an entry sets `window.location.href` to the rendered `mailto:` and logs the activity as before; the select stays on its placeholder so the same template can be chosen twice; a line under it says out loud: "Opens in your own mail client with the fields filled in — nothing is sent until you press Send there." Scans updated (the select, the blank option, the option label with the subject, the choose path, the hint; the row list gone). Re-verified: six gates, 735/735, 153/153, 430/430 mutations, `index-s255BQ_M.js`. No schema change. **SHIPPED — the EIGHTEENTH ship.**

### 0.123 Lead scoring: read against the code — v1.5 AND Phase 2 already shipped; one default config, the engine in the harness, "Web Form" inbound, a readiness line (12 Sep, thirteenth session — Jeff: "this is a recommended enhancement by you. Lets work on lead scoring — finish Lead Scoring v1.5, which is spec'd, has its DB hooks already live (the leadId rescore in activities.mjs), and is a genuine differentiator at this price tier")

**What was found, before building anything (§0a8's own lesson, applied).** The recommendation Jeff quoted is the horizon entry §0a8 already struck: **v1.5 is live** — `activities.lead_id`, the lead-touch logging, `rescoreLead` on the activity POST/PUT/DELETE, five `op:'event'` rules, events passed by `leads.mjs`, `lead-intake.mjs` and the nightly batch. **And Phase 2 is live too**, though its horizon line still read as open: `score-lead.mjs` carries `leadFeatures` (five signals, `status` excluded as label leakage), `computeSourceWinRate`, `trainLeadModel` (per-org logistic regression, standardised, L2) and `predictLead`; `score-leads-batch.mjs` trains per org on its decided (Converted / Dead) leads once `predictive.enabled` and the threshold (`minClosedRecords`, default 150) are met, stores the model on `settings.extra.leadScoring.predictive.model`, and every lead's breakdown then carries a 0–100 `probability`; the panel has the switch, the threshold and the model status; the Leads tab shows "P<n>" and "Win probability" in the why-this-score popover. "License-gated like Dispatch" is the per-org Admin switch — the same shape as `dispatchEnabled`. Read-only on the shared database: every org with a saved config has ten engagement rules and no trained model (`model: null` — the jsonb null reads as present to `IS NOT NULL`, so count leads with a probability instead: zero); the dev org has 1 decided lead of 25, so the threshold is years away there.

**What was NOT right, fixed here (bundle AND functions; no schema change).** (1) **The panel kept a MIRROR of the engine's defaults and it had drifted:** five engagement rules where the engine has ten (no event rules), `minClosedRecords` 200 where the engine says 150, a predictive block shaped `{ lastTrainedAt, coefficients }` where the batch stores `model`. A brand-new org's first Save from that panel, or any "Reset to defaults", wrote the five-rule config — silently undoing v1.5 for that org. Now ONE copy, `src/utils/leadScoringDefaults.js` (pure), re-exported by the engine so every function and test keeps importing it from `score-lead.mjs`; the panel imports it (first load and Reset) and the mirror is gone. (2) **"Web Form" earned no inbound points** — the source the web-to-lead form stamps (§0.121) was not in `f_src_inb`; added (orgs with a saved config keep theirs — the Admin can add it there). (3) **The engine's own `tests/score-lead.test.mjs` (18 tests) was never in the harness's `SUITES`** (the §0.95 class): registered, with three engine mutants that it catches — event rules ignored, the bucket taking the lower axis, recency not decaying. (4) **`isDecidedLead` / `DECIDED_STATUSES`** name what the model trains on, shared by the batch and the panel. (5) **A readiness line in the panel:** "No model yet. <n> of <threshold> decided (Converted / Dead) leads so far — and predictive scoring is off." from the Admin's own lead list — before this an Admin had no way to see how far from training the org was.

**Not done, Jeff's call:** a "Train now" action for an Admin (the batch trains nightly and only above the threshold; a manual trigger would be a small Admin-only function); a lower threshold for a workspace that wants an early, noisier model (the field is editable today, 20 rows is the engine's floor); the batch's `extra` write is a read-then-overwrite of the whole settings blob and could lose a concurrent Admin save (a narrow window, once a night — not changed).

**Tests:** +1 file `tests/lead-scoring-defaults.test.mjs` (6: the engine re-exports the shared object, ten rules with five events and the predictive block, Web Form scores as inbound, decided statuses and the batch's use of them, the panel's import/no-mirror/Reset/readiness scans, a web lead scored end to end — New + fresh recency = 45 → warm), `score-lead` registered. **Mutants (5):** event rules ignored; the bucket takes the lower axis; recency stops decaying; Web Form drops out of inbound; the Demo rule vanishes. **Verified:** six gates (162 files; 91 function files), 741/741 unit, 153/153 integration, 435/435 mutations after a printed green baseline, build guard OK 2,495 kB `index-Af3POXBZ.js`, `dist/` cleared. **OBSERVED in the pane signed in as Jeff (19:50 UTC 12 Sep, `index-Af3POXBZ.js` served from 19:47:12):** Settings → Sales process → Lead scoring — 19 rule rows read from the controls, nine fit (four `matchesAny`, three `gte`, two `in`) and ten engagement (four `equals`, one `recency`, five `event` labelled Demo logged / Meeting logged / Meeting scheduled / Call logged / Email logged); the Predictive card: "No model yet. 1 of 150 decided (Converted / Dead) leads so far — and predictive scoring is off." The dev org's SAVED inbound rule still reads "Website, Webinar, LinkedIn" — a saved config keeps its own; Reset would bring "Web Form" (Jeff's call). **Seen and fixed the same hour (a follow-up commit):** the Engagement card's description still said "Behavioral events arrive in a later release." — stale since v1.5; now "Behavioral events are live: an ‘activity logged’ rule scores a Demo, Meeting, Call or Email logged against the lead, decaying by its half-life." (a first attempt with straight double quotes broke the JSX attribute — `check:tdz` and `check:dupes` both went red and the unit scan of the whole tree failed; typographic quotes, and the scan now pins the sentence). Re-verified: six gates, 741/741, 153/153, 435/435 mutations, `index-DS-lL6jZ.js`. **OBSERVED by Jeff on dev (12–13 Sep): "working as designed."**

**PROD SHIPPED — the EIGHTEENTH ship (Jeff: "push master", 13 Sep).** Pre-flight: tree clean, `dev` == `origin/dev` == `b54a2a7`, `origin/master` (`dab2655`) an ancestor of `dev`, 17 commits after it (seven of them code: `04ad57b`, `d936baf`, `59cd6c3`, `51675be`, `f422a92`, `cf752b5`, `0810167`); `git diff --stat origin/master..dev` over `db/schema.ts` EMPTY — no schema change; five function files (`lead-intake.mjs` new, `settings.mjs`, `send-slack.mjs`, `score-lead.mjs`, `score-leads-batch.mjs`), one `netlify.toml` rewrite, `package.json` (the integration-test list only), the bundle. `git push origin dev:master` at 13:14:46 UTC: `master` `dab2655` → `b54a2a7`. salespipelinetracker.com served `index-COkIzXNn.js` from 13:15:31 UTC (the hash poll) with the `pk_live_` key inlined. Landing proof by the served bundle's literals ("Web-to-lead form", "Copy embed code", "Email templates", "Choose a template…", "decided (Converted / Dead) leads so far", "Behavioral events are live" — all present) and by the functions: GET salespipelinetracker.com/lead-form/<junk> answered the intake function's own 404 page ("This form is not available", its CSP without frame-ancestors, noindex) — the rewrite and the new function are live on prod — and the JSON POST with an unknown token answered 404 { ok: false, error: 'Form not found' } (13:16 UTC). The Netlify deploy record was not read (no deploy id to hand); the served hash and the function's own answers are the proof. **What prod does differently from this moment:** an Admin can turn on the web-to-lead form (OFF for every workspace until then — the ship itself exposes no form and sends nothing), `/lead-form/<token>` and the intake function exist, a "New lead from the web form" Slack switch exists (§0.121); an Admin can write email templates and a rep's ✉ Email ▾ offers them, sent by the rep's own mail client (§0.122); the Lead scoring panel reads the shared defaults (ten engagement rules on Reset, "Web Form" inbound), the engine and batch use the same, the readiness line shows (§0.123). Every "NOT shipped" in §0.121–§0.123 above is superseded by this paragraph. **§0.121–§0.123 OBSERVED on dev by Jeff before the ship**; NOT yet observed on prod.

### 0.124 Light workflow automation — the stalled-deal signals reach the rules engine; a rule's task is linked and owned; one trigger vocabulary; update_field takes an allowlist (13 Sep, fourteenth session — Jeff: "work on this enhancement that was suggested by claude in an earlier conversation: light workflow automation ('when deal enters Proposal, create task X; alert if silent 14 days' — your stalled-deal cards show you already compute the signals, they just don't act)")

**What was found, before building anything.** An engine already existed — `automations` / `automation_runs`, `dispatch-automations.mjs`, Settings → Integrations → Automations with a four-step builder — and the first half of the ask was nearly expressible: `opportunity.stage_changed` + a condition + `create_task`. Read against the code, five things were not right. (1) **The hourly job (`pipeline-alerts.mjs`) computed silent / stuck / close-lapsed and emailed, texted and posted them, and never fired a rule** — "alert if silent 14 days" as an automation was impossible. (2) The panel offered `task.overdue` and `task.completed`; nothing fired either (tasks.mjs fired only the webhook). (3) The panel's condition field was free text ("Field (e.g. stage, arr)") — a typo is a rule that never matches, logged as skipped forever. (4) `create_task` wrote `account` and `notes`, two keys the tasks table has no column for (drizzle drops them silently), never linked the task to the deal (`opportunityId`), never stamped `ownerId` — and its placeholder promised "{account}" while nothing rendered a merge field. (5) **`update_field` wrote whatever column the rule named** — `set({ [p.field]: p.value })` — so an Admin's own rule with field `orgId` would move a deal into another tenant's workspace: a cross-tenant write by configuration (guide §18b37, new).

**What was built (functions AND bundle; no schema change).** `src/utils/automationEvents.js` (pure, new): `AUTOMATION_TRIGGERS` — ten: Pipeline created / stage_changed / won / lost; **Deal health** `opportunity.silent` / `.stuck` / `.close_lapsed` (kind `hourly`); Leads created / converted; Tasks completed; `task.overdue` dropped — `EVENT_FIELDS` / `conditionFields(trigger)` (the keys each event carries, doubling as merge fields), the builders `dealEventData` / `leadEventData` / `taskEventData` (stored row → snake_case payload, numbers as numbers, nothing undefined), `renderMerge` (`{{key}}`; an unknown or null key STAYS VISIBLE — the emailTemplates rule), `taskFromAction` (rendered title and notes; a due date in whole days from now, none for blank or negative; the rule's assignee else the deal's rep; `opportunityId` AND `relatedTo` for a deal event), `UPDATABLE_OPPORTUNITY_FIELDS` / `updateFieldPatch` (forecastCategory, probability 0–100, nextSteps, vertical, territory, team — never stage, org, owner, rep or money). **The engine:** refuses a trigger outside the vocabulary before any read (fail closed); `create_task` builds the row from the module and stamps `ownerId` via `resolveOwnerId(assignedTo, orgId)` — THIS org's roster; no match → unowned and the run says "no user named …"; ambiguous → unowned, "ambiguous" (reported, never guessed); `send_email` renders subject and body, escapes the body, and with a blank To reaches the lead's email or the deal's rep resolved in the org (one active match or nothing); `update_field` goes through the allowlist and its write is `and(id, orgId)`; the run-counter update is org-scoped too, so `dispatch-automations.mjs` left the org-scoping test's skip list; a skipped or refused action's reason lands on the run record (the panel's run history shows it). **The hourly job** fires the three signals right after each Slack post — inside the dedup block, so once per deal per signal per DEDUP_DAYS (7), at the rep's alert hour, for a deal whose rep is an active user with an email: the conditions under which the signal itself is raised (momentum and score drop do not fire — flagged). **Every call site** hands the engine the builder's payload — opportunities twice, leads twice, lead-intake — and tasks.mjs now fires `task.completed` behind its first-flip gate beside the webhook. **The panel:** the shared vocabulary in four groups with a line under Deal health saying how it fires; the condition field is a SELECT over the event's fields; a Notes field on a task action; a merge-field hint under task and email actions listing the event's keys; switching an action to create_task keeps the defaults its inputs show; the review names the trigger; the table names it and marks a rule saved under a retired trigger "(never fires)"; the footer says what is true.

**Not done, flagged:** `task.overdue` (nothing computes it — a signal for the minute job, if wanted); momentum and score-drop as triggers; a roster picker for "Assign to" (free text today, resolved by name in the org); Slack as an action; a rule fires only when the job raises the signal — a deal with no rep, or whose rep has no email or is inactive, raises none (the job's own rule, unchanged); the run record's `error` column carries skipped-action notes as well as errors (one column, shown in red in the run history).

**Tests:** +1 file `tests/automation-events.test.mjs` (13: the vocabulary, the fields, the three builders, renderMerge, taskFromAction ×2, updateFieldPatch, the engine scan, the scoping/stub scan, the job scan, the call-site scan, the panel scan), registered in `SUITES`; +1 integration file `tests/integration/dispatch-automations.itest.mjs` (5: a Proposal move in A creates ONE task in A owned by A's "Karen Rep" — B holds a user of the same name and gets no task, B's rule never ran, the paused rule never ran, the counter moved in A alone; a skipped run; the silent signal — the `orgId` write refused, the allowlisted write landed, the rep emailed with merge fields rendered and HTML escaped; an unknown trigger is a no-op; a ghost assignee leaves the task unowned and says so). **Mutants (9):** the silent dispatch removed; the trigger check dropped; ownership not stamped; update_field writes the rule's column; orgId joins the allowlist; an unknown merge field vanishes; the deal link lost; a negative offset dates the past; the condition field free text again. **Verified:** six gates (163 files; 91 function files), 754/754 unit, 158/158 integration, 444/444 mutations after a printed green baseline, build guard OK 2,499 kB `index-CDir98BZ.js`, `dist/` cleared. **OBSERVED IN PART by Jeff on dev (13 Sep, minutes after landing): "created. moved the deal. task created for Karen (account was assigned to her)" — the rule built through the new picker, a deal moved into Proposal, the task created for the deal's rep. Still his: the run history rows, the task as Karen, update_field with forecastCategory and the refused orgId. **Seen by Jeff in the pane the same hour, fixed the same hour (a follow-up commit, bundle only):** "the pop up screen when you hit this 3 button is being cut off by the top frame. I am sure if we had a list the bottom one would be cut off by bottom frame." The row's ⋯ menu was `position: absolute` inside the Rules card, which is `overflow: hidden`; it opened UPWARD for the last three rows — with one rule, the only row — and the card clipped it at its top edge; a long list would have clipped the last rows' menus at the bottom edge. Now the popover rule (`getBoundingClientRect()` + `position: fixed`, the CLAUDE.md convention): `menuPlacement(rect)` (module scope) puts the menu below the button when it fits the viewport and above it otherwise, at the button's right edge; a scroll (capture) or resize closes it, since a fixed menu does not follow the page. The same absolute-inside-overflow pattern is in `WebhooksDetail.jsx` and `ApiKeysDetail.jsx` — flagged, not changed. Also seen in his run-history screenshot and flagged: "Triggered by" shows the deal's raw id (the run row stores only the id — a name would be a nullable column or a lookup), and the table's RUNS column counts executed runs only, so a Skipped row is in the history but not in the number (pre-existing). +1 scan, 1 mutant; six gates, 755/755 unit, 445/445 mutations after a printed green baseline, build guard OK 2,499 kB `index-DqfNZYvt.js`. **LANDED on dev** — accelerep.netlify.app served `index-DqfNZYvt.js` at 19:19:41 UTC (the hash poll). **OBSERVED IN PART by Jeff on dev before the follow-up:** the rule built, a deal moved into Proposal, the task created for Karen (the account's rep), a move to another stage a Skipped row beside the Success row in the run history (his screenshot). Still his: update_field with forecastCategory and the refused orgId. **The menu follow-up OBSERVED by Jeff on dev: "the menu is working correctly."** **The ownership check OBSERVED by Jeff (three screenshots, 13 Sep):** the run history with three rows (Success · 1 action, then two Skipped · 0 actions for moves to other stages); **Karen's Tasks tab (Mine, All time): "Send proposal to ZZFX Beacon Metals" due Tuesday — her account's deal — and nothing of Jeff's**; Jeff's Tasks tab (Mine): "Send proposal to ZZTest Alpha Industries" due Tuesday — his account's deal. Each rep sees the task the rule made for their own deal and not the other's: ownership by user id. **Third follow-up, the same session (a third commit, bundle only — Jeff: "before we wrap this session I would like you to fix the opp names in the run history and fix the ... pop ups that overrun the frame you found"; then, on the Update field check: "it did not change it to commit"; then "I just opened the app in the claude browser - check my automation").** (1) **One row-menu hook for the three integration panels:** `menuPlacement` and `useRowMenu(btnPrefix, menuPrefix)` moved to `integrations/shared.jsx`; Automations, Webhooks and API keys all render their ⋯ menu `position: fixed` through it (below the button when it fits, above otherwise; outside click, scroll and resize close it); the three direction-assuming arrows are gone; the two panels' local outside-click effects are gone. (2) **The run history names the record:** the run row stores only the id, so `recordName(run, lists)` (module scope) looks it up by the trigger's entity in the lists the app already holds (`useApp()` — opportunities by name or account, leads by name or company, tasks by title) and falls back to the id — never blank; the id stays on hover. (3) **The Update field action saves what it shows** — read in the pane through the page's own session: Jeff's "Field Test" rule had saved `params: {}` (no entity, no field, no value) and its one run read "update_field: skipped — unsupported entity" — the entity select DISPLAYED "Opportunity" but a type switch stored `{}` (the same display-versus-stored gap as the task's due days). Now a switch to Update field stores `{ entity:'opportunity', field:'', value:'' }`; the Field is a SELECT over the allowlist (`UPDATABLE_FIELD_OPTIONS`, the same six keys — the flagged dropdown, done); Create refuses an update-field action with no field and a webhook action with no URL; the review line shows "field = value" or "no field chosen"; and `updateFieldPatch` reads a missing entity as opportunity (the only one) so an older rule with a field and no entity works. **His "Field Test" rule is still the empty one — delete it and build it again with the dropdown.** +3 scans (one rewritten), 5 mutants; six gates, 757/757 unit, 450/450 mutations after a printed green baseline, build guard OK 2,499 kB `index-DpgxUDFK.js`. **LANDED on dev** — accelerep.netlify.app served `index-DpgxUDFK.js` at 19:42:53 UTC (the hash poll). **OBSERVED in the pane (Jeff had opened the app in the Claude browser, signed in as himself; the tab read `index-DpgxUDFK.js` after a reload):** Settings → Integrations → Automations → the first rule's ⋯ → the menu opened fully BELOW the button, outside the card (Pause, View run history, Delete); View run history → five rows reading "Triggered by: ZZFX Ashgrove Renewal", "ZZTest Alpha Renewal" (Success and Skipped), "ZZFX Dovetail Upgrade", "ZZFX Beacon Trial" — names, not ids. **The rebuilt Field Test rule OBSERVED by Jeff (two screenshots, 13 Sep):** the deal ZZFX Ashgrove Renewal moved to Negotiation/Review with Forecast category reading **Commit** (0 days in stage, the rule's write on the reloaded record), and the rule's run history "Success · 1 action · Triggered by: ZZFX Ashgrove Renewal". And the guard (his third screenshot): a new rule "Test2" with Update field and no field chosen — the review line "Update field — no field chosen" in red and Create refused with "Update field: choose the field to set". Every check from the §0.124 walkthrough is now observed on dev except the Webhooks and API keys menus (the same hook as the Automations one he saw). **PROD SHIPPED — the NINETEENTH ship (Jeff: "ship to master", 13 Sep).** Pre-flight: `dev` == `origin/dev` == `b2218b2`, `origin/master` (`b54a2a7`) an ancestor of `dev`, 14 commits after it (three code: `427fd65` §0.124, `892c978` the menu follow-up, `ba9aefb` the third follow-up), `git diff origin/master..dev -- db/schema.ts` EMPTY — no schema change; seven files under `netlify/` and `package.json` (the six functions of §0.124 and the integration-test list), `netlify.toml` untouched; the working tree carried the UNCOMMITTED fourth follow-up (Edit an existing automation, its harness still running) — a push of `dev:master` ships the committed branch only, and that follow-up is NOT in this ship. `git push origin dev:master` at 19:59:53 UTC: `master` `b54a2a7` → `b2218b2`. salespipelinetracker.com served `index-BfMnj4VB.js` from 20:00:24 UTC (the hash poll, 31 s after the push; was `index-COkIzXNn.js`). Landing proof by the served bundle's literals — "Deal gone silent (no activity for 14 days)", "Merge fields:", "Update field: choose the field to set", "Choose a field…", "no field chosen", "never fires" all present; `pk_live_` inlined (the one `pk_test_` string is Clerk's SDK constant `s4="pk_test_"`, not a key) — and by the function: GET `/.netlify/functions/automations` on prod answers 401 `{"error":"Unauthorized: no token"}` (the function is deployed and behind Clerk). The Netlify deploy record was not read. **What prod does differently from this moment:** the hourly job fires `opportunity.silent` / `.stuck` / `.close_lapsed` rules at each rep's alert hour (prod alone runs the jobs — `JOBS_ENABLED`); a rule's task is linked to the deal and owned by the assignee; `task.completed` fires; the panel offers the shared vocabulary, a field select, merge fields, notes; `update_field` is allowlisted (an `orgId` write refused); the row menus are fixed popovers in three panels; the run history names the record; the Update field action saves what it shows. **The three ships' rules on prod are Jeff's to create** — prod holds no automation rule until an Admin makes one. Every "NOT shipped" above for §0.124 and its first three follow-ups is superseded by this paragraph; the fourth follow-up (Edit) is NOT shipped. **NOT yet observed on prod.** **Fourth follow-up, the same session (a fourth commit, bundle only — Jeff: "There is not an edit option for an existing automation. Can we add that").** The ⋯ menu gains **Edit** (above View run history): the four-step modal is now ONE component, `AutomationModal({ rule, onClose, onSaved })` — a new rule with no `rule`, an edit seeded from it (name, trigger, conditions, actions; an empty action list falls back to the default task action) — saved through the endpoint's existing PUT with the rule's id (POST for a new one); the header reads "Edit automation" with the past run count, the button "Save changes", and while editing every step is opened directly from the stepper. The list replaces the edited row in place (a new rule is still prepended); closing forgets the rule. No endpoint change: `automations.mjs` PUT already applied name, triggerEvent, conditions, actions and active, org-scoped. +1 scan, 2 mutants; six gates, 758/758 unit, 452/452 mutations after a printed green baseline, build guard OK 2,500 kB `index-De3J8tLC.js`. **LANDED on dev** — accelerep.netlify.app served `index-De3J8tLC.js` at 20:03:21 UTC (the hash poll). **OBSERVED by Jeff on dev: "edit works and the field test rule is deleted."** The dev org keeps "Proposal > Task" alone. **NOT shipped** — `dev` ahead of `origin/master` by this ONE code commit (`b88ee9d`, bundle only) and the docs; a ship is `git push origin dev:master`, Jeff's call. NOT shipped.** **SHIPPED — the TWENTIETH ship (Jeff: "ship to master", 13 Sep), together with §0.125: `master` `b2218b2` → `7fd83de` at 20:43:16 UTC; salespipelinetracker.com served `index-DdSH6fY8.js` from 20:43:41 UTC (the hash poll), "Edit automation" in the served bundle. The "NOT shipped" for this follow-up above is superseded by this line; OBSERVED on prod by Jeff ("tests passed - edit and change org").**

### 0.125 Every mount-time load keys on an active org — the §9 "401 then 200" race read and fixed, and the org switch made honest on the client (13 Sep, fifteenth session — Jeff picked it from the flagged list: "Fix the 401-then-200 load race")

**What it was.** Recorded at §0.109's pane pass (§9, 9 Sep): `coaching-notes`, `calendar-events` and `spiff-claims` each GET once before the Clerk token (401, "DB error 401" in the console) and once after (200) — "a load-order race in the hooks that fire on mount, unread as to which hook fires early." Read against the code this session: all three call sites already awaited `waitForToken()`; what set them apart from every other load was the GATE. The main load (`App.jsx`) runs only when `clerkUser && organization?.id`; the spiff-claims effect ran on mount (`[]`), the coaching hook's effect on mount, the calendar strip's on the Home tab — each on `waitForToken` alone, and `waitForToken` gives up after 8 seconds and resolves ANYWAY (its own comment: "resolve anyway — dbFetch will send without token and get 401"). So whenever no org was active for eight seconds — the in-app `<SignIn />` with Clerk's MFA step, the "No Organization Found" page — the three requests went out with no Authorization header, the function answered 401 "no token", and nothing retried: a `[]` effect runs once. The 200 that followed in the pane's log was the NEXT page load (Clerk's redirect after sign-in) — the pane's request log spans page loads (three loads read this session; every request appears once per load). Reproduced against dev as Jeff (Admin, `index-De3J8tLC.js`, warm reloads): every function GET once, all 200, no 401 — the race needs the slow path, and the code is the proof of it. The same read found a second thing: the header renders Clerk's `OrganizationSwitcher`, the main load re-runs when `organization` changes (`[clerkUser, organization]`), and NONE of the three did — after a switch the previous org's spiff claims, coaching notes and calendar strip stayed on screen until a refresh. A tenant-isolation gap on the client: the server never returned another org's rows; the client kept them.

**What was built (bundle only; no function, no schema change).** `App.jsx` derives ONE gate, `const activeOrgId = (clerkUser && organization?.id) || null` — the active-session user (0.65) and the active org — declared after the token getter's effect so the getter is set first in the same commit. The spiff-claims effect keys on it: clears the list (another org's claims never survive a switch), returns with no org, otherwise waits for the token and loads, dropping a load in flight when the org changes; a refused response is not parsed as claims (`r.ok ? r.json() : null`). `useCoachingNotes` takes `orgId` (null by default → no load): its effect resets the list and the loaded flag, returns with no org, loads for THIS org, and re-runs per org. The calendar strip's Home-tab auto-fetch returns with no org and, remembering the org it fetched for (`calendarOrgRef`), re-arms its once-per-session guard when the org changes. `waitForToken`'s give-up now warns ("waitForToken: no Clerk token after 8 s — the request will go out unauthenticated (401)") before it resolves — with every load gated it should never print; if it does, a caller is fetching before sign-in again. Every other `waitForToken` caller (documents, quotes, settings, the profile read, DispatchTab) already runs inside the main load or inside a tab, and App renders the tabs only after `clerkUser` and `organization` (its two early returns) — none can reach the give-up.

**Not done, flagged:** §9's neighbour, the `GET /users` 403 noise for non-admin roles (reps now receive a directory read — re-check whether anything still 403s before spending time on it); the calendar strip keeps the previous org's events on screen for the length of the new fetch (the fetch replaces them; a clear-first would flash "no meetings"); `waitForToken` still resolves on give-up rather than rejecting — a rejection would surface as an unhandled rejection in `loadData`, which has no catch around its await, so the loud resolve is the safer path; the org-switch reload is unobserved (Jeff's account on dev holds the orgs; a switch is his to make).

**Tests:** +1 file `tests/auth-gated-loads.test.mjs` (6: the gate and its position after the active-session user; the spiff effect's gate, clear, ok-check, cancel and deps; the hook told the org, its gate, reset and deps; the calendar effect's gate, switch detection and deps; the main load's gate and the getter effect's position; `waitForToken` — immediate with a getter, resolved once one appears, the loud give-up), registered in `SUITES`; the coaching-notes scan re-pinned to the new call. Five mutants: the spiff gate removed; the hook not told the org; the hook's gate dropping the org; the calendar deps losing the org; the give-up silent again.

**Verified:** six gates (163 files; 91 function files), **764/764 unit** (6 new), **158/158 integration**, **457/457 mutations after a printed green baseline**, build guard OK 2,500 kB `index-s-O_Wnog.js`, `dist/` cleared. **LANDED on dev** — accelerep.netlify.app served `index-s-O_Wnog.js` from 20:26:50 UTC 13 Sep (the hash poll, 30 s after the push of `83646ca` at 20:26:20). **OBSERVED in the pane (Jeff signed in as himself, Admin; three orgs in the switcher):** a reload on the new bundle fired spiff-claims, coaching-notes and calendar-events ONCE each, all 200, no 401, no give-up warning in the console; the avatar strip's OrganizationSwitcher → Accelerep (`org_3B8Lh…`): no page load, the token's org changed, the main load re-ran AND the three fired again, once each, 200 — the previous org's rows replaced (the panel showed that org's blank profile); back to Accelerep Test (`org_3BDQ…`): the same once more, Jeff's profile back. **OBSERVED by Jeff as Karen (role User) — his screenshot, 13 Sep, accelerep.netlify.app:** Home after signing in as Karen shows "Notes from your manager" with Jeff's 3 Sep note (coaching-notes loaded), Spiffs → My claims with two APPROVED claims (spiff-claims loaded) and the "Connect your calendar" prompt (calendar-events answered, no calendar connected) — the three loads populated on a rep's Home with no refresh; the console is not in the screenshot, and whether the sign-in went through the in-app form (the 8-second path) or an existing session is not stated. **Then the slow path itself, OBSERVED by Jeff (13 Sep): "logged in through private window and received no errors" — a fresh sign-in as Karen through the in-app form, no 401 in the console. Every check of §0.125 is observed.** **SHIPPED — the TWENTIETH ship (Jeff: "ship to master", 13 Sep).** Pre-flight: `dev` == `origin/dev` == `7fd83de`, `origin/master` (`b2218b2`) an ancestor of `dev`, 11 commits after it (two CODE: `b88ee9d` the Edit follow-up, `83646ca` this batch — both bundle only), `git diff origin/master..dev -- db/schema.ts` EMPTY, nothing under `netlify/` in the diff, tree clean. `git push origin dev:master` at 20:43:16 UTC: `master` `b2218b2` → `7fd83de`. salespipelinetracker.com served `index-DdSH6fY8.js` from 20:43:41 UTC (the hash poll, 25 s after the push; was `index-BfMnj4VB.js`). In the served bundle: the give-up literal ("waitForToken: no Clerk token after 8 s") once, "Edit automation" once, `pk_live_` inlined (the one `pk_test_` string is Clerk's SDK constant, as at the nineteenth ship); the automations function answers 401 unauthenticated on prod (deployed, behind Clerk; no function changed in this ship). **What prod does differently from this moment:** the spiff-claims, coaching-notes and calendar loads key on the active org — nothing before sign-in completes, again per org, the previous org's rows cleared — and an automation rule can be edited from its ⋯ menu. ~~**NOT yet observed on prod.**~~ **OBSERVED on prod by Jeff (13 Sep): "tests passed - edit and change org" — Edit on an automation rule and the org switch reloading Home, both on salespipelinetracker.com. The rep sign-in in a private window on prod is not stated (observed on dev the same hour).** The Netlify deploy record was not read. Rep-path note: this change has no ownership or role branch — the gate is auth readiness, the same for every role — so an Admin's read of the request log after the landing is the meaningful browser check; the org-switch reload is Jeff's (§5 of the handoff).

### 0.126 The automation's Assign-to is a roster picker stored by id and name; the calendar strip clears on an org switch; §9's users-403 item struck by code (13 Sep, fifteenth session — Jeff: "lets just keep fixing those here")

**What it was.** Flagged at §0.124: "a roster picker for Assign to (free text today, resolved by name in the org)". A typed name that matched no roster row left the task unowned (the run record said so, but unowned); a name two users share was ambiguous and unowned; and a rep renamed in the roster after the rule was saved stopped matching. Flagged at §0.125: the calendar strip kept the previous org's meetings on screen for the length of the new fetch after a switch. And §9's "client skips `GET /users` for non-admin roles (currently eats an expected 403 with console noise each load)" — read against `users.mjs`: a non-admin GET has returned 200 directory rows (id, name, active) since §0.109's directory read, so there is no 403 to skip; struck by code, nothing to fix.

**What was built (functions AND bundle; no schema change — the two keys live in the rule's `actions[].params` JSON).** `src/utils/automationEvents.js` (pure): `assigneeOptions(roster, params)` → the select's value and options — "The deal's rep (default)" first, then the org's active users by name (base sensitivity, then id, so two names differing only by case order deterministically); the value is a saved id that is in the roster, else a saved name matched to ONE user case-insensitively, else the saved name kept as a `name:` value labelled "by name — not in the roster" or "by name — more than one user", so a rule saved before the picker is never blanked; `assigneeParams(value, roster)` → both keys at once (`assignedTo` the roster name, `assignedToId` the id; a `name:` value the name alone; an unknown id nothing). The panel's task action renders a SELECT (`ActionEditor` takes `roster` and `setAssignee`; the modal reads `useApp().settings.users` — the roster an Admin already holds — and `setAssignee` spreads both keys into the params); the review line names the assignee. `_lib.mjs` exports `rosterUserById(id, orgId)` — THIS org's cached roster, else null; an orgId is required. The engine (`dispatch-automations.mjs`) tries the id first: in the event's org → owned by it and `assignedTo` refreshed to the roster's current name (rename-proof, never ambiguous); a miss (no id, or an id from any other org) → the §0.124 name path unchanged, with its notes. `App.jsx`: the calendar effect's switch branch clears the events before re-arming the fetch.

**Not done, flagged:** the picker offers every active roster user, Technicians included (a task can be owned by anyone; a role filter is a product call); the Update field and Send email actions keep their typed inputs; the rest of §0.124's list — Slack as an action, `task.overdue`, momentum and score-drop triggers.

**Tests:** +2 in `tests/automation-events.test.mjs` (the two helpers — the default, an id, a one-name match, legacy and ambiguous names, both keys always, an unknown id never stored; the panel / engine / `_lib` scans), the engine's and the panel's import pins re-pinned; +1 line in `tests/auth-gated-loads.test.mjs` (the calendar clear); +1 integration test in `tests/integration/dispatch-automations.itest.mjs` (A's Karen by id with a stale name → owned by the id, the name refreshed; B's Karen's id in A's rule → a miss in A, the name resolves in A; B still empty). Five mutants: `_lib` trusting any id; the engine's id path dead; the picker storing the id alone; an inactive user offered; the calendar not cleared.

**Verified:** six gates (163 files; 91 function files), **766/766 unit** (2 new), **159/159 integration** (1 new), **462/462 mutations after a printed green baseline**, build guard OK 2,502 kB `index-qwmC8w1K.js`, `dist/` cleared. **LANDED on dev** — accelerep.netlify.app served `index-qwmC8w1K.js` from 21:03:25 UTC 13 Sep (the hash poll, 41 s after the push of `324c04e` at 21:02:44). **OBSERVED in the pane (Jeff signed in as himself, Admin, Accelerep Test):** Settings → Integrations → Automations → + New automation → a name, Stage changed, Next, Next → the Actions step's "Assign to" is a SELECT reading "The deal's rep (default)", then Jeff Russell, Karen Russell, Ryan Algie, Savannah Miller — the org's roster by name; the modal closed without saving (no rule created). NOT observed: a rule saved through the picker and its task owned by the id (the integration test proves the engine; Jeff's check is in the handoff's §5). ~~NOT shipped.~~ **SHIPPED — the TWENTY-FIRST ship, 21:47:59 UTC 13 Sep (§0.128, last paragraph).**

### 0.127 Post to Slack as an automation action; deal momentum and AI score drop fire the engine (13 Sep, fifteenth session — the next two of §0.124's flagged list, Jeff: "lets just keep fixing those here")

**What it was.** Flagged at §0.124: "momentum and score-drop as triggers; Slack as an action". The hourly job (`pipeline-alerts.mjs`) computed five deal signals and posted all five to Slack, but fired the engine for three (silent, stuck, close-lapsed) — a rule on a deal gaining momentum or on an AI score drop could not exist. And the engine's four actions had no Slack: a rule could email, fire a raw webhook, create a task or set a field, while the org's Slack webhook (Settings → Connected apps) sat one import away.

**What was built (functions AND bundle; no schema change).** Two hourly triggers in the shared vocabulary: `opportunity.momentum` ("Deal gaining momentum (2+ stages in 14 days)", fields `stage_count`, `days_since_created`) and `opportunity.score_drop` ("AI score dropped below 40", fields `score`, `verdict`); the job fires each right after its Slack post, inside the same dedup block (once per deal per signal per week, at the rep's alert hour — the pattern §0.124 set), so five signals now fire and the Deal health group offers five. The `send_slack` action: the engine renders `params.message` with the event's merge fields (a blank message still says what fired: "Automation: <subject>") and posts through `sendSlackToOrg(orgId, { text })` with NO alert type — the rule IS the switch; the org's webhook and its master switch still gate the post, and an unconnected Slack is a skipped action whose run record says "Slack is not connected for this workspace" (never an error). The panel offers "💬 Post to Slack" with a Message textarea, the merge-field hint and a note on where the webhook lives; a type switch stores `{ message:'' }`; Create refuses a blank message ("Post to Slack: a message is required"); the review reads → Slack: "…"; the footer lists the new triggers and the action.

**Not done, flagged:** `task.overdue` (still nothing computes it — the hourly job could scan open tasks past due, once per task per week; a product call on the dedup); the Slack action posts text only (no blocks — the pipeline alerts use `slackTemplates`; a rule's message is the Admin's own words); the Update field and Send email actions keep their typed inputs.

**Tests:** the vocabulary asserts raised (twelve triggers, five hourly, the two new field sets), the job scan's pair list and count (five signals), the panel footer pin; +1 unit test (the engine's Slack case — no alert type, the skip reason — and the panel's option, textarea, default, guard and review); the integration suite stubs `send-slack.mjs` and the silent-deal rule gains a Slack action: one post to A's Slack, merge fields rendered ("Acme — HVAC has been silent 16 days"), no alert type, the run's executed count 3. Four mutants: the post gated by a pipeline switch; a blank message accepted; the momentum dispatch removed; the score-drop dispatch removed.

**Verified:** six gates (163 files; 91 function files), **767/767 unit** (1 new), **159/159 integration**, **466/466 mutations after a printed green baseline**, build guard OK 2,503 kB `index-CiwV447T.js`, `dist/` cleared. **LANDED on dev** — accelerep.netlify.app served `index-CiwV447T.js` from 21:17:42 UTC 13 Sep (the hash poll, 41 s after the push of `ac50a91` at 21:17:01). NOT observed in a browser (the pane went to §0.128 the same hour; the Slack action and the two triggers are Jeff's checks in the handoff's §5 — the triggers on prod only). ~~NOT shipped.~~ **SHIPPED — the TWENTY-FIRST ship, 21:47:59 UTC 13 Sep (§0.128, last paragraph); the two triggers now run on prod's hourly job.** **The two new signals cannot be observed on dev** — `JOBS_ENABLED` is set on prod alone (state §0.100); a rule on them fires there after a ship, at the rep's alert hour, for a deal that meets the signal, once per week.

### 0.128 The lead score's "Why this score" popover is a fixed popover — the Triage rows no longer clip it or grow a scrollbar (13 Sep, fifteenth session — Jeff: "When I click on a lead score the review gets truncated ... and a side slider is added to the window ... only ... in these single line categories (call today, needs first touch, and working). The all leads works the way I would like")

**What it was.** `LeadScore` (LeadsTab.jsx) rendered its breakdown card `position: absolute; top: 100%` under the badge. The Triage rows — Call today, Needs first touch, Working — are horizontal card strips with `overflow: auto`, so the card was clipped at the strip's bottom edge and the strip grew a scrollbar to hold what it clipped (his screenshot: "WHY THIS SCORE" cut off after two rows, a scrollbar at the right of the strip). The All leads table has room below each row, so the same card looked right there. The same absolute-inside-overflow pattern as the automation row menu (§0.124's follow-up, then `useRowMenu` for three panels).

**What was built (bundle only; no function, no schema change).** `src/utils/popoverPlacement.js` (pure, new): `popoverPlacement(rect, { width, height, gap = 6, margin = 8, viewport })` → `{ left, top }` below the trigger when the height fits the viewport, `{ left, bottom }` above it when that fits, else as high as needed for the bottom edge to stay on screen and never above the top margin; the left edge is the trigger's, kept `margin` px inside the viewport on both sides; the viewport is injectable, so the arithmetic is unit-tested. `LeadScore` now opens the card `position: fixed` at that placement (measured from the badge's `getBoundingClientRect()` at the click, width 232, a 300 px height allowance), and closes it on an outside click (the wrapper contains both the badge and the card, so a click inside either stays open), a scroll anywhere (capture) or a resize — a fixed box does not follow the page. Before, only a second click on the badge closed it. The table's copy uses the same code and is unchanged to the eye. `menuPlacement` (integrations/shared.jsx) stays as it is — it anchors to the right edge for a row menu; a fold of the two is a small follow-up.

**Not done, flagged:** fold `menuPlacement` into `popoverPlacement` (an `align: 'right'` option); the card's height allowance is a constant (300) — a card with many signals still places by that number (measure after mount if it ever matters); the other absolute popovers in the tabs (RepPickerPopover, the status picker) were not audited this batch.

**Tests:** +1 file `tests/lead-score-popover.test.mjs` (2: the placement — below, above, both edges clamped, neither fits taller than the viewport, neither fits shorter, room above only, a null rect, gap and margin as parameters; the LeadScore scan — the import, viewport state, placement at open, `position: 'fixed'`, the absolute card gone, the three listeners on and off, the wrapper ref, the toggle), registered in `SUITES`. Four mutants: always below; the left edge unclamped; absolute under the badge again; no scroll listener.

**Verified:** six gates (164 files; 91 function files), **769/769 unit** (2 new), **159/159 integration**, **470/470 mutations after a printed green baseline**, build guard OK 2,504 kB `index-838pOcA_.js`, `dist/` cleared. **LANDED on dev** — accelerep.netlify.app served `index-838pOcA_.js` from 21:26:43 UTC 13 Sep (the hash poll, 31 s after the push of `e2dceca` at 21:26:12). **OBSERVED in the pane (Jeff signed in as himself, Admin; Leads → Triage in "Mine", the Working strip one card wide at the pane's width):** the 57 badge → the card opened `position: fixed` at viewport (35, 524), 232 × 200, directly under the badge (496 + 22 + 6), its offset parent the viewport, painted in full over the Distribute leads panel beside the strip; the strip's scrollHeight stayed equal to its clientHeight (132) — no scrollbar grew; a click elsewhere closed it and the strip was unchanged. (A first read one second after the click showed the strip grown and the card 485 px low — a measurement taken mid-layout; the settled read and the screenshot are the record. **Read again after Jeff's report: that first read was the HOVER case, not mid-layout — the pane's pointer was still over the card from the click; see the follow-up below.**) NOT shipped.

**Follow-up, the same hour — the popover is a PORTAL (Jeff, on the first cut: "when i click on the lead score nothing pops up except a scroll bar on the right ... on dev after hard refresh in brave or in edge"; then: "oddly the list appears and the scroll bar disappears if I move the cursor off the lead after clicking").** Read against the code: the Triage card (`LeadCard`, LeadsTab.jsx) lifts on hover — `transform: hov ? 'translateY(-1px)' : 'none'` — and a transformed ancestor is the CONTAINING BLOCK for a `position: fixed` descendant. With a real pointer over the card, the fixed popover was positioned relative to the card, not the viewport: back inside the strip, clipped, and the strip grew a scrollbar to hold it; the cursor leaving the card dropped the transform and the box snapped to the viewport — his second observation exactly. The pane's clicks are synthetic and never hover, which is how the first pass measured a correct box and missed it; reproduced in the pane with a `hover` action first: the card's computed transform read `matrix(1, 0, 0, 1, 0, -1)`. Now the popover renders through `createPortal(…, document.body)` (the precedent: TimeDropdown, ContactsTab, DocumentsTab, IndustriesDetail), so no ancestor's transform can contain it; the outside-click check covers the wrapper AND the portal's box (`cardRef`), since the box is no longer a DOM child of the wrapper. The hover lift stays. +6 scans in the test file (the import, the portal, the body, the ref, the two-ref outside click, the lift still present), 1 mutant (the portal removed — the box a child of the card again). Six gates, **769/769 unit**, **159/159 integration**, **471/471 mutations after a printed green baseline**, build guard OK 2,504 kB `index-BIIXAgGT.js`, `dist/` cleared. **LANDED on dev** — accelerep.netlify.app served `index-BIIXAgGT.js` from 21:43:24 UTC 13 Sep (the hash poll, 32 s after the push of `eec5ae2` at 21:42:52). **OBSERVED in the pane under Jeff's conditions (a real pointer over the card):** Leads → Triage → All → the pointer hovered over the Call today card (its computed transform `matrix(1, 0, 0, 1, 0, -1)` — the lift) → a real click on the 100 badge with the pointer still there → the card still transformed at the measurement, the popover's parent `document.body` (not inside the card), `position: fixed` at (37, 477) — the badge's bottom 471 + 6 — 232 × 241, on screen and painted in full over the Distribute leads panel; the strip's scrollHeight equal to its clientHeight (132): no scrollbar. Before this follow-up the same sequence put the box inside the card. **OBSERVED by Jeff on dev (13 Sep): "working perfectly."** **SHIPPED — the TWENTY-FIRST ship (Jeff: "ship to master", 13 Sep), with §0.126 and §0.127.** Pre-flight: `dev` == `origin/dev` == `ae0cae8`, `origin/master` (`7fd83de`) an ancestor of `dev`, `git diff origin/master..dev -- db/schema.ts` EMPTY, three function files in the diff (`_lib.mjs` +12, `dispatch-automations.mjs` +21/−2, `pipeline-alerts.mjs` +2 — Netlify rebuilds every function), four CODE commits (`324c04e` §0.126, `ac50a91` §0.127, `e2dceca` and `eec5ae2` §0.128), tree clean. `git push origin dev:master` at 21:47:59 UTC: `master` `7fd83de` → `ae0cae8`. salespipelinetracker.com served `index-BnV7qKA1.js` from 21:48:30 UTC (the hash poll, 31 s after the push; was `index-DdSH6fY8.js`). In the served bundle: "The deal's rep (default)" once, "Post to Slack" twice, "Deal gaining momentum" once, "AI score dropped below 40" once, "Why this score" once, `pk_live_` inlined (the one `pk_test_` string is Clerk's SDK constant); the automations function answers 401 unauthenticated on prod (deployed, behind Clerk). **What prod does differently from this moment:** an automation's Assign to is a roster picker and the engine owns the task by the id in the event's org first; a rule can post its own message to the org's Slack; deal momentum and AI score drop fire rules from the hourly job (prod alone runs it — `JOBS_ENABLED`), once per deal per signal per week, at the rep's alert hour; the calendar strip clears on an org switch; the lead score's "Why this score" card is a fixed popover through a portal, never clipped by the Triage strips. **NOT yet observed on prod.** The Netlify deploy record was not read. **The rule this adds to the popover rule (guide §22 / CLAUDE.md "Popovers: getBoundingClientRect() + position: fixed"):** fixed is not enough under an ancestor that may carry a transform, filter, perspective or `will-change` — a hover lift is the common one — so a popover under any styled card renders through a portal to `document.body`; the placement math is unchanged. The guide had this rule already (§16, the kebab/dropdown menu rule: portal + fixed, "any ancestor with a CSS transform ... traps position:fixed"); CLAUDE.md's one-line index of it omitted the portal — the index line now says so (an index-versus-doc disagreement, fixed in this commit). **Flagged, not done:** the three integration row menus (`useRowMenu`, §0.124's follow-up) are fixed WITHOUT a portal — correct today because no ancestor of those rows transforms, and the same latent trap if one ever lifts on hover; a portal there is the same small change.

### 0.129 The "Tonal & Form" signal-system migration — Phase 0 (the spine collision) and Phase 1 (the colour audit) delivered; NOTHING migrated (13 Sep, sixteenth session — Jeff: "I had claude design update the look of the application. I have attached the design files for your reference. I would like to make these changes.")

**What it is.** Jeff had Claude Design restyle the app's signals and attached its handoff, `handoff-signal-tonal/` — copied into the repo at `docs/design/handoff-signal-tonal/` so the spec is committed truth. The deliverable is `src/tokens-signal-tonal.jsx`: `RAMP` (a `tint` / `base` / `deep` step per semantic — rise, fall, attend, critical, info, neutral — each step with exactly one job: tint = fills only, base = strokes and lines, deep = text on stone and fills behind light text), `SIGNAL` (flat aliases plus one near-black `focus`), `FORM` (signal pills SOLID deep fill with light text at radius 2; a 3px inset left spine on a flagged row; 1/2/3-of-3 severity dots; bare trend deltas with no pill; sparkline area fills; deep left borders on metric cards), `SPINE_CONFLICT`, `SIGNAL_MIGRATION` (ok → rise; warn → attend; danger SPLITS three ways — fall, critical, and destructive which stays `T.danger`; info new; inkMuted-as-status → neutral) and `SIGNAL_EXCLUSIONS` (gold, the ink text ramp, the header band, file types, document categories, tiers, stages — categorical and ordinal keep their tinted ROUNDED pill form on purpose, so a signal is distinguishable from a label). README §4 is a phased prompt with two gates of its own: Phase 0 (report every "selected" left spine, propose a resolution, STOP), Phase 1 (audit only, change no files, STOP), Phase 2 (tokens + four primitives `SignalPill` / `SignalRow` / `SeverityDots` / `TrendCard` + a visual test page, no call sites), Phase 3 (five batches: trends → events → status pills → info → the rest), a contrast gate in both directions as a unit test, and `MIGRATION-NOTES.md` at the end. Its hard rules: never collapse `T.danger`; honour the exclusions; no status text on `inkMuted`; at most one focus treatment per screen; no new hex in components; contrast is a test, not a judgement; refactor nothing else.

**What was found, before building anything.** (1) The brief's `SPINE_CONFLICT.affected` names Claude Design's mock artboards (`dispatch-customers-a/b/c.jsx`, `documents-*.jsx`, `contact-account-rails.jsx`); none exist here. Read against `src/` (every `inset [3-6]px 0 0` and `borderLeft: [3-6]px`): FOUR selection spines in three different colours — `ListView.jsx` 252 (gold), `ContactsTab.jsx` 464 (ink), `LeadsTab.jsx` 758 (the lead's score-band colour), `DispatchTab.jsx` 3126 (the customer's plan-tier colour) — and the Dispatch customer list renders `overdueBy` and `renewalDays` in the selected row, so the collision the brief warns about is LIVE today. ONE severity spine already in the design's form (`DispatchTab.jsx` 809, a technician lane over hours). About sixty notice boxes already tint + 3px spine (the Settings banners, the Home attention cards, the job-editor notes) — callouts, not rows, no collision. About twenty categorical/ordinal spines, two of which are severity by another name (job priority `prioColor` in `T.danger`/`T.warn` at 3px AND 4px; rep health / metric tone). Thirteen gold section-heading rules (`borderLeft: 3px goldInk; paddingLeft: 10`) — headings, never rows. (2) **The token object is not shared.** `settings/shared/tokens.js` is imported by the Settings panels only; 24 other files declare their own `T` (the tabs, the modals, the views), and `ReportsTab.jsx` declares EIGHT (`T`, `T2`, `T2b`, `T2c`, `T3`, `T4`, `TS`, `T_ACTIVITY`). A value changed in the shared file reaches none of them — the brief's "ship primitives, not tokens" is the only route that reaches every screen. (3) Guide §16 line 740, "Pills: `borderRadius: 999`", contradicts the design for signal pills (r2) while categorical pills stay rounded — the rule changes either way; flagged in SPINE_COLLISION §5 Q2, NOT edited until Jeff decides. (4) Signals sit on the ink header band: `AppHeader.jsx` 325, 330, 443 (notification badges in `T.danger`), 448 (`T.warn`) — the ramp's deep steps (`#4a1109`) on `#2a2622` would vanish; the header needs its own rule and the contrast gate a third background. (5) A Tailwind-family off-brand palette (`#2563eb`, `#ef4444`, `#dc2626`, `#f59e0b`, `#10b981`, `#e2e8f0`, `#94a3b8`, `#64748b`, `#1e293b` …) still lives in `ModalLayer.jsx` (130 uses), `LeadImportModal.jsx` (88), `App.jsx` (74), `OutlookImportModal.jsx`, `ContactModal.jsx`, `UserModal.jsx`, `ViewingBar.jsx`, `QuotaRepCard.jsx` — guide §16 already forbids it; the brief's rule 8 keeps it OUT of this migration; recorded in the audit §4 for a separate cleanup. (6) Charts have their own palettes (`funnelColors`, `SOURCE_RAMP4`, the avatar palettes, the heat ramp) — categorical, excluded; one ranked series uses signal tokens (`lossBarColors = [T.danger, T.warn, T.inkMid, …]`, ReportsTab 3016) — flagged. `KPIThresholdsDetail`'s "Color palette" card is display-only and prints `#4d6b3d` etc. as text — it will read wrong the day the values move. (7) Fourteen local pill components are the natural `SignalPill` call sites (or "leave alone, categorical"): `LeadStatusPill`, `TasksTab.StatusPill`, `QStatus`, `ScoreBadge`, `PlanBadge`, `DPill`, `QPill` ×2, `ConnectedApps.Pill` / `StatusDot`, `StatusChip`, `DispatchVehicles.StatusPill`, `OpportunityModal.StatusBadge` / `SignalChip`, `StagePill`, `TypePill`.

**Delivered — Phase 0, `docs/design/SPINE_COLLISION.md`.** Every left spine grouped by meaning (selection, severity, notice box, categorical/ordinal, brand); the collision named concretely (the Dispatch customer list, the Leads cockpit); **recommendation A** — severity owns the 3px left spine in the ramp's `deep`; selection becomes `background: T.surface2` + a 5px INK spine, which is what `ContactsTab.jsx` already does, so A is "make the other three lists do what the company list does" (ListView's gold → ink, the Leads score band → ink with the `LeadScore` badge still in the row, the Dispatch tier → ink with the `PlanBadge` still in the row; nothing lost). Why not B (a right spine is invisible on the strips that scroll sideways) and not C (dots alone vanish in a forty-row list). Two decisions the code forces that the brief does not ask: job priority → severity (emergency → critical, high → attend, else no spine) or dots; crew/team colour spines stay categorical at 4px, rep health and metric tone become `TrendCard`'s deep border. The brief's §7 questions answered from the code where it answers (Q1 tiers resolve under A; Q4 the header band; Q5 the chart palettes; Q6 the density is ReportsTab, DispatchTab and the Settings panels — one file per commit for the two 5,000-line files); Q2 (radius 2 everywhere) and Q3 (destructive stays `T.danger`, recommended) are Jeff's. §7 lists what is needed from him before any list is migrated.

**Delivered — Phase 1, `scripts/audit-colors.mjs` → `docs/design/color-audit.md` and `color-audit.csv`.** Read-only over `src/` (164 files): every hex, `rgb()`/`rgba()` and every `T.ok` / `T.warn` / `T.danger` / `T.info` / `T.inkMuted` reference (the ReportsTab copies included; `res.ok` and friends excluded by matching the token identifiers only; the 531 lines inside token-object definitions excluded from every table) — file, line, enclosing component, value, the line itself, the CURRENT FORM (`pill r999`, `pill r8–r20`, `r2`, `dot`, `tint bg`, `solid bg`, `1px border`, `spine Npx`, `text`, `svg`) and a classification from the brief's fixed set (`trend` · `event` · `info` · `neutral-status` · `categorical` · `brand` · `type` · `destructive`) plus `?` where the line does not say and `info?` for `T.info` outside a notice. The classification is a heuristic over the line and the two above it, with the token names stripped first (`T.warn` is not evidence of a warning — the first cut counted every `T.warn` line as an event); the md says so and treats every `?` and every `destructive` / `event` split of `T.danger` as Jeff's decision. Counts: **5,514 uses**; the signal tokens — ok 435, warn 328, danger 664, info 314 (1,741 signal sites, all listed in §7); shapes today — bare text 585, tinted box 193, outlined box 112, spine 104, rounded pill 92, lozenge pill 38, r2 40, dot 21, svg 14; `inkMuted` 1,675 uses of which 316 sit on a status line (§8); the off-brand literals by file (§4); the 30-odd token-object declarations by file and line (§5); per-file counts (§6). `node scripts/audit-colors.mjs` regenerates both; `--stdout` prints the summary only.

**NOT done, by the brief's own gates:** no token, no primitive, no call site changed; `RAMP` / `SIGNAL` / `FORM` are not in `src/`; guide §16 not edited. Phase 2 waits on Jeff's answers in SPINE_COLLISION §7 and his read of the audit. **Verified (docs and one script under `scripts/`, no `src/` or `netlify/` change, no schema change):** six gates (164 files; 91 function files), **769/769 unit**, build guard OK 2,504 kB — the hash UNCHANGED, `index-BIIXAgGT.js` — `dist/` cleared; the integration suite and the mutation harness NOT run, nothing they cover changed. Not pushed at the time of writing; the handoff says.

### 0.130 One design-token object — `src/tokens.js` replaces the 32 local copies, and the three reads that were silently `undefined` (14 Sep, sixteenth session, continued — Jeff: "lets do that single token fix")

**What it was.** Guide §16 said it plainly: "Every component file declares its own design-token object." Twenty-four files did — every tab, every modal, every rail, the views, the header, the Documents atoms — and `ReportsTab.jsx` declared EIGHT (`T` inside a JSX IIFE, `T2` three times in three scopes, `T2b`, `T2c`, `T3`, `T4`, `TS`, `T_ACTIVITY`) plus seven `const T = TS;` and one `const T = T_ACTIVITY;` aliases, one per report template. Only the Settings panels imported `settings/shared/tokens.js`. The signal-migration audit (§0.129) surfaced it as the reason a token change reaches nothing; Jeff picked it up on its own.

**What was found, before writing anything (the parser, not grep).** Every T-family object literal in `src/` was parsed (`@babel/parser`; a token object = a `const T…` whose literal has `surface` and `ink`) and diffed key by key. **The colours never drifted** — all 14 palette keys identical in every copy. Everything else had: (1) five spellings of `serif` (`"Source Serif 4", Georgia, serif` · `Georgia, "Source Serif 4", serif` · `Georgia, "Tiempos", serif` · `Georgia, serif` · `Georgia,serif`) — and neither "Source Serif 4" nor "Tiempos" is loaded by `index.html` or `index.css`, nor installed on this machine, so every screen rendered Georgia regardless; unified on `Georgia, serif`, the shared file's value. (2) `sans` in three whitespace/quote spellings — CSS-identical; unified. (3) `r: 4` in DispatchTab (100 reads) against 3 everywhere else — kept, as `const T = { ...TOKENS, r: 4 }` with a comment: the one recorded divergence. (4) Four alias keys — `ink2` = `inkMid`, `ink3` = `inkMuted`, `surface3` = `bg`, `radiusMd` = `rMd` (and `good` = `ok`, defined in atoms and read nowhere) — 150-odd reads across the rails and the Documents surfaces; renamed to the canonical names. (5) Extra keys some copies carried — `stages` (identical in all four; the eleven-stage map), `mono`, `tint`, `surfaceInk`, `surfaceInkFg`, `rSm`/`rMd`/`rLg` — now on the one object. (6) **Three reads of keys the local copy never had, each `undefined` today with no error anywhere:** `T.r` in HomeTab (line 585, a bordered box, and 753, the Connect-calendar button — no radius at all; now the app's 3), and `T.info` inside the report templates where `T` aliased `TS`, which had no `info` (line 3223, a Call activity's colour falling through to muted grey where the sibling template at 1263 uses slate; line 3693, a metric chip's `borderLeft: 3px solid undefined` — no border). These three are the only behaviour changes in the batch, each accepted by hand and named in the script's `KNOWN_NEWLY_DEFINED`.

**How it was applied.** A scratchpad script (run from the repo root so `@babel/parser` resolves — the memory rule) resolved every `<Name>.<key>` read in each file to the innermost declaration in scope — the `const T = TS` aliases included — and compared the value it resolved to with the canonical value BEFORE writing; any read that differed, other than the font unification and the three accepted `undefined`s, blocked `--apply`. The first cut reported false differences (exported declarations cut twice; `TYPE_COLORS` matched as a token object; ReportsTab's scopes conflated) — each a script defect, fixed before anything was written. Then: the declaration statements cut (whole statements, their lines closed up), `T2.`/`T3.`/`TS.`/… renamed to `T.` within ReportsTab, the alias keys renamed across all of `src/` (the seven Documents files that import from atoms included — missed by the first pass, caught by the resolve-every-read check), one import line added after each file's last import, every rewritten file re-read from disk and re-parsed. `src/tokens.js` is new (the union, commented); `settings/shared/tokens.js` and `documents/atoms.jsx` re-export it so their importers are untouched.

**Tests.** +1 file `tests/single-token-file.test.mjs` (5): the palette values and the exact key set (a runtime import of the module); exactly one token literal under `src/`, at `src/tokens.js`; every file that reads `T.` imports it from the module or a re-export; every `T.<key>` read names a key the object has (no alias, nothing undefined); Dispatch's override is the only `...TOKENS` in the tree. Four mutants (the ok green drifting; a local copy returning in AccountPicker; Dispatch dropping its override; the alias reads returning in atoms). Guide §18b39 (new) and §16 rewritten; `docs/design/color-audit.md` §5 (the token-copy map) now describes the state before this batch.

**Verified:** six gates (165 files; 91 function files), **774/774 unit** (5 new), **159/159 integration**, **475/475 mutations after a printed green baseline**, build guard OK 2,489 kB `index-D0WIMRUE.js` (was 2,504 kB `index-BIIXAgGT.js` — 15 kB of duplicate objects gone), `dist/` cleared. Browser: see the handoff for what was observed. No function, no schema change — bundle only. **SHIPPED — the TWENTY-SECOND ship, 16:02 UTC 14 Sep, with §0.131 (its last paragraph has the record).**

### 0.131 Every tab announces itself the same way — Tasks, Dispatch, Documents, Reports and Settings brought onto the one title block (14 Sep, sixteenth session, continued — Jeff, with his screenshots of every tab: "All the tabs have the correct look for announcing the tab ... The tabs done differently are (Dispatch, Documents, Reports (located too close to the title bar not where the rest are), Settings wrong font and script")

**What it was.** Seven tabs shared one title block — `fontSize: 28, fontFamily: T.serif, fontStyle: 'italic', fontWeight: 300, letterSpacing: -0.8, lineHeight: 1, marginBottom: 5`, then a 12px `inkMuted` caption with the counts in `ink` at 600 — inside a `.tab-page` (1.5rem padding). Five did not: **Tasks** had the block but with an uppercase eyebrow "TASKS · TODAY IS MON, SEP 14" above the title; **Dispatch** put "DISPATCH" as the eyebrow and the VIEW's label ("2 dispatch customers", the board's date range) as a 24px title behind a 3px gold rule, in its own bordered header bar; **Documents** was a 26px bold sans title under an eyebrow, in a 1280px column centred on the page; **Reports** had the right title but no page padding at all — the title touched the header band and sat 28px left of every other tab's; **Settings** was a 26px bold sans title behind a gold rule with a 13px caption.

**What changed (five files, presentation only).** `TasksTab.jsx` (the header near line 1262): the eyebrow removed; title → "Today is {date}" (12px, `inkMuted`) → the counts line — the date "moved under the Tasks label and above the 0 due today", as asked. `DispatchTab.jsx` (the dispatcher header near 5666 and the technician header near 5645): "Dispatch" is the title; the caption reads "{view label} · {n} techs available · {n} jobs" with the view label in `ink` at 600 (the urgent-unassigned note unchanged after it); the technician's reads "My jobs"; the header bar's own padding, border and gold rule gone — it is now `padding: 0 0 14px` inside the `.tab-page` like the others. `DocumentsTab.jsx` (line 127): the wrapper is `padding: 1.5rem` with no `maxWidth` / `margin: 0 auto`; the title block is the shared one with "{n} files · {size}" as the caption. `ReportsTab.jsx` (line 245): the root column gains `padding: 1.5rem; boxSizing: border-box` (its `height: 100%` kept — the sub-tab panels scroll inside it as before); the title block's hard-coded `#2a2622` / `#8a8378` / 'Georgia, serif' read `T` (§0.130 made that possible). `SettingsTab.jsx` (line 19): the shared title block; the gold rule stays on the panel headings, where it is the section device. Not touched: the six tabs already on the block, the Reports sub-tab nav, the Dispatch sub-tab strip, the Home greeting (a greeting, not a tab name).

**Verified:** six gates (165 files; 91 function files) — the first cut failed three of them on a JSX comment placed between `return (` and the root element in ReportsTab (a parse error the gates caught before any test ran; moved inside the element), **774/774 unit, 159/159 integration, 475/475 mutations after a printed green baseline**, build guard OK 2,489 kB `index-B_6zu6fm.js`, `dist/` cleared. Browser: the handoff says what was observed. **NOT shipped.** Bundle only.
**Follow-up, the same hour — the gutter is the class, not a number (`the follow-up commit`).** The first cut gave Documents and Reports `padding: 1.5rem` inline. The other tabs get their gutter from `.tab-page`, which is 1.5rem on a desktop and 0.75rem under 640px (`index.css` 698) — so at the pane's phone width the two titles sat 12px right of and below every other tab's (x 32 / y 80 against 20 / 68, measured), and on a desktop they would have matched by coincidence. Both wrappers now carry `className="tab-page"` with `gap: 0` inline (the class's column gap would have opened 1.25rem between the header, the toolbar or sub-tab nav, and the table — each page spaces itself). Same six gates, 774/774, 159/159, 475/475 mutations, build guard OK `index-CyIfr4JI.js`. **OBSERVED by Jeff on dev, desktop (14 Sep): "updates to tabs looks great."** Then Jeff: "push master" → **PROD SHIPPED — the TWENTY-SECOND ship (Jeff: "push master", 14 Sep): `git push origin dev:master` at 16:01:27 UTC, `master` `ae0cae8` → `e2cba64`; salespipelinetracker.com serving `index-D0HaH5AU.js` from 16:02:06 UTC with the live key (`pk_live_` ×2 — the key and the SDK constant; `pk_test_` ×1, the SDK constant), 2,549,159 B; §0.130 (one token object), §0.131 (every tab announces itself the same way) and its gutter follow-up on prod — three code commits `10a78bc`, `4bb37ef`, `c9bf692`, all bundle only; no function, no schema change (`git diff --stat origin/master..dev -- db/schema.ts netlify/` EMPTY at pre-flight); the batch's literals in the served bundle ("Today is ", "techs available", "Workspace admin console", "My jobs", the Reports caption). Pre-flight: `dev` == `origin/dev`, `origin/master` an ancestor.** **OBSERVED on prod by Jeff (14 Sep): "All tabs look great on production."**


### 0.132 The enhancement thread's last three: task.overdue is computed by the hourly job; the Slack action posts Block Kit; an Admin can Train now (14 Sep, sixteenth session, continued — Jeff: "lets work on this item", the three sub-items of §9's enhancement bullet)

**What it was.** §9's enhancement bullet ("Web-to-lead, email templates, lead scoring … all shipped. Left over from that thread") had three items, each flagged in §0.124 / §0.127 / §0.123: `task.overdue` — offered by the panel for months while nothing computed it, struck from the vocabulary in §0.124 (the webhooks list still names it; nothing fires it there either); the `send_slack` action — text only since §0.127, where every pipeline alert posts Block Kit; and a manual "Train now" for the predictive lead model — the batch trains nightly and only above the org's `minClosedRecords`, so an Admin with 40 decided leads and a threshold of 150 could never see a model.

**(1) `task.overdue` — computed, not merely offered.** `automationEvents.js`: the trigger `{ value: 'task.overdue', label: 'Task overdue (past its due date)', group: 'Tasks', kind: 'hourly', entity: 'task' }` — thirteen triggers now, six hourly; `TASK_FIELDS` gains `priority` and `due_date` (both events), `task.overdue` adds `days_overdue`; `taskEventData` carries `priority` and `due_date`. `pipeline-alerts.mjs`, a new section after the deal loop (Signal 7): ONE read of the orgs that have an active `task.overdue` rule — an org with none is not scanned and writes nothing to the ledger; for each open task (`!completed && status !== 'Completed'`) whose `dueDate < todayStr` and whose assignee (`userByName[task.assignedTo]`) is an active user WITH the task's `orgId` (cross-tenant: a same-named user of another org never fires it), at that assignee's alert hour (the deal loop's `localHourToUtc` rule), deduped by `wasRecentlyAlerted(orgId, assignee, task.id, 'taskOverdue')` — the task id in the ledger's `opportunityId` column, the task-reminders precedent — then `logAlert` and `dispatchAutomations(task.orgId, 'task.overdue', taskEventData(task, { days_overdue }))`, `days_overdue = daysBetween(dueDate, todayStr)` (today minus due, positive). No email, SMS or Slack of its own — the rule is the switch, exactly the §0.127 shape; the summary line counts `overdueFired`; the section never fails the run. A `create_task` from it links the same deal (`opportunity_id`) and goes to the same assignee by default (`taskFromAction` already did both for a task entity). The panel's Tasks group says how its two triggers fire (completed: on the tick; overdue: hourly, once per task per week, at the assignee's alert hour).

**(2) The Slack action posts Block Kit.** `automationSlackMessage({ message, ruleName, triggerLabel, subject, appUrl })` (pure, `automationEvents.js`): `text` is the plain rendered message (the notification preview and the no-blocks fallback); `blocks` = a `section` (the message, mrkdwn-escaped — `<`, `>`, `&`), a `context` line ("Automation: *rule* · trigger label · record"), and an `actions` button "Open Accelerep →" to `APP_URL` when one is given. `send-slack.mjs` wraps it as `slackTemplates.automation` with the module's `APP_URL`; the engine's `send_slack` case hands `sendSlackToOrg` that template with the rule's name (`executeAction` now receives the rule) and the trigger's label from the vocabulary. Still no alert type: the rule is the switch; an unconnected Slack is still a skip that says so.

**(3) Train now.** `score-leads-batch.mjs`: the per-org work is `export async function scoreOrg(orgId, { force = false })` — the settings read, the decided count, the gate (`predictive-off` → `below-engine-floor` (`MODEL_MIN_ROWS = 20`, the engine's own floor, checked BEFORE force) → `below-threshold` unless forced → train and store on THIS org's settings row), then every lead scored as before; it returns `{ leadsUpdated, decided, threshold, modelTrained, reason, leadScoring }` (`{ skipped: 'disabled' }` when scoring is off). The nightly `run` loops the orgs calling it WITHOUT force — the batch never forces. New `train-lead-model.mjs`: POST, `verifyAuth` → `requireRole(['Admin'])` → `scoreOrg(auth.orgId, { force: true })`; the body carries nothing (no org, no threshold, no flag from the client); the answer carries `ok`, `reason`, `decided`, `threshold`, `minRows`, `leadsUpdated` and the org's saved `leadScoring` (the model on it when trained). The panel (`LeadScoringDetail.jsx`, the Predictive card): a "Train now" button, disabled while the form is dirty (training reads the SAVED config — a stale button would lie) and while training; on success the returned config becomes the `settings` prop (the effect re-seeds the form; the status box reads the server's truth); the message is `trainNowMessage(data)` (pure, `leadScoringDefaults.js`): "Model trained on N decided leads · A% training accuracy." or the gate that refused — "Needs at least 20 decided (Converted / Dead) leads to train — this workspace has N.", predictive off, scoring off, training failed, or the server's own error for a refused request.

**Tests.** `tests/automation-events.test.mjs`: the vocabulary test says thirteen (HOURLY_TRIGGERS ends with `task.overdue`, kind hourly, group Tasks); +4 tests — the payload and merge fields for `task.overdue` (and a follow-up from it links the same deal), the job's task section (gate → dedup → ledger → the rule, in order; the cross-tenant check; the alert hour; NO email/SMS/Slack in the section; exactly two `'task.overdue'` literals in the job), `automationSlackMessage` run (shape, escaping, no button without a url), the engine's use of the template + the panel's Tasks note; the §0.127 send_slack scan updated to the template. `tests/train-now.test.mjs` (new, 4): `trainNowMessage` run for every gate; `train-lead-model.mjs` scanned (POST, verify → Admin gate → force, no body read, the config in the answer); `score-leads-batch.mjs` scanned (the floor before force, the batch never forces); the panel scanned (disabled while dirty, POST with no body, adopts the config, a refusal shown). `tests/integration/dispatch-automations.itest.mjs`: the send-slack mock now carries the real `automationSlackMessage` as its template; the silent-deal case asserts the three blocks; the outside-vocabulary case uses `task.snoozed`; +1 case — a `task.overdue` rule with a `days_overdue >= 3` condition: 2 days late is a skipped run, 5 days late creates "Chase: Send SOW (5d late)" linked to the overdue task's deal, owned by A's Karen, both runs naming the task, B untouched. `tests/integration/train-lead-model.itest.mjs` (new, 4; namespace `itest_train_*`): A with 30 decided leads and threshold 150 — unforced refused `below-threshold` with every lead still scored; forced trains (n 30, five coefficients, source win rates, `trainedAt`), stored on A's row alone, every lead of A carrying a probability, B untouched, the threshold not rewritten; B with 10 decided refused `below-engine-floor` force or not; predictive off → `predictive-off`; scoring off → `{ skipped: 'disabled' }`. `tests/lead-scoring-defaults.test.mjs`: the panel's import line now carries `trainNowMessage`. Nine mutants (the three job gates — orgs with a rule, cross-tenant, dedup; the trigger's kind; the plain-text post returning; the unescaped section; force skipping the floor; the Admin gate; the batch forcing). The first itest cut seeded a PARTIAL `leadScoring` config (no fit/engagement rules) and scored nothing — a real config is the full object the panel saves; the seed now spreads `DEFAULT_LEAD_SCORING`.

**Not done, flagged:** the webhooks vocabulary (`webhooks.mjs`) still lists `task.overdue` and nothing fires a webhook for it — the hourly job could, beside the rule, a product call; a task's `days_overdue` is counted in UTC days (the job's `todayStr`), like every deal signal — a due date is a wall-calendar day, so a task due "yesterday" in Chicago can be "not yet overdue" until 00:00 UTC; the Slack context line prints the deal or lead name from `eventSubject` (a task event prints the task title); Train now trains on the SAVED config and says so — an unsaved threshold change is not what trains; the training itself is the nightly engine unchanged (400 iterations, L2 0.01) — a large org's Train now runs inside the function's timeout like the batch does.

**Verified:** six gates (165 files; 92 function files — `train-lead-model.mjs` new), **782/782 unit** (8 new), **164/164 integration** (5 new), **484/484 mutations after a printed green baseline**, build guard OK 2,492 kB `index-DL6xNwD_.js`, `dist/` cleared. Browser: the handoff says what was observed. ~~NOT shipped.~~ **SHIPPED — the TWENTY-THIRD ship, 18:54 UTC 14 Sep (§0.133, last paragraph).** Functions AND bundle (`pipeline-alerts.mjs`, `dispatch-automations.mjs`, `send-slack.mjs`, `score-leads-batch.mjs`, `train-lead-model.mjs` new); no schema change.

**Follow-up, the same hour — the modal's error renders on whichever step raised it (`88148ab`; Jeff: "I can't move a new automation from trigger stage to conditions").** Reproduced in the pane on dev before reading further: with the Name blank, Next did nothing visible and the modal stayed on step 1; with a name, it advanced. The cause, read against the code: `Next →` refuses step 1 without a name and calls `setError('Name is required')`, but the only `{error && …}` render sat INSIDE the `step === 4` block — the message existed and was never on screen for steps 1–3, so the button looked dead. Pre-existing since the modal was built (§0.124); it surfaced now because the trigger Jeff wanted is new. Fix (bundle only): the one error render moved out of step 4 to the modal body after the step blocks, styled as the app's callout (tint, 3px danger rule), so every step shows what stopped it. +1 scan (exactly one render, after the last step block, the step-4 line gone, the step-1 gate intact), 1 mutant (the render suppressed). Same six gates, 783/783 unit, 164/164 integration, 485/485 mutations, build guard OK `index-DoNj0dG6.js`. **OBSERVED in the pane as Jeff on `index-DoNj0dG6.js`:** New automation → Next with the Name blank → the modal stays on step 1 and "Name is required" is on screen, in the danger callout (a `2.67px rgb(156, 58, 46)` left rule measured), no console error; closed without saving. **OBSERVED by Jeff on dev (14 Sep, his Slack screenshot: "created new rule, saw no name warning, used slack, and posted the bottom message"):** a new rule "Test #3" on Stage changed with a Post to Slack action; the blank-name warning on step 1; a deal moved → #sales-alerts received the Block Kit post — the section "sales rep Karen Russell:ZZFX Harborline Service moved to Negotiation/Review" (his merge fields rendered), the context line "Automation: Test #3 · Stage changed · ZZFX Harborline Service", the "Open Accelerep →" button — beneath the same event's company-switch "Stage changed" alert from the pipeline events.

### 0.133 The report builder runs real queries — the preview is the query, a saved report opens and saves in place, the period is real, the controls with nothing behind them are gone (14 Sep, sixteenth session, continued — Jeff: "lets fix this from your list. Report builder running real queries. The preview is illustrative today.")

**What it was.** §9 carried "Report builder: actual query execution against live DB (currently preview is illustrative)". Read against the code: the Reports → Saved reports → + New report → Blank canvas builder let a user pick a source (Opportunities, Accounts, Leads, Activity, Quotes), group-by chips, measure chips, a chart type (eight, two of them never drawn), a Period ("This quarter", a label), filters ("+ Add filter", a dead button), Compare to ("Previous quarter", a label) and a Format tab (labels) — and its Update preview drew ONE fixed picture whatever was picked: `openPipelineByRep(reportsOpps, 5)` (§0.68 had made that picture real, "titled as a preview whose dimensions and metrics are not applied yet"). Save wrote the picks to `saved_reports` under the name "Untitled report" (no name field), and the library card for such a report said "Built in the report builder — opening it is not wired yet". The AI mode is a separate fixed view (stuck deals by rep) and says so; the six template Save buttons save a `config.templateId` that opens its template — both untouched.

**The engine — `src/utils/reportQuery.js` (pure; no React, no db).** `REPORT_SOURCES` (four — no Quotes, the tab holds none), `REPORT_PERIODS` (All time, This fiscal year, Q1–Q4 — the tab's own keys), `REPORT_CHARTS` (bar, stacked, line, funnel, table, kpi). `fieldsFor(source)` → `{ dims, metrics, defaults }`, every field backed by a column: deals — Owner, Stage, Close date (a closed deal's close DAY per §0.68, an open deal's forecast, by month), Industry (`vertical`), Territory, and advanced Forecast category, Loss reason (`lossBucketOf`), Created date, Last activity date (from the activities), Product line (`products`); Revenue (sum arr), # of deals, Avg deal size, and advanced Days to close (the median `cycleDaysOf` of closed deals), Days in stage (open deals, since `stageChangedDate`), AI score (avg of what exists), # activities (linked); accounts — Owner, Industry (`industry || verticalMarket`), Territory, Tier, Segment, State, Created date; # of accounts, Revenue (won) and Open pipeline and # of deals from the org's deals joined by `accountId` or by name; leads — Assigned to, Lead source, Status, Score bucket, Created date; # of leads, Est. ARR, Avg lead score (a blank score is not a 0), # converted; activities — Rep (`activityRepOf`), Type, Date, Outcome; # of activities, Minutes logged, Deals touched (distinct). The old picker's Competitor, Deal tier and Lead source on a DEAL had no column and are not offered. `resolveField(source, kind, ref)` finds a field by id, by label (case-insensitive), by the id a label makes, and the two legacy ids (`arr` → revenue, `count` → deals) — so `{ id: '_of_deals', label: '# of deals' }` from a report saved before today resolves; a field with no column is DROPPED and named in a warning, never faked. `runReport(def, data, { today, fiscalStart })`: the source's rows, the period through `periodRange` on the source's own day (a deal by `forecastedCloseDate || createdDate` — the tab's `reportsTimedOpps` convention; an activity by `date`; a lead or account by `createdAt`), grouped by the dimension values, every metric aggregated per group (`sum` / `count` / `avg` / `median` / `distinct`) and in total; rows sorted by month or stage order (the org's `funnelStages`, the closes last) else by the first metric descending; an optional `limit` (12 for charts, 200 for the table) with a "Showing the top N of M groups" warning; `{ rows, totals, count, scanned, period, dropped, warnings }`. `chartData(result, chart)`: bar = first dim × first metric; stacked = first dim rows × second dim segments; line = a month dim in order; funnel = the Stage dim in stage order; table = every group × every measure; kpi = the totals; a chart the definition cannot support falls back and SAYS so on screen (a stacked bar with one dimension → a bar; a line without a date dimension → a bar; a funnel without Stage → a bar; no dimension → the totals); `heatmap` / `scatter` from an old saved report → a table. `formatMetric` (money short, days, score, int) and `formatKey` (yyyy-mm → Mon yyyy).

**The chart — `src/components/ReportChart.jsx`** (module scope, no state): draws every kind from the shared tokens — horizontal bars, stacked bars with a legend, an SVG line with an area fill and an emphasised endpoint, a funnel, a grid table, KPI cards — and prints the fallback note and the engine's warnings above the drawing.

**The builder (`ReportsTab.jsx`, `SavedReportsTab`).** New state: `builderPeriod`, `builderName`, `editingReportId`, `builderResult`; `runBuilder()` = `runReport` over `{ reportsOpps, accounts, leads (new prop, the sliced `reportsLeads`), activities (the sliced set), settings }` with the org's fiscal start; **Update preview runs it**; the preview is `<ReportChart result={builderResult} chartType={builderChart}/>` under the report's name, source and period, with a footer that says how many rows of the source were in the period and how many groups; the Data tab's chips come from `fieldsFor(builderSource)` (basic always, advanced behind the toggle) and store the engine's ids; choosing a source resets the chips to that source's defaults; the Filters tab is ONE real Period select over `REPORT_PERIODS` with a line saying which day each source counts by; the Chart tab lists the six charts; the Format tab, the add-filter button and the compare-to picker are gone (they computed nothing). **A saved report opens:** the library card calls `openSavedReport(r)` (a template card still opens its template) — the saved source / dims / metrics / chart / `filters.period` seed the builder, the query runs, the builder shows "Saved / Saved report", and Save PUTs the same id (the endpoint's owner-or-Admin upsert) and replaces the card in place; a new report POSTs. The blank builder has a **name input** (a report is no longer always "Untitled report"). The picker's Blank canvas card resets the builder for a NEW report and sets the tab key to `Data` — the + New report button had set `'data'` (lowercase), so the Data tab rendered nothing until a tab was clicked (pre-existing, fixed).

**Found while building, each caught by a gate or a test before the pane:** (1) the first Close-date dimension read `closeDayOf(o) || forecastedCloseDate` — `closeDayOf` falls back to the stage-change day for an OPEN deal, so an open deal grouped under the month it last moved; the dimension now reads the close day for closed deals and the forecast for open ones (the engine test caught it: an open October deal grouped under September). (2) The name input was first placed inside `BuilderHeader` — a component declared INLINE in the render — and `check:inline` reported it user-visible: the input would remount and lose focus on every keystroke (guide §16); it lives in the builder body now, and the scan pins that the header holds no form control. (3) `runReport` built "today" with `toISOString().slice(0, 10)` — the date-local scan (§18b26) refused the UTC truncation; `isoLocal(today)`.

**Tests.** `tests/report-query.test.mjs` (new, 12): the vocabulary (four sources, six periods, six charts, every field typed, no Quotes/heatmap/scatter, no competitor/deal_tier/lead_source on a deal); `resolveField` by id, label, label-made id and the two legacy ids, a dropped field named, de-duplication, an unknown source; `runReport` grouping two dimensions with three metrics and the totals, the advanced metrics (median cycle 79d over a 70d and an 88d close, days in stage over open deals only, AI score over what exists, activities per deal), the date and loss dimensions (close day vs forecast, months in order with None last, `Not lost`, last activity month), the period on every source (Q3 / Q4 / FY / Q1 counts, `scanned` vs `count`), accounts joined by id AND by name, leads and activities, `limit` with its warning, stage order; `chartData` for every kind and every fallback; `formatMetric`. Scans: the tab imports the engine and the chart, sources and charts are the module's, Update runs the query, the preview is `ReportChart`, the chips store ids, the period is a select, the labels are gone, the tab keys are `Data/Filters/Chart`; a saved report opens and PUTs in place, the name input exists and the inline header holds no form control, the leads reach the builder as `reportsLeads`, no lowercase `'data'`; `ReportChart` draws every kind at module scope. Three older scans updated to the new shapes (`pipeline-report` — the preview line; `reports-controls` — the save handler's signature and the card's click). Seven mutants (the open-deal close date, the period ignored, the label match dropped, the stacked fallback dropped, Update not running the query, a reopened report saving as new, the card not opening).

**Not done, flagged:** row filters beyond the period (a `field = value` filter — the old dead button's promise) and Compare-to (a prior-period delta — `priorRange` exists for it) are not built; the AI mode is still the fixed stuck-deals view and says so; a saved report's chart is drawn from the builder's scoped sets at open time — a Manager opens a rep's report against the Manager's scope; the Product line dimension groups by the free-text `products` string as typed; Days in stage counts from `stageChangedDate` (or creation) to the local today; the chart limit (12 groups) is a constant; a report older than today with a dropped field opens with a warning naming it — the field is not restored.

**Verified:** six gates (167 files; 92 function files), **796/796 unit** (12 new), **164/164 integration**, **492/492 mutations after a printed green baseline**, build guard OK 2,508 kB `index-BNgFKZx-.js`, `dist/` cleared. Browser: the handoff says what was observed. ~~NOT shipped.~~ **SHIPPED — the TWENTY-THIRD ship (next paragraph).** Bundle only — no function, no schema change (`saved_reports` already had `filters` and `config`; the endpoint already had PUT).

**PROD SHIPPED — the TWENTY-THIRD ship (Jeff: "ship master", 14 Sep): `git push origin dev:master` at 18:53:25 UTC, `master` `e2cba64` → `c17e2e8` (the §0.133 handoff commit; the three code commits `9893ddc` (§0.132, functions AND bundle), `88148ab` (the modal's error, bundle) and `ed34ca9` (§0.133, bundle) under it). Netlify's record, read after the landing: deploy `6aa842a7…`, branch `master`, context production, commit `c17e2e8`, published 18:53:57 UTC, deploy time 28 s, **82 functions** — NINE bundles carry the ship's build stamp (18:53:56–57 UTC): the five changed files `pipeline-alerts`, `dispatch-automations`, `send-slack`, `score-leads-batch` and `train-lead-model` (new on prod), plus `leads`, `opportunities`, `tasks` and `lead-intake`, the four endpoints that import the automation module; the other 73 kept their earlier stamps (Netlify re-uploaded only what changed — not "every function", as the handoff had expected); the four schedules unchanged (`digest` and `pipeline-alerts` hourly, `score-leads-batch` 06:00 UTC, `task-reminders` every minute); 3 redirect rules, 1 header rule, secret scan clean over 449 files, "2 new files uploaded — 1 generated page and 1 asset". salespipelinetracker.com serving `index-Ct7x96Jv.js` from 18:54:02 UTC (the hash poll, 37 s after the push; before it `index-D0HaH5AU.js`) with the live key (`pk_live_` ×2 — the key and the SDK constant; `pk_test_` ×1, the SDK constant), 2,568,009 B. No schema change (`git diff e2cba64..c17e2e8 -- db/schema.ts` empty). The tree was clean at the ship; the Resend webhook has targeted prod since the twenty-second ship (untouched). **From this hour the hourly `pipeline-alerts` on prod computes task.overdue** — a rule on prod is Jeff's to create; nothing fires until one exists. ~~NOT yet observed on prod~~ **OBSERVED on prod by Jeff (14 Sep): "All tests passed."**

### 0.134 Drag-to-reschedule on the dispatch week board — a card drags to another day or another technician’s row, through the crew builder’s gates, with a partial PUT that never sets status (14 Sep, seventeenth session — Jeff: "Drag-to-reschedule on the dispatch week grid")

**What it was.** §9 had carried "drag-to-reschedule on the week grid (still OPEN — nothing in DispatchTab is draggable)" since the board was built, restated by every reading. The week board (`WeekBoardView`: technician rows × seven day columns, plus a "Needs a crew" row for dated jobs with no crew) rendered every job as a card whose only gesture was a click — to the crew builder for a job still to schedule, to its record otherwise. Moving a scheduled job by a day meant opening its record and editing the date; handing it to another technician meant the crew builder. The Kanban board has dragged deals between stages since the first sessions.

**What a drop means (`src/utils/weekDrop.js`, pure — no React, no db; the board’s reads are injected).** `nextCrewAfterDrop(crew, fromTechId, toTechId)`: a job appears in every crew member’s row, so a card dragged from A’s row onto C’s row means "C instead of A" — the seat swaps and the lead seat stays the lead (`[A, B]` from B’s row onto C is `[A, C]`); dropped on a row whose technician is already on the crew it is a date move only; from the Needs-a-crew row it takes that technician as its crew. `planWeekDrop({ job, fromTechId, toTechId, toDate, lookup, blockTypes })`: a drop on the same cell — or onto a crew-mate’s row on the same day — is a NO-OP (nothing written, nothing said); a crewed job on the Needs-a-crew row, a stale crew (the row’s technician no longer on the job), an unknown technician and a won-opportunity placeholder (`isBridge`, no `dispatch_jobs` row) are refused in words. Then the crew builder’s own gates, in the same words as `handleSchedule`: every technician who has to be free on the target day — the newcomer on a swap, the WHOLE crew when the day moves under them — must be rostered that day (`shiftForDate`), not marked out all day (`blocksOnDate`), and, for a job with a placed start, inside their shift, clear of a partial-day block, and not already on another job that overlaps (that technician’s placed jobs on that day, the job itself excluded). A held crew (`start == null`, §0.115) moves by day and by seat with the time gates skipped — a busy day is not a clash until a time is chosen. `lookup(techId)` is the parent’s: `{ tech, shift, dayBlocks, rivals }` read from the board’s techs, blocks and jobs for the target date.

**The body is partial and never carries status.** `payload = { id, scheduledDate? , assignedTechId? + coTechIds? }` — the date only when it moved, the crew only when it changed. The dispatcher PUT merges per field (`if (f in data)`), so nothing else is touched: a scheduled job keeps `scheduledStart`/`scheduledEnd`/`timeSlot` and its status, a held crew stays `unscheduled`. Sending a status would have done two wrong things — promote a held crew to `scheduled` (a customer confirmation for a job with no time) or, as `'unscheduled'`, release the job’s reserved equipment on the server. The server’s own logic reads the change as it should: `notifyCustomer` (§0.111) sees a scheduled job whose appointment changed and sends the confirmation the org has switched on; the status history is untouched because the status is.

**The parent (`handleWeekMove`, DispatchTab).** The plan first; a refusal is shown, a no-op says nothing. Equipment on a new day (§0.116): `equipmentConflicts` over the target day refuses when a required kind is short; a job HOLDING units (`assignedEquipment`) has them re-picked for the new day through `pickEquipmentFor` over that day’s `overlappingRivals`, and the body carries `assignedEquipmentIds` — a unit is never held by two jobs at once. Then the soft blockers: `scoreTech` on the newcomer with the target date, minus the three the hard gates already ran (Not rostered / Off / Double-booked) — a missing skill, a licence too low or unset, an expired cert, over-hours — go through the app-wide `showConfirm` ("… does not meet this job’s requirements: …. Move it anyway?"), the same confirmation the builder’s Override asks for; Cancel simply closes it. The write: `dbFetch` PUT with the partial body, a 403 read as "Your role cannot reschedule jobs."; the client row adopts the server’s `scheduledDate`, the plan’s crew, `queueWindow(saved)`, the re-picked units, the server’s `publicToken`/`customerNotifications`; the raw row (`jobsRaw`, the Jobs view’s) follows — which `onScheduled` never did (flagged below). Audit `dispatch.reschedule` / `dispatch.reschedule.override`: "Moved from <date · tech> to <date · tech>", the re-reserved units, the overridden blockers. The result — "Moved <job> to <date · tech>." — shows for 8 s in the strip above the grid; a refusal stays until dismissed.

**The board.** HTML5 drag: every card is `draggable` unless a write is in flight or it is a placeholder; `dataTransfer.setData` for Firefox; the cell under the card carries a dashed gold outline; the card in hand fades. Every cell in every technician row AND in the Needs-a-crew row is a drop target — the planner decides what a drop there means. A strip above the grid reads the hint ("Drag a job to another day to reschedule it, or onto another technician’s row to hand it over."), "Moving…", the refusal (`role="alert"`, Dismiss) or the result (`role="status"`). Mouse only — no touch handlers (the Kanban’s touch handling is its own).

**Found while building.** `tests/crew-next-step.test.mjs` pinned "one refusal banner" as a count over the WHOLE file; the week board’s strip is a second `role="alert"`. The pin is now the crew builder’s own segment (one alert, one status) plus the week board’s (one of each). And the state header’s **Verified at** line still carried the §0.126–§0.128 counts (769 / 471 / 159 / `index-BIIXAgGT.js`) after §0.130–§0.133 had recorded 796 / 492 / 164 — the line was not bumped at those batches; it is bumped now and the old text kept inside it for the record.

**Observed in the pane (localhost:8888, Jeff, Admin — the rep-path pass as Karen is in the handoff §5), 14 Sep 19:43 UTC, the Accelerep Test org (`org_3BDQ…`):** Job Board → This week showed the hint strip; the Tue 15 card on Savannah Miller’s row (JOB-2026-0007, 08:00–10:00, scheduled) dragged to Wed 16 → one PUT, 200, the server row `scheduledDate 2026-09-16`, `scheduledStart 08:00` / `scheduledEnd 10:00` / `status scheduled` / the crew UNCHANGED — and, as §0.111 says, a confirmation email to the test customer’s address (Jeff’s own) appended to the trail; dragged back → "Moved Dispatch Customer Test to 2026-09-15." in the strip, a second PUT, the row restored (and a second confirmation — the rule, not new). Dragged onto Jax Miller’s Sunday → the strip read "Jax Miller is not rostered on Sunday." with Dismiss and NO PUT; dropped on its own cell → nothing said, no PUT. Dragged onto Jax’s Tuesday → the app-wide confirm: "Jax Miller does not meet this job’s requirements: License not set · job needs Apprentice. Move it anyway?" — whose button read **Delete**: the handler had passed the DANGER variant, whose button label is Delete, not the info variant’s Confirm. Cancelled (no write); the flag is `false` now (the info variant, as the Renew confirm uses; the follow-up commit; observed reading **Confirm** after HMR). The hand-over itself was NOT completed in the pane (it would have changed a test job’s crew under a licence override); its path is the planner’s, tested by running.

**Tests.** `tests/week-drop.test.mjs` (new, 12): the crew after a drop (swap, co-tech seat, a crew-mate’s row, the Needs-a-crew row, null); the no-op; a date move’s body (date only, the whole crew checked, no status/time); a hand-over’s body (lead and co-techs, the date only when moved); a held crew (no time gates); the Needs-a-crew row both ways; every gate’s words; a day move refused by the CO-TECH’s PTO; the refusals (unknown, stale, crewed-on-the-row, bridge, no job, no date); `dayNameOf` local parts and `fmtHour`; and two source scans over DispatchTab — the cards draggable (both rows), the cells drop targets, the guard on a placeholder or a write in flight, the strip; the handler’s refusal, the equipment gate on a new day only, the re-pick, the partial body, the row adopting the plan and the server, the raw row, the soft-blocker filter, the confirm, the audit action, the 403 text, the wiring. **8 mutants**, all caught: a held crew promoted to `scheduled` on a day move; the roster gate gone; a day move checking only the newcomer; a same-cell drop writing; a hand-over ADDING instead of replacing; the refusal ignored; the equipment gate gone; the crew rows’ cards not draggable.

**Not done, flagged:** the day board (hour lanes) and the month grid do not drag — the week board was the ask; dropping a crewed job on the Needs-a-crew row is refused, not an "unassign" (a destructive gesture deserves its own control); a drop cannot change the TIME (the row is the technician, the column the day — the time stays; the record or the builder changes it); `hoursThisWeek` on the technician rows is computed at load and not moved by a hand-over (as after `onScheduled`), so the builder’s over-hours blocker reads the load-time figure until a refresh; `onScheduled` and `onHeld` still leave `jobsRaw` stale after a crew-builder write (the Jobs view shows the old crew until a refresh) — the drop handler updates both, they should too; touch is not supported.

**Verified:** six gates (168 files; 92 function files), **808/808 unit** (12 new), **164/164 integration**, **500/500 mutations after a printed green baseline**, build guard OK 2,515 kB `index-B4AZRwho.js`, `dist/` cleared — then the follow-up `5bbd813` (the confirm variant): six gates, 808/808 unit, build guard OK 2,515 kB `index-BULS4hTf.js`, the harness re-run green (the handoff has the count). Browser: observed in the pane as above; **then OBSERVED by Jeff on dev (accelerep.netlify.app, `index-BULS4hTf.js`, 14 Sep): "it works now very well."** (His first try was on the unpushed site — "I am unable to drag a job" — the push was the fix.) **And as Karen (role User), 15 Sep: a scheduled job dragged both ways, a drop on a technician's day off refused with the warning — the rep path observed.** ~~NOT shipped.~~ **SHIPPED — the TWENTY-FOURTH ship (§0.137’s last paragraph).** **NOT shipped.** Bundle only — no function, no schema change.

### 0.135 Saved reports: sharing, scheduled email or Slack delivery, pin to Home — and two tab preferences that reset (14 Sep, seventeenth session — Jeff: "Saved reports: sharing, scheduled email or Slack delivery, pin to dashboard"; "Aren’t the tabs supposed to maintain your last selections until they are changed?")

**What it was.** §9’s "Saved reports: sharing, scheduling (email/Slack cadence), pin-to-dashboard". Read against the code: `saved_reports` had `is_shared` and the endpoint stored it — and its GET returned EVERY row in the org whatever the flag said (its own comment claimed "own + shared"); nothing in the UI set it. The PUT was the CLAUDE.md sanitize shape a THIRD time: `sanitize({ ...data, orgId, ownerId: userId })` — a partial body would have rebuilt the row from the body (dims, metrics, chart gone) and re-stamped the owner with the CALLER (an Admin renaming a rep’s report would take it); only the builder’s full-row save had hidden it. Nothing scheduled anything: `export_schedules` is a separate feature with no runner; the four scheduled functions are the digest, pipeline alerts, task reminders and lead scoring. The library’s "Pinned" section is four hard-coded live tiles, not a pin. The Tasks range pill (`useState('week')`) and the Dispatch board range (`useState('today')`) were plain state — the tabs unmount on navigation, and only what a tab persists by name (`tab:<tab>:<key>`, scope and sub-view) survives; Jeff’s All time and This week did not.

**Sharing.** GET: `orgId AND (ownerId = me OR isShared)`; an Admin reads the org. PUT: read the row (404 if none — the old "unknown id upserts" is gone), owner or Admin (403), then `sanitize({ ...existing, ...data, ownerId: existing.ownerId, isShared: 'isShared' in data ? … : existing, config: configFor(…) })` — a share toggle is `{ id, isShared }` and keeps the definition; an Admin’s rename keeps the owner. The library shows "Your reports" and "Shared with you" (the card’s Share / Unshare for the owner or an Admin, a SHARED mark, the owner’s name); a stranger sees no Share, Deliver or delete on a shared card (the endpoint refuses them anyway).

**Scheduled delivery — the shape (`src/utils/reportDelivery.js`, pure).** `cleanDelivery(raw)` is an ALLOWLIST (guide §18b37): `{ enabled, cadence: daily|weekly|monthly, hour 0–23, weekday 0–6, dayOfMonth 1–28, timezone (an IANA zone Intl knows, else UTC), emailTo: [roster ids], slack: bool, lastDeliveredAt, lastError }` — a body’s `orgId` through the schedule is dropped, not stored. It lives at `config.delivery` (no new column; dev and prod share the database — nothing to migrate); the endpoint’s `configFor` merges a body’s config keys over the stored ones and never lets a body’s config overwrite the delivery key, so the builder’s save in place (`config: null`) keeps the schedule; `deliveryFor` limits `emailTo` to THIS org’s roster and carries the job’s stamps from the stored copy (a client cannot write `lastDeliveredAt`). `deliveryDue(delivery, now, last)`: the wall clock in the schedule’s zone (`localClock`, Intl `formatToParts`, never the server’s hour) must be the hour; weekly the weekday, monthly the day; and not within the cadence’s window of the last send (20 h / 6 d / 27 d — Netlify’s cron is not to the second). `deliveryTable(result)` → header / formatted rows / a total row, capped at 25 with `more`; `deliveryText` (monospace), `deliveryHtmlTable` (every cell escaped), `slackBlocksForReport` (header, context, the table in a code block, an Open button), `deliverySummary` ("Every week · Monday 08:00 America/Chicago · email to 2 · Slack").

**The job (`netlify/functions/report-deliveries.mjs`, hourly).** `withHeartbeat('report-deliveries', run)` — in `SCHEDULED_JOBS` and netlify.toml, so the Jobs tile reads it and only the site with `JOBS_ENABLED` runs it (prod). `run`: every saved report (the one cross-tenant read, like the digest), each handled BY ITS OWN ORG; due ones through `deliverReport(row)`: the owner on that org’s roster (by `clerkUserId` = the row’s `ownerId`; gone → an error, nothing sent), the data as the OWNER reads it — `canSeeAll(owner.role)` → the org’s opportunities / accounts / leads / activities, else the rows whose `ownerId` is the owner’s app id (the server’s rule; a Manager gets the org) — plus the settings row for the fiscal start; `runReport` (the tab’s engine; the drizzle rows carry the same camelCase fields); email to the schedule’s roster ids resolved IN THAT ORG (active, a real address) through `emailTemplates.reportDelivery`; Slack through `sendSlackToOrg` (no alert type — not gated by the pipeline-alert switches; an unconnected workspace is a named error); then the stamp — `lastDeliveredAt` when anything went, `lastError` — written by id AND org. **"Send now"** is `POST saved-reports?action=deliver&id=` (owner or Admin) → the same `deliverReport` → `{ ok, sent: { email: [...], slack }, errors, rows, delivery }`, a 400 when nothing could be sent (no recipients, no owner).

**Pin to Home.** `pinnedReports` (ids, strings only, capped at 50) is a key on my own profile blob — added to `SELF_EDITABLE_KEYS` (§18b34) and to `users.mjs` sanitize’s profile packing, so `PUT users?me=true { id, pinnedReports }` merges over my row and `flatten()` reads it back. The card’s Pin / Pinned writes first, then `setMyProfile` (the setter joined the app context — `App.jsx`). Home: a "Pinned reports" section after Worth your attention — the library rows loaded once per active org and resolved caller (§18b38: nothing before an org), each pinned report run with `runReport` over `visibleOpportunities` / accounts / leads / activities (what THIS viewer can see — the app’s visibility, not the tab’s name-based role gate; flagged) and drawn by `ReportChart`; a template report links instead. "Open →" sets `tab:reports:subTab = custom` and `tab:reports:openReport = id`, and the library consumes the id once its list has loaded and opens the report (into the builder, or its template).

**The library card and the dialog (`ReportsTab.jsx`).** `LibraryCard` (module scope, data as props): the source, a SHARED mark, the name, the description, the schedule’s summary when on (red with "last send failed" when the stamp says so), the owner, and Pin · Share · Deliver · × — Share / Deliver / × only when `userRole === 'Admin' || r.ownerId === clerkUserId`; Deliver never for a template report. `DeliveryDialog` (module scope): Delivery on; How often; the weekday or day of month; At (24 slots); Time zone (an input, red when Intl does not know it; the browser’s zone by default); Email to (the roster as checkboxes, "(no email)" greyed); Post to Slack (enabled only when `settings.connectedApps.slack`); the last send and its error; Close · Save & send now · Save schedule. Save is a partial PUT of `delivery`; Save & send now saves then POSTs deliver and prints "Sent to a@b, Slack — 12 rows in the period." or the errors. A refusal in the library (403, a failed pin) is a banner with Dismiss.

**The two preferences.** Tasks: `tab:tasks:range` (today / week / month / all, validated on read, default week). Dispatch: `tab:dispatch:boardRange` (today / week / month, default today); the anchor DATE is deliberately not persisted — a stored week would strand a returning dispatcher on a stale week — the range re-anchors on today. The convention (`tab:<tab>:<key>`, preference only, never data, validated on read) is unchanged; these two controls had simply not opted in.

**Found while building.** (1) The sanitize rule a third time (above) — CLAUDE.md’s count is updated. (2) §9’s "Report builder: actual query execution" line was not struck at §0.133 — struck now. (3) The report-query and reports-controls scans and one harness anchor pinned the OLD inline card markup; repointed to `openCard`. (4) The job-health test pinned FOUR scheduled jobs in three places; five now.

**Tests.** `tests/report-delivery.test.mjs` (new, 11): the allowlist (every key coerced, a smuggled orgId dropped, the stamp kept only as a string), no-channel, the summary, `localClock` in Chicago / Tokyo / UTC across a UTC midnight, `deliveryDue` for every cadence, the hour, the day, the dedup window from the argument and from the stamp; the table / text / cap / totals from a real `runReport`; the Block Kit shape and the escaped html; scans of the endpoint (own + shared, Admin, read-then-merge, the owner kept, the roster filter, the stamp, Send now, no upsert, org-scoped update), the job (heartbeat, the owner by org, recipients by org, canSeeAll, the rep’s rows, the stamp by org, Slack), netlify.toml, the registry, the template, the allowlist and the packing, the library (module-scope card and dialog, mayTouch, the gated buttons, the partial PUT adopting the row, the pin write, Send now, the two sections, Slack only when connected, the open request consumed once), Home (the §18b38 gate, the pins, runReport over what the viewer sees, the chart, the open request), the context setter, and the two tab preferences. `tests/integration/saved-reports.itest.mjs` (new, 6, mocked mailer): a rep reads own + shared and NOT another member’s unshared; an Admin the org; org B nothing of A; a share toggle keeps dims and metrics; an Admin’s rename keeps the owner (the regression); a stranger 403; an unknown id 404; org B’s PUT by id 404; the schedule stored in the allowlisted shape with org B’s member and a stranger dropped and the client’s stamp refused; a builder-style save keeps it; a config merges beside it; Send now over a User owner’s two deals (not the rep’s, not org B’s), one mail to the chosen member, the stamp on the row; an Admin owner’s report over the org’s three; no recipients → 400 and nothing sent; a stranger 403; org B 404; DELETE gated. **13 mutants**, all caught: GET returning the org; PUT re-stamping the owner; recipients unfiltered; the stamp from the body; a rep’s report over the org; recipients resolved without the org; weekly firing daily; the dedup gone; cleanDelivery keeping every key; the html unescaped; the pin local only; Home loading before an org; the Tasks range not persisted.
...
**Verified:** six gates (169 files; 93 function files), **819/819 unit** (11 new), **170/170 integration** (6 new), **513/513 mutations after a printed green baseline**, build guard OK 2,532 kB `index-ODWcjiOD.js`, `dist/` cleared. Browser: the handoff says what was observed — and Jeff's screenshots (14 Sep) show the delivered EMAIL and the SLACK post rendered as designed (the table, the total, the "14 more rows" line, the Open button); as Karen (15 Sep) the shared report with Pin only, pinned onto her Home. ~~NOT shipped.~~ **SHIPPED — the TWENTY-FOURTH ship (§0.137’s last paragraph).** Functions AND bundle, netlify.toml; no schema change.

### 0.136 The blue-era modals on the tokens — 549 Tailwind literals in six files rewritten by a parser-driven script, by role; LeadImportModal on CsvImportModal’s chrome (14 Sep, seventeenth session — Jeff: "The blue-era modals. Yesterday’s audit put numbers on this: ModalLayer, LeadImportModal, App.jsx, OutlookImportModal, ContactModal and UserModal carry roughly 400 off-brand Tailwind colours … It is independent of the paused signal migration.")

**What it was.** The §0.129 audit’s §4 table: 542 off-brand literal uses in 22 files, 417 of them in the six Jeff named — the Tailwind palette written by hand (slate greys `#1e293b`/`#64748b`/`#94a3b8`/`#e2e8f0`, `#2563eb` buttons and links, `#fef2f2`/`#eff6ff`/`#d1fae5` washes, `#dc2626`/`#10b981`/`#f59e0b` signals) in the confirm / prompt / blocked-delete dialogs and the spiff-claim and lost-reason chrome (`ModalLayer`), the lead CSV importer, the Outlook importer, the contact and user forms, and `App.jsx`’s sign-in screen, no-org screen, offline banner, badge dots and meeting-prep drawer. Guide §16 has forbidden them since the palette moved; nothing had swept them. `LeadImportModal` also wore its own chrome — a white panel at radius 14 with a light header — beside `CsvImportModal`’s dark drag-handle band. And the legacy `.modal` CSS class the confirm dialogs still use painted `#ffffff`.

**The script (`scripts/migrate-blue-era.mjs`, kept as the record; the §0.130 method).** `@babel/parser` over each file; every string literal, template quasi and JSX attribute carrying a hex is rewritten to the token it means. The map is explicit (slate → ink / inkMid / inkMuted / border / borderStrong / surface / surface2; red → danger; green → ok; amber → warn; the -50/-100 washes → the token with an alpha, `${T.danger}14`; the -200 tints → alpha 33/40); the BLUES go by ROLE, read from the literal’s context — the CSS property it sits under (`background`/`fill` → a primary button → `T.ink`, the near-black button guide §16 names; `color`/`border*`/`outline`/`stroke` → `T.info`), the argument slot of a `btn(bg, fg)` helper, or a hover handler’s `e.currentTarget.style.background = …`. An 8-digit hex (`'#dc262614'`) keeps its own alpha on the token. A string with a hex inside other text (`'1px solid #e2e8f0'`) becomes a template literal; an exact hex becomes the bare token; a JSX attribute (`fill="#2563eb"`) becomes `fill={T.ink}`. The import is added when the file lacks it. KEPT, by what the line SAYS (never by line number — the first version excluded by number, and a second run after the import had shifted every line converted half the avatar palette; caught by reading the dry run, restored from git, re-run): `#1c1917` (the allowed dark header), the pipeline’s stored default colour (`name: 'New Business', color: '#2563eb'` — a value the settings row holds), the stage pill map (`{ bg, text }` pairs and the two closed entries — what `getStageColor` keys on app-wide; a design decision, not a sweep), the avatar initials map. The dry run prints every decision (`line [role] from → to`) and every kept line with its reason; the apply re-parses each result and lists what is left.

**By hand after the script.** Four named `'white'`s the hex pass could not see → `T.surface`; `OutlookImportModal`’s `var(--text-secondary)` beside one → `T.inkMid`; `index.css` `.modal { background: #fbf8f3 }` (the class is still what the confirm, prompt and blocked-delete dialogs wear — the guide forbids the classes in NEW work; swapping those three shells for inline chrome is a separate job, flagged); `LeadImportModal`’s shell → `CsvImportModal`’s (the surface at radius 12 with a border, the `#1c1917` drag-handle band with the title "Import Leads from CSV" in the surface colour and the subtitle / × in `surfaceInkFg`).

**The numbers (the audit regenerated, `docs/design/color-audit.md`).** Off-brand uses 542 → 140 tree-wide, 22 → 18 files. The six: ModalLayer 130 → 4 (the avatar map), LeadImportModal 88 → 0, App.jsx 74 → 11 (the pipeline default and the stage map), UserModal 45 → 0, ContactModal 40 → 0, OutlookImportModal 40 → 0 — 417 → 15, every one of the 15 a kept palette. 549 literals rewritten (the audit counted 417 because its regex, like the script’s first, stopped at six digits and did not see `#dc262614`; and because the same line can carry several). The remaining 140 are QuotesTab (20), ViewingBar (18), useSettings (16 — data: the KPI threshold colours), QuotaRepCard (14), QuickLogFab (10), the Account and Contact rails (9 each), ErrorBoundary (7), OpportunityModal (7), the two merge modals (5 each) and five single-line files — the next sweep, the same script with more files.

**Found while building.** (1) The line-number exclusion list (above) — a migration keyed on line numbers is wrong the moment it inserts a line; the script now excludes by content and says so. (2) `tests/fetch-status.test.mjs` pinned the offline banner’s two hexes; repointed to `T.warn : T.danger`. (3) The audit’s regex misses 8-digit hexes — a note for its next run, not fixed here.

**Tests.** `tests/blue-era-sweep.test.mjs` (new, 5): the six files carry no hex outside the allowed lines and each imports `T`, no named white; the blues went by role (the confirm button danger-or-ink, its wash with an alpha, Cancel on the surface; the lead importer’s `btn(T.ink)` / `btn(T.surface2,T.inkMid)` / drag-over `T.info`; the sign-in mark, the offline banner, the meeting-prep health colour); LeadImportModal’s chrome; `.modal`’s background; the script’s content-based exclusions, alpha handling and role rule. **3 mutants**, all caught: the confirm button blue again; the importer’s primary button blue again; `.modal` white again.
...
**Verified:** six gates (169 files; 93 function files), **824/824 unit** (5 new), 170/170 integration (unchanged — nothing server-side changed; not re-run), **516/516 mutations after a printed green baseline**, build guard OK 2,532 kB `index-5_oMz9NV.js`, `dist/` cleared. Browser: the handoff says what was observed — and Jeff on dev (14 Sep, `index-5_oMz9NV.js`): "everything testing correctly". ~~NOT shipped.~~ **SHIPPED — the TWENTY-FOURTH ship (§0.137’s last paragraph).** Bundle only — no function, no schema change.

### 0.137 Home’s plate row shows the whole stage name — a 136px label column, the stage on its own guarded line (15 Sep, seventeenth session’s last fix — Jeff, with a screenshot: "Would you make adjustments so all sales stages can be displayed in this on your plate section without bleeding in to the activity")

**What it was.** `PlateRow` (HomeTab) gave the time / urgency label a 68px column and printed `{fmtArr(arr)} · {stage}` under it in one unguarded div. "$18K · Proposal" fit on two lines; "$84K · Negotiation/Review" wrapped after the dot and the stage — one word the browser cannot break — overflowed the column and drew across "Follow-up · ZZFX Ashgrove Holdings" in the title column (his screenshot). Any stage longer than the column did the same: Evaluation (Demo), Qualification.

**The fix.** The column is 136px (`flexShrink: 0, minWidth: 0`) — the longest stage name fits on one line at 11px. The amount and the stage are two lines: the amount, then the stage as a block span in its stage colour (`getStageColor(stage).text`, the same colour the pill uses), each with `overflow: hidden` / `textOverflow: ellipsis` / `whiteSpace: nowrap`, and the whole line carries a `title` so a clipped value still reads in full on hover — the guard for a stage name longer than any today. The title column keeps `flex: 1, minWidth: 0`, so it shrinks by the 68px the label gained instead of pushing the Open deal button out.

**Tests.** `tests/plate-row.test.mjs` (new, 1 scan): the 136px column, the 68px one gone, the stage as its own guarded line in its colour, the guarded amount · stage line with the hover title, the title column still shrinking. No mutant — a layout scan; the harness was not re-run (516/516 at §0.136 stands).

**Verified:** six gates, **825/825 unit**, build guard OK 2,533 kB `index-oOwpiwjs.js`, `dist/` cleared. Browser: NOT observed — the pane’s session had signed out overnight (the sign-in screen showed instead, and read as §0.136 says: the mark `#2a2622`, its line `#4d6b3d`, the subtitle in inkMuted, the page on `T.bg`); signing in is Jeff’s. ~~NOT shipped.~~ **SHIPPED — the TWENTY-FOURTH ship (next paragraph).** Bundle only.

**PROD SHIPPED — the TWENTY-FOURTH ship (Jeff: "ship prod", 15 Sep): `git push origin dev:master` at 14:50:55 UTC, `master` `c17e2e8` → `a4f4589` (the docs commit after the Karen observations; the FIVE code commits `9fb744a`, `5bbd813` (§0.134, bundle), `62dc20a` (§0.135, functions AND bundle, netlify.toml — `report-deliveries` NEW on prod), `f0705c0` (§0.136, bundle) and `9bce063` (§0.137, bundle) under it). Pre-flight: `git diff origin/master..dev -- db/schema.ts` EMPTY; 40 files across docs, netlify, scripts, src and tests; `dev` == `origin/dev`; the tree clean but `deno.lock` (the CLI's two lines, uncommitted as at the last two ships). salespipelinetracker.com serving `index-zv28zhMt.js` from 14:51:57 UTC (the hash poll, 62 s after the push; before it `index-Ct7x96Jv.js`) with the live key (`pk_live_` ×2 — the key and the SDK constant; `pk_test_` ×1, the SDK constant), 2,593,345 B. Netlify's record, read after the landing: deploy `6aa95b51…`, branch `master`, context production, commit `a4f4589`, published 14:51:52 UTC, deploy time 52 s, **83 functions** (82 before — `report-deliveries` NEW), FIFTEEN bundles carrying the ship's build stamp (14:51:49–51 UTC): `report-deliveries`, `saved-reports`, `send-email`, `_selfProfile`, `users`, `_heartbeat`, `_customerNotify`, `dispatch-jobs`, `job-status`, `opportunities`, `digest`, `pipeline-alerts`, `score-leads-batch`, `task-reminders`, `train-lead-model` (the changed files and what imports them); the schedules now FIVE — `digest`, `pipeline-alerts` and `report-deliveries` hourly, `score-leads-batch` 06:00 UTC, `task-reminders` every minute; 3 redirect rules, 1 header rule, secret scan clean over 458 files, "2 new files uploaded — 1 generated page and 1 asset". **From this hour the hourly `report-deliveries` runs on prod** — a schedule on a prod report fires at its own hour; the Jobs tile shows the job as "never" until then. ~~NOT yet observed on prod.~~ **OBSERVED on prod by Jeff (15 Sep): the plate row "clear", "Dispatch worked", "Reports worked", "Leads worked".****

### 0.138 The rest of the audit’s off-brand table on the tokens — 126 Tailwind literals in fifteen more files; the tree-wide count 140 → 31, all of them data palettes (15 Sep, seventeenth session — Jeff: "fix the 140 off-brand literals before I start a new conversation then")

**What it was.** After §0.136 the audit’s §4 table still listed 140 off-brand uses in 18 files: QuotesTab 20, ViewingBar 18, useSettings 16, QuotaRepCard 14, QuickLogFab 10, the Account and Contact rails 9 each, ErrorBoundary 7, OpportunityModal 7, the two merge modals 5 each, ReportsTab 1 and four surfaces with a single `#fef2f2` each. Jeff asked whether they came from the design he proposed (§0.129) — they do not: they are the app’s original Tailwind-era styling, the same class as the six modal files, written before the warm palette; guide §16 had forbidden them since; the signal migration stays paused and is untouched by this.

**The pass.** `scripts/migrate-blue-era.mjs` with fifteen more files in its list — `useSettings.js` deliberately NOT among them: its sixteen literals are stored DEFAULTS (a pipeline’s colour, the KPI tolerance colours, the block types), data the settings row holds. Three rules added for this pass and kept: (1) a literal inside an ARRAY is a palette (a chart’s series, a stage map, avatar initials) and is never rewritten — reported as kept; (2) in these files the warm palette’s OWN values (`#fff`, `#f5f1eb`, `#2a2622`, `#e6ddd0` …) are left as they are — the ask was the off-brand set, and a white → surface swap across QuotesTab and ReportsTab would be a visible change nobody asked for; the six modal files of §0.136 had those converted, and stay so; (3) `T` may reach a file through `documents/atoms.jsx` or `settings/shared/tokens.js` — any binding named `T` counts, so the import is added only where there is none (the first run added a second import to a documents file and the re-parse refused it — caught by the script’s own check, fixed, re-run). Four Tailwind values the first pass’s map lacked were added: the purple accents `#7c3aed` / `#6b21a8` (by role, like the blues) and the wash `#f3e8ff` (QuotesTab), the greens `#4ade80` / `#bbf7d0` (OpportunityModal). 126 replacements: QuotesTab 32, ViewingBar 18, QuotaRepCard 14, ErrorBoundary 7 (its retry button was a blue background → the ink button), QuickLogFab 10, the two rails 10 each (the do-not-contact and merge notices), OpportunityModal 10, the merge modals 5 each, ReportsTab 1, the four `#fef2f2` surfaces 1 each; imports added to ViewingBar, QuotaRepCard, QuickLogFab, ErrorBoundary and the two merge modals.

**The numbers.** The audit regenerated: off-brand 140 → 31 tree-wide, 22 → 3 files — useSettings 16, App.jsx 11 (the pipeline default and the stage pill map), ModalLayer 4 (the avatar map): every remaining use is a stored data palette, kept on purpose. From the audit’s first count (13 Sep): 542 → 31.

**Not converted, on purpose (in these fifteen files).** The warm palette’s own literals (~200 uses — `#e6ddd0`, `#2a2622`, `#8a8378`, `#fbf8f3`, the stage colours); OpportunityModal’s forecast-category badge palette (twenty-odd bg/text pairs — `#eaf3de`/`#3b6d11`, `#e6f1fb`/`#185fa5` … — a deliberate per-category set, not Tailwind’s, and not in the audit’s off-brand table); the merge modals’ warm custom tints; every array palette. A pass that names those by token is a different decision (value-identical, no visual change for the warm ones) and stays open.

**Tests.** `tests/blue-era-sweep.test.mjs` +1: the fifteen files bind `T` and carry no Tailwind value outside a palette array. **2 mutants**, all caught: ErrorBoundary’s retry button blue again; QuotaRepCard’s quota bar blue again. (The fetch-status, house-dialogs and role-vocabulary pins were unaffected — 825 green before the scan was added.)

**Verified:** six gates, **826/826 unit**, 170/170 integration (unchanged; not re-run), **518/518 mutations after a printed green baseline**, build guard OK 2,533 kB `index-DIxtA4Km.js`, `dist/` cleared. Browser: the handoff says what was observed. **NOT shipped.** Bundle only — no function, no schema change.
...
~~**Settings auto-save fires for users who can never save.**~~ **DONE — the §0.54 no-change guard (`if (json === lastSavedRef.current) return;` in useSettings.js) is exactly the fix this asked for; struck by reading in §0.109.** `useSettings.js:223`
...
**Jeff's dispatch picks, in his order (9 Sep, §0.110 was the first):**
- ~~**Customer-facing notifications**~~ **DONE — §0.111:** confirmation on schedule and on a date/time change, "on the way" on `en_route`, by email now and by SMS the day Twilio's three variables land on a site AND the A2P campaign is approved (Twilio showed Error 30034 on 9 Sep — brand approved, campaign not); `/status/<token>` is the public page, token-only. Per-org switches, OFF by default.
- **Quote → job → invoice → QuickBooks** — jobs carry `invoiceAmount`/`invoiceStatus`/`invoicePaidAt` but no UI writes them; no quote-to-job link; no invoice record with lines; no QuickBooks code. The QuickBooks half needs Jeff's hand first: an Intuit developer account, an app with OAuth redirect URIs for BOTH sites, a sandbox company. Build the quote-to-job link and the invoice record first; the export waits on the app.
- **Tech mobile experience** — deferred by Jeff to one dedicated mobile pass.
...
~~**Skip / defer a single plan occurrence.**~~ **DONE — §0.110:** `dispatch_plan_visits` records a skipped or deferred occurrence per customer per plan, honoured by the queue, the customer list and the hourly renewal alert through one pure module.
...
- ~~**"Test auto-create"** — `DispatchJobTemplatesDetail`.~~ **REMOVED (§0.109) — pinned absent by `tests/dispatch-stubs.test.mjs`.**
- ~~**"Manual pick"** — `DispatchTab` / `CrewBuilderView`.~~ **REMOVED (§0.109), with the copy that pointed at it.**
- ~~FunnelStages "+ Add stage" · LeadConversion "Recompute from history" (needs a new Netlify function) + "Save changes" · PainPoints "Import CSV" · CustomerTypes auto-classification "Edit" · Industries drag handles.~~ **Struck by reading (§0.109):** "+ Add stage" calls `addStage`; PainPoints "Import CSV" opens a real file input; the Industries drag handles reorder; the LeadConversion header stubs and the CustomerTypes "Edit" no longer exist in the tree.
...
- ~~**Saved reports:** sharing, scheduling (email/Slack cadence), pin-to-dashboard~~ **DONE — §0.135 (14 Sep): own + shared visibility with a Share toggle; a per-report schedule delivered by the hourly job by email and/or Slack, with Send now; Pin to Home on my profile, drawn on Home.** Left there: a schedule’s recipients are roster members (no outside addresses); the day/time is one hour slot; Home draws a pinned report over what the viewer can see (the app’s visibility), the tab over its own role gate by name.
- ~~**Report builder:** actual query execution against live DB (currently preview is illustrative)~~ **DONE — §0.133 (14 Sep); this line was missed at that batch and is struck now (§0.135).**
...
- ~~**Lead Scoring Phase 2**~~ **DONE — struck by reading (§0.123, 12 Sep):** the per-org logistic regression (`trainLeadModel`, `predictLead`, five leakage-free features), source win-rate learning (`computeSourceWinRate`), the nightly training above `minClosedRecords`, the stored model, the probability on every lead and in the why-this-score popover, and the Admin switch that is the "license gate" — all shipped. What remains is Jeff's call: a manual "Train now"; nothing else from this entry.
...
- ~~**Inline-component audit** — `TasksTab`, `LeadsTab`, `PipelineTab` still contain inline sub-components to hoist.~~ (Duplicate of the struck entry under "Resolved this batch" — those three were already clean, §0.33.)
- **This session's follow-ups:** Kanban view (PipelineTab) — verify closed deals leave the board correctly now that closing is reachable; ~~sweep other modals (`components/modals/`) still on blue-era palette~~ **DONE — §0.136 (14 Sep) for the six the audit named, then §0.138 (15 Sep) for the rest: 31 off-brand uses remain tree-wide, every one a stored data palette (useSettings’ defaults, the stage pill map, the avatar map)** / missing Esc + overflow handling (LostReasonModal was one of a likely cohort); consider exempting task completion from the rep ownership gate, or allowing the assignee specifically (a rep completing a teammate's task currently 403s → rollback reverts the checkbox — correct per matrix, maybe wrong for workflow); align other tabs' delete-confirm copy with the soft-delete undo reality (done for ContactsTab).
- ~~**Dispatch — technician model.**~~ **DONE — struck by reading (§0.109): `isTechnician` in auth.mjs, jobs FK the technician row, a technician's PUT is filtered to `TECH_WRITABLE` (status, techNotes, completionNotes, photosCount, customerSignature) with `TECH_STATUS_ALLOWED` transitions in dispatch-jobs.mjs, and `dispatchTechProfiles`, `DispatchTechDetail` and `u.dispatchEnabled` no longer exist in the tree.** Was: Step 3 **DONE** (old stores retired, crews repointed). Remaining — Step 4: migrate any real `settings.users[].dispatch*` data into `dispatch_technicians`, then retire those fields, the dead `settings.extra.dispatchTechProfiles` key, and `DispatchTechDetail` (it filters on `u.dispatchEnabled`, which nothing sets any more, so it now always renders empty). Step 4: **`Technician` role + scoping** for mobile — add `isTechnician()`, resolve `userId → dispatch_technicians.id` server-side (jobs FK the technician row, *not* the user), filter to `assignedTechId` or membership in `coTechIds`, and apply a **per-field write whitelist**: job status transitions, `techNotes`, `completionNotes`, `photosCount`, `customerSignature` — and nothing else (no reassignment, scheduling, or customer edits). Note `requireWrite` currently treats every non-ReadOnly role as fully write-capable, so the role and its scoping must land together.
- **Priority vocabulary normalization** to `low|normal|high|emergency` everywhere. **Code half DONE — struck by reading (§0.109), the alias collapsed in its own commit as this entry asked:** the `priority:'standard'` literal is gone, `PRIORITY_ALIASES` in DispatchTab translates legacy `urgent`/`standard`/`medium` at the read boundary and DispatchJobTemplatesDetail translates on read, and `prioColor2` is collapsed into `prioColor`. NOT verified: whether any `dispatch_jobs.priority` row still holds a legacy value — a read-only `SELECT DISTINCT priority` first, then the additive `UPDATE` by Jeff's hand if any does. Was: Surface is small: the `priority:'standard'` literal in the "New template" button, the `autoJobs` synthesiser in `DispatchTab`, a settings-blob pass over `dispatchJobTemplates`, and one additive `UPDATE` on `dispatch_jobs`. Then collapse the dual-accept `prioColor`/`prioColor2` maps back to one. **Do as its own commit** — not bundled with feature work. Store `emergency`, label it "Emergency".
- ~~**`DispatchJobTemplatesDetail` bug:**~~ **DONE — struck by reading (§0.109): the name input writes `updateTemplate('name', …)`, and the file's header comment records the split.** Was: the "Template name" input and the customer-type select both write `updateTemplate('ctype', ...)` — the same field — so templates have no distinct name and the list renders customer types as names. Split `name` from `ctype` (migrate by copying `ctype` → `name`). **Blocks** making templates selectable in the New Job modal.
...
- ~~**Job templates selectable in New Job**~~ **DONE — struck by reading (§0.109): a job carries `templateId` and the New Job flow resolves it against `settings.dispatchJobTemplates`.**
...
- ~~**Dispatch smaller gaps:**~~ **DONE ×4 — struck by reading (§0.109):** `jobNumber` is issued by `nextJobNumber` inside `withNumberRetry` on create and is immutable on PUT; `preferredTechId` is set from the customer form's select; `dispatch_equipment` has the Vehicles & equipment UI and the planner's availability checks; mass-schedule honours `crewSize` and skips a partial crew with a reason. Was: `jobNumber` is never assigned; `preferredTechId` cannot be set anywhere; `dispatch_equipment` has an endpoint and no UI; mass-schedule ignores `crewSize`.
- **Dispatch board follow-ups:** ~~drag-to-reschedule on the week grid (still OPEN — nothing in DispatchTab is draggable)~~ **DONE — §0.134 (14 Sep): the week board’s job cards drag to another day or another technician’s row; the day and month boards do not drag (flagged there).** ~~`Save draft` in the crew builder is still inert~~ (no such control exists any more — §0.109); SMS notify is disabled on purpose pending Twilio A2P approval (the button says so).
- ~~**Settings panels swallow save errors**~~ **DONE — §0.109: the class was already closed by `putSettings` (guide §18b1) for every panel but three live writes — the team and territory delete cascades and the lead-conversion benchmark save — which now report in a banner; pinned by `tests/settings-cascade-errors.test.mjs`.** The Add-then-Save clarity question stands (below, "Add-vs-commit").
- **Non-transactional dispatch job create** — still OPEN (§0.109 re-read it): the New Job flow is three client-side writes (customer, location, job) to three functions, so no server transaction can span them; making it atomic means one server endpoint for the whole create. If the service-location write fails, the customer from step 1 already exists; retry finds them rather than duplicating, but state is not rolled back.
...
- ~~**Seed data cleanup:**~~ **STRUCK by a read-only query (§0.109, 9 Sep):** one `@test.com` row exists in the whole database — `smiller@test.com` (Savannah Miller, Technician, the dev org) — and it is LINKED to a Clerk identity, a real test account. Nothing to delete. Was: ten `@test.com` users exist in the `users` mirror and in no Clerk account.
- **Security follow-ups:** ~~**`PUT /users?me=true` accepts a self-supplied `role`**~~ **DONE — §0.109 / guide §18b34: the role column was already pinned to the stored value and the client's gates read Clerk, not the roster; the self-save now takes the `SELF_EDITABLE_KEYS` allowlist, so quota, team, territory, `active`, the quotas and the forecast calls cannot be set by a member about themselves either, and the blob's `userType` copy is pinned too;** ~~non-admin settings auto-save now 403s~~ (the §0.54 no-change guard stops the load-time mirror-back PUT for every role); role/ownership sweep of `documents.mjs` (DELETE has `ownerId` but no gate) + all `dispatch-*` endpoints + `products`/`saved-reports`; Netlify env-var & source-map audit item (dashboard config); Security-health sub-page — wire mock events feed + score to the real audit log; migrate name-based ownership to `ownerId` userId columns (post-launch); Manager **team-scoped** writes (Phase 2 — full org-wide write in v1 by design); align delete-confirm copy on other tabs with the soft-delete undo reality; ~~client skips `GET /users` for non-admin roles (currently eats an expected 403 with console noise each load)~~ **STRUCK by code (§0.126, 13 Sep): a non-admin GET returns 200 directory rows since §0.109 — no 403 to skip;** ~~**NEW (§0.109's pane pass, 9 Sep): `coaching-notes`, `calendar-events` and `spiff-claims` are each fetched once BEFORE the Clerk token is attached (401, "DB error 401" in the console) and once after (200) on every page load as Karen — a load-order race in the hooks that fire on mount, not a permissions gap; three console errors per load, harmless to data, unread as to which hook fires early.**~~ **DONE — §0.125 / guide §18b38 (13 Sep): the three fired from mount on `waitForToken` alone, whose 8-second give-up resolves without a token (the in-app sign-in with MFA takes longer than that); the "once after" was the next page load. Every mount-time load now keys on `activeOrgId`, reloads per org and clears the previous org's rows — the OrganizationSwitcher had left them on screen.**
