...
**Tests:** +1 file `tests/lead-scoring-defaults.test.mjs` (6: the engine re-exports the shared object, ten rules with five events and the predictive block, Web Form scores as inbound, decided statuses and the batch's use of them, the panel's import/no-mirror/Reset/readiness scans, a web lead scored end to end — New + fresh recency = 45 → warm), `score-lead` registered. **Mutants (5):** event rules ignored; the bucket takes the lower axis; recency stops decaying; Web Form drops out of inbound; the Demo rule vanishes. **Verified:** six gates (162 files; 91 function files), 741/741 unit, 153/153 integration, 435/435 mutations after a printed green baseline, build guard OK 2,495 kB `index-Af3POXBZ.js`, `dist/` cleared. NOT observed on dev at the time of writing; NOT shipped.
