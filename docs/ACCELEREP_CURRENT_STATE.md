...
**What it was:** dev and prod share one Neon database, and since the tenth ship both sites have run every scheduled job — `pipeline-alerts` and `digest` hourly, `task-reminders` every minute, `score-leads-batch` daily — against the same orgs, deals, preferences and Slack webhook. A qualifying pipeline signal at the top of the hour went out twice: one email, one SMS and one Slack post per site (not observed — no signal has qualified since the ship; the 22:33 UTC Closed Won post was an event post from the deal save, one site). With §0.101 shipped, every digest would go twice as well, and the first manager digest on Monday 14 Sep would arrive in duplicate.
