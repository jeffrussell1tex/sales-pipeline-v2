...
- **Dispatch board follow-ups:** drag-to-reschedule on the week grid (still OPEN — nothing in DispatchTab is draggable); ~~`Save draft` in the crew builder is still inert~~ (no such control exists any more — §0.109); SMS notify is disabled on purpose pending Twilio A2P approval (the button says so).
