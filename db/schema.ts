...
export const users = pgTable('users', {
    // APP-OWNED AND PERMANENT. Generated by us as `usr_<uuid>` and never
    // rewritten. It used to hold the Clerk userId, which meant the primary key
    // CHANGED the moment an invited user accepted -- silently detaching anything
    // already keyed to the pending_ row. An identity that can change is not an
    // identity. Clerk's id now lives in clerkUserId below, as an attribute.
    id:            text('id').primaryKey(),                          // usr_<uuid> -- never reassigned
    // The Clerk identity this roster row is linked to. NULL until an invited
    // user accepts. Unique per org (see users_org_clerk_uq); Postgres treats
    // NULLs as distinct, so any number of un-accepted invites coexist.
    clerkUserId:   text('clerk_user_id'),
    name:          varchar('name', { length: 255 }).notNull(),
    // Unique PER ORG, not globally. The global .unique() meant one email address
    // could exist in exactly one Accelerep organization across the entire
    // customer base: inviting a consultant who already worked for another
    // customer failed with "A user with that email address already exists",
    // which both blocked a legitimate invite and confirmed to one tenant that
    // the address existed in another. Clerk supports multi-org membership; this
    // column is what stopped us from representing it.
    ...
    // One row per email per org, and one row per Clerk identity per org.
    uniqueIndex('users_org_email_uq').on(t.orgId, t.email),
    uniqueIndex('users_org_clerk_uq').on(t.orgId, t.clerkUserId),
